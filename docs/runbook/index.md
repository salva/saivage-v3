...
For rewritten files, only `IndeterminatePublicationError` means publication may have occurred and closes mutation admission. Validation, temporary create/write/fsync/close, and other failures known to precede rename are nonterminal: fix the reported cause and retry the application operation normally. Saivage does not scan history, reconcile a retry, or treat a duplicate as success. For growing files, uncertainty starts immediately before the first append write syscall; an append-open failure is nonterminal.
