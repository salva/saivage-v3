...
`saivage stop` delegates to `POST /api/runtime/stop-project`. Project Stop is containment, not cancellation: it never calls `cancel_card` and writes no card/root outcome, ToolResult, provider round, or Stop event. It first closes continuation/admission, then Stop-claims only open owners; result/cancel winners remain authoritative and only suppress continuation. A cleanup/process failure returns containment failure and retains closing ownership, so all controls conflict until terminal restart. Success leaves the durable running chain unchanged for a later Run, which installs every ancestor owner in structural wait and activates only the deepest card. The server, Analyst, MCP resources, and lifecycle lock remain live. Pause is cooperative: one flag parks one active admission frontier.
