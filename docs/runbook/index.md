...
Audit compacted history under the session's owner root: Analyst sessions use `.saivage/agents/conversations/<encoded-session-id>/`, while planner/executor/reviewer card sessions use `.saivage/outputs/cards/<cardId>/conversations/<encoded-session-id>/`. `index.json` names the active version and frozen versions; numbered `<N>.jsonl` files contain immutable raw or compacted versions; `summaries.jsonl` contains cached per-round summaries used to rebuild merged summaries. Frozen versions are audit evidence and should not be edited during normal operation.
