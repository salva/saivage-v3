...
An uncertain durable write makes the serving application mutation-unhealthy until restart. Read-only inspection remains available, while readiness reports not ready and later durable mutations fail before filesystem access. Restart performs deterministic restabilization and strict loading; complete malformed state remains for operator repair.
