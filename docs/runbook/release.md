...
Use Node.js 24 with the npm range declared in package engines (`node >=24 <25`, `npm >=10 <12`) before running release checks; this matches the CI setup-node profile. Use the named validation profiles from the repository root so routine checks stay consistent with package scripts. `validate:docs` intentionally runs only `npm run docs:verify`; it does not run `npm test` or the Vitest operator smoke guard, because documentation verification must stay lightweight and fast enough for routine doc drift checks.
...
The repository includes a path-aware GitHub Actions workflow at [`.github/workflows/validation.yml`](../../.github/workflows/validation.yml). Pull requests, pushes to `main`, manual dispatches, and the scheduled nightly backstop use least-privilege `contents: read` permissions, cancel superseded runs for the same workflow/ref, set up Node.js 24 with `actions/setup-node@v4` and npm caching, and install dependencies with `npm ci`. The `routine-docs` job always runs `npm run validate:routine` and `npm run validate:docs` so CI exercises the same fast package profiles operators run locally.
