...
**Process store schema change.** The current `ProcessRecord` carries `card_id`, `agent_session_id`, and `owner_kind` (`'agent' | 'operator' | 'runtime'`). This design replaces those three fields with a single `owner_id: string`. The executor passes its activation id as `owner_id`; the analyst passes its session id. `wait_process` and `kill_process` verify that the caller's `ownerId` matches the record's `owner_id`. The runtime kills processes by `owner_id` scope on activation settlement or session end — no actor-instance iteration needed. Existing process records are not migrated; the process store is ephemeral (`.saivage-work/processes/`), so a clean cut-over is safe.
...
6. Move Analyst audit logging and event broadcasting into Analyst-handler pre/post hooks around `invokeTool`. Derive shaping from `data` content instead of `preview`/`errorEnvelope`.
