...
Stable sessions are `planner:<card-id>`, `executor:<card-id>`, `reviewer:<card-id>`, and shared `analyst:global`. Each has one canonical `conversation.jsonl`; there are no versions, active-version inventories, sidecars, summary caches, or reconstructed provider continuations. `AnalystSessionActor` is the one stable submission/source-round owner and retains one child `ConversationLLMActor`; the child performs owner-delegated provider-turn, continuation, and compaction publication without introducing another conversation, cache, cursor, or projection authority.
