...
Model-facing scoped filesystem references use canonical triple-slash URLs: `project:///`, `record:///`, `tmp:///`, read-only `work:///`, and `system:///`. The workspace layer parses and emits these URLs through one string-based helper that validates raw path segments symmetrically and rejects old two-slash forms. Record URLs are derived from card id, slot filename, and version when projected; they are not persisted in record slot index entries. Record card ids from query parameters, default card context, and record search paths are validated before any slot path is constructed.
