...
Runtime process ownership is centralized in `startApp()`. It parses only enough input to resolve the project root and `--create-runtime`, acquires `.saivage/locks/runtime.lock` through the canonical runtime-lock helper before any `.saivage` mutation, registers the release with the root resource scope, and only then performs locked `--create-runtime` initialization before config/environment loading and server startup. `createServer()` and `startServer()` remain lock-free composition/listen helpers. `saivage reset` acquires the same lock atomically, holds it while deleting generated roots and running `initProjectTree`, preserves the held `runtime.lock` while cleaning other lock entries, and releases it only after reinitialization succeeds or fails.
