...
Provider-turn interruption authority lives above the provider adapters. Each `ConversationLLMActor` issues one opaque immutable invocation lease per provider-bearing turn and registers its Saivage wrapper before preprocessing or raw work starts. The wrapper races cancellation against preprocessing, compaction/summarization, gate admission, and provider completion, observes a late raw rejection, and suppresses delivery after revocation. Activation/session operation trackers separately retain processor or Analyst-loop wrappers and their admitted completion callbacks. Join first waits for wrappers, consumer callbacks, and admitted completion-persistence callbacks, then takes the actor-core lifecycle-settlement snapshot; an abort-ignoring raw dependency is reported as external abandonment rather than physical quiescence.
...
That same signal object is passed to `RuntimeGate.waitUntilOpen`; gate admission never receives the task-only signal. Ordinary gate closure is reversible Pause. Terminal gate closure rejects current and future waiters, removes their abort listeners, and makes later open attempts fail. Accepted completion persistence uses only `CompletionPersistenceAdmission.admit(invocation, persist)`: its synchronous prefix validates the exact current invocation, rejects duplicate or revoked admission, and registers one tracked callback. The callback is invoked at most once and released internally in `finally`; callers receive no acquire/release lease. OpenAI Responses private-plus-visible completion rows are one callback persistence unit and define no parallel commit authority.

The durable-ownership cutover has an internal foundation that does not change this lifecycle contract. Four immutable process-local identities—root generation, leaf activation, Analyst turn, and installed MCP revision—form the complete currentness vocabulary; an MCP invocation authority is the product of its discriminated caller authority and installed revision, not a fifth identity. A composition-owned `MutationLane` executes synchronous callbacks immediately, rejects recursive or Promise-returning callbacks, and checks exact authority currentness immediately before application. Its dedicated MCP delivery operation additionally compares the current caller-kind admission and returns typed stale delivery without invoking the callback. Stores move to this lane only in complete store-plus-caller cutovers; authority-free current store contracts and the existing `RuntimeGate`/`InvocationLease` settlement tracking remain current until their corresponding cutover. No public `pausing` state, unified Run control, or quiescent-Pause claim is introduced by this foundation.
...
State remains project-local under `.saivage/`. Card and authored-record authority consists of strict self-contained version artifacts under `.saivage/cards/<cardId>/{card,brief,status,review}/versions/`; adjacent indexes are deterministic projections rebuilt from those artifacts. One project persistence authority is opened with a live root-bound runtime lifecycle-lock handle and owns one multi-caller writer. Authorized Analyst and planner requests share its authority-wide serializer; no card-mutation project lock, per-card lock, slot lock, or per-file queue exists. Each canonical artifact and derived index replacement is independently durable, so semantic operations may expose an accepted prefix rather than cross-file atomicity.

Normal opening first performs mutation-free complete canonical project-root observation. Only after that proof may target-local temporary cleanup, incomplete non-root creation cleanup, index rebuilding, and full tree/brief validation run. Plain start always chooses normal mode. Init, reset, and `start --create-runtime` choose card-store bootstrap only after strict read-only fresh/reset-empty verification; reset retains the lifecycle-lock handle while deleting generated roots and then explicitly bootstraps. Bootstrap creates the root through the same private ordinary writer and exposes it only after the normal observation/restabilization transition. Fresh-root project identity is separate: only Init creates it through the lane-backed `ProjectIdentityStore` while holding bootstrap-unbound ownership. Server start and reset require bound project identity.

Runtime process ownership is centralized in `startApp()`. It acquires one strict bound `.saivage/locks/runtime.lock` with `O_EXCL`, opens the project persistence authority in an explicit mode, and passes that already-open authority into server composition. After listen, the same exact owner atomically adds the actual endpoint metadata; current lifecycle controls do not consume it until their later atomic cutover. `createServerServices()` constructs `CardStore` only as an injected façade over the authority's reader and singular writer; server, tools, actors, and recovery cannot reopen card/record persistence from a path. Init and reset enter `withDirectMutationComposition`, which owns the same strict lock plus one private composition authority and synchronous lane for the command lifetime. Every pre-existing lock blocks; diagnosis never removes it, and binding, endpoint publication, and release verify the exact instance, PID-start, and canonical-root owner identity immediately before changing the lock file.
