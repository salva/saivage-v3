...
Card identity is `project` or one-to-five opaque 28-letter lowercase hierarchical segments. Parent, depth, namespace, exact card/record streams, and fixed-role conversation paths are forward-derived. Backend `listChildren`, logical-path, and reorder behavior uses positions. The web nested and direct-child projections instead follow each reached parent's committed `children` array order and expose `logical_path`; neither assigns identity meaning to a segment.

One non-root attempt asks its segment factory once and exclusively claims `.saivage/cards/project/children/<s1>/.../children/<sn>/`. Its exact successful order is closed initial `brief.jsonl` and card-version-1 `card.jsonl` publication → first complete-publication proof inside `publishInitialCard()` → fresh parent reread/admission → second complete-publication proof in `CardService` → cumulative parent `children` append → parent `child_link` history, card, and runtime effects.

Successful-path visibility checks occur only after the confirmed parent-link append: they may read the child through the parent's link and linked list/tree projection. Readable initial child streams before that link are not accepted membership. If the parent-link append reports an error, its outcome is unknown and neither production nor verification may read, list, retry, reopen, reconcile, reconstruct a reader, or inspect any canonical path, stream, namespace, or artifact to infer what committed; only required cleanup of the already-open descriptor remains permitted. A normal reader derives the bounded root-to-target chain from the requested ID, validates each reached exact card stream and required exact brief stream, requires the next ID in the active parent's committed `children`, and only then touches the exact `children/<segment>` component. A terminal tombstone is the final row of `card.jsonl`: it makes that card absent from every card-domain detail/history/version/diff/record projection and terminates traversal before descendants. Direct exact-known conversation I/O remains a separate lifecycle-ungated path. Namespace-only, brief-only, temp-only, unlinked malformed, and arbitrary noncanonical entries are silent invisible orphans. They are never scanned for allocation, validated in aggregate, cleaned, warned about, adopted, quarantined, or repaired.
...
The singular `ReadModelChanges` broadcaster feeds `SyncHub`; `SyncHub` alone debounces and coalesces successful owner hints before `LiveSyncSocket` sends canonical invalidation frames. Clients treat those frames as lossy prompts and refetch authoritative REST state. The Cards registration is a distinct `resource: 'cards'` variant with `onRefetch?: never`: every delivered frame invokes its refetch immediately, including while another Cards invocation is active, and observes each returned promise's rejection independently without publishing SyncClient success. Non-Cards registrations and conversations retain their existing single-flight/trailing behavior. There is no owner-side suppression, polling, queue, generation, acknowledgement, dropped-frame recovery, replay, or alternate publication path.
...
For Cards specifically, one CardStore owns the canonical flat collection, keyed detail, history data, list-response currentness, and the sole nested `CardTreeNode.childNodes` projection from parent-owned child-ID links, with independent operation state for each resource. One disposable call-local ID map resolves the already-delivered flat collection: projection starts only at `project`, recursively follows each reached record's `children` IDs in array order, skips a linked ID with no active row, and omits every unlinked row. The same rule supplies direct-child projections; child `parent`, child `position`, and flat response order do not establish membership or sibling order. The map is discarded with the call and is not store state, an index lifecycle, or a cache. CardStore's sequence/abort checks reject an older collection success, failure, or final loading state after an immediately dispatched invalidation starts a newer list read. Keyed-detail requests mutate only selected-detail loading/error state; they do not mutate collection loading/error, replace the canonical collection, or assume selection ownership. The router alone owns selected presentation identity. `CardsView` passes the exact route card ID into the tree and has no fetch or optimistic insertion; the tree computes and discards the route-selected ancestor set used for render reveal. No store field, watcher, event bus, selection synchronization, or resize coordinator shares that responsibility. The layout chain is the bounded workspace stack → flex-column workspace content → optional non-growing auth banner → remaining-space route host → contained Cards route → existing inspector panes. The route host is the normal page scroller for other routes; Cards suppresses combined overflow and delegates vertical overflow to its existing tree and detail pane owners without fixed viewport heights or JavaScript sizing.
