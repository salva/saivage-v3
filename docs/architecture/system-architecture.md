...
One server-composed `ConversationStore` owns initialization, batch append, summary-cache append, and compaction publication through named synchronous methods. Callers pass domain payloads, never capabilities or callbacks. Compaction rechecks the active numeric version and source-content digest immediately before publishing the next version. The store publishes scoped conversation and `agents` freshness only after successful durable effects. UI chat and timeline surfaces refetch the active highest validated version; inactive pre-compaction versions are not merged into current operator transcripts, summaries, or last-activity calculations. Active-version row order is semantic transcript order. Provider exchanges remain app-log-backed and outside conversation token budgeting.
...
One composition-owned `AppLogStore` is the only writer for event, error, control-action, provider-exchange, and content-review entries. Its named append method is synchronous and checks shared application persistence health before the file effect. Startup truncates only an incomplete final JSONL row and rejects malformed complete rows. The former app-log lock, generic JSONL ledger writer, event-bus persistence projections, and root-taking append helpers do not exist.
