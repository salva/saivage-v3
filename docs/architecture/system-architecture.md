...
Normal opening first performs mutation-free complete canonical project-root observation. Only after that proof may target-local temporary cleanup, incomplete non-root creation cleanup, index rebuilding, and full tree/brief validation run. Plain start always chooses normal mode. Init and `start --create-runtime` choose bootstrap only after strict read-only fresh/reset-empty verification; reset retains the lifecycle-lock handle while deleting generated roots and then explicitly bootstraps. Bootstrap creates the root through the same private ordinary writer and exposes it only after the normal observation/restabilization transition.

Runtime process ownership is centralized in `startApp()`. It acquires `.saivage/locks/runtime.lock`, opens the project persistence authority in an explicit mode, and passes that already-open authority into server composition. `createServerServices()` constructs `CardStore` only as an injected façade over the authority's reader and singular writer; server, tools, actors, and recovery cannot reopen card/record persistence from a path. Init and reset open and close the same factory for their command lifetime while retaining the lifecycle lock.
