...
Planner child admission uses the narrow operation-local `CardService.readActivationAdmission()` projection. Its complete read graph is exactly one `listCards(projectRoot)` call → one `readCardIndex(projectRoot)` call → one `scanCardIndex(cardsRoot(projectRoot))` call. Strict parsing and graph validation use that returned current-card set; the service builds one disposable ID map, projects the target plus dependency ID/status rows in declared order, and discards the cards and map with the call. It does not use the general two-scan `CardService.state()`, retain a cache/index, or authorize the later status write. Malformed canonical artifacts and missing declared dependencies fail strictly; only an absent target returns null.

The common planner request order is one canonical scan → disposable ID map → target immediate-parent and exact-`done` dependency validation → `children.get()`. `children.get()` may construct and register a `CardActor`, construct its processor, and call the idle processor actor's `start()`; that lifecycle initialization is not processor activation or provider execution. An admitted new activation continues with `running` status write → exact live-ownership claim → current-leaf transfer → `CardActor` running transition → processor activation/provider work. An admitted running join instead continues with `awaitSettlement()` and its parent `enterChild`/`resumeParent` relationship, without reactivation.
...
Round summarization is the explicit structural exception. Its dedicated provider path persists no invocation or completion rows to the actor conversation, so the synthetic `summary:<round>` invocation session and fresh input UUID remain distinct from the stable actor source session carried by the projection and its rows. Merge summarization constructs same-identity `summary:merge` rows. This exception is a separate non-persisting path, not a caller-selectable bypass of ordinary ownership checks.
...
The strict schema admits only one complete compaction mode: literal `enabled: true`, positive budget, and a structured exact summarizer candidate. `ProviderRegistry` projects implicit account identity by object identity and preserves every explicit account name and slash-bearing model verbatim. Immediately after Registry construction, `createRuntimeApplication()` validates candidate membership exactly once, before constructing routing/invocation services or selecting even an injected runtime factory. It then retains identity only in a fixed one-element summarizer invocation closure and passes an identity-free numeric/fraction/snap policy plus that provider port to required autonomous wiring. `PreparedCompaction`, compactor calls, summarizer prompts, and episode context carry no candidate/model label. Each new autonomous activation builds its exact prompt and ordered tool surface once, then `prepareCompaction()` derives one immutable ephemeral policy and rejects invalid fractions, completion, widths, static trigger capacity, or hard capacity before stabilization and any conversation, summarizer, router, or provider I/O. Autonomous `LLMActor.turn()` accepts only prepared input and runtime-rejects stale/cast unprepared input before transition or persistence.

Process-local availability compares structured candidate triples directly. `/api/providers` has one Registry-backed projection with ordered `{ candidate, state, reason?, untilMs? }` entries; route composition requires that read model and has no config-derived fallback or account-only list. A running server has no disabled status because invalid compaction cannot boot. `init` and `start --create-runtime` do not generate provider/model/compaction configuration. Analyst/manual continuations remain ordinary unprepared calls and are outside this autonomous guarantee.
...
Actor compaction weighting and provider wire admission are distinct contracts. Compactor message estimates use visible row content plus structural characters, with `provider_private` fixed at zero actor weight while the marked visible row is counted and its bundle remains indivisible; rendered-context estimates use UTF-8 bytes; rejection diagnostics retain their independent final-section character estimates. For each provider candidate, the actual request builder maps the projection once, canonicalizes the complete protocol body once, estimates `ceil(UTF8 bytes / 4)`, and hashes those exact bytes. This deterministic best-effort heuristic may skip obvious oversize but is not a fit proof. Admission, retries, and the final transport send reuse that exact serialized body without rebuilding or normalizing it, so Responses wire admission includes private output bytes present in the sent body. Prepared autonomous requests prohibit ordinary `modelParams.maxTokens`; Invocation Service uses only prepared completion tokens for candidate admission and options. Chat emits `max_tokens`, public Responses emits `max_output_tokens`, and Codex emits neither while retaining admission/options authority.

Invocation Service owns one ordinary route pass and keeps existing admission skips and non-context retry/failover behavior. `ConversationLLMActor` retains turn-start append, source consumption, completion persistence, output, state, and tool delivery; its protected provider-call hook is the only seam. Prepared `LLMActor` planner/reviewer/executor turns use that seam to recognize an eligible first-pass `input_context_exhausted` only before any accepted completion/output/tool effect or completion-persistence entry. The rejection ends pass one, the actor consumes its sole allowance, and successful forced compaction precedes exactly one fresh ordinary pass. The second call keeps the same input/source identity and all fields except provider projection, without re-appending turn messages. The actor combines and contiguously reindexes both passes' exchanges under that identity before the existing singular success/final-failure persistence. A second context rejection or any pass-two failure is final; there is no third pass or candidate-route × compaction loop. Non-context behavior is unchanged.

Clean no-smaller and non-cancelled typed summary-construction failure are normal final actor error outcomes: pass-one exchanges attach to one final model issue and no provider retry occurs. Validation/projection-identity, summary-exchange publication, and append failures are fatal local outcomes with no later triggering-input projection or conversation write; cancellation remains cancellation without a model issue. If cancellation follows a successful append, the one canonical row remains. `compaction/summarizer.ts` separately owns summary-attempt evidence: each round/merge call has a `summary:<round-id>`/`summary:merge` session and fresh input UUID, projects settled attempts exactly once through `InvocationService.projectProviderExchanges()` with empty assistant output IDs, and never merges them into the triggering input. Projection failure is outcome-unknown and is not retried. Analyst/manual calls and summarizers are unprepared and receive no last-chance recovery.
...
| `compaction` | `completion_reserve_fraction,enabled,escalate_merge_line_fraction,escalate_summary_line_fraction,input_budget_tokens,merge_line_fraction,snap,summarizer_candidate,summary_line_fraction,trigger_fraction` | `src/agents/config-schema.ts:157` |
| `compaction.summarizer_candidate` | `account,model,provider` | `src/agents/config-schema.ts:151` |
