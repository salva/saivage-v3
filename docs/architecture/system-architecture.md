...
The Analyst is the global card-mutation owner for user-requested changes. Analyst card mutations go through the canonical card application service, which must not start autonomous work directly. Planners may mutate only direct children of the goal they own; they do not directly target ancestors, siblings, unrelated cards, or deeper descendants. Recursive operations such as cancelling or deleting a direct child may affect that child's subtree as a runtime consequence. Cards may be reordered among siblings where supported, but cross-parent movement is not a supported operation.
...
One server-composed `ConversationStore` owns initialization, batch append, summary-cache append, and compaction publication through named synchronous methods. Callers pass domain payloads, never capabilities or callbacks. A new session publishes non-empty `1.jsonl` atomically; compaction publishes non-empty `N+1.jsonl` atomically; later transcript and summary-cache writes append strict envelopes. Compaction rechecks the active numeric version and source-content digest immediately before publishing the next version. Loaded identities, active versions, summary keys, and scoped conversation/`agents` freshness update only after successful durable effects. UI chat and timeline surfaces refetch the active highest validated version; inactive pre-compaction versions are not merged into current operator transcripts, summaries, or last-activity calculations. Active-version row order is semantic transcript order. Provider exchanges remain app-log-backed and outside conversation token budgeting.
...
Each session directory has an exact all-entry inventory: canonical positive numeric `1.jsonl..N.jsonl` and optional `summaries.jsonl`. Every version validates before `N` becomes active. Unknown entries, gaps, alternate names, symlinks, malformed complete content, noncanonical URI encoding, duplicate decoded identities, and wrong owner placement fail without normalization. Session grammar is restricted to current producers. Tombstoned card namespaces are classified first and their retained sessions never enter active discovery. Provider input and operator projections read only `N`; prompt-identity checks may inspect validated numeric history.
...
Recoverable-result body dropping is the only conversation-content removal and happens only inside compaction. Source-recallable results, stash-backed webfetch results, and process-output-backed stdout/stderr results keep pointer data that the model may issue to `read` later. Every summary row includes a deterministic `Recoverable evidence` section generated by the compactor, not by the summarizer. Those pointers are not retention claims: `.saivage/work` is disposable, and stopped-runtime cleanup may remove a referenced stash or process log.

The summary cache stores immutable per-round `summary_text` plus recoverable-evidence descriptors in strict append-only envelopes. Its first entry is atomically published and duplicate cache keys fail. On re-compaction, existing `context_compaction` rows are treated as already-compacted history; they are not re-summarized. The merged summary is rebuilt from cached per-round entries, prior summary rows are replaced rather than duplicated, and the active version maintains exactly-once coverage: each still-relevant round appears once as merged cached summary content, as a per-round compacted summary, or as verbatim tail rows. Version entries record consumed `summary_ids` for audit.
...
Model routing and live availability are separated at the provider boundary. `ModelRouter` resolves the full configured and capability-compatible route order for a role: base model candidates first, then configured equivalent models and failover models, with duplicate concrete candidates emitted only once. It does not filter on live candidate availability. `AgentLlmInvocationGateway` resolves transport configuration fresh for every provider attempt and does not cache credential-bearing provider gateways. Local setup resolution emits typed permanent `local_setup_error` failures before provider I/O: missing providers/accounts, explicit account/provider auth-profile misses, ambiguous implicit profile matches, sanitized auth-profile store read/JSON/schema/IO failures, and OpenAI Codex missing or structurally unusable credentials needed to derive `chatgpt-account-id`. An absent auth-profile store is not a store-load error: explicit profile selections fail as `missing_auth_profile`, implicit optional no-profile resolution may proceed, and implicit required Codex credentials fail as `missing_required_credential`. `InvocationService` owns live availability decisions and the recovery state machine for every LLM invocation, including compaction summarizer invocations: non-rate-limit transient failures retry the same candidate first and block alternates while waiting; rate-limit/`Retry-After` cools that candidate while allowing later currently eligible untried candidates; exhaustion or the fixed two-hour deadline ends recovery. Its composition-owned `CandidateAvailabilityStore` strictly replays one JSONL file after startup discards only an incomplete final tail. Every success/failure append calls the named synchronous availability-store method directly after a fresh application-persistence-health check. The store has no subordinate PID lock, constructor I/O, malformed-row skip, writer lifecycle, or fallback instance. Empty routes, capability incompatibility, local setup errors, and permanent auth/configuration/input/protocol failures fail fast.

`InvocationService` accumulates and indexes provider-exchange records in memory without persisting successful completion metadata ahead of conversation state. The LLM actor first commits its assistant/error/tool-call conversation batch, then asks the invocation service to append provider exchanges independently through the shared `AppLogStore`. Durable identity is `(session_id, source_input_id, attempt_index)` and the app-log id is deterministic. Successful entries carry the already-durable visible assistant/tool-call output id; failed attempts accumulated before a later success are projected in the same later phase.

One composition-owned `AppLogStore` is the only writer for event, error, control-action, provider-exchange, and content-review entries. Its strict discriminated union binds each type directly to a closed payload schema and correlates outer identity/time with payload identity/time. The owning startup reader validates every envelope and payload before exposing typed filtered reads. The first non-empty envelope is atomically published; later envelopes append. Startup truncates only an unterminated final physical suffix and rejects zero-byte or malformed complete state. The former `card_deleted` member, downstream casts/drop filters, app-log lock, generic JSONL ledger writer, event-bus persistence projections, and root-taking append helpers do not exist.

All four growing families—app log, provider availability, conversation versions, and summary cache—use exact newline-terminated `{ version: 1, type: "rows", rows: [...] }` envelopes with non-empty rows and closed durable schemas. The first envelope uses the one-target durable replacement primitive; an existing loaded target is append-only and is never replaced or compacted. Any append write, fsync, or close failure after the first write may have changed the target and terminally reports through the shared `ApplicationPersistenceHealth`; loaded state and freshness remain unchanged. Known pre-publication failures are nonterminal, while indeterminate first publication is terminal. Startup removes only exact owned replacement temporaries and exact empty unpublished sessions, truncates only an incomplete final suffix, and otherwise fails without normalization. The cutover is reset-only.

Runtime lifecycle state, actor cursor snapshots, and outstanding recovery diagnostics are likewise composition-owned stores. `RuntimeStateStore` preserves the version-1 runtime envelope and existing Pause/Resume meanings. The internal `RuntimeControlService` is the sole owner of state persistence and one control-action audit for each current Start/Pause/Resume request; the supervisor exposes lifecycle mechanics, while Analyst, REST, runtime-facade, and CLI adapters delegate without duplicate writes. `ActorSnapshotStore` and `RecoveryDiagnosticsStore` retain their named synchronous mutation contracts. Their readers are strict and side-effect free, and startup restabilization removes only owned durable-replacement temporaries before validation.
...
That same signal object is passed to `RuntimeGate.waitUntilOpen`; gate admission never receives the task-only signal. Ordinary gate closure is reversible Pause. Terminal gate closure rejects current and future waiters, removes their abort listeners, and makes later open attempts fail. After provider I/O returns, the invocation lease and application persistence health are checked before the actor calls the synchronous conversation store. OpenAI Responses private-plus-visible completion rows are one atomic conversation batch. No completion-persistence admission wrapper, release tracker, or parallel commit authority remains.

Durable owners expose direct named synchronous methods with concrete non-Promise returns. One serving composition injects the same process-local `ApplicationPersistenceHealth` into every durable owner and mutation admission boundary. The first uncertain publication outcome permanently closes later durable mutation until restart while reads remain available. Async actors retain local invocation ownership and recheck before delivery; no local owner identity reaches persistence. Ordinary Analyst mutation may await read-only preparation, then performs health, live intervention-readiness, permission, and current-domain rechecks followed by one synchronous named commit and audit without yielding. Preparation modules receive only read facets and cannot import persistence owners, durable writers, runtime/MCP/process mutators, or mutation application services. Named top-level commit operations receive narrow card, configuration, notification, or brief-record services rather than the full tool context. Navigation, reads, and confirmation intents stay outside that mutation runner. The exposed-tool registry classifies every tool and every conditional `webfetch` target/output branch by effect. MCP topology edits and reconcile remain rejection-only, while MCP tool invocation retains its named manager owner and checks persistence health before invocation and after transport delivery.

The runtime application also owns one process-local intervention binding. Start and Resume clear it before autonomous admission. Stopped becomes ready only without a live autonomous activation; Pause closes provider admission immediately but becomes intervention-ready only after admitted mutation-capable work has settled. Public Pause does not await that private settlement. `RuntimeControlService` owns these control-boundary changes without renaming or merging the current lifecycle operations.
...
State remains project-local under `.saivage/`. Card and authored-record truth consists of strict self-contained version artifacts under `.saivage/cards/<cardId>/{card,brief,status,review}/versions/`. One composition-owned `CardStoreRepository` loads the active model at startup and updates affected in-memory card projections only after successful direct writes. There are no card/record indexes, generations, callback request wrappers, capability façades, subordinate locks, queues, or path-based reopening writers. A strict root `tombstone.json` reserves a deleted id and excludes all retained evidence from active consumers. Independent file effects may expose an accepted prefix; no transaction or rollback protocol is implied.

`ResolvedConfigAuthority` is the application owner for the one startup-selected path and frozen interpolation environment. It reads current YAML, applies and validates a named semantic change, then passes the final serialized document to its `ConfigFileStore`; the store has no mutation callback or configuration precondition logic. Auth refresh similarly rechecks the expected profile revision in `replaceRefreshedAuthProfile` and passes one complete final profile document to `AuthProfileRepository`. Analyst MCP desired-config mutation and explicit reconciliation remain rejection-only and reach neither config storage nor MCP reconciliation.

Normal opening performs complete canonical project-root observation, exact temporary/incomplete-prefix cleanup, and strict tree/brief loading before ingress. There is no index rebuilding, replay decision, generation refresh, or callback admission wrapper. Plain start always chooses normal mode. Init, reset, and `start --create-runtime` choose card-store bootstrap only after strict read-only fresh/reset-empty verification; reset retains the lifecycle-lock handle while deleting generated roots and then explicitly bootstraps. Card bootstrap owns only card namespaces. `RuntimeStateStore` is the sole runtime-state writer and initializes the default separately. Fresh-root project identity is separate: only Init creates it through `ProjectIdentityStore` while holding bootstrap-unbound ownership. Server start and reset require bound project identity.

Runtime process ownership is centralized in `startApp()`. It acquires one strict bound `.saivage/locks/runtime.lock` with `O_EXCL`, creates one shared `ApplicationPersistenceHealth`, opens card persistence in an explicit mode, and passes the already-created repository into server composition. Server read models receive only repository readers; planners receive direct-child card operations and record authors receive named record operations. Init and reset enter `withDirectMutationComposition`, hold the lifecycle lock throughout, and use the same owning stores without alternate path writers. Every pre-existing lock blocks; diagnosis never removes it, and binding, endpoint publication, and release verify the exact instance, PID-start, and canonical-root owner identity immediately before changing the lock file.
...
`ServerAvailability` contains API, runtime, MCP, and persistence components. Persistence health is read directly from the shared in-memory owner, not from the app log. Mutation-unhealthy projects return readiness 503, and the same bounded redacted diagnostic flows through runtime/operator availability and `/api/debug/state`; existing app-log timeline/error reads remain usable and no failure-report path recursively logs to the uncertain target.
...
Server composition injects one `ResolvedConfigAuthority` created by `loadEnvironment()` into runtime/Analyst tools, operator config handlers, and `McpManager`. It owns the selected absolute path, selection source, and startup interpolation snapshot. Named synchronous config changes read the latest raw YAML, apply one closed-union change, validate canonical schema and model-role constraints, and durably replace the selected file. There is no mutation callback, promise queue, path-keyed lock, cleanup-on-read, or second writer. Raw-document changes preserve placeholders while effective consumers receive interpolated values.
...
| `runtime` | `continuous_improvement,max_review_retries,process_timeouts` | `src/agents/config-schema.ts:117` |
