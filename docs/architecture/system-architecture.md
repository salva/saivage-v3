...
Each session directory uses a versioned layout: `index.json`, numbered active/frozen `<N>.jsonl` version files, and `summaries.jsonl`. Legacy `seg-NNN.jsonl` conversation segments are not a supported current format. The validated index reader used for prompt identity is read-only; the canonical cleanup-on-read index path separately owns orphan JSONL cleanup. Before the first provider turn, the shared `ConversationLLMActor` boundary applies the exact prompt identity lookup to direct conversation actors and recoverable `LLMActor`s alike. It scans indexed active and frozen versions, so compaction may omit `system_prompt` rows from the active projection without losing that session fact. A same-id wrong-kind row is a fail-fast durable-contract violation, not a writer-level duplicate. Code that builds provider input from disk and operator/UI transcript projections reads only the active version. Frozen versions remain audit and prompt-identity recovery evidence. Id-only append idempotency scans only its active-version target file. Compaction runs at one hook site only: `LLMActor.onBeforeProviderCall`, invoked from the base `_on_enter__calling_provider` path after the actor is entering a provider call and before the request is sent. If compaction runs, it refreshes the in-memory generic provider projection from the new active version's provider-visible rows and rebuilds the active Responses replay projection before the provider call.
...
State remains project-local under `.saivage/`, not under user-global state. Durable card metadata, record slots, card-owned conversations, and card-owned actor cursors live under `.saivage/cards/<cardId>/`. Deleting or archiving a card removes that live card namespace; deleted ids remain reserved only by `.saivage/state/deleted-card-ids.json`, with no restore/archive side-file contract. Durable operator/security inputs include `.saivage/saivage.yaml`, `.saivage/auth-profiles.json`, `.saivage/config/prompts/<cardType>/<role>.md`, `.saivage/skills/index.json`, and `.saivage/instructions/`. Persisted auth profile files, card records, and card history snapshots are strict persisted contracts: unknown or obsolete keys are rejected rather than stripped, and auth profile file `version` is required on read. The persisted runtime-state projection lives under `.saivage/state/runtime.json`; provider availability replay state lives at `.saivage/state/provider-availability.jsonl`; current actor recovery diagnostics live at `.saivage/state/recovery-diagnostics.json`. Event, error, control-action, content-review, and optional deleted-card forensic timelines are logical projections from the single app log at `.saivage/logs/app.jsonl`; blocked-content reviews store only sanitized summaries, never raw blocked content, quarantine metadata, or browseable quarantine paths. Runtime, state, app-log, project-mutation, actor-snapshot, recovery-diagnostics, and provider-availability locks live under `.saivage/locks/`. Disposable operational work lives under `.saivage/work/`, including card tmp files, card-owned process logs at `.saivage/work/cards/<cardId>/processes/<procId>/`, non-card process logs at `.saivage/work/processes/<procId>/`, and oversized webfetch stashes at `.saivage/work/tmp/stash/`. `.saivage/work` may be wiped while the runtime is stopped and is recreated on startup.

Initialization is defined by the current generated layout, not by project identity alone. `.saivage/project.json` without `.saivage/cards/project/`, current state/log/lock/work roots, and default runtime/app-log files is not initialized. `initProjectTree`, `saivage init`, `saivage reset`, and `start --create-runtime` all use that invariant. The initializer preserves durable operator/security inputs, including prompt overrides, and creates missing current generated roots plus the root project card when the card store is absent.

Runtime process ownership is centralized in `startApp()`. It parses only enough input to resolve the project root and `--create-runtime`, acquires `.saivage/locks/runtime.lock` through the canonical runtime-lock helper before any `.saivage` mutation, registers the release with the root resource scope, and only then performs locked `--create-runtime` initialization before config/environment loading and server startup. The runtime-lock layer has one validated payload parser and one lock-state reader used by acquire/status/stale-cleanup/live-holder paths. The reader distinguishes missing, malformed-content, valid, and unreadable states: missing is absent/retryable, malformed content and valid dead-PID locks are removable, valid live-PID locks block regardless of age, and unreadable existing lock files throw without unlinking because ownership is unknown. `createServer()` and `startServer()` remain lock-free composition/listen helpers. `saivage reset` acquires the same lock atomically, holds it while deleting generated roots, `.saivage-work/`, and generated legacy markers (`.saivage/outputs`, `.saivage/views`, plus obsolete internal roots), preserves the held `runtime.lock` while cleaning other lock entries, runs `initProjectTree` under the held lock, and releases it only after reinitialization succeeds or fails.
