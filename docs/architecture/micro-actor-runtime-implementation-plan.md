...
- Notification delivery uses the card-owned `deliverNotificationsForInput(inputId)` contract at safe pre-provider-call points. Terminal `emit_result` validation does not sample pending notification state, and successful terminal settlement is not deferred solely because notifications are pending (see [P3](#p3-cardactor-owns-authoritative-cancellation-and-activation-id-settlement)).
...
- Terminal contract handling uses the contract repair loop (`runContractRepairLoop`) shared by the planner, reviewer-inner, and executor processors — this is accepted current architecture, not pending work. Correctable terminal-contract failures are unbounded inside the activation: they are always returned to the model as tool results or repair directives, and the loop terminates on a valid terminal, a non-repairable `error` outcome, or activation abort.
...
- A no-progress/stuck-loop policy for repeated identical terminal-contract failures remains a deferred follow-up. The current repair loop has no count budget; natural bounds are activation abort, provider limits, and operator cancellation.
...
- On `done` activation settlement, terminal validation must not defer the report merely because undelivered main-agent notifications are pending. Notifications are delivered only while constructing provider input: initial planner/executor turns, non-terminal tool continuations, failed terminal `emit_result` repair continuations, and plain-text repair continuations. If validation succeeds with no further provider call, pending notifications remain queued for a future safe delivery point. `CardActor` never flips `done` to `changed` because of pending notifications. `changed` is exclusively an edit/subtree-mutation signal.
...
- Pending notifications are delivered at the next safe planner provider-input point, including initial turns, non-terminal tool continuations, failed terminal repair continuations, and plain-text repair continuations.
...
Current status: implemented for the current actor path. Reviewer execution is planner-owned and contract-terminal-only, self-citation without durable evidence is rejected, and corrections return planner rework feedback. Reviewer turns cannot drain main-agent notifications; queued main-agent notifications remain for later planner/executor delivery. Reviewer approval currentness is based on actual assessed card/subtree/record changes, not on pending notification state alone (see [P5](#p5-reviewer-cannot-reach-main-agent-notification-delivery)).
...
- If relevant card/subtree/record changes arrive while reviewing, invalidate reviewer success or divert back to planning. Pending main-agent notification state alone is not reviewer-success invalidation.
...
**Problem.** The earlier shell-leader model could lose control when a shell exited before its background descendants. The current model instead retains an in-memory POSIX process-group control record for every managed launch. Scoped termination remains `stopByOwner` for activation/session cleanup and `stopRuntimeOwned` for internal shutdown; do not add a second process-scope taxonomy unless a future multi-project runtime deliberately changes the one-active-project invariant.
...
- A live process group is the control handle even after its direct child exits. `stopProcess`, `stopByOwner`, and `stopRuntimeOwned` signal negative PGID with TERM, wait for absence, then KILL stragglers. Process records settle only after absence; there is no durable reattachment after runtime restart.
- `wait()` uses the shared group settlement monitor. A zero timeout inspects liveness, a positive deadline returns the running view, and only group absence returns a terminal result.
...
- Terminal-report validation in the contract repair loop (`onTerminalTool` handler shared by planner and executor processors) must not defer a `done` report merely because undelivered main-agent notifications are pending. That earlier terminal-time deferral statement is stale. The current safe-point contract is: terminal validation validates the terminal payload, required records, completion gates, and reviewer rework only; it does not sample pendingness and does not add notification text to failed terminal repair errors. Pending notifications are appended only while preparing provider input for the next planner/executor call, including failed terminal `emit_result` repair continuations and plain-text repair continuations. If terminal validation succeeds and no next provider call occurs, notifications remain queued for a future safe delivery point. `CardActor.commitOutcome()` therefore never needs to flip `done` to `changed` — it only commits clean outcomes. Remove `reopenDoneWithPendingNotifications()` and the `_on_enter__done`/`_on_recover__done` pair that exists only to compensate for it. Notifications arriving on an already-inactive `done` card after settlement are queued for future delivery; they do not mutate lifecycle state. `changed` is produced only by card edits/subtree mutations, never by notification delivery.
...
Acceptance: late provider/tool/process/child outcomes cannot mutate a cancelled card's lifecycle; running descendants are cancelled through their live `CardActor.cancel()`; `cancelled` is a first-class parent-visible activation outcome; pending main-agent notifications at terminal-report time are not a terminal validation gate and are delivered only at the next safe provider-input point; `reopenDone` and its compensating recover hook are gone; `cancelled` is terminal in the lifecycle state machine (no cancelled→backlog/changed transition, no propagation flip); recovery removes all actor snapshots for cards whose durable status is `cancelled`, so a crash during running cancellation does not leave stale active snapshots or outstanding diagnostics.
...
Acceptance: reviewer code has no method capable of draining planner/main-agent notifications; planner/executor delivery still drains exactly once and records markers at safe pre-provider-call points; terminal validation does not sample notification pendingness; no new delivery-policy abstraction is introduced.
