...
The composition projector is the single request-composition boundary. Its provider conversion places one concise system provenance boundary after prepared blocks and before any historical/current conversation suffix; only an accumulated summary receives the additional `Historical summary:` label. At the direct canonical branch, successful current process results are strictly accepted and copied for the primary audience: each done+complete stream loses only its URL and every successful copy is canonically serialized with its copied hash updated. Prepared blocks and retained instructions are unchanged. Summarizer requests instead retain the exact durable URL-bearing process bytes as source alongside prepared blocks, inherited history, and other new source. This distinguishes synthesized history from current-node authority without demoting applicable owner requirements or claiming that replay proves execution, transition, or approval. Chat, Responses, and Codex serialize the same ordered primary items; adapters add no authority rules. Summarizer guidance preserves attribution, actual work and decisions, applicable requirements, unresolved uncertainty and evidence references; it distinguishes proposed from executed and draft from accepted/approved, and does not treat a final successful newline-separated shell command as proof that earlier commands passed (`pipefail` concerns pipelines).
...
Each shipped template retains one shared Planner system-prompt artifact selected for both `project` and `goal`. Its prose interprets the separately supplied frozen `cardType`: project Planner owns coherent strategy through complete evidenced acceptance, while goal Planner owns local decomposition, coordination, and repair within its delegated outcome. The distinction is not a workflow-agent placeholder—`contractDescription` remains that host's only value—and needs no card-specific Planner artifact, specialized fragment, selector, role classifier, or agent. Typed review-to-plan remains the concrete accepted-review revision handoff. Typed architecture uses the shared Reviewer with node-owned component and system scope; immutable versioned review URLs let each Reviewer node start a clean cycle of the same record without a specialized agent template.

Shared planning policy now has one role-level home for strategy and remedy judgment, complete direct-child inspection and reuse, lifecycle repair order, metadata and capability boundaries, and operation-result handling. Typed plan assesses current facts and chooses an outcome; typed recover rebuilds that assessment from durable facts without reconstructing discarded graph position. They no longer restate generic child-management recipes.

Accepted-review notification handling deliberately retains two complementary artifacts in the unchanged compiled closure. The durable `review-to-notifications` edge owns historical framing and carries accepted summary and immutable versioned review evidence, not a rejection. The separately prepared `handle-notifications` destination owns current action, mandatory status update, and re-review; its body is not part of transition context. Shared Reviewer owns independent inspection and representative-family method, while specialized planning and architecture nodes own subtree, component, or system scope.

The shared Planner prompt also owns prompt-level distinctions among bounded research delivery, implementation, evidence promotion, and project acceptance. It preserves required scope while removing only locally invented sequencing and requires a material input, decision, correction, or different approach before retrying the same blocker. Root acts only on its own direct goals and leaves grandchild repair to the goal Planner; a deliberately restricted named-agent configuration remains a real capability boundary, not permission to bypass it. Typed planning and recovery apply only current-node assessment and recovery deltas. The shared Executor owns branch-by-branch reference/specification comparison, owning-primitive repair, verification interpretation, and evidence reuse: later successful invocations do not erase earlier failed or unresolved checks, and evidence remains reusable only while relevant inputs are unchanged and it applies to current criteria. Typed retry exhaustion, verify evidence reuse, and research discriminator rules remain process-node policy at their existing outcome owners. These are composed instruction policies, not runtime gates, workflow state, or new admission mechanisms.
