...
- Pause support may be centralized as a small runtime composition primitive (the `RuntimeGate` service) for work that explicitly waits on the gate before starting new external work (new provider calls, new process spawns, new card dispatch). The gate is NOT part of the micro-actor framework (`BaseActor`/`micro-actor.ts`); it is a composition-root service injected into the actors that need it. Do not implement pause by globally suppressing `_on_enter__{state}`: that creates half-entered states and makes recovery ambiguous. The gate never blocks completion settlement of already-admitted work — tool results, child outcomes, and provider responses must be delivered even while paused; only follow-up autonomous side effects wait at the gate.
...
- `paused`: parked; root project may exist, but the runtime gate is closed. No new provider calls, runtime-owned process spawns, or card/processor dispatches are admitted (enforced by `RuntimeGate`, not by the supervisor actor itself). Already-admitted work may still persist facts and settle to durable boundaries.
...
| pause runtime | `RuntimeSupervisorActor.pause()` plus runtime gate close | Closes the global scheduling/delivery gate; does not mutate card statuses. |
| resume runtime | `RuntimeSupervisorActor.run()` plus runtime gate open | Reopens the gate from paused so waiters blocked at provider/spawn/dispatch seams proceed in normal actor order. |
...
- No new provider call, runtime-owned process spawn, child dispatch, or root card dispatch starts while paused.
- Completion facts produced by already-admitted work may persist and settle to durable boundaries while paused. If follow-up autonomous work would start, it waits at the provider/spawn/dispatch gate before doing so.
