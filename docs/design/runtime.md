...
6. **Crash recovery**: Read `.saivage/tmp/state/runtime.json`. If a
   ...
   authoritative file exists, `src/runtime/state.ts#symbol:readRuntimeState` migrates it
   once; if both files exist, `src/runtime/state.ts#symbol:assertNoMixedRuntimeStateLayout` refuses the
...
- idle runtime states with `current_card_id === null` cannot retain a
  ...
  migration/refusal is anchored by `src/runtime/state.ts#symbol:runtimeStatePath` and
...
every significant state change. `src/runtime/state.ts#symbol:runtimeStatePath` defines
that path, and `src/runtime/state.ts#symbol:initRuntimeState` /
`src/runtime/state.ts#symbol:saveRuntimeState` are the canonical init/save writers:
...
(`src/runtime/state.ts#symbol:readRuntimeState`); if both paths exist, runtime-state
helpers throw `RuntimeStateLayoutError` instead of choosing one
(`src/runtime/state.ts#symbol:RuntimeStateLayoutError`, `src/runtime/state.ts#symbol:assertNoMixedRuntimeStateLayout`). After server restart/reload, `/api/state` must
