...
[src/agents/agent-adapter.ts](../../../../src/agents/agent-adapter.ts#L211)
...
1. Resolves the candidate chain via the model router and creates a persisted
   ...
   [agent-adapter.ts#L225-L237](../../../../src/agents/agent-adapter.ts#L225-L237)).
...
3. For each candidate, it enters an inner `for (;;)` loop ("same-candidate
   ...
   ([agent-adapter.ts#L275-L388](../../../../src/agents/agent-adapter.ts#L275-L388)).
4. Each turn calls `buildLlmOptions(role, 'tools', turnTools, ...)`
   ([agent-adapter.ts#L284](../../../../src/agents/agent-adapter.ts#L284))
...
- **Plain message branch**
  ([agent-adapter.ts#L287-L320](../../../../src/agents/agent-adapter.ts#L287-L320)).
  Since commit `a2a6f05`, a `result.kind === 'message'` no longer kills the
  candidate. The adapter appends the assistant text and a `system /
  model_repair` row containing a hand-written nudge string:
...
- **Terminal-tool branch**
  ([agent-adapter.ts#L335-L350](../../../../src/agents/agent-adapter.ts#L335-L350)).
  ...
  ([agent-adapter.ts#L340-L344](../../../../src/agents/agent-adapter.ts#L340-L344)).

- **Non-terminal tool-call branch**
  ([agent-adapter.ts#L353-L382](../../../../src/agents/agent-adapter.ts#L353-L382)).
...
([agent-adapter.ts#L386-L388](../../../../src/agents/agent-adapter.ts#L386-L388)):
...
([agent-adapter.ts#L407-L440](../../../../src/agents/agent-adapter.ts#L407-L440)),
...
([invocation-recovery-policy.ts#L131-L137](../../../../src/agents/invocation-recovery-policy.ts#L131-L137))
it returns:
...
[agent-adapter.ts#L435-L440](../../../../src/agents/agent-adapter.ts#L435-L440):
...
`token_budget_exceeded`, `parse_error`, `cancelled`, `unknown`
([invocation-recovery-policy.ts#L100-L155](../../../../src/agents/invocation-recovery-policy.ts#L100-L155)).
...
pre-`a2a6f05` adapter threw `terminal_tool_missing` from the post-loop path.
With the mitigation in place, the plain-message branch
([agent-adapter.ts#L287-L320](../../../../src/agents/agent-adapter.ts#L287-L320))
...
`activate_card` ([agent-adapter.ts#L353-L382](../../../../src/agents/agent-adapter.ts#L353-L382))
...
([llm-failure.ts#L8-L18](../../../../src/agents/llm-failure.ts#L8-L18))
that mixes:
...
- `OpenCodeGoClassifier`
  ([llm-failure-classifiers.ts#L86-L110](../../../../src/agents/llm-failure-classifiers.ts#L86-L110))
...
Commit `a2a6f05` inlined the plain-message repair directly into the inner
turn loop ([agent-adapter.ts#L287-L320](../../../../src/agents/agent-adapter.ts#L287-L320)):
...
`src/schemas/types.ts` is consumed nowhere. The mitigation does, badly,
exactly the work the redesign needs to do well.
...
- The message uses a single message kind (the `model_repair` kind already
  present in `src/schemas/types.ts`, but now produced *only* by the
  verifier).
...
- `MessageKind` in `src/schemas/types.ts` already carries `model_repair`,
  `model_issue`, and `model_recovered`. The repair message kind must be
  emitted by the verifier and only by the verifier. The `model_recovered`
  directive currently emitted from
  [agent-adapter.ts#L249-L254](../../../../src/agents/agent-adapter.ts#L249-L254)
...
- `envelopeToPlannerResult / envelopeToExecutorResult /
  envelopeToReviewerResult` ([agent-adapter.ts#L48-L62](../../../../src/agents/agent-adapter.ts#L48-L62))
...
- `TERMINAL_TOOL_NAMES` duplicated in `src/contracts/llm-exchange.ts` must
  be removed or replaced with whatever name the done signal takes; no
...
- `sanitizeRecoveryMessage`
  ([invocation-recovery-policy.ts#L60-L73](../../../../src/agents/invocation-recovery-policy.ts#L60-L73))
...
- `invokeWithRecovery` ([recovery.ts#L93-L177](../../../../src/agents/recovery.ts#L93-L177))
...
- The repair loop lives **inside** `agentFn`, on the same candidate, and
  consumes its own repair budget. Repair-budget exhaustion converts to a
  transport-layer failure (a new `no_progress` or `repair_exhausted`
  kind) so the existing failover behaviour stays intact.

### 3.5 Skills, system prompts, and self-checks

- `AgentRoleRunner.applySelfCheck`
  ([agent-adapter.ts#L168-L170](../../../../src/agents/agent-adapter.ts#L168-L170))
...
4. **What happens when repair fails N times in a row with the *same*
   obligation report?** Options: escalate to transport-layer failover,
   abort the invocation, demote the verifier check (last-resort
   acceptance), or rotate to a different candidate carrying the repair
   history. The design must pick one and articulate the criterion.
