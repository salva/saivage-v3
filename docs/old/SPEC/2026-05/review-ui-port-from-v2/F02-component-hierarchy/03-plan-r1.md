# F02 — Component hierarchy / UI primitive layer — Implementation plan (r1)

Writer round 1 of the implementation phase. Translates the approved
design [02-design-r3.md](02-design-r3.md) (DESIGN-APPROVED) into a
concrete, commit-by-commit checklist with verification gates,
validation commands, and rollback. Inputs:
...
compatibility. Every bespoke v3 selector enumerated in analysis §4
is **deleted in the same commit** that introduces its replacement
primitive. No `@deprecated` re-exports, no `.legacy-*` holdovers,
no alias period, no `index.ts` barrels under `ui/` / `content/` /
...
| `web/package.json` | `"sideEffects": ["./web/src/utils/tool-presenters/**"]` per F05 r3 §3.4 | C4 |
...
| `web/src/components/layout/AppShell.vue` | auth banner rewrite onto `<Card tone="danger">` + `<Button>`; remove `data-modal-open` shortcut suppression already added by C2 | C7 |
...
| `web/src/components/nav/NavRail.vue` | `<Button icon-only>` for `.api-token-btn` and surface-local layout styles | C15 |
...
! rg -n "from\s+['\"](.*/components/(ui|content|conversation)/)" web/src/components/ui/{Pill,Card,PanelHeading,StatusDot,Overlay}.vue
# Spinner is the ONLY ui/ file allowed to import lucide-vue-next.
rg -l "lucide-vue-next" web/src/components/ui | tee /tmp/f02-c2-lucide.txt
diff <(sort /tmp/f02-c2-lucide.txt) <(echo "web/src/components/ui/Spinner.vue") || { echo "Lucide leak in ui/"; exit 1; }
...
**Modifies:** `web/package.json` adds
`"sideEffects": ["./web/src/utils/tool-presenters/**"]`. Every
consumer import of `'../../utils/tool-presenters'` resolves to the
new directory (no path change at the call site, since the
analysis-canonical path was always the directory).
...
rg -n '"sideEffects"\s*:' web/package.json
npx vue-tsc --noEmit
npm run lint
...
```bash
# No consumer imports the deleted single file.
...
[ ! -s /tmp/f02-c4-leaks.txt ] || { echo "Unexpected import shapes in /tmp/f02-c4-leaks.txt"; cat /tmp/f02-c4-leaks.txt; exit 1; }
# barrel-integrity test passes (every file in the directory is reachable from index.ts).
npm test -- --run utils/tool-presenters/barrel-integrity
# registry.test passes (default + every EXPECTED_TOOL_NAMES is registered after one barrel import).
npm test -- --run utils/tool-presenters/registry
# InlineParts template uses <router-link>/<a> but the SFC does NOT import vue-router.
rg -n "from\s+['\"]vue-router['\"]" web/src/components/content/InlineParts.vue && exit 1 || true
...
### C6 — `auth/ApiTokenEntry.vue` rewrite on `Overlay` + `Button`

**Adds:** updated SFC; no new tests beyond rewrite of
[api-token-entry.test.ts](../../../../web/src/__tests__/api-token-entry.test.ts)
per analysis §5.2.

**Modifies:** `auth/ApiTokenEntry.vue`, `nav/NavRail.vue` (only the
`.api-token-btn` line; the rest of NavRail is deferred to C15),
...
rg -n "document.body.dataset.modalOpen|modalOpen" web/src/components/layout/AppShell.vue web/src/components/ui/Overlay.vue
```

### C7 — `layout/AppShell.vue` auth banner + `layout/WorkspaceHeader.vue` chip cluster

**Modifies:** `layout/AppShell.vue` (auth-required banner now
`<Card tone="danger" role="alert">` + `<Pill tone="danger">` +
`<Button>` + icon-only dismiss `<Button>` per analysis §6.1),
`layout/WorkspaceHeader.vue` (chip cluster now `<Pill>` + `<StatusDot>`
siblings per analysis §4.3), `app-shell-auth-banner.test.ts` and
`workspace-header.test.ts` selector rewrites.
...
itself, which survives as a layout-only class).
...
#     One-shot grep across all .vue files; the gate fails on any hit in non-styles/* SFCs that contain
#     a color/background-color/border-color/border-radius rule.
rg -n --multiline -U \
...
# (k) `data-modal-open` shortcut suppression is single-sourced in Overlay.vue
...
Per project guideline (architecture-first, no backward
...
the deleted selectors.

Per-commit rollback notes:

| Commit | Rollback shape |
| --- | --- |
| C1 | `git revert` is safe; only the new SFC, its test, and one ESLint block are affected. |
| C2 | `git revert` is safe; SFCs and tests are additive. If the F02 r2 extension rules were appended to `patterns.css` in this commit, the revert removes them — C8/C10/C14 will then fail their visual checks if they were ever merged ahead. Recovery: re-land C2. |
...
| **C4** | `git revert` restores `web/src/utils/tool-presenters.ts` (single file) and removes the directory. The new content/ SFCs (`JsonView`, `FormattedContent`, `InlineParts`) and `json-tokenize.ts` are reverted in the same operation. **C5 must be reverted FIRST if C5 already merged**, because C5 consumes the directory barrel. |
| **C5** | `git revert` restores in-line `.tool-chip*` in `AnalystChatPanel.vue` and removes the new `conversation/*` SFCs + the adapter + the pairing utility. Selector-migration tests revert with them. Because C5 is the cross-batch boundary co-owned with the F03 PR, the revert MUST be coordinated with F03's owner. |
| C6–C15 | `git revert` of each is safe and idempotent; each commit is bounded to one surface or one auth/overlay scope. The deleted selectors come back with the revert. After the revert, surface tests pass against the restored selectors. |

**No partial rollback (alias period) is permitted.** If a commit
must be rolled back, every consumer change inside that commit is
rolled back atomically.

Recovery procedure (after a revert):

1. `git revert <sha>` (single commit) or `git revert -m 1 <merge-sha>`
   (if merged via merge commit).
2. Push the revert branch, open a PR, get one approving review.
3. Re-land the commit with the fix in a new SHA.
...
| R7 | The auth-shortcut suppression flag (`data-modal-open`) could be left set if Overlay crashes mid-open. | Low | Overlay's `onMounted`/`onUnmounted` symmetrically increment/decrement the module-level counter (design §9.3). Vitest covers crash-on-open by simulating the watch-side error path. |
...
[ ] C4  utils/tool-presenters/ directory + content/{JsonView,FormattedContent,InlineParts} + json-tokenize; delete utils/tool-presenters.ts
...
[ ] C6  auth/ApiTokenEntry.vue rewrite on Overlay + Button
[ ] C7  layout/{AppShell,WorkspaceHeader}.vue rewrites
[ ] C8  views/DashboardView.vue rewrite
[ ] C9  views/FilesView.vue rewrite
[ ] C10 views/DebugView.vue rewrite
