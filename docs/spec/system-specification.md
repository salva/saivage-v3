...
Planner sessions are goal-lived. A goal planner uses deterministic identity derived from the goal card, is created lazily the first time the goal needs an LLM agent, and receives multiple activation requests over that goal's lifetime as the same logical agent session. When reactivated through normal card activation after safe recovery, card processors load the persisted conversation prefix for the session, append a non-provider-visible activation marker, persist the current turn's runtime-provided context rows, and then call the provider with the persisted prefix plus those newly persisted rows. Runtime-provided planner state, notifications, continuation-hook directives, reviewer descendant context, and reviewer currentness context are transcript rows, not ephemeral side channels. Mid-flight restart recovery is implemented through the root-card cascade: interrupted running cards are reached through replayed `activate_card` calls, and their in-flight provider calls or waiting tool calls are reissued or replayed when each reached processor lazily recovers its LLM without constructing a fresh input on recovery branches. This recovery behavior is orthogonal to planner session identity. Future implementations may unload dormant planner agents from memory and recover them from durable session/card storage; that is not a user-facing restart/reset capability.
