...
Classic is unchanged: `project` and `goal` independently use `plan -> optional review -> plan` with `recover` for STOPPED, while the other seven types each use one `execute` node.
...
Default Planner has the exact 20-tool inventory, Reviewer the 12-tool inventory without MCP, Executor the 17-tool inventory with unrestricted configured MCP invocation, and Analyst the 43-tool inventory.
...
`edit_card` is only for a genuine immediate-child metadata change to `title`, `tags`, `priority`, `urgency`, or `related`; it cannot edit a brief, dependency, parent, type, or lifecycle and is not a generic reopen.
Planner has no `reopen_card`.
...
Analyst reopening changes eligible work to CHANGED; a BLOCKED child normally needs only Planner notification and activation.
...
Accepted transition context carries immutable `record:///review.md?card=<id>&v=N` evidence before the next edge/node prompt, so component evidence remains readable after system review opens a newer clean version and revision returns to `draft`.
...
Planner activation and cancellation delegate through a port bound to the exact parent owner.
...
Card stream format v3 keeps the newline envelope at `version: 1`, `type: 'rows'`, and accepts only `card-version` and `card-tombstone` rows with `format_version: 2`.
Artifact row format 1, records with durable `children`, and streams containing more than one artifact format fail strict reads.
...
Every durable snapshot has one status authority at `lifecycle.status` and contains no top-level `status`, persisted `parent`, persisted `depth`, or `allowedActions`.
...
It admits no further activation; an accepted Analyst record mutation or explicit `reopen_card` reopens it to changed, while Planner `edit_card` rejects it, and deletion and reorder retain their separate operation rules.
...
An accepted Analyst record mutation, explicit `reopen_card`, or real immediate-child Planner metadata delta reopens it to changed; an equal Planner patch does not.
...
Update provenance is fixed planner/runtime, delete is Analyst/runtime, and every other family is runtime/runtime.
Planner edit changes only title, tags, priority, urgency, or related.
...
At node entry the owner appends named-agent context, then immediate lifecycle/edge transition context when present, then the node prompt.
...
The conversation LLM actor separately owns nullable process-local compaction progress `{strategy,startedAt,foldsDone,foldInFlight}`. It begins at zero/false for each preventive, local-exact-admission, or authoritative-recovery compaction, including zero-call structural work. `foldInFlight` becomes true only after a fold is admitted immediately before logical invocation; validated success increments `foldsDone` and clears it. Transient provider attempts do not increment the count. Ordinary completion, failure, cancellation, or disposal clears only current ownership. Publication-outcome uncertainty is delivered to the fatal owner first and permits no progress clear or later notification.
...
Application teardown instead closes admission, directly disposes the session and Conversation LLM owner, revokes or aborts work that does not yet own completion, and joins owned completion.
...
Disposal while merely parked rejects later result admission; disposal after nested continuation admission follows that invocation's existing owned-completion rules.
...
Disposal cannot replace entered completion, evidence, handoff, required notice, or their exact failure; only later optional runtime/read-model projection is suppressible.
...
Ordinary web content fetches read through the bounded body path and retain the configured byte limit, text inference, binary metadata, save, and stash behavior.
...
Every successful metadata-only, binary, inline, stash, save, redirect-final, and Analyst brief-write result contains only `redacted_url`, never a raw `url`; durable call rows follow that same redaction.
Status, selected headers, byte/content policy, and canonical stash/save/write identities remain unchanged.
...
Configuration requires literal `enabled: true`, a positive route-independent `input_budget_tokens`, and a strict structured `summarizer_candidate` with exact non-empty `provider` and `model` plus required nullable `account`.
...
`composeContextProjection` is the singular primary selection and ordering owner. It receives the prepared dynamic blocks explicitly (there is no empty default), selects them once in prepared order, then emits the accumulated historical summary when present, the existing newest-only recovery/refusal facts, and the uncovered canonical suffix in canonical order.
...
Specialized process prompts remain startup-compiled **dynamic process context** delivered at node-entry, transition, correction, and recovery positions; they are never part of `instructionText` or prefix bytes.
...
Canonical conversation rows carry one exhaustive, mutually exclusive row policy.
`text` and `model_repair` are direct primary-and-summarizer content; `content_policy_retry` is code-owned retry text; `activity` is an activation boundary omitted from primary and summarizer prose; `model_issue` is structural provider failure whose following recovery notice owns semantics; `model_recovered` is a model-facing synthetic system notice, never evidence-only; `content_policy_refusal` renders fixed synthetic refusal text and never replays the raw provider response; `provider_private` is selectable only with its one marked visible mate; `tool_call` and `tool_result` form one bundle projecting the pre-execution call template and the settled pair.
...
Diagnostic, control-action, process, agent-session, and MCP tool result bodies remain arbitrary and are bounded only when exact compaction materialization and request admission admit a subsequent provider request.

Preparation validates positive capacities, `0 <= tail_fraction <= trigger_fraction`, `trigger_fraction + completion_reserve_fraction <= 1`, a completion reserve of at least 2,000 tokens, requested primary completion not exceeding that reserve, and invariant static-token trigger/hard capacity. The sole tail fraction defaults to `0.25`; the removed merge/summary and escalation fractions are invalid unknown keys, not aliases.
The card dynamic block is exactly one system-role activation-local block containing canonical `{cardId,cardType,title,brief}` JSON, where `brief` is the complete accepted configured bootstrap content. The Analyst dynamic block remains its existing bounded project orientation. Both are prepared before ingress, frozen through initial calls, tool and notification continuations, plain repair, pinned retry, preventive/local/authoritative compaction projection, and the one authoritative retry, and are never reconstructed by rereading current card/tree state. Tool effects, canonical results, and broadcasts remain outside retry seams. A later card-node activation or Analyst submission prepares anew; confirmed restart remains model-free.
...
Selected-config workflow compilation derives the distinct participant set—the configured global Analyst agent plus every named node agent in every selected compiled graph, including specialized-card-type nodes—and requires each participant's selected route `max_tokens` to be no greater than `floor(input_budget_tokens * completion_reserve_fraction)`.
Requested values remain exact and are never clamped.
Every offending `agents.<name>.model_route` is reported in one aggregate diagnostic, and an impossible relationship fails selected-config loading before runtime composition or generated root-card access.
Configured agents unused by the selected compiled graphs are exempt from this participant rule.
...
The successor segment's compacted genesis is self-contained and carries the accumulated history: the accumulated summary text, a fixed-size coverage commitment (source session/version, covered-through message ID, covered source-group hash, accumulated-summary hash), a fixed-size disposition commitment (counts of summarized, evidence-only, and superseded groups), exactly two nullable bounded required-model-fact slots (latest recovery notice and latest covered refusal), and an explicit continuation that is `between_rounds` or `inherited_open_round` carrying only the validated marker ID, canonical input UUID, and active initial/repair segment kind.
...
Compacted genesis is the sole bounded prior-history input to the next compaction: generation N+1 merges the inherited accumulated summary before newly selected summary/evidence groups and never opens a predecessor segment for current projection.
...
Before construction, compaction selects at most two distinct positive increasing safe endpoints: the preamble and closed rounds outside the backward `floor(input_budget_tokens * tail_fraction)` tail, using configured snap behavior, and the furthest legal closed/open atomic fallback. Empty inherited rounds contribute no endpoint. Equal, absent, and zero endpoints are omitted; no all-cutoff array or candidate memo exists.
Preventive and authoritative compaction construct the base first and stop immediately when it is accepted; only then may they construct the fallback. Preventive accepts an actual composed estimate at or below its trigger and otherwise throws the terminal preventive-capacity error, including when no endpoint exists. Authoritative recovery accepts only a projection strictly smaller than the rejected estimate and otherwise returns `no_smaller_projection`. Local exact admission constructs every selected endpoint, requires all demanded construction to succeed, and chooses the smallest actual composed byte estimate strictly below the rejected projection, tying toward the furthest endpoint; otherwise it returns `no_smaller_projection` and the actor retains exact local-admission diagnostics with `local_compaction_attempted:true`.

Every refine request contains, in order, the dedicated replacement-summary instruction, every selected frozen prepared current block as full read-only orientation, the genuine accumulated history when present, and labeled new source. Static agent instruction is not duplicated. Tool arguments and results are separate ordered source components; evidence-only content uses its existing evidence projection, and private result bytes never enter summary source.
The actual-use packer assigns each immutable projected component exact UTF-8 byte ranges at code-point boundaries, total bytes, source hash, and `omitted_source_bytes=0`; empty components carry an explicit empty range. It first exact-admits the next code point or empty component, tries the whole remainder, and after whole rejection grows prefixes by doubling until the first rejected probe. A rejected next-component minimum flushes already admitted material. Sent ranges reconstruct every projected component exactly once without gaps or overlaps, and each admitted serialized body/hash is retained unchanged for send.
Every probe includes actual wrappers, escaping, labels, prepared orientation, genuine accumulator, and requested output. With fixed candidate context `C`, the fixed 2,000-token output request is admitted only when estimated input plus 2,000 is at most both the input budget and `floor(0.8*C)`, in addition to generic declared capability limits. Prepared orientation and inherited summary are never split, trimmed, or hypothetically reserved.
Cancellation is observed with its exact reason before `compact()` reads or prepares source and before each accumulator advance validates its cutoff, slices or projects source, or begins packing. One `compact()` permits at most 16 logical refine invocations across both endpoints. Immediately before each invocation it checks abort, checks the ceiling, increments, then sends; a needed seventeenth call is not made and no successor publishes. Ordinary Invocation Service transient attempts remain internal to one logical invocation.
Every strictly increasing advance computes its next summary, inherited-fact fold markers, and cutoff transactionally and commits that call-local state only after the entire advance succeeds. After each successfully validated fold is recorded complete, cancellation is observed again before packing resumes.
Failure leaves the preceding committed advance state unchanged and stops the `compact()` operation; there is no later candidate work or retry. Invocation and progress accounting is not committed advance state and is not rolled back: admitted invocations and started folds remain counted, while only successfully validated folds are recorded complete.
...
Refine calls execute sequentially, at most one summarizer provider call is in flight per `compact()` operation, and abort or the first failure stops every later admission.
...
constant.summarizer-output-max-bytes = {"unit":"bytes","value":12000}
...
The byte heuristic is deterministic and best-effort, not a fit proof.
...
`admitted` requires conjunctively capability support, declared context/output limits, prepared completion-reserve compatibility, candidate output capacity, and exact estimated input plus requested completion fitting both the prepared input budget and the candidate context window.
A failed capability match, undeclared required limit, reserve/output incompatibility, or insufficient output maximum is non-size-fixable `candidate_ineligible` carrying only the bounded canonical `CapabilitySkipReason` enum reasons; a capability-and-output-compatible candidate whose exact serialized projection violates the local input budget or context window is `projection_too_large`.
...
A clean `no_smaller_projection` result appends no compaction, performs no second provider pass, and produces the ordinary final model-issue/provider-exchange failure for the first rejection.
Direct summary-result validation owns `SummaryResultValidationError`; at the compactor boundary, and only there, that exact error from transport-successful summary output containing tool calls, empty text, the prohibited recoverable-evidence section, or more than 12,000 UTF-8 bytes after trimming becomes `CompactionSummaryConstructionError` with the validation error as its exact `cause`. `SummaryConstructionLimitError` has only `request_context_capacity` and `fold_limit` reasons plus numeric counts/limits and is wrapped at that same boundary. Provider, abort, invariant, evidence, and publication failures retain their existing owners.
Preventive summary provider exhaustion remains the exact Invocation Service `ProviderTurnFailure`; abort remains its exact reason; summary-exchange projection/publication, publication-unknown, source/projection/hash/invariant, and append failures retain their exact owning identities.
...
If summary recovery exhausts with `ProviderTurnFailure` after summary attempts were published, the actor validates and publishes the original triggering planner context-failure attempts exactly once under the planner input, with null terminal conversation output and no assistant output.
Publication failure or uncertainty remains authoritative and creates no marker.
Only after successful triggering-attempt publication does the actor throw fieldless `LastChanceSummaryProviderUnavailableError`, whose sanitized message and exact summary `ProviderTurnFailure` cause prevent generic rejection from reidentifying or republishing summary attempts or fabricating a planner `model_issue`.
...
Historical Analyst list arrays, bounded session wrappers, and earlier `emit_result` data therefore remain valid because they satisfy the current `unknown` field, not because of a compatibility reader or legacy-shape branch.
...
Every other admitted `work:///` text preview, including process logs and webfetch stashes, receives text defense and sensitive-redacted metadata; normal denial, containment, binary, size, listing, and card behavior is unchanged.
