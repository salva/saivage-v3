...
The Analyst may inspect process state and terminate processes owned by its current session when canonical process control supports it. Analyst-started commands are session-owned operator work; losing a browser WebSocket does not clean up the shared `analyst:global` session or its processes. Application disposal owns that cleanup, while internal server/application disposal also terminates runtime-owned running processes.

Process handling uses a launch-and-monitor model rather than unbounded synchronous shell tools. Each managed POSIX launch owns a detached process group for the current runtime lifetime, not merely its shell leader. The group remains live and controllable after the leader exits while descendants remain; owner cleanup, explicit kill, runtime shutdown, and resource-scope disposal signal the group with TERM, wait for group absence, then use KILL if needed. An agent may launch a project command, inspect its evolving status and logs over time, wait for completion with a bounded `wait_process` operation, and then decide whether to wait again, terminate the process, or continue other work. `timeout_ms: 0` is non-blocking group-liveness inspection; a positive timeout returns the running view without killing the group when the deadline expires. A terminal result is published only after the owned group is absent: natural completion preserves the leader outcome and runner-initiated termination is `killed`. After a runtime restart, process ids and groups from the previous runtime are unknown.
...
CLI initialization uses the same invariant as runtime startup: `.saivage/project.json` alone is not an initialized project. A project is initialized only when the current generated layout and canonical root project card exist. `saivage init` completes missing generated layout while preserving durable inputs and prompt overrides, and reports “already initialized” only when that invariant holds. `saivage start --create-runtime` acquires `.saivage/locks/runtime.lock` before bootstrap and uses the same initializer before loading config and starting the server; plain `start` also owns the runtime lock but does not silently initialize missing projects.

`saivage status` reads persisted runtime state from `.saivage/state/runtime.json` and reports a live PID from `.saivage/locks/runtime.lock`. Runtime lock payloads are valid only when they contain a finite positive integer `pid` and an ISO `started_at` timestamp matching the current writer format. A valid readable lock is held when its PID is live; lock age never expires a live-PID lock. Dead-PID locks and malformed lock content are removable by acquisition/stale cleanup. Existing lock files that cannot be read fail closed: start, reset, status/holder, and stale-cleanup helpers throw and do not unlink or proceed based on unknown ownership. `saivage reset` is a locked runtime-state reset: it atomically acquires `.saivage/locks/runtime.lock`, refuses without mutation when a valid live runtime owns that lock regardless of age or when the existing lock cannot be read, removes generated roots (`cards`, `agents`, `state`, `logs`, `locks` contents except the held runtime lock, `work`, and optional `stages`), the external generated `.saivage-work/` root, and obsolete/generated legacy roots (`runtime`, `tmp`, `archive`, `supervision`, `notes`, `outputs`, and `views`), calls initialization while still holding the lock, then releases it. After a successful reset, durable credentials/config/operator inputs and source/docs are preserved, `.saivage/state/runtime.json` contains default runtime state, `.saivage/logs/app.jsonl` exists and is empty, `.saivage/locks/` exists without `runtime.lock`, and `.saivage/cards/project/` exists as the root project card.
