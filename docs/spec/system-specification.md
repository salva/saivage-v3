...
Supervisor processor activation observes both activation rejection and either path's subsequent result-settlement rejection. A settlement-phase invariant rejection starts or joins this same singular `runtime_failure` halt, retains the original invariant error as the current owner's halt and settlement failure evidence, and is never left as an unobserved promise rejection; publication-outcome-unknown activation rejection retains its distinct immediate fatal boundary.
