...
Provider candidate availability is owned by one composition-created `CandidateAvailabilityStore`. Startup truncates only an incomplete final JSONL tail and strictly reads every complete row; malformed or empty complete rows fail startup. Provider success and failure use direct synchronous appends and update memory only after success. There is no threshold compaction, candidate-specific PID lock, constructor mutation, malformed-row skip, close/dispose phase, or fallback writer.
