...
Agent summaries require `id`, `agent_name`, `session_scope`, `card_id`, `started_at`, and exactly one valid liveness pair: `status:'active'` with `activity:'busy'`, or `status:'inactive'` with `activity:'idle'`.
...
Every autonomous, Analyst, repair, continuation, individual-summary, and merge-summary invocation receives a fresh opaque UUID source input generated by its owner.
...
Transport wire encoding may differ per protocol, but provider adapters serialize instructions before all conversation content and project tools only from the compiled contracts.
...
Preparation validates budget, fraction ordering, positive reserved/requested completion tokens, requested completion not exceeding reserve, trigger plus reserve, escalated tail/middle widths no greater than normal, and static-token trigger/hard capacity.
Tool-result and repair calls retain the activation's exact prefix, tools, and prepared object while refreshing source input ID and canonical projection; a later activation or Analyst submission prepares anew.
...
One call-local incremental materializer owns a genuinely nullable accumulated summary: it is initially the inherited summary or null, and only a successful content-bearing advance can establish new genuine accumulated text.
...
A structural-only advance after genuine current-segment material returns that accumulator unchanged, while structural-only coverage with inherited history but no current-segment material fails rather than claiming new coverage.
...
Deterministic safe-prefix evaluation walks explicit round state across multiple closed rounds and into a partial open round with strictly advancing first-time cutoffs.
Each distinct extension materializes leaf work only from the disjoint source-row slice after the preceding cutoff, then validates and measures the complete candidate.
A repeated cutoff, including normal/escalated overlap, reuses its complete call-local memoized candidate without another summary call, reconstruction, validation, metric update, or publication; an uncached backward cutoff is an invariant failure.
A strategy of `local_exact_admission` evaluates every safe increasing cutoff in one call and selects the smallest exact composed projection, with an exact-byte tie resolved toward the furthest cutoff, while preventive and authoritative strategies retain their bounded order and acceptance criteria.
When no new safe coverage exists, compaction returns `no_smaller_projection` and appends and publishes nothing.

Each summary-eligible leaf body and each reduction request is materialized through one exact serialized-request estimator and packer: instructions, labels, wrappers, the genuine nullable prior accumulator, the exact semantic projection of selected required-model-fact text, protocol bytes, and the completion reserve in the [exact context and compaction limits](#exact-context-and-compaction-limits) are all accounted, oversized leaf/prior/reduction inputs are chunked deterministically, and per-level progress must be provable or construction fails.
Every strictly increasing advance computes its next summary, inherited-fact fold markers, current-segment-material flag, and cutoff transactionally and commits that call-local state only after the entire advance succeeds.
Failure leaves the preceding materializer state unchanged and stops the `compact()` operation; there is no later candidate work or retry.
...
Summary leaf and reduction calls execute sequentially, at most one summarizer provider call is in flight per `compact()` operation, and abort or the first failure stops every later admission.
...
The strict durable compacted payload stores only fixed-size commitments: mode, selected band, input budget, trigger/completion fractions, the selected band's merge and summary fractions, snap, the coverage/disposition commitments, required-model-fact slots, and the otherwise-unreconstructible static estimate.
Derived completion, thresholds, and window budgets are never persisted.
...
Summary leaf and reduction attempts are projected exactly once to the existing app log under the internal `internal:compaction-summary:<source-session-hash>` evidence session and fresh source-input identity, with no assistant output IDs, and are never merged into the triggering persisted input.
