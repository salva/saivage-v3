...
The runtime-owned subset of `BlockedResult` is exactly primary `content-policy-refusal` or `{kind:'compaction-summary-blocked',summary:'Internal conversation summarization was blocked by the provider after bounded recovery. No further automatic retry was attempted.',session_id,summary_input_id}`. The latter requires a canonical owning conversation session and UUID summary-input identity, has no content-policy marker/evidence URL, and is invalid in DONE or FAILED.
...
A sole final unmatched call has no active, waiting, pending, or snapshot meaning. When a later ordinary card workflow selects that exact configured session for imminent activation, or a later explicit submission is newly admitted to the configured global Analyst session in an already-running server, the consuming owner may pair it with one permanent synthetic failed result stating exactly `Prior activation ended without a recorded tool result. External or domain effects may or may not have happened. The prior call will not be replayed.` The result preserves the old source input, call, tool, and persisted result-policy template bytes/hash, carries exactly `{outcome_unknown:true}` and evidence `none`, and is appended immediately before the fresh activation marker. It neither continues nor replays the old invocation and appends no recovery notice.
...
An error escaping the exact invoked-tool await after its durable call and before its result rejects that submission with the original error. When the error is not publication uncertainty, the parked provider turn is successfully abandoned, and application disposal has not won, the session returns to idle with restart confirmation cleared so a later explicit submission is a fresh activation. Preparation and project-context errors outside invocation, result/continuation publication errors, failed abandonment, and cleanup errors still poison the session; publication uncertainty remains fatal. The later submission strict-reads only its selected global session and applies the final-unmatched rule above before fresh ingress. It never automatically retries or continues the old tool.
...
Multiple or nonfinal unmatched calls and malformed canonical data fail directly at card-activation or Analyst-submission use, before its activation marker or provider request. Lifecycle admission is unchanged. Settlement publication uncertainty, or uncertainty while publishing the following activation, is fatal and authorizes no read, retry, second result, context append, provider request, cleanup, or reconciliation.
...
Global Analyst startup and ordinary read-only projections never enter either bounded settlement path.
...
The Analyst `pause_runtime` tool admits only `running`, and `resume_runtime` admits only settled `paused`; every wrong public status is an ordinary failed tool result with `runtime_status`, including guidance to use `start_project` when stopped. The model may therefore choose another action in the same turn. These tool-boundary checks do not weaken the Supervisor's synchronous Run-identity/currentness authority or convert its invariant/publication failures. Analyst `stop_project` has no status precheck: it always delegates to the Supervisor so stopped/no-live remains `{status:'stopped',contained:false}`, live or closing callers join the one halt, and a retained halt failure remains the same rejection.
...
Each workflow-node activation or global-agent operation (Analyst submission or Oversight check) builds one immutable `PreparedInvocationContext` before any durable ingress effect: a `StaticInvocationPrefix`, one ordered `CompiledInvocationToolContract` array, its internal-tool-contract hash, ordered typed dynamic `ContextBlock`s with their canonical hash, and one prepared compaction value.
...
The exact typed prompt-policy flag is the sole protocol-error exception. In `internal-summary` purpose its first occurrence permits one immediate retry of the identical retained request/candidate bytes, with no delay or availability mutation. A second flag is terminal even when capacity remains, and the ordinary four-attempt ceiling still applies across mixed failures; transport retries do not add logical refine calls, so the complete compaction remains bounded by 16 logical calls and at most 64 transports. After all settled summary attempts are projected once under the internal evidence session, persistent flagging becomes a fixed-safe `SummaryPromptPolicyBlockedError` carrying the final summary input UUID. It is ineligible for correction, range changes, alternate endpoints/candidates, retained fallback, partial publication, or uncompressed continuation.

The consuming actor handles that typed signal before generic provider/construction wrapping. Preventive and local-exact strategies create no primary turn/error row. Authoritative recovery first projects the real original primary context-failure attempts exactly once with null terminal output, then propagates the summary signal; any publication failure supersedes it. Autonomous-card ownership returns strict `compaction-summary-blocked` with the canonical source session and summary input UUID through the ordinary runtime-owned BLOCKED path. Global-agent ownership returns the ordinary fixed-safe error outcome and settles its lease: Analyst appends its ordinary safe unavailability notice and the retained owner accepts a later newly admitted explicit submission, while an Oversight check settles failed and ProjectOversight creates a fresh check only at the later normal cadence. Publication uncertainty remains fatal before handoff, notice, lifecycle settlement, cleanup, or reuse claims.
...
A distinct factual protocol-error evidence value, `prompt_policy_rejection`, is attached only to an opened HTTP-200 terminal whose direct code/type is `invalid_prompt` and whose direct message contains the supported “prompt was flagged as potentially violating our usage policy” phrase, compared case-insensitively. Authentication, rate-limit, server-transient, context ambiguity/exhaustion, and recognized content-policy classifications retain precedence. Plain `invalid_prompt`, nested data, wrapped exception prose, and other wording do not gain this evidence. Primary execution remains terminal for this protocol error.
...
Only owned non-provider construction failures become `CompactionSummaryConstructionError`. Its fixed safe message reports reason, logical invocation count/limit, correction count/limit, and available summary-byte/target or projection-token/ceiling measurements; it never stringifies arbitrary provider prose. Eligible reasons are `empty_output`, `tool_calls`, `incomplete_output`, `request_context_capacity`, `fold_limit`, `residual_capacity`, and `no_reduction`. Provider failures—including typed output/input capacity and recognized content policy—remain the exact original `ProviderTurnFailure` when no candidate/correction succeeds, except that successfully projected exhausted prompt-flag evidence becomes the narrow typed summary-policy signal described above. Abort, summary-exchange projection/publication, publication-unknown, source/projection/hash/invariant, and append failures retain their exact owning identities.
...
Last-chance context recovery has narrow publication ownership after the triggering planner provider boundary has already been entered.
If summary recovery exhausts with ordinary `ProviderTurnFailure` after summary attempts were published, including the narrow Chat refusal/protocol conversion after its successful exchange was published once, the actor validates and publishes the original triggering planner context-failure attempts exactly once under the planner input, with null terminal conversation output and no assistant output. An exhausted typed summary prompt flag performs the same original-attempt publication and then returns the strict blocked result rather than the last-chance wrapper.
...
`{kind:'autonomous-card',cardId}` must exactly match a card-scoped session; `{kind:'global-agent'}` must match a global session.
CardProcess, AnalystSession, and each fresh OversightSession check are the explicit production constructors.
...
Global agents and direct summarizers never produce the pinned variant.
...
Persistent typed internal-summary flagging instead settles the running card BLOCKED with `compaction-summary-blocked`, the fixed safe summary, owning session ID, summary input UUID, null `completed_at`, and no refusal marker or content-policy metadata.
...
`card.json` returns the last row's projected document; `card.json?v=N` returns exactly row N's document from the same validated fold. An ordinary version document is exactly `{format_version:4,kind,entry_id,card_id,version,published_at,card,change}`; a tombstone substitutes `final_card` and retains `prior_card_version`. The required nullable change is the same ordinary-only projection used by history and artifact hashes; these wrappers retain publication identity/time and contain no pending-notification field.
...
Typed research treats `evidence_ready` as readiness to assess the bounded question, including reasonably exhausted or inconclusive investigation, rather than support or exhaustive coverage. A return to exploration requires concrete obtainable evidence or materially different useful analysis capable of changing a named assessment or decision; the status names that discriminator, the change it could support, and the next work. When the bounded question remains uncertain, successive recorded hypotheses have been refuted without convergence, and no useful discriminator remains, assessment selects an honest bounded-inconclusive report; a conclusively refuted question remains refuted. Completing the report accepts only that bounded research deliverable: unresolved questions, limits, required exhaustive follow-up, and actionable handoffs remain explicit, while evidence promotion, policy approval, and parent or project acceptance remain their respective owners' decisions. An accepted brief remains authoritative; a required deliverable that cannot proceed without specific scope, input, or a decision uses the existing blocked route rather than being silently narrowed.

Typed data validation distinguishes correct rejection of invalid samples from a schema defect and technical acceptance from owner or policy approval. Only an actually mandatory missing decision or input blocks; ordinary engineering and accepted graph nodes require no new human signoff. Across both templates, shared Executor guidance compares the affected path branch-by-branch with the applicable reference or specification—including state transitions, ordering, and error behavior—and repairs the owning primitive within the brief rather than accumulating symptom patches. It preserves earlier failed or unresolved checks despite a later successful tool call and reuses evidence only while relevant inputs remain unchanged and the evidence applies. Changed checks, currently required checks, and unresolved verification questions require execution; prior failure remains explicit and required validation is never waived. Typed code red owns adequate reproduction and diagnosis of relevant branch and boundary behavior without beginning repair; green owns the complete coherent primitive repair and affected-path regression coverage. Typed refactor may reuse recent valid focused evidence after a reasoned no-change decision unless current criteria require a rerun.

Typed test diagnosis classifies adequate meaningful coverage plus passing focused tests as `coverage_ready`, including on fresh entry or re-entry after interruption. That route requires no manufactured test, source, or metadata change: it reuses `test-to-verify` and enters the existing `verify` node, which owns affected-suite and coverage acceptance and remains the only node that can complete the test card successfully. Verify may cite immediately preceding same-card focused-suite evidence only while relevant inputs are unchanged and that coverage establishes the cited part of acceptance; it records provenance and applicability, runs every remaining affected or acceptance-required check, and reruns after relevant changes or unresolved verification questions.

The existing retry outcomes at typed code green, test repair, and data implement require materially changed diagnosis or implementation evidence supporting a concrete useful next attempt. Their existing `failed` outcomes apply when the bounded task remains unsatisfied after materially distinct reasonable attempts with named hypotheses and observed failures, no diagnosis or implementation is converging, and no justified useful next attempt remains, or when evidence establishes that the task cannot succeed within its accepted constraints. Existing status records the attempts, exhausted hypotheses, residual uncertainty, and remaining work without claiming global impossibility. Their `blocked` outcomes are reserved for a specifically identified genuinely unavailable required input, access, or decision, not ordinary uncertainty or difficult engineering. This is prompt policy at the current outcome owners; it adds no retry counter, state, preflight, or engine mechanism and does not redefine Planner strategy exhaustion or bounded-inconclusive research.
...
Shared Planner owns generic project/goal responsibility, strategy and remedy judgment, complete direct-child inspection and reuse, lifecycle repair order, metadata boundaries, blocker materiality, configured capability limits, and operation-result handling. Typed plan and recover prompts are current-node deltas: plan assesses the current brief, status, children, records, delivered context, and review evidence before choosing a current outcome; recover rebuilds that assessment from durable facts without reconstructing a discarded graph position. They do not repeat or replace the shared child-management policy.

Delivered notifications and peer context are queued evidence with provenance, not instructions that override the current role, node, generated contract, or applicable owner requirements. For accepted planning review with pending recipient context, the durable `review-to-notifications` handoff owns only historical framing: accepted summary and immutable versioned review URL remain evidence, not a rejection. The separately prepared `handle-notifications` node owns current action: assess delivered context against accepted evidence and unchanged owner authority, coordinate justified work, mandatorily update current-node status, and return through `admit_review` or an existing justified terminal route. Neither site infers hidden queue state or completion, and the physical prompt closure and graph topology remain unchanged.

The shared Reviewer independently inspects relevant implementation, tests, records, and artifacts; traces load-bearing claims rather than accepting author status; and, for family criteria, assesses representative members and boundary cases without waiving expressly exhaustive criteria. It judges only criteria assigned to the current node. Specialized planning review applies that method to the brief and subtree, treating descendant status as evidence pointers rather than acceptance by itself. Reviewer has no command-execution authority and this prompt policy adds none.

Architecture uses the shared Reviewer prompt and one declared `review.md`.
...
Card stream format v5 keeps the newline envelope at `version: 1`, `type: 'rows'`, and accepts only `card-version` and `card-tombstone` rows with `format_version: 4`.
Former artifact row formats, including row format 3, records with durable `children`, snapshots containing removed `tags` or `related` members, and streams containing more than one artifact format fail strict reads.
...
Each check performs bounded, proportionate observation of objectives, linked work, records, conversations, process evidence, and source, distinguishing facts, interpretations, uncertainty, and current evidence from historical evidence. Sustained non-progress may be evidenced across checks or from bounded history inspected during one check; another check is not a prerequisite.
Relevant evidence includes a running card accumulating retry or repair executions over a sustained window without substantive accepted artifacts, milestones, or relevant implementation commits, or repeated unresolved findings directed at a terminal implementation owner that cannot consume or act on them. Mere status versions, repeated old revision citations, and repeated defect restatement do not establish convergence, but an ordinary review mentioning a terminal card does not establish futility either.
Reads are non-atomic observations and never authorize a later write; partial or failed reads do not prove absence. Healthy slow work, truthful red cycles with relevant active commits or other substantive progress, and ordinary deep review do not become futile solely because of elapsed time, ordinal magnitude, call volume, difficulty, or a red test.
On newly actionable evidence of sustained non-progress or futile terminal-owner feedback, Oversight submits concise, qualified `normal` advice to the nearest responsible nonterminal card whose workflow reports `planning_target: true`, identifying the observed window, looping card, retry or repair evidence, unchanged acceptance gap, and any terminal owner. It asks that Planner to reconsider the approach or decomposition or coordinate repair at the actual owning card; it does not notify the terminal card, broadcast, prescribe automatic reopening, or claim strategy failure as fact.
Project guidance supplies context but cannot waive owner scope or acceptance. Planners retain discretion and may reasonably disagree. A short no-intervention response is successful when there is no new actionable concern, but does not certify the whole project.
Oversight uses ordinary conversation and compacted history, repeats equivalent advice only when new evidence or a stated material reason adds value, and corrects disproved advice. Previously queued advice with no observed handling is neither evidence of delivery or failure nor, by itself, new value warranting repetition or escalation.
This advisory criterion does not interrupt active descendant work and may await the designated recipient's existing handling opportunity under §6. Enqueue proves neither receipt, consideration, action, eventual delivery, nor immediate containment. The criterion does not itself satisfy or broaden the separate exceptional urgent predicate in §7 and requires no mechanical normal-plus-urgent pair.
Oversight records unavailable recipients and owner-decision needs in its inspectable conversation without promising an operator alert; no runtime detector, delivery SLA, intervention ledger, deduplication registry, or guaranteed-attention channel is introduced.
