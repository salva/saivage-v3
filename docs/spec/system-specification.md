...
The persisted runtime-state schema and actor-snapshot schema remain unchanged. One server composition constructs and restabilizes their shared-lane stores before runtime startup; missing runtime state is initialized exactly once through `RuntimeStateStore`, while reads never initialize or repair it. Snapshot and recovery-diagnostics reads are also strict and non-mutating. Recovery cleanup and diagnostics replacement use composition authority, while live snapshot changes use the exact current actor authority. This ownership cutover does not reinterpret Pause: Pause remains provider-admission scheduling control, and Resume retains the behavior specified above.
...
Current provider-exchange ordering is conversation-first: `InvocationService` only accumulates and indexes settled envelopes in memory. The LLM actor commits its assistant/error/tool-call conversation batch first, then independently appends provider exchanges through the lane-owned `AppLogStore`. Successful payloads contain the already-durable visible output id in `assistant_output_ids`; they are not written early with an empty array. The app-log store is also the sole writer for events, errors, control actions, and content reviews; it fsyncs synchronous authorized appends, truncates only an incomplete final row at startup, and rejects malformed complete rows.
...
Project configuration defaults to `.saivage/saivage.yaml`, but `--config` takes precedence over `SAIVAGE_CONFIG`, which takes precedence over that default. Startup resolves this choice once to an absolute path and records the winning source. Every server-side config view, Analyst mutation, and MCP initialization/reload addresses only that selected file; a missing or invalid selected file never probes or falls back to another path. Startup also snapshots the interpolation environment once, so later process-environment changes do not change effective interpolation. Startup restabilization and default creation use composition authority. Mutations synchronously enter the process-wide mutation lane with the exact current Analyst turn, then read the latest raw YAML, apply one typed change, run canonical schema and model-role validation against the startup snapshot, and atomically replace the same file. There is no private config queue, cleanup-on-read, fallback constructor, or authority-free mutator. Raw YAML placeholders, comments, and unrelated ordering survive mutation rather than interpolated credentials being serialized. Agent prompt overrides are file-level replacements in `.saivage/config/prompts/` using the same `<cardType>/<role>.md` paths as the shipped defaults; omitted files use built-in defaults. The rendered `(cardType, role)` template is the complete provider-visible system prompt for that agent slot. Before its first provider turn, every newly created conversation actor resolves its once-per-actor/session prompt identity from indexed active and frozen session-version rows, including provider reissue during recovery and an ordinary later activation. A row with that exact id but a non-`system_prompt` kind fails the turn before provider I/O. Operator and model transcript reads remain active-version-only.

Auth profiles are owned by one composition-created `AuthProfileRepository` backed by the same synchronous mutation lane. Startup alone removes exact interrupted replacement temporaries and restores the credential file's restrictive mode; ordinary reads never clean up or rewrite. Provider credential resolution receives that repository rather than constructing a path-local store. OAuth refresh HTTP remains outside the lane, and publication requires the originating current root-plus-leaf or Analyst-turn authority plus the exact expected profile revision. If the profile changed while the request was in flight, replacement fails without overwriting the newer credential. Malformed JSON, invalid schema, and I/O failures fail closed with credential-safe diagnostics; there is no authority-free save/delete helper or fallback writer.

Provider candidate availability is owned by one composition-created `CandidateAvailabilityStore` on the process mutation lane. Startup removes exact interrupted compaction temporaries, truncates only an incomplete final JSONL tail, and strictly replays every complete row; a malformed or empty complete row fails startup. Provider success and failure updates require the originating current invocation authority, append one complete newline-terminated row, and fsync before publication. Threshold compaction atomically replaces that same JSONL file with the current per-candidate projection. There is no candidate-specific PID lock, constructor mutation, malformed-row skip, close/dispose phase, or fallback durable writer.
