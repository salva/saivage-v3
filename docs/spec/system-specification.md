...
`saivage status` reads persisted runtime state from `.saivage/state/runtime.json` and reports a live PID from `.saivage/locks/runtime.lock`. `saivage reset` is a locked runtime-state reset: it atomically acquires `.saivage/locks/runtime.lock`, refuses without mutation when a live runtime owns that lock, removes generated roots (`cards`, `agents`, `state`, `logs`, `locks` contents except the held runtime lock, `work`, and optional `stages`) plus obsolete old roots (`runtime`, `tmp`, `archive`, `supervision`, and `notes`), calls initialization while still holding the lock, then releases it. After a successful reset, durable credentials/config/operator inputs and source/docs are preserved, `.saivage/state/runtime.json` contains default runtime state, `.saivage/logs/app.jsonl` exists and is empty, `.saivage/locks/` exists without `runtime.lock`, and `.saivage/cards/project/` exists as the root project card.
