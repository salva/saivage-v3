...
Archiving is not a card status. Deleting or archiving a card publishes a strict `.saivage/cards/<id>/tombstone.json`, removes the id from every active card/history/version/record/conversation/snapshot/recovery surface, and reserves it permanently for the current generated state. Retained namespace files are opaque evidence, not an archive API; there is no restore or archive-content contract.
...
The Analyst has limited card authority on behalf of the user. All ordinary Analyst card mutations require the serving runtime's process-local intervention facet to report stopped-ready or settled-paused; the persisted status alone is not admission. Stopped is ready only with no live autonomous activation, and public Pause may return while already-admitted work is still settling and the private facet remains not ready. An operation may first await read-only preparation. It then freshly checks shared persistence health, intervention readiness, permission, and every mutable domain fact before invoking a named synchronous application-service commit without yielding. In those ready states, the Analyst may manage cards through semantic operations such as create card, reorder direct children where supported, cancel dormant work, and delete cards/subtrees from the active tree with deleted-id reservation. It may also update the goal/instructions/acceptance brief of any existing card whose current status is exactly `backlog`, `done`, `failed`, or `running` by calling `write` or `edit` on `record:///brief.md?card=<id>&v=next`, or by using fetched content from `webfetch.save_as` as preparation for that same brief-record commit. Direct brief edits to `changed`, `blocked`, or `cancelled` cards fail before publication. Brief close and lifecycle propagation are independently durable synchronous publications. A propagation failure reports accepted partial completion; startup does not replay or converge it. `edit` loads the latest closed brief as its source and never rewrites a historical closed version in place. No Analyst auth-profile mutation tool is exposed; provider-owned profile refresh is not an Analyst capability.
...
The internal `RuntimeControlService` sits behind the existing `start_project`, Pause, and Resume adapters. It alone persists the current lifecycle request's runtime-state effect and appends its single `runtime.start_project`, `runtime.pause`, or `runtime.resume` control-action outcome. Analyst, REST, runtime-facade, and direct offline CLI adapters do not duplicate those writes. Public names, responses, and Pause timing are unchanged. Direct offline Pause/Resume holds one lifecycle-lock handle across state and audit; a live owner is still contacted through the existing REST delegation path.
...
Each provider-bearing turn has one immutable invocation lease. A revoked turn cannot deliver a late provider result, tool call, conversation notification, actor transition, or continuation. After provider I/O, the runtime rechecks that lease and submits the exact captured root-plus-leaf or Analyst-turn authority to the synchronous `ConversationStore`. For OpenAI Responses, the paired private replay row and visible assistant/tool-call projection are one atomic batch. There is no completion-persistence admission wrapper, release tracker, separate completion lease, or interruption fence.
...
`webfetch` is classified by branch rather than by tool name. Calls without `save_as` are read-only when they return metadata, errors, binary descriptions, or inline text. Oversized no-save text retains the direct disposable stash write and requires neither intervention readiness nor application-persistence admission. A project, tmp, or system `save_as` is admitted by the external workspace-file owner after the fetch. A brief-record `save_as` performs the fetch as read-only preparation and then enters the ordinary fresh recheck/named-commit boundary. MCP desired-topology add/edit/remove and `mcp_reconcile` remain unconditionally rejection-only; `mcp_tool_call` is the separate manager-owned invocation path.
...
`/api/state` returns the persisted runtime-state projection from `.saivage/state/runtime.json`; the `runtime` field may be `null` when that file is absent. Live server availability is a separate projection sourced from the running runtime application. If that live runtime-status read fails, availability reports a degraded diagnostic for `runtime-application`; `runtime-state` is not an availability component source. The availability map also carries the shared in-memory persistence component. Its terminal unavailable state makes `/health/ready` return HTTP 503 with `status: not_ready`, while `/health`, runtime/operator reads, card/file reads, and Debug remain available. `/api/debug/state` includes the same bounded redacted persistence-health snapshot, so an uncertain app-log append remains diagnosable without recursively attempting another app-log row. Runtime event, error, control-action, and content-supervision review APIs continue to expose previously durable rows from the single application log at `.saivage/logs/app.jsonl`. When supervision blocks content, it appends only a sanitized `content_review` summary; the blocked raw content is not stored, no quarantine side files are created, and no browseable quarantine path is exposed.
...
CLI initialization uses the same invariant as runtime startup: `.saivage/project.json` alone is not an initialized project. A project is initialized only when the current generated layout and canonical root project card exist. Fresh-root `saivage init` acquires bootstrap-unbound lifecycle ownership, creates strict project identity through the command-scoped persistence-health owner, binds that same lock owner, and then creates generated state through named synchronous stores. On an identified project, Init uses bound ownership and completes missing generated layout while preserving durable inputs and prompt overrides. `saivage start --create-runtime` requires existing strict project identity and may fill only missing generated runtime layout before ingress.
...
`saivage reset` uses the same bound direct-command composition as Init and one command-scoped `ApplicationPersistenceHealth`. It removes generated roots while retaining the held runtime lock, reinitializes through named synchronous owners, and then performs matching-owner release. Durable credentials/config/operator inputs and source/docs are preserved. No raw helper, queue, capability, or alternate writer composition is supported.
...
The persisted runtime-state schema and actor-snapshot schema remain unchanged. One server composition constructs and restabilizes their synchronous stores before runtime startup; missing runtime state is initialized exactly once through `RuntimeStateStore`, while reads never initialize or repair it. Snapshot and recovery-diagnostics reads are strict and non-mutating. Every mutation checks the same process-local persistence-health object. This ownership cutover does not rename or reinterpret the public Pause and Resume contracts.
...
Current provider-exchange ordering is conversation-first: `InvocationService` accumulates settled attempts in memory. The LLM actor commits its assistant/error/tool-call conversation batch first, then independently appends provider exchanges through the composition-owned `AppLogStore`. The app-log store is also the sole writer for events, errors, control actions, and content reviews. Its owning reader validates the complete strict type-discriminated payload union before filtering by type; malformed payloads are never cast, skipped, or silently dropped.

OpenAI `openai-responses` turns atomically add one provider-private `provider_private` replay row plus one visible assistant text/tool-call projection row marked with `provider_projection.kind: openai_responses` in one conversation batch. The private row stores the completed OpenAI `response.output` array unchanged, including reasoning items and `reasoning.encrypted_content`, so stateless tool continuations can replay exact prior Responses output and append local `function_call_output` tool results. Generic Chat/Codex prompts, token-budget summaries, operator conversation APIs, Raw LLM Exchange, logs, and provider exchange metadata must not expose provider-private rows, private row ids, raw `response.output`, encrypted reasoning, API keys, or tool result bodies through diagnostics. A marked visible Responses row without its paired private row, an orphan private row, duplicate pairs for a source input, or mismatched bidirectional ids is fatal malformed transcript state; unmarked assistant/tool-call rows remain valid generic history and never imply a missing private row.
...
Current conversation directories contain only a gap-free sequence of numbered `<N>.jsonl` version files and optional `summaries.jsonl`; the highest fully validated numeric version is active. Session ids and placement are strict: planner/executor sessions use `project` or canonical `card-N`, reviewer sessions repeat the same card id in `assessment-<card-id>-1`, and Analyst sessions are `analyst:global` or canonical Telegram ids. Index files, aliases, gaps, unknown entries, malformed versions, and misplaced sessions are unsupported current state and fail strict loading.

App log, provider availability, conversation versions, and summary caches share one physical growing-file contract. Every logical append is one newline-terminated strict `{ "version": 1, "type": "rows", "rows": [...] }` envelope with a non-empty row array; readers flatten rows in physical-line and array order. OpenAI Responses private replay plus visible projection rows occupy one envelope. The first envelope is atomically published through same-directory durable replacement; later envelopes are append-only. Startup may truncate and file-sync only an unterminated final physical suffix. Complete malformed envelopes, unknown fields, unsupported versions/types, duplicate logical identities, empty lines, and zero-byte canonical files fail for operator repair. Cleanup removes only exact owned replacement temporaries and an exactly empty unpublished conversation session directory. Generated-state format changes are reset-only, with no compatibility reader, migration, probing, or mixed-version operation.
...
Until the runtime exposes completed quiescent Pause, Analyst `mcp_add`, `mcp_edit`, `mcp_remove`, and `mcp_reconcile` requests fail before configuration mutation or reconciliation with `persisted: false` and `reconciled: false`. Scheduling-only Pause is not intervention readiness. Boot-owned reconciliation remains available before ingress; the final lifecycle cutover enables Analyst MCP desired-state mutation and explicit reconciliation only under its stronger paused/current-turn contract.

Project configuration defaults to `.saivage/saivage.yaml`, with `--config` ahead of `SAIVAGE_CONFIG` and then the default. Startup resolves one absolute path and snapshots interpolation once; missing or invalid selected configuration never falls back. Named synchronous changes read current raw YAML, apply one typed change, validate canonical schema and model roles, and durably replace the selected file under shared persistence health. Ordinary Analyst config changes require live intervention readiness and a fresh synchronous recheck after any read-only preparation. MCP topology variants remain unconditional rejection-only operations. Raw placeholders, comments, and unrelated ordering survive mutation. Prompt overrides retain their current paths and rendered contract. Conversation prompt identity is resolved from validated numeric session history; a same-id wrong-kind row fails before provider I/O, while operator/model transcript reads remain active-version-only.

Auth profiles are owned by one composition-created `AuthProfileRepository`. Startup alone removes exact interrupted replacement temporaries and restores restrictive mode; ordinary reads never clean up or rewrite. OAuth refresh HTTP occurs before the named synchronous replacement, which requires the exact expected profile revision. A concurrent change fails without overwriting newer credentials. Malformed JSON, invalid schema, and I/O failures fail closed with credential-safe diagnostics; there is no fallback writer.

Provider candidate availability is owned by one composition-created `CandidateAvailabilityStore`. It uses the shared strict envelope contract, atomically publishes its first non-empty envelope, and appends thereafter. Startup truncates only an incomplete final physical suffix and strictly reads every complete envelope and row; malformed or zero-byte canonical state fails. Provider success and failure update loaded memory only after durable success. There is no threshold compaction or compaction configuration, candidate-specific PID lock, constructor mutation, malformed-row skip, close/dispose phase, or fallback writer.
...
- archiving publishes the card namespace tombstone, removes the card from active runtime state, and reserves its id without an `archived` status or archive side files;
