...
A strict raw provider rejection, malformed provider-attempt envelope, pre-provider failure, ordinary processor failure, or executor-cleanup failure is not indefinitely active work. For those ordinary failures the backend publishes the selected failed card lifecycle before resolving the waiting parent's existing failed `activate_card` tool result and before releasing the completed activation owner. A terminal CardProcess actor-main invariant failure is different: it rejects the owning activation, publishes no ordinary failed lifecycle or parent result, and starts or joins the singular `closing -> error/not_ready` halt. The exact actor error remains visible at its local log/lifecycle boundary, while a structurally earlier cleanup failure may be the halt's retained error. Service restart is required. This exception adds no web implementation, API field, control, color, or new runtime status. Card and actor projections therefore do not retain stale `running` state for a normally published failure; existing REST refetch and invalidation behavior exposes the corrected durable card. Fatal-handler server logs are not a durable UI diagnostic; these strict failures add neither a synthetic provider exchange to Raw LLM Exchange nor a `model_issue` entry.
