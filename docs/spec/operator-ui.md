...
Card tree rows show only the represented-hierarchy depth-indented state ball, human-friendly display-path level number when represented, card title, and card kind. Tags and priority are not displayed in card rows or card detail. The backend projects every full card used by detail, parent/children slices, tools, history, and diffs: title, lifecycle result summary and blocked resume reason, lifecycle error, operator-summary error, status text, and notification content are redacted prose. IDs, types, statuses, tags, tag-filter values, notification source, allowed actions, and result/blocker/action discriminants remain exact even when credential-shaped. `runtime.cardRuns` uses the same title/status-text rule while preserving current/breadcrumb card IDs, breadcrumb card types/order, and dormant goal/session IDs exactly. Card detail renders the one retained actions array that the backend authorizes for that card. Blocked warning state, stopped recoverable state, error/stale state, phase, completion, and blocking-child presentation derive from lifecycle and their explicit fields; no action-count presentation exists. Stopped is green with a black-ringed circular status marker in the tree and card badge, while cancelled remains neutral/gray and terminal; the ring distinguishes recoverable stopped work from green running work. The card action vocabulary is exactly `card.start`, `card.create`, `card.cancel`, `card.delete`, and `card.reorder_child`. It never contains `card.restart`, and a blocked card exposes neither start nor restart: only its exact parent planner can re-enter it through `activate_card` and configured `BLOCKED`. Those projected card actions are distinct from the Dashboard's capability-gated, exactly confirmed **Restart server** control. Card detail does not render Related or Hierarchy sections; represented children and ancestors remain navigable through the lazy card tree. Cards has no search, status, type, Clear, or other filter controls and does not claim that currently loaded slices are a complete project inventory. The former card-inventory Timeline route and primary navigation item do not exist. Debug's distinct event timeline and agent-conversation timelines remain.
...
Public MCP status omits integration-owned errors, and tool inventory omits title, description, input/output schemas, annotations, `_meta`, and other opaque integration metadata. The UI continues to preserve exact server/tool topology names and `${server}:${tool}` statistics keys, process-lifetime totals, nested current-tool statistics, derived totals, and the existing **All Invocation Stats** table. Omission does not reset, re-key, or reinterpret statistics.
