...
The exact opaque card ID in `/cards/:id` is the only selected-row authority. Exactly the tree row with that ID receives the visible selected treatment and semantic `aria-current` state; its lifecycle state ball remains independently visible. For rendering, ancestors of that route card are revealed even when explicit collapse intent would otherwise hide the selected row. That route-required reveal takes precedence only while needed: explicit collapse intent remains recorded and resumes after navigation leaves the branch. Selection is not inferred from the last click, synchronized into a store, or scrolled into view.
