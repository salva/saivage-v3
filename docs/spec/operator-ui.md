...
The web client has one owner for each separate core resource family. CardStore alone owns response-defined hierarchy slices, selected detail, an ordered map of the selected card's configured record descriptors/states, separately lazy history and entry, and the current-relative diff. Each record name has one exact abortable request owner. Records components present CardStore state and actions and own no request or cache. Selection reset and selected-detail 404 abort and clear every dynamic record owner before state. Runtime and Agent stores retain their separate authorities.
...
Files and card-record panels are logical projections of canonical artifacts and never expose writable physical paths. Card detail supplies ordered descriptors; the UI renders every configured name, schema, bootstrap/optional status, writers, local version, commit timestamp, and Markdown content. Initial 404 is an error for the bootstrap descriptor and accepted empty for an optional descriptor. Display order is descriptor order; there is no fixed three-row list or project-wide ordinal.
