...
When application persistence becomes mutation-unhealthy, existing read surfaces remain usable while `/health/ready` returns 503 and the existing availability projection's persistence component reports that mutation is unavailable until restart. Runtime and state projections carry that same availability, Debug state carries the bounded redacted health snapshot, and the availability detail indicator explains the restart requirement. The diagnostic remains visible even when the app log itself is the failed target. Strict physical growing-file envelopes, conversation version inventories, and persistence-health internals are not exposed as editable UI state; APIs return flattened typed logical rows. Returned `work:///tmp/stash/...` URLs retain their existing presentation but refer to disposable work output with no retention guarantee.
...
These projections are backed by strict reads from the composition-owned runtime-state, actor-snapshot, and recovery-diagnostics stores. The storage ownership change does not alter the operator contract: Pause remains the existing scheduling/provider-admission gate and Resume continues waiting work in normal ordering; neither control gains a quiescence or lifecycle-rewrite meaning.
