...
Cards expose immutable hierarchical Stable IDs whose segments come from each parent's local spreadsheet sequence, plus mutable `logical_path` outline labels derived from active indices in committed parent order. Consumed namespace segments do not give UI membership or order: the complete cumulative parent `children` array is the sole linked-membership and semantic-order authority, including retained tombstone links, and the current card shape has no `position`. Incomplete or complete-but-unlinked namespaces are invisible, with no UI warning, fallback, repair, or compatibility state. The list response is an ordered flat collection whose `children` values are parent-owned child-ID links. CardStore alone derives the nested `CardTreeNode.childNodes` projection by starting at `project` and following each reached record's `children` IDs in their committed array order. A linked ID with no active row is omitted, as for a retained tombstone link, without changing the order of visible rows; an unlinked flat row is not part of the nested projection. Components neither infer membership from child `parent` nor use flat response order as sibling authority. CardStore owns collection request currentness. A delivered post-link create invalidation reaches CardStore immediately even during an active refresh, starts a newer list read, and prevents the older collection response from becoming current. `CardsView` only renders CardStore state; it performs no local fetch, watcher, polling, or optimistic row insertion. These hints remain lossy, with no dropped-frame or failed-read recovery. Route-owned selection, ancestor reveal, expand/collapse, independent loading/errors/scrolling, direct navigation, Back behavior, and generic Files API/VFS/UI behavior are unchanged.
...
Within the Cards workspace at desktop widths, the card tree and selected-card detail are independently vertically scrollable panes. Tree overflow does not move the detail, detail overflow does not move the tree, and normal Cards content does not create one combined Cards/page scrollbar. A valid loaded tree remains mounted, interactive, and at its existing scroll position while selected-card detail loads or fails; detail loading and error presentation is confined to the detail pane. At widths below the Cards split breakpoint, `/cards` shows the tree alone and `/cards/:id` shows the detail alone; the detail's **Back to Cards** action restores the tree.
...
The runtime dashboard is current-state-only. It shows process-local runtime status and active-card evidence without a runtime-state file, command/run ledger, control audit, actor snapshot, or offline mutation. It exposes one **Stop project** action while starting/running/pausing/paused; closing, stopped, and error disable it. The current Dashboard has no Pause/Resume action or disabled-reason affordance, while genuine paused status remains displayed as runtime state. Stop is containment, not domain cancellation: it does not itself change card status or root result, and open cards remain durably `running`. While the request remains closing, a cancellation that already won may complete its durable `cancelled` publication and caller settlement; the UI must not show Stop success before that settlement. The result distinguishes contained Stop from already stopped; containment failure must not claim stopped and leaves the UI in closing/conflict state. A successful later Run reuses the cards that remain in the running chain, installs all ancestor owners, and activates only the deepest card.

An `error` caused by a reported reconstructed immediate-child result-append failure is a degraded, non-ready retained-owner state, not an ownerless or still-executing state. Existing status and actor projections retain the current-card/card/agent evidence, while the runtime-state active-run field is null. The Dashboard disables **Stop project**; Analyst Run and Stop reject; and explicit card cancellation is unavailable until the server process restarts. The supervisor-side cancellation rejection occurs before its own card read, owner lookup, claim, settlement, or map mutation, but the UI does not promise that upstream Analyst/tool validation performed no reads. Independent notifications remain unchanged. Application shutdown cleanup is the sole retained-owner settlement exception. The restart requirement does not make a control available: the existing exactly confirmed **Restart server** action is shown only when the server is already bearer-auth-capable and reports `restart_server_available:true`. No repair, retry, cancellation, or other new UI control is added.
...
Agent conversation surfaces render the one stable append-only session in physical row order. They may show `model_recovered` uncertainty, the original-call paired failed tool result, and a later fresh UUID continuation as separate facts; source UUIDs have no sequence or timestamp meaning. Notification-defeated terminal candidates remain visible as ordinary failed `emit_result` rows followed by operator context. Reviewer deferral shows discard/result effects without inventing a durable reviewer phase, delivery marker, or recovered outcome. After accepted reviewer rework, the planner transcript shows the handoff as one ordinary canonical user-text row containing the exact closed `review.md` URL and reviewer summary; it requires no special UI row type or API contract.

Ordinary restart recovery may therefore show an outcome-unknown failed result, while a same-Run reconstructed immediate-child barrier may show the child's exact real result after strict association. If Stop is initiated synchronously by result publication, that one result remains in the transcript but no fresh activation marker, reconstruction notice, notification rows, projection, or provider work follows. A later Run sees the already paired call and does not append another result. This adds no live pending-call promise or presentation contract.
...
Last-chance compaction adds no live agent activity status or phase. In particular, the current UI must not claim that `compacting` is truthfully projected. A successful preventive or forced compaction becomes visible only through its durable canonical conversation row; completion or failure then appears through the existing card, conversation, and provider-exchange projections. Live activity-status redesign remains separately gated on operator involvement.
...
Card tree rows show only the depth-indented state ball, human-friendly display-path level number, card title, and card kind. Tags and priority are not displayed in card rows or card detail. Card detail does not render Related or Hierarchy sections; children and ancestors remain navigable through the card tree. Cards has no search, status, type, Clear, or other filter controls; it displays the complete canonical card tree.

The exact opaque card ID in `/cards/:id` is the only selected-row authority. Exactly the tree row with that ID receives the visible selected treatment and semantic `aria-current` state immediately during card-to-card navigation; its lifecycle state ball remains independently visible. For rendering, ancestors of that route card are revealed even when explicit collapse intent would otherwise hide the selected row. That route-required reveal takes precedence only while needed: explicit expand/collapse intent remains recorded throughout detail loading or failure and resumes after navigation leaves the branch. Selection is not inferred from the last click, synchronized into a store, or scrolled into view.

Cards default project expansion is derived from the current canonical card list plus explicit expand/collapse intent. A project arriving after route mount expands without a watcher or route fetch, while an explicit collapse survives list refreshes. Card list replacement or WebSocket-triggered REST refetch recomputes the displayed row from the unchanged route without creating another selection authority. Canonical non-empty `brief.md`, `status.md`, and `review.md` record content is rendered as Markdown in card detail; an out-of-code `[[card:<id>]]` reference remains a card-route link. Following that link changes `/cards/:id`, and the same route-derived selection and ancestor reveal apply. Card Conversations filters the canonical agent-store list reactively by card ID: mount and card-ID changes make no list request, and its explicit Refresh makes one canonical list request while preserving accepted rows and reporting refresh failure separately.
...
Card management is Analyst-owned and process-lifecycle-gated. A displayed or persisted `stopped`/`paused` status is not by itself mutation admission: the private runtime facet must report stopped-ready or settled-paused after admitted work has settled. The Analyst may then use supported semantic card operations such as creating cards, reordering direct children where supported, cancelling dormant work, and subtree deletion. Scoped file URLs shown by the UI use canonical triple-slash form (`project:///`, `record:///`, `tmp:///`, `work:///`, `system:///`). The UI may show relevant record URLs and metadata, but it must not perform mutations directly.
