...
Root bootstrap requests only `project/children`. Expanding an idle node requests only that node; collapse requests nothing, re-expansion of a successful slice requests nothing, and only a settled error exposes node-local Retry. Successful slices are never refreshed by route reveal, live hints, or reconnect and remain the displayed snapshot until CardStore reset. Same-parent callers share the promise in one exact keyed request-owner object. Success, failure, finalization, and owner removal update state only while that exact object remains current; reset aborts and clears all keyed owners before clearing slices, so old completions cannot alter reset or later state. Different parents remain independent.

On a cold deep route, Cards loads root and each required ancestor-parent slice in order, and proceeds only across membership represented by the containing slice. Already successful slices are consumed without refresh. If a retained stale slice lacks the next edge, reveal stops without requesting the descendant, inferring membership, or changing the slice. Current detail may still return 200 and remain visible while no selected tree row or Path can be rendered; detail 404 reports only detail-resource absence and does not clear, retry, or certify hierarchy. Rapid route changes use separate action-local monotonic ownership: a superseded reveal stops after each await and before issuing any later ancestor request, but it never aborts shared children work or changes request/slice authority. Route-owned selection, represented ancestor reveal, expand/collapse, independent loading/errors/scrolling, direct navigation, Back behavior, and generic Files API/VFS/UI behavior remain.
...
The web client has one owner for each separate core resource family: the runtime store owns `/api/state` together with `/api/runtime/status`; CardStore owns response-owned hierarchy slices, selected detail, and separately lazy selected history; and the agent store owns `/api/agents`, selected conversations, and selected LLM exchanges. Debug's State presentation uses runtime and agent owners and its distinct diagnostic resources; it has no card inventory, histogram, complete child projection, or duplicate aggregate owner. Application bootstrap starts runtime, root hierarchy, and agent reads independently of WebSocket availability. Token identity change resets CardStore and then performs an ordinary root load. Card live hints and reconnect do not refresh hierarchy/detail/history; scoped invalidation is deferred. Loaded hierarchy titles may decorate Dashboard or Agents, otherwise those surfaces show the stable card ID. Files and Analyst child shortcuts consume only an explicitly loaded exact slice and do not trigger or imply a complete inventory. `/api/state` contains no `cardIndex` or global card counts.
