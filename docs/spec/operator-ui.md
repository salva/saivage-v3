...
Files reconnect refresh is a browser-local registration owned by the sync client. It does not imply or accept a Files invalidation frame. The wire has no `files` or `processes` unscoped invalidation resource; its unscoped resources are only `runtime` and `timeline`.
