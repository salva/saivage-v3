...
Boot requires `compaction.enabled: true`, route-independent `compaction.input_budget_tokens`, and one exact structured summarizer candidate. `/api/config` therefore exposes a complete enabled policy on a running server. `/api/providers` uses only the Registry-backed ordered structured candidate projection: `account: null` means implicit, while explicit `_implicit` and `_` remain exact names; no account-only list or internal sentinel is exposed. Runtime status has no disabled-protection state because that server cannot boot, and the UI provides no warning-only mode or configuration editor. Exact role/card prompt and tool-schema capacity is prepared once for each stable autonomous planner, reviewer, or executor activation/repair loop before any transcript append or model I/O; continuations retain it while refreshing context. Diagnostics distinguish static-capacity failure from the deterministic best-effort full-request byte/4 candidate heuristic and strict authoritative provider input-context evidence. An eligible rejection with no accepted output or tool effect may yield exactly one forced canonical compaction and one retry under the same source-input identity. Clean no-smaller ends without a retry, a second rejection is terminal, and fatal local/cancellation outcomes use existing failure visibility; none creates a new live status. Analyst/manual calls are outside this autonomous guarantee: one submission may continue through multiple distinct tool calls without compaction or post-rejection replay.
