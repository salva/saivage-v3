...
An `error` caused by a reported reconstructed immediate-child result-append failure is a degraded, non-ready retained-owner state, not still-executing agent work. Runtime/card projections retain current-card and card-owner evidence; retained LLM session history is inactive. The Dashboard disables **Stop project**; Analyst Run and Stop reject; and explicit card cancellation is unavailable until the server process restarts. Independent notifications remain unchanged. Application shutdown cleanup is the sole retained-owner settlement exception. The restart requirement does not make a control available: the existing exactly confirmed **Restart server** action is shown only when the server is already bearer-auth-capable and reports `restart_server_available:true`.
