...
- A growing Saivage-owned JSONL file is append-only and is never replaced. Each logical append is exactly one newline-terminated physical line containing one strict, versioned and type-discriminated envelope with a non-empty `rows` array. A multi-row logical append occupies one physical line, with its rows in semantic order. Owning readers strictly validate each complete envelope and every logical row, then flatten rows in physical-line and array order.
- Growing-file restabilization may truncate only an identifiable incomplete final envelope: a final physical suffix without its terminating newline. A complete malformed envelope, unsupported version or type, invalid logical row, or other complete invalid state must remain present and fail clearly for operator repair; it must not be discarded, normalized, or recovered.
- Rewritten and newly published versioned files use the singular permitted one-target durable-replacement primitive: write one same-directory temporary file, `fsync` that file, rename it over its one target, and `fsync` the containing parent directory. The parent-directory `fsync` completes only that target's replacement and does not coordinate it with any other file.
- Application-built or custom transaction protocols are categorically forbidden regardless of their name, implementation quality, or claimed simplicity. Do not build write-ahead logs, journals, two-phase commit, commit manifests, multi-file or cross-line coordination, recovery engines, transaction emulation, or equivalent protocols.
- Storage must have one owning writer path per file. Do not add subordinate storage locks, PID locks, promise queues, writer registries, generations, generic ledgers, or other storage coordination mechanisms. Application authority, currentness, or admission mechanisms must not be used as same-file concurrency control. In particular, the current `MutationLane` remains application authority/currentness/admission; this policy-only change neither removes nor redesigns it and does not make it a storage lock or writer queue.
- The runtime lifecycle lock remains the process-level exclusion boundary between the serving process and direct mutating CLI commands. A direct offline mutation must own one lifecycle-lock handle from before its first state or audit mutation through the complete state-and-audit operation. A command that observes a live owner delegates through the live service and must not fall back to direct writes.
- Durable-format changes use a reset-only cutover. Do not add migrations, compatibility readers, format probing, adapters, dual paths, or legacy normalization. Operators must stop the service, reset generated persistence while preserving configuration, credentials, operator inputs, source, and documentation, and then start the current binary; mixed-version operation and rollback against current-format state are unsupported.
...
- Existing persistence that replaces growing JSONL, writes bare logical rows, coordinates logical appends across lines, uses subordinate storage coordination, lacks the required strict envelope/restabilization behavior, or otherwise conflicts with this policy is technical debt. The separately planned implementation cutover must correct it; this policy-only change does not claim that current persistence already complies and does not authorize storage-code changes by itself.
