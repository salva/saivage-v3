...
- Card identity is exactly the fixed root ID `project` or a fresh opaque canonical UUID for a non-root card. Generate one UUID per non-root creation attempt and exclusively claim that namespace once; an existing path fails directly. Never derive an ID from namespace discovery or use allocation scans, counters, collision retries, adoption, or reuse.
