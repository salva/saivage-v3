...
- Card identity is the fixed root `project` or `card-<segment>[-<segment>...]`, with one to five lowercase alphabetic segments. Each parent owns a durable spreadsheet sequence (`a` through `z`, then `aa`, and so on) represented by strict `card-child-reservation` rows in its exact card stream. Append one reservation before claiming the child namespace; failed publication consumes the reservation and gaps remain. Canonical membership exists only after the complete child is published and one cumulative `children` snapshot is appended to its parent stream. Never derive allocation or membership from namespace discovery, scans, active children, positions, collision retries, adoption, or reuse.
