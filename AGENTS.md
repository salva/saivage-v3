...
- Card identity is the fixed root `project` or `card-<segment>[-<segment>...]`, with one to five opaque 28-letter lowercase segments. Generate exactly one segment per non-root creation attempt and claim its deterministic hierarchical namespace once; an existing path fails directly. Canonical membership exists only after the complete child is published and one cumulative `children` snapshot is appended to its parent stream. Never derive identity or membership from namespace discovery, scans, counters, collision retries, adoption, or reuse.
