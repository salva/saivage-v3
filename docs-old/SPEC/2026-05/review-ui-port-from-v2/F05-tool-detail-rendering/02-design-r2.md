# F05 — Tool detail rendering: Design (R2)

Writer round 2 for [02-design-r1.md](02-design-r1.md). Binding
critique: [02-design-review-r1.md](02-design-review-r1.md). Approved
...
[F03 r2 analysis](../F03-conversation-rounds/01-analysis-r2.md),
...
public surface. Every replaced helper/type/template/route field is
removed in the same commit set as its replacement.

---

## 0. Coverage map (review → r2)

| # | Reviewer-required item ([02-design-review-r1.md](02-design-review-r1.md)) | Addressed in |
| - | ------------------------------------------------------------------------- | ------------ |
| 1 | Drop the `registerAlias` / `ALIASES` subsystem. Multiple tool names sharing behaviour use a **shared factory**; each tool name registers explicitly via `registerToolPresenter`. No alias side channel; `_registryKeysForTest()` returns only a flat list of names. | [§3.1](#31-registry-and-types), [§3.2](#32-shared-factories-replace-aliases), [§3.3](#33-per-tool-files-with-shared-factories) |
| 2 | Define raw-content ownership. Presenters return ONLY structured `headline`/`detail`. The chip prop bag (owned by F03 r2 §7.2) carries `callContent`/`resultContent` as separate string props; `<ToolChip>` mounts F05's `<FormattedContent>` from those. `ToolCallPresentation`/`ToolResultPresentation` do NOT get a `rawContent` field. | [§1.2](#12-public-presenter-contract-unchanged-from-r1), [§4.1](#41-toolchip-prop-bag-f03-r2-72-verbatim), [§4.2](#42-formattedcontent-and-jsonview-stay-content-owned) |
| 3 | Single canonical public import path. F03 r2 §7.3 imports from `'../../utils/tool-presenters'`. r2 locks that path: presenters live under `web/src/utils/tool-presenters/` and `index.ts` is the public surface; `web/src/utils/tool-presenters.ts` is **deleted**. No re-export shim, no parallel surface. | [§3.1](#31-registry-and-types), [§5](#5-canonical-import-paths-and-file-deletions) |
| 4 | Registry initialization contract. Production code imports only `'../../utils/tool-presenters'` (the `index.ts` barrel). A `barrel-integrity.test.ts` asserts every file in the directory is imported by `index.ts`. A `registry.test.ts` asserts default + every expected tool name are registered after one barrel import. Vite/Rollup retains side-effect imports because `index.ts`'s sub-imports are statements (not unused namespace bindings) and because `package.json` marks the directory `"sideEffects": ["./web/src/utils/tool-presenters/**"]` — restated explicitly here. | [§3.4](#34-initialization-contract-and-tree-shake-rule), [§6.5](#65-registry-and-barrel-integrity-tests) |
| 5 | Reconcile the per-tool matrix. `run_project_command` result detail is `process pid` (matches existing v3 source [tool-presenters.ts L218](../../../web/src/utils/tool-presenters.ts#L218)). The "stdout-tail" wording in F05 analysis r2 §5 is dropped here. Single source of truth: [§3.5](#35-per-tool-coverage-table). `coverage.test.ts` asserts every `EXPECTED_TOOL_NAMES` entry has BOTH a registered call presenter AND a registered result presenter (no accidental `__default__` fall-through). | [§3.5](#35-per-tool-coverage-table), [§6.5](#65-registry-and-barrel-integrity-tests) |
| 6 | Browser-facing seam tests: `ToolChip` ARIA/DOM (no nested interactives), `InlineParts` routing, `FilesView` query handling, agent integration, analyst integration, registry entrypoint. | [§6.1–§6.6](#6-test-plan) |

Strengths from r1 retained (cross-referenced): Proposal B selected
([§2](#2-recommendation-recap)); `InlinePart` union, `json-tokenize`,
`JsonView`, `FormattedContent` shapes ([§4](#4-shared-deliverables));
status-derivation rules ([§3.1](#31-registry-and-types));
non-button `<div role="group">` with sibling links
([§4.1](#41-toolchip-prop-bag-f03-r2-72-verbatim)); 1 MB
`JsonView` raw fallback ([§4.2](#42-formattedcontent-and-jsonview-stay-content-owned)).
...
- F03 r2 design [§7.2](../F03-conversation-rounds/02-design-r2.md#72-toolchipvue--prop-bag-and-template-fixed):
  `<ToolChip>` carries `{ call, result, callContent, resultContent, status, expanded, detailsId, timestamp? }` and is the **single** chip
  renderer used by both the agent surface (F03) and the analyst
  surface (F04).
- F04 r3 [§4.1](../F04-chat-surface-style/01-analysis-r3.md#41-adapter-signatures):
  binds via `v-bind="adaptChatMessageToToolChip(...)"` /
  `v-bind="adaptPendingInvocationToToolChip(...)"`.

### 1.2 Public presenter contract (unchanged from r1)

```ts
// web/src/utils/tool-presenters/index.ts — public exports
export type { InlinePart, ToolStatus, ToolCallPresentation, ToolResultPresentation } from './types';
...
Per review item 2, neither presentation carries a `rawContent`
field. Raw producer payloads flow through `<ToolChip>` as separate
string props (F03 r2 §7.2 — `callContent`, `resultContent`).
Presenters remain pure functions of their own payload only; no
hidden coupling.
...
selected direction, unchanged from r1 §10. Proposal A (single
`tool-presenters.ts` with two maps) is documented in r1 §A and is
not redeveloped here; it remains a viable fallback if the registry
boot story ever breaks in practice. r2 fixes the four blockers the
review raised against Proposal B; nothing about Proposal A changes.
...
```ts
// (see §1.2 for InlinePart / ToolCallPresentation / ToolResultPresentation / ToolStatus)
...
v3 source at [tool-presenters.ts L218](../../../web/src/utils/tool-presenters.ts#L218);
detail is **process pid**, not stdout-tail — review item 5):
...
3. **`sideEffects` manifest preserves the bare-statement imports.**
   ...
   Existing `*.css` side effects are preserved; new line is the
   tool-presenters directory. (The current `web/package.json` does
   not declare `sideEffects`; default is "all side effects
   preserved" — but the project rule is to make this explicit so
   future hardening does not silently drop the registry.)
...
5. **Tests use the canonical entrypoint.** Per-tool unit tests
   ...
   tree-shake mitigation we are testing (review item 4). The
...
(review item 5). It overrides every prior table in F05 (analysis
r2 §5, design r1 §A.3 / §B.4). Tone column applies on the success
path; the error path always uses `danger` (§1.2). Detail for
command tools is `process pid` (matching the existing v3 source,
...
### 4.1 `ToolChip` prop bag (F03 r2 §7.2 verbatim)

The chip lives in `web/src/components/conversation/ToolChip.vue`
and is implemented by F03 r2 design §7.2. F05 references its
contract here for completeness; F05 does NOT redefine it.

```ts
// web/src/components/conversation/ToolChip.vue (F03-owned)
defineProps<{
  call: ToolCallPresentation;          // from F05 — always present
  result: ToolResultPresentation | null; // from F05 — null when no result yet
  callContent: string;                 // RAW producer payload, fed to <FormattedContent> on expand
  resultContent: string | null;        // RAW producer payload, fed to <FormattedContent> on expand
...
Per review item 2: `ToolCallPresentation` / `ToolResultPresentation`
remain pure structured outputs. The raw producer payloads (`call.content`
and `result?.content` for persisted entries; a synthesised text for
pending invocations) travel as separate string props from the
adapter (F04 r3 §4.1) and the agent-side `toolChipPropsFor(pair)`
helper (F03 r2 §7.3). The chip then mounts F05's
`<FormattedContent :content="callContent" />` and (when
...
in r2; no edits required by the review affect those files. Only
the registry surface (§3) and the test plan (§6) change.
...
to the `files` route only through Vue Router's resolved `<RouterLink>`
component, not by importing the store.

### 4.3 `FilesView` routing (unchanged from r1)

[FilesView.vue#L234-L250](../../../web/src/views/FilesView.vue#L234-L250)
becomes:
...
- The `expandedDetail` / `toolCallView` / `toolResultView` helpers
  ...
  [AgentConversationView.vue](../../../web/src/components/agents/AgentConversationView.vue#L63-L103)
  — deleted (replaced by `<ToolChip>` per F03 r2 §10).
- The in-line `<button class="tool-chip">` markup and scoped CSS
  in [AnalystChatPanel.vue#L36-L75 / L298-L347](../../../web/src/components/chat/AnalystChatPanel.vue#L36-L75)
...
the chip-contract test file. F05 r2 lists the cases that protect
F05's contract (the `InlinePart` field names, the routing of file
parts, the raw-content body via `<FormattedContent>`):
...
F03 r2 §11.1 already lists overlapping cases; F05 r2 owns these
...
- `presentToolResult > error path: per-tool result presenter is NOT invoked` (spy via stub registration in a sandboxed registry, see below)
...
import { _registryKeysForTest } from '../../utils/tool-presenters/registry';
...
describe('tool-presenter registry coverage', () => {
  ...
  it('every expected tool name has BOTH a call and a result presenter', async () => {
    // The ToolPresenter interface in types.ts requires both; this is a
    // sanity check that no per-tool file accidentally registers a partial
    // object via `as ToolPresenter` casts.
    const { _internalRegistryEntriesForTest } = await import('../../utils/tool-presenters/registry');
...
(A second test-only enumerator `_internalRegistryEntriesForTest()`
returns `[string, ToolPresenter][]`; both `_*ForTest` helpers are
namespaced with the underscore prefix and excluded from production
import linting via the ESLint rule in §3.4.)
...
F04 r3 §9.1 (`analyst-chat-panel.test.ts`). F05 r2 adds NO new
files at the integration layer; it only ensures the cases below
are present in those existing files:
...
schema-driven, MarkdownText-for-everything) is unchanged in r2.
No new alternatives are introduced; the review's blockers were all
fixable inside Proposal B.
...
- Absolute path: `/home/salva/g/ml/saivage-v3/SPEC/2026-05/review-ui-port-from-v2/F05-tool-detail-rendering/02-design-r2.md`
- Chosen proposal: **B (Registry-based presenters)**, with the
  four review blockers resolved:
  1. Alias subsystem removed; shared factories in `helpers.ts`
     replace it (§3.2).
  2. Raw-content ownership lives on the chip (F03 r2 §7.2's
     `callContent` / `resultContent`), not on the presentations
     (§1.2, §4.1).
  3. Single canonical public path: `web/src/utils/tool-presenters/`
     with `index.ts` as the only production import target
     (§3.1, §3.4, §5).
  4. Explicit initialization contract: barrel-only imports,
     ESLint `no-restricted-imports`, `sideEffects` manifest,
     `assertDefault()` runtime guard, barrel-integrity test
     (§3.4, §6.5).
  5. Per-tool matrix reconciled (run_project_command detail =
     `process pid`; coverage test asserts both halves registered
     per name; no fallthrough) (§3.5, §6.5).
  6. Browser-facing seam tests added: `ToolChip`, `InlineParts`,
     `FilesView` routing, agent + analyst integration (§6).
