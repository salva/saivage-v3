...
Saivage v3 is an autonomous multi-agent runtime for software-development work. A top-level planner decomposes goals into cards, executors perform scoped work, reviewers verify results, and the operator workspace projects cards, agents, files, strict app-log events/derived errors, and runtime state while the Analyst chat is the ordinary mutating user surface, with direct Dashboard **Stop project** and confirmed bearer-only **Restart server** as bounded exceptions. Debug > Errors—not Dashboard—is the durable command, activation, actionable, and runtime-error surface.
...
Activation ownership is callback-free supervisor state: one `activationOwners` map contains plain prepared, admitting, active, settling, uncertain, and containment-retained owners. `CardProcessActor` remains the micro-actor; `ConversationLLMActor` is a direct provider/tool phase state machine.

The direct Conversation LLM owner has no `BaseActor` queue or lifecycle. Every autonomous and Analyst LLM tool call uses the complete LLM-owner-built invocation context; an admitted child lease makes the exact planner wait until supervisor structure/currentness is released and invalidated. Runtime controls are delegation-only and readiness is supervisor-owned. Natural root completion atomically retires a pending or settled Pause before publishing stopped, and Dashboard Stop remains available for retained publication-uncertainty `error`.
...
**Operator egress:** Backend-owned typed projectors preserve structural identity while redacting only schema-classified prose, secrets, URLs, and opaque leaves. Effective config guarantees replacement of provider/account API keys and every stdio MCP environment value regardless of key spelling. Files recognizes the startup-selected config through its admitted lexical path or resolved alias and renders projected effective config; all other admitted work text, including process logs and webfetch stashes, receives text defense. Card detail/children, runtime breadcrumbs, tools, history, and diffs redact card title/status/lifecycle/notification prose while preserving exact card/type/tag/tag-filter/discriminant and dormant planner identities, even when credential-shaped.

The invocation projector recognizes the exact 41 Analyst tools, planner-only `edit_card`/`activate_card`, and terminal `emit_result` (44 identities) in complete, call-row, and result-row shapes. Unsupported, malformed-JSON, and schema-invalid known calls remain readable without invented mates. Complete Agent conversations allow zero or one exact active-in-flight unmatched call with no pending row, exactly one barrier-matched waiting call that is the sole pending row, and no inactive orphan. Bounded `read_agent_session` keeps its exact suffix and counts: isolated results remain valid but never satisfy waiting; inactive allows zero call-only rows, active zero or one without invention, and waiting exactly one matching its sole pending row.

Provider exchange, event, and control rows are source-projected before publication and on read. Provider endpoint scheme/host/port/path and adapter identity remain exact while userinfo/query/fragment are removed. Public MCP status/tools omit integration errors, descriptions, schemas, annotations, `_meta`, and other opaque metadata while preserving exact topology names, `${server}:${tool}` keys, nested stats, browser totals/table, and manager-level process-lifetime statistics. Every `webfetch` result exposes `redacted_url` and no raw URL, and active/waiting call rows expose no raw query. Text/key/path and frontend display heuristics remain likely-accident defense only: arbitrary unmarked secrets in free text are not guaranteed to be detected and credential-like spelling never makes an identifier secret.
...
The application log has one strict `{type,data}` row union with only `event`, `control_action`, and `provider_exchange` lanes. Event kinds are exactly `runtime_diagnostic`, `runtime_actionable_error`, and `mcp_tool_invocation`; Debug Errors derives both runtime kinds and failed MCP invocations. One mandatory boundary performs all preparation/redaction/projection, validation/serialization, directory creation, first publication, and append. Owners order work as settle unrelated operation, publish the required row, issue the exact direct SyncHub hint, then continue. Any boundary failure is one `AppLogPublicationError` with outward precedence over the operation it was trying to record; catches preserve that exact object and perform no read, retry, second append, normal failed result, later attempt, write, or hint. The current propagation graph is direct Conversation LLM to plain Analyst or CardProcess to centralized Supervisor, and the only consuming roots are global Fastify, the registered Analyst WebSocket message observer, and Supervisor's retained project-owner activation rejection callback.

`EventQueryService` is the sole event/error reader. `/api/events` supports strict oldest-page or newest-tail selection with a maximum of 1000; Debug explicitly requests the newest 1000 and one existing `timeline` live-sync callback refreshes both `/api/events` and derived `/api/debug/errors` as two requests. There is no EventBus, generic read-model broadcaster, `/api/debug/timeline`, dedicated error lane, or content-supervision surface. `InvocationService` owns provider-exchange logging independently, and the Analyst mutation settlement runner owns control-action production.
