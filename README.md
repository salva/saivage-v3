...
`init` has six ordered phases: (1) read project identity to select a bound or bootstrap-unbound lock record; (2) exclusively publish that init lifecycle lock; (3) publish missing `.saivage/saivage.yaml`; (4) load and validate the effective configuration and selected workflows; (5) read identity again and, only when absent, create it and bind the held lock; and (6) classify generated state, accepting a strictly valid existing project card or publishing initial state only when all four exact generated roots—`.saivage/cards`, `.saivage/agents`, `.saivage/logs`, and `.saivage/work`—are absent.
...
The application log has one strict `{type,data}` row union with only `event`, `control_action`, and `provider_exchange` lanes. Event kinds are exactly `runtime_diagnostic`, `runtime_actionable_error`, and `mcp_tool_invocation`; Debug Errors derives both runtime kinds and failed MCP invocations. After preparation and validation, the owner strictly reads the exact stream, truncating only an identifiable unterminated suffix and rejecting retained complete malformed data, then validates global logical-ID uniqueness over the existing-plus-candidate rows before one candidate append attempt. Preparation, validation, strict reads without mutation, directory, and acquisition failures remain ordinary. Owner pre-read suffix truncation and the direct append/replacement primitives create `PublicationOutcomeUnknownError` when canonical mutation becomes uncertain; catches preserve that exact object until immediate fatal delivery, with no read, retry, second append, failed result, later attempt, write, hint, halt, API response, or WebSocket frame.
