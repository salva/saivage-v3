...
MCP server entries in `.saivage/saivage.yaml` use `transport: stdio` or `transport: streamable-http`. Stdio entries inherit only the shared safe command environment (`PATH`, `HOME`, `USER`, `LANG`, `TERM`, and `LC_*`), not ambient Saivage, provider, or deployment credentials. Declare every required non-base variable explicitly in that server's `env`; explicit entries override inherited base values.
