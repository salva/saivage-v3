...
Conversation rows use the current strict context-policy format. Each tool binder/catalog entry owns one fixed internal result-policy template distinct from its provider-wire definition; generated terminal, MCP, and unsupported calls declare theirs directly. Mode-typed execution separates the visible `ToolResult` from internal none/observational/canonical evidence. Calls commit exact policy bytes/hash before execution; results commit settlement origin, exact canonical visible-result hash, matching call-policy hash, and result-derived evidence. Validation pairs only the composite session/source-input/call identity and keeps Responses private/visible/tool-result bundles atomic. Internal evidence never enters provider-visible result content. This incompatible conversation-row shape is a reset-only durable-format cutover.
