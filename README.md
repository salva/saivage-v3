...
The application log directly validates and appends only each incoming row without a prior full read. Strict full reads retain complete malformed-data and global duplicate-ID rejection. Physical append completes before the producer's one existing effect or return; only a committed provider exchange whose session strictly satisfies the canonical Agent conversation grammar prompts one Agent refresh, while a throwing strict observer authorizes no rollback or retry. `InvocationService` owns provider-exchange logging independently, and the Analyst mutation settlement runner independently owns new control-action production. Planner reorder and notification operations append no control-action rows; their canonical card/history/notification state and normal tool results remain their evidence. Valid retained current-format control-action rows, including historical Planner rows, remain returned unchanged by `/api/control-actions`. `summary:*` provider evidence and event, error, control-action, and content-review rows do not prompt Agent refresh.
