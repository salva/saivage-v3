...
Activation ownership is callback-free supervisor state: one `activationOwners` map contains plain prepared, admitting, active, settling, uncertain, and containment-retained owners. `CardProcessActor` and `ConversationLLMActor` remain actors. Every autonomous and Analyst LLM tool call uses the complete actor-built invocation context; an admitted child lease makes the exact planner wait until supervisor structure/currentness is released and invalidated. Runtime controls are delegation-only and readiness is supervisor-owned. Natural root completion atomically retires a pending or settled Pause before publishing stopped, and Dashboard Stop remains available for retained publication-uncertainty `error`.
