...
The invocation projector recognizes the [exact shipped tool identities](docs/spec/system-specification.md#exact-shipped-tool-identities) for call arguments. Unsupported, malformed-JSON, and schema-invalid known calls remain readable without invented mates. Current providers return only constructor-created nominal action outcomes; one settlement authority projects them and creates the generic strict success/failure `ToolResult`, whose optional `data` remains opaque. Terminal `emit_result` validates terminal-specific data and uses those same constructors. Complete Agent conversations permit zero or one unmatched tool call only when it is the final durable source row. That prefix has no active, waiting, pending, or snapshot meaning. Bounded `read_agent_session` validates the complete stream before keeping its exact suffix and counts, without expanding the suffix to recover a matching call.
...
Authored records use one reusable current URL, `record:///<name>?card=<id>`, for reads, writes, edits, and record-targeted Webfetch saves; only ordered numeric `&v=N` addresses immutable history. A first admitted write creates an absent dynamic record, subsequent mutations resolve or reuse its current draft, and framework acceptance—not a client head token—closes card-agent drafts. Saved Webfetch text reports `write:{kind,data}`: plain/project/tmp/system destinations carry workspace write data and record destinations carry record mutation data, never a nested `ToolResult`. Audited Analyst record saves preflight before the sole network request and recheck cancellation, intervention readiness, and fresh full current admission after it. Denial and every other structured mutation rejection are top-level failed tool calls.
...
Before starting, configure the required global named-agent catalog, selected global Analyst, named model routes, and the project's card types in `$TARGET_PROJECT/.saivage/saivage.yaml`. The source contract permits exactly two forms: a complete `card_types` map, or omission of `card_types`, which selects the bundled `classic` definitions. `saivage init [--profile <classic|classic-typed>]` materializes the complete selected template once: the full prompt tree into `.saivage/config/prompts`, a provenance marker into `.saivage/config/template.json`, and the complete config YAML — including its explicit `card_types` map — published last. After init the instance is the single runtime authority; templates are data used once at init and never consulted at runtime. An unknown `--profile` name fails before any write. The deleted `card_type_set` selector key now fails strict source validation as an unknown key. `ResolvedConfigAuthority` resolves the source once and supplies one complete effective `card_types` map to all compiler/runtime and outbound effective-config consumers.

| Source | Purpose |
| --- | --- |
| omitted `card_types` | Bundled `classic` default: unchanged project/goal plan-review flows and one-node execution for the other seven types. |
| `init --profile classic` (default) | Materializes the complete classic template: config with explicit classic `card_types` plus the 14-file prompt closure. |
| `init --profile classic-typed` | Materializes the complete typed template: purpose-specific code, test, research, data, architecture, and planning flows (doc and ops remain one-node) plus the 51-file prompt closure. |
| complete `card_types` | Advanced hand-authored replacement of the entire map; never an overlay on bundled definitions. |

To use the typed flows, run `saivage init --profile classic-typed` on a fresh project directory, or copy the classic-typed template's prompt tree into an existing stopped instance (see the upgrade note below). The engine does not branch on template names: it compiles one complete map and uses it everywhere, including read-only Debug Graphs and record descriptors. Under classic-typed, project/goal/architecture expose `brief.md`, `status.md`, and `review.md`; code/test/doc/data/research/ops expose `brief.md` and `status.md`. Record tabs and history always follow those selected compiled declarations rather than a fixed UI list.
...
The following abbreviated shape shows the normal source contract. `saivage init` materializes the complete `classic` template, whose explicit `card_types` map contains the nine definitions in this order: `project`, `goal`, `architecture`, `code`, `test`, `doc`, `data`, `research`, `ops`.
...
server:
  ...
  host: "0.0.0.0"
card_types:
  # init materializes the complete classic map; abbreviated here
  # ...
```
...
As the advanced alternative, hand-author the complete map directly. The following is only a structural excerpt; a source map must include every referenced definition and each definition's complete workflow and records:
...
A hand-authored map replaces the entire default map; there is no partial-map overlay, inheritance, merge, alias, or missing-reference fallback. A map that references typed-only prompts (for example `code-red` or `specialized-plan`) must also carry those prompt files in `.saivage/config/prompts`, because the bundled root is the classic 14-file closure.
...
Changing the template, replacing an explicit map, adding a card type, or changing matching prompt/config inputs is an ordinary stopped configuration change: stop, edit or re-init, and start so one complete effective map is resolved and compiled; no unconditional generated-state reset is required. Strict startup validates every reached active or tombstoned card and retained parent/type/record admission before optional generated-state effects. Matching names and hierarchy do not guarantee compatibility. If startup rejects the selection, restore the compatible map or template and restart, or intentionally perform the existing stopped whole-generated-state reset for a fresh history. Saivage never probes, falls back, merges, migrates, aliases, selectively repairs, or normalizes generated state, and rolling back to a binary/configuration that cannot admit it is unsupported.

**Hand-authored explicit `card_types` maps referencing typed-only prompts:** on this binary the bundled prompt root is the classic 14-file closure, so a hand-authored specialized-graph map without its own prompt tree fails at startup when a typed-only reference (for example `code-red`, `specialized-plan`, `research-explore`, `architecture-draft`) misses at every selection tier. Preserving that behavior requires carrying the specialized prompt files in `.saivage/config/prompts`: a fresh `init --profile classic-typed` materializes the complete 51-file tree, and a one-time stopped copy of the classic-typed template's `prompts/` tree into the existing instance provides it for hand-authored cases. Classic-only references keep resolving byte-identically from the bundled classic root.
...
Prompt files use one singular purpose-first tree: `agents|process|fragments/<cardType|_shared>/<reference>.md`. Card hosts select project card-specific, project shared, bundled card-specific, then bundled shared; the global Analyst checks only project shared then bundled shared. Only exact absence advances. Agent filenames always use `agents.<name>.prompt`, not the agent name. Agents sharing one prompt reference share the same applicable override; configure distinct references for independent content. Process templates allow only raw `&#123;&#123;cardType&#125;&#125;` and render at startup. Hosts may directly include one-level fragments with `&#123;&#123;> fragment-id&#125;&#125;`; nested includes are rejected. Every effective workflow-agent system prompt includes `&#123;&#123;contractDescription&#125;&#125;` exactly once. Packaging validates each registered template standalone and requires its physical bundled tree to equal its own compiled closure exactly—`classic` is locked to 14 files (four shared agent prompts and ten shared process prompts), `classic-typed` to 51—and copies each tree per-template under `dist/src/config/system-templates/<name>/prompts/`. Changes require restart.
...
Current card IDs and their depth ceiling follow the [exact card identity contract](docs/spec/system-specification.md#exact-card-identity-contract), with segments allocated from a parent-local spreadsheet sequence and nested parents restarting that sequence. A card at the final permitted depth must select a card type whose compiled permitted-child set is empty. Every creation starts at the first segment and directly attempts exclusive creation of each exact candidate namespace, advancing only when that `mkdir` returns `EEXIST` and never inspecting or enumerating the collision. A successful namespace claim remains consumed even if publication or linking later fails; membership begins only after complete initial publication and one parent version appends the child to both monotonic `child_membership` and complete `active_child_order`. Widening the unchanged ID field grammar from the former bound to the current source-derived bound is a same-format forward deployment and requires no reset; after deeper cards are created, do not roll back to a binary that accepts only the former bound.

Card streams use aggregate format v3: each physical line remains an envelope `{version:1,type:'rows'}` containing only card artifact rows with `format_version:2`. One strict `cardRecordSchema` defines current, history, and tombstone snapshots. Each parent has monotonic `child_membership` in link chronology and a complete same-set `active_child_order`; both retain tombstoned links, and only the carrier filtered through exact live child folds determines semantic sibling order. Directory creation claims identity, complete initial publication proves the child, and one later parent append adds the child to both arrays. A real reorder changes only `active_child_order`, placing requested active IDs first and retained tombstones afterward in prior carrier-relative order; an active-order identity request writes nothing and reports zero. Generic card patches cannot write either relationship field or lifecycle. See the canonical system specification and the runbook's incompatible-cutover procedure.
...
Card type is selected at creation and is immutable for the card's entire durable lifetime. Directory-owned child claiming, the two-kind card stream, removal of card `position`, and the card-stream-v3 relationship split are named reset-only durable changes. Applying one of those incompatible cutovers requires each affected deployment to stop, preserve configuration, credentials, operator inputs, source, skills, instructions, and canonical documentation, run the matching current built `saivage reset` to replace generated persistence wholesale, and then start that binary; no compatibility check or apparently unchanged history exempts an affected installation. Card artifact row format 1, old `children` records, mixed rows, and old `card-child-reservation` rows are unsupported and rejected, never compatibility-read or migrated. A successful candidate `mkdir` consumes its segment even if later publication or linking fails, and only the parent's committed monotonic `child_membership` grants membership.
...
Recovery from a dirty shutdown guarantees only that the reconstructed runtime is internally structurally consistent and runnable; semantic completeness is best-effort and is not guaranteed (a stopped card may resume, a done card may reactivate if its trace was lost, a duplicate activation is harmless). Explicit Run traverses complete canonical linked membership and proves every running card belongs to one unique project-rooted chain before writing. It then processes each selected card leaf-to-root as one unit: derive that card's deterministic named sessions from its compiled workflow, stabilize all eligible conversations, and immediately publish that same card `stopped` before advancing. An exact existing `model_recovered` row remains read-only conversation state but never waives the card stop or authorizes resume/redispatch. Only project starts through configured STOPPED; descendants require later ordinary immediate-parent activation. An unmatched parent `activate_card` is settled as ordinary interrupted outcome-unknown work; terminal child results are never reconstructed or replayed. A partial reset is not atomic: the first error stops the attempt, stopped descendants below the remaining running ancestor prefix are valid, and a later Run freshly selects that prefix or stopped project. Separately, natural root settlement requires the strict pre-publication chain to be exactly `[project]`; invalid topology cannot commit a terminal root version. Intervention-ready Analyst configured-record/card edits and immediate-parent named-agent edits preserve stopped.

Conversation and app-log mutation use direct synchronous domain-owner functions. Stable role conversations are append-only: compaction never replaces a version or writes a cache. Configured card-node and Analyst persisted turns all use the singular prepared conversation actor. Each activation freezes one immutable static prefix and prepares typed dynamic context and one exact prompt/tool budget. Provider tool definitions are wire-only; executors return nominal action outcomes, while the separate compiled invocation contracts own evidence policy and the settlement authority alone creates the post-outbound `ToolResult`, canonical bytes, hash, and evidence. Bounded discovery/read/version fit checks use those exact post-redaction canonical bytes; current and immutable card summaries and notifications share one projector. The conversation append owner returns the settled facts through the actor, so immediate Analyst REST and WebSocket activity passes the complete durable result unchanged while projecting only arguments. Compaction retains its existing coverage and admission behavior.
...
`npm run check:export-consumers` is the phase-1 semantic external-consumer guard for exactly the tracked TypeScript modules under `src/contracts/**` and `src/schemas/**`; it is not whole-repository dead-export coverage. A production external consumer is a supported non-test TypeScript module outside the declaring module that semantically references the exact export, and a test external consumer is a supported test TypeScript module outside the declaring module that does the same. Both production-consumed and test-only exports pass. An export used only within its declaring module is local-only, and one with no semantic reference is zero-use; both fail. Stale imports, unresolved governed-module edges, unsupported whole-module or reflective uses, and stale or invalid exceptions also fail rather than establishing consumption. Import bookkeeping, re-export routing without a terminal semantic reference, broad namespace reflection/spread, and source-text `?raw` reads do not count as consumers.

Run `node scripts/check-export-consumers.js --report-test-only` to print the complete sorted phase-1 test-only set and its exact semantic test locations without changing pass/fail. `scripts/export-consumer-allowlist.json` is a strict top-level array and exceptional allowlist for exact unobservable entrypoints or exact reflective consumers only: entries require one exact governed module/export, one exact tracked non-governed consumer, concrete evidence, and a permitted kind; wildcard, directory-wide, module-wide, test-only, generic, baseline, invalid, and stale entries fail. The expected allowlist is empty.

`npm run lint` runs the export-consumer guard before the existing stamp-producer, ESLint, backend import-boundary, and web-component boundary checks. The backend import-boundary scan currently reports accumulated debt as advisory findings; this is not a permanent waiver of the intended package boundaries.

Run the validation profile that matches the change type. The checked-in GitHub Actions workflow at [`.github/workflows/validation.yml`](.github/workflows/validation.yml) runs only on pushes to `master`. It is least-privilege and secret-free (`contents: read`, no `secrets.*` or token-like env assignments), cancels superseded runs for the same workflow/ref, and sets up Node.js 24 with npm caching. The `routine-docs` job runs `npm run validate:routine` and `npm run validate:docs` on every selected push. A fail-closed `classify-changes` job gates `backend-jest-build`, `backend-e2e`, `ui-vitest`, `browser-smoke`, and `dependency-hygiene` by changed paths, and `validation-required` aggregates every applicable conclusion. The `backend-jest-build` job performs the dual clean install—root `npm ci`, then web `cd web && npm ci`—before the root build and non-E2E Jest suite. The independently visible `backend-e2e` job uses a root clean install and runs `npm run test:e2e` for backend, package/workflow, and fail-closed full-validation changes; it needs no web install, browser, secret, or external service. For every applicable UI path, `ui-vitest` runs exactly `npm run web:typecheck && npm run web:test`; browser smoke remains a separate Playwright job. Dependency hygiene applies to package/workflow pushes (including fail-closed full validation) and runs the production `npm run audit:security` gate.
...
Root `npm test` remains the complete non-E2E backend authority. It runs the ordinary parallel Jest set first, followed by the exact serial real-terminal-child suite after the ordinary workers exit. Use `npm run test:parallel -- <Jest arguments>` or the unchanged `test:direct` helper for focused tests in the ordinary Jest set; that set excludes the real terminal-child suite. Use `npm run test:terminal-child` as the focused command for that exceptional suite. `validate:release` runs singular `npm test` and then the distinct `npm run test:e2e` tier exactly once; CI likewise keeps non-E2E Jest in `backend-jest-build` and backend E2E in `backend-e2e`.
...
`npm run web:test:e2e:smoke` runs the complete self-contained browser profile and composes exactly the two current Playwright owners: `npm run web:test:e2e:preview-smoke` covers every production-preview smoke test, while `npm run web:test:e2e:browser-client-smoke` covers the one source browser-client test. The preview owner builds and starts the production preview server; the browser-client owner starts the Vite dev server because that test imports source modules directly. Neither owner contacts a live Saivage deployment. Install its local Chromium prerequisite with `npm run web:test:e2e:install`; on hosts that require browser system libraries, install those Chromium host dependencies before running the profile.
...
The backend build gate and `validate:release` package each registered template's prompt tree under `dist/src/config/system-templates/<name>/prompts/` and then run a compiled per-template prompt-composition smoke against those packaged trees. Run `npm run test:compiled-prompt-composition` to repeat that focused packaged-layout check against existing build output.
...
its constituent commands remain available for focused diagnosis, but release
validation does not require an additional manual backend-E2E invocation.
