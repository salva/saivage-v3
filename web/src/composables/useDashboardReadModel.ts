...
export function useDashboardReadModel(options: {
  ...
  cardsStore: Pick<ReturnType<typeof useCardStore>, 'currentCard' | 'childrenOf'>;
}): DashboardReadModel {
  const displayedGoalId = computed<string | null>(() => options.cardsStore.currentCard?.id ?? null);
  const goalChildren = computed<CardRecord[]>(() => displayedGoalId.value ? options.cardsStore.childrenOf(displayedGoalId.value) : []);
