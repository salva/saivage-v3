...
function arrayCount(ctx: ResultPresenterContext, noun: string) {
  ...
  return { headline: list ? textPart(`${list.length} ${noun}${list.length === 1 ? '' : 's'}`) : textPart(ctx.rawContent, 96) };
}
...
export const TOOL_PRESENTERS = {
  ...
  glob: { action: 'Glob', group: 'context', call: (a) => ({ icon: '📂', headline: pathParts(a.directory), detail: textPart(a.pattern) }), result: (ctx) => { const n = Array.isArray(ctx.dataRecord?.matches) ? ctx.dataRecord.matches.length : null; return { headline: n === null ? textPart(ctx.rawContent, 96) : textPart(`${n} match${n === 1 ? '' : 'es'}`) }; } },
  grep: { action: 'Grep', group: 'context', call: (a) => ({ icon: '🔎', headline: textPart(a.pattern, 80), detail: a.path === undefined ? undefined : pathParts(a.path) }), result: (ctx) => { const n = Array.isArray(ctx.dataRecord?.matches) ? ctx.dataRecord.matches.length : null; return { headline: n === null ? textPart(ctx.rawContent, 96) : textPart(`${n} match${n === 1 ? '' : 'es'}`) }; } },
  ...
  mcp_tool_call: { action: 'MCP', call: (a) => ({ icon: '🔌', headline: textPart(`${str(a.serverName)}/${str(a.toolName)}`), detail: a.args === undefined ? undefined : textPart(a.args, 72) }), result: (ctx) => ({ headline: textPart(ctx.data, 96) }) },
  ...
  read_agent_session: { action: 'Session', group: 'context', call: (a) => ({ icon: '🧵', headline: textPart(`session ${str(a.sessionId)}`) }), result: (ctx) => { const n = Array.isArray(ctx.dataRecord?.messages) ? ctx.dataRecord.messages.length : null; return { headline: n === null ? textPart(ctx.rawContent, 96) : textPart(`${n} message${n === 1 ? '' : 's'}`) }; } },
  ...
  read_runtime_errors: { action: 'Errors', group: 'context', call: (a) => ({ icon: '🩺', headline: textPart(`errors × ${str(a.limit ?? 50)}`) }), result: (ctx) => describeJsonlTail(ctx, 'errors', 'errors') },
  read_runtime_events: { action: 'Events', group: 'context', call: (a) => ({ icon: '📜', headline: textPart(`events × ${str(a.limit ?? 50)}${a.kind ? ` [${str(a.kind)}]` : ''}`) }), result: (ctx) => describeJsonlTail(ctx, 'events', 'events') },
  ...
  websearch: { action: 'Search', group: 'web', call: (a) => ({ icon: '🌐', headline: textPart(a.query, 80) }), result: (ctx) => { const n = Array.isArray(ctx.dataRecord?.results) ? ctx.dataRecord.results.length : null; return { headline: n === null ? textPart(ctx.rawContent, 96) : textPart(`${n} result${n === 1 ? '' : 's'}`) }; } },
