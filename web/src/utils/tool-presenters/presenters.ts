...
export const TOOL_PRESENTERS = {
  ...
  diff_card_versions: { action: 'Diff', group: 'context', call: (a) => ({ icon: '🔀', headline: cardPart(a.card_id), detail: textPart(`v${str(a.from_version)} → ${a.to_version === 'current' ? 'current' : `v${str(a.to_version)}`}`) }) },
  ...
  get_card_version: { action: 'Version', group: 'context', call: (a) => ({ icon: '🕘', headline: cardPart(a.card_id), detail: textPart(`v${str(a.version)}`) }) },
  ...
  list_card_versions: { action: 'Versions', group: 'context', call: (a) => ({ icon: '🕘', headline: cardPart(a.card_id) }), result: (ctx) => arrayCount(ctx, 'version') },
