    ...
    <div class="debug-content">
      <div v-if="localActiveTab === 'state'" class="debug-tab-content">
        <section class="debug-section" data-testid="debug-runtime-state">
          ...
          <div v-if="runtime" class="debug-grid">
            ...
            <div class="debug-grid-item">
              <span class="dg-key">Started:</span
              ><span class="dg-value">{{ fmtDate(runtime.started_at) }}</span>
            </div>
            <div class="debug-grid-item">
              <span class="dg-key">Current Card:</span
        ...
        <section class="debug-section">
          <div class="debug-section-header operator-header">
            <div>
              ...
              <p class="operator-subtitle">
                Inspect runtime state here. Ask the Analyst to Run, Pause, or Shutdown the runtime;
                use Debug &gt; Errors for durable command, precondition, activation, and
                actionable-error evidence.
              </p>
            </div>
            <div class="operator-actions-inline">
              <button
                class="sv-fetch-btn"
                :disabled="operatorPanelBusy"
                @click="refreshOperatorControl"
              >
                Refresh
              </button>
            </div>
          ...
          <div v-else class="operator-runtime-card">
            <div class="operator-runtime-summary">
              <div class="debug-grid-item">
                <span class="dg-key">Status:</span
                ><StatusBadge :status="statusForRuntimeStatus(runtimeStatusLabel)" />
              </div>
              <div class="debug-grid-item">
                <span class="dg-key">Current Card:</span
                ><span class="dg-value mono">{{ currentCardId || 'none' }}</span>
          ...
          <div v-for="entry in errorSourceEntries" :key="entry.source" class="error-source-group">
            ...
            >
              <div class="error-header">
                <span class="error-severity-badge" :class="'sev-' + err.severity">{{
                  err.severity
                }}</span>
      ...
      <div v-if="localActiveTab === 'agents'" class="debug-tab-content">
        <section class="debug-section">
          ...
          />
          <StatusBanner
            v-if="sessionsRefreshing"
            tone="stale"
            message="Refreshing agent sessions…"
          />
          <ViewState
            v-if="sessionsLoading"
            state="loading"
            title="Loading agent conversations..."
          />
          <ViewState
          ...
          <div v-else class="agent-debug-layout">
            <aside class="agent-debug-sidebar" aria-label="Persisted agent sessions">
              ...
              >
                ...
                <span class="agent-debug-session-meta"
                  >{{ session.agent_name }} · {{ session.session_scope }}</span
                >
              </button>
            </aside>
            <div>
      ...
      <div v-if="localActiveTab === 'mcp'" class="debug-tab-content">
        ...
        <ViewState
          v-else-if="mcpStore.error"
          state="error"
          title="Failed to load"
          :message="mcpStore.error"
        />
        <ViewState
          v-else-if="mcpStore.serverCount === 0"
          state="empty"
          title="No MCP servers configured or running."
        />
        <div v-else class="mcp-content">
          <section class="debug-section">
            ...
            <div class="debug-grid">
              <div class="debug-grid-item">
                <span class="dg-key">Servers:</span
                ><span class="dg-value">{{ mcpStore.serverCount }}</span>
              </div>
              <div class="debug-grid-item">
                <span class="dg-key">Tools:</span
                ><span class="dg-value">{{ mcpStore.toolCount }}</span>
              </div>
              <div class="debug-grid-item">
                <span class="dg-key">Invocations:</span
                ><span class="dg-value"
                  >{{ mcpStore.totalInvocations }} ({{ mcpStore.totalErrors }} errors)</span
                >
              </div>
              <div v-if="mcpStore.lastRefreshed" class="debug-grid-item">
                <span class="dg-key">Last Refreshed:</span
                ><span class="dg-value">{{ fmtDate(mcpStore.lastRefreshed) }}</span>
              </div>
            </div>
          </section>
          <section v-for="server in mcpStore.servers" :key="server.name" class="debug-section">
            <h4 class="debug-section-title mcp-server-title">
              ...
              <span class="mcp-server-badge" :class="'mcp-status-' + server.status">{{
                server.status
              }}</span>
              ...
              <div class="mcp-tool-stats">
                <span class="mcp-stat-item" title="Total invocations"
                  ><svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                    ...
                    <line x1="4" y1="9" x2="8" y2="9" stroke="currentColor" stroke-width="1" /></svg
                  >{{ tool.stats.total }}</span
                >
                <span class="mcp-stat-item mcp-stat-success" title="Successful invocations"
                  >✓ {{ tool.stats.success }}</span
                >
                <span class="mcp-stat-item mcp-stat-error" title="Failed invocations"
                  >✗ {{ tool.stats.error }}</span
                >
                <span
                  v-if="tool.stats.lastInvokedAt"
                  class="mcp-stat-item mcp-stat-time"
                  title="Last invoked"
                  >{{ fmtDate(tool.stats.lastInvokedAt) }}</span
                >
              </div>
            </div>
          </section>
      ...
      <div v-if="localActiveTab === 'processes'" class="debug-tab-content">
        <div class="debug-section-header operator-header">
          <div>
            ...
            <p class="operator-subtitle">
              Inspect Saivage-managed process records using redacted commands and contained log
              references.
            </p>
          </div>
          <div class="operator-actions-inline">
            <button
              ...
              @click="debugStore.fetchProcesses()"
            >
              Refresh
            </button>
          </div>
        ...
        <ViewState
          v-else-if="processesError"
          state="error"
          title="Failed to load"
          :message="processesError"
        />
        <ViewState
        ...
        <div v-else class="processes-list">
          ...
          >
            <div class="process-header">
              ...
              <span class="process-status-badge" :class="'ps-' + proc.status">{{
                proc.status
              }}</span>
            ...
            <div class="process-details">
              <div class="pd-row">
                <span class="pd-key">Command:</span
                ><span class="pd-value mono wrap">{{ proc.command }}</span>
              </div>
              <div class="pd-row">
                <span class="pd-key">Card:</span
                ><span class="pd-value mono">{{ proc.card_id }}</span>
              </div>
              <div class="pd-row">
                <span class="pd-key">Session:</span
                ><span class="pd-value mono">{{ proc.session_id || 'none' }}</span>
              </div>
              <div class="pd-row">
                <span class="pd-key">Owner kind:</span
                ><span class="pd-value mono">{{ proc.owner_kind || 'unknown' }}</span>
              </div>
              <div class="pd-row">
                <span class="pd-key">Owner id:</span
                ><span class="pd-value mono">{{ proc.owner_id || 'unknown' }}</span>
              </div>
              <div class="pd-row">
                <span class="pd-key">Working directory:</span
                ><span class="pd-value mono wrap">{{
                  proc.cwd || 'Unavailable or unsafe to display'
                }}</span>
              </div>
              <div v-if="proc.ended_at" class="pd-row">
                <span class="pd-key">Ended:</span
                ><span class="pd-value">{{ fmtDate(proc.ended_at) }}</span>
              </div>
              <div class="pd-row">
                <span class="pd-key">Exit code:</span
                ><span class="pd-value mono">{{ proc.exit_code ?? '-' }}</span>
              </div>
              <div v-if="proc.timed_out" class="pd-row">
                <span class="pd-key">Timed out:</span><span class="pd-value">Yes</span>
              </div>
            </div>
              ...
              <div v-else>
                <div v-for="logEntry in processLogEntries(proc)" :key="logEntry.key" class="pd-row">
                  ...
                  <span v-if="logEntry.value" class="pd-value mono wrap"
                    >{{ logEntry.value }}
                    <button class="process-link-button" @click="browseProcessLog(logEntry.value)">
                      Browse
                    </button></span
                  >
      ...
      >
        <section class="debug-section">
          <div class="debug-section-header operator-header">
            ...
            <button
              class="sv-fetch-btn"
              :disabled="graphsLoading || graphsRefreshing"
              @click="debugStore.fetchGraphs()"
            >
              Refresh
            </button>
          </div>
          <StatusBanner
            v-if="graphsRefreshing"
            tone="stale"
            message="Refreshing compiled graphs…"
          />
          <StatusBanner
            v-else-if="graphsRefreshError"
            tone="warning"
            :message="graphsRefreshError"
          />
      ...
      <div v-if="localActiveTab === 'doctor'" class="debug-tab-content">
        <section class="debug-section">
          ...
          <template v-else>
            <div
              class="doctor-status-banner"
              :class="doctorStatus === 'ok' ? 'doctor-ok' : 'doctor-issues'"
            >
            ...
            <div class="doctor-checks-list">
              ...
              >
                ...
                <div class="check-body">
                  ...
                  ><span v-if="check.details" class="mcp-sep" aria-hidden="true">·</span
            ...
            <div v-if="doctorIssues.length > 0" class="doctor-issues">
              ...
              >
                <span class="issue-severity-badge" :class="'iss-' + issue.severity">{{
                  issue.severity
                }}</span
...
.error-item.sev-error {
  border-left-color: var(--danger);
}
.error-item.sev-warning {
...
.error-severity-badge.sev-error {
  background: var(--entry-danger-bg);
  color: var(--danger);
}
.error-severity-badge.sev-warning {
  background: var(--entry-warn-bg);
  color: var(--warn);
}
.error-severity-badge.sev-info {
  background: var(--entry-user-bg);
  color: var(--accent-2);
}
.error-type {
...
.agent-debug-session-id {
  color: var(--accent-2);
}
.agent-debug-session-meta {
  font-size: 11px;
  color: var(--text-muted);
}
.agent-debug-toolbar {
...
.mcp-server-badge.mcp-status-running {
  background: var(--entry-accent-bg);
  color: var(--accent);
}
.mcp-server-badge.mcp-status-stopped {
  background: var(--surface-3);
  color: var(--text-muted);
}
.mcp-server-badge.mcp-status-error {
  background: var(--entry-danger-bg);
  color: var(--danger);
}
...
.process-status-badge.ps-running {
  background: var(--entry-accent-bg);
  color: var(--accent);
}
.process-status-badge.ps-exited {
  background: var(--entry-user-bg);
  color: var(--accent-2);
}
.process-status-badge.ps-failed {
  background: var(--entry-danger-bg);
  color: var(--danger);
}
.process-status-badge.ps-killed {
  background: var(--entry-warn-bg);
  color: var(--warn);
}
.process-time {
