  ...
  <div data-testid="route-agents">
    ...
    >
      <template #list>
        ...
        />
        <ViewState
          v-else-if="sessionsUnauthorized && !agentStore.sessionsLoaded"
          class="agents-unauthorized"
          state="unauthorized"
              ...
              <div class="session-list">
                ...
                >
                  ...
                  <div class="session-meta">
                    <button
                      v-if="session.card_id"
                      type="button"
                      class="session-card-link"
                      @click.stop="goToCard(session.card_id)"
                    >
                      {{ cardTitle(session.card_id) }}
                    </button>
                  </div>
                  <div class="session-time">
                    Started:
                    <span :title="timestampTitle(session.started_at)">{{
...
const selectedSessionId = computed(() =>
  routeSession.value.kind === 'valid' ? routeSession.value.sessionId : null,
);
...
function goToCard(id: string): void {
  void router.push({ name: 'card-detail', params: { id } });
}
...
onMounted(() => {
  closeAgents = liveSync.openAgents(async (frame) => {
    await agentStore.reconcileMembership(frame);
  });
});
onUnmounted(() => {
  closeAgents?.();
...
.agents-content > :deep(.status-banner) {
  margin: 0 0 12px;
}
.agents-loading,
.agents-unauthorized,
