...
export const useCardStore = defineStore('cards', () => {
  ...
  function selectRecordVersion(name:LiveSyncCardRecordName,version:number):Promise<void>{
    ...
    const versionController=new AbortController();let versionOwner!:RequestOwner;const versionPromise=getRecordVersion(id,name,version,versionController.signal).then((selected)=>{if(recordVersionOwners.get(name)!==versionOwner||selectedCardId.value!==id||cardRecords.value[name]?.selectedVersion!==version)return;cardRecords.value=withKey(cardRecords.value,name,{...cardRecords.value[name]!,selected,selectedLoading:false,selectedError:null});}).catch((error:unknown)=>{if(recordVersionOwners.get(name)!==versionOwner||aborted(error))return;cardRecords.value=withKey(cardRecords.value,name,{...cardRecords.value[name]!,selectedLoading:false,selectedError:message(error,'Historical version not found')});}).finally(()=>releaseRequestOwner(recordVersionOwners,name,versionOwner));versionOwner=markRaw({controller:versionController,promise:versionPromise});replaceRequestOwner(recordVersionOwners,name,versionOwner);
