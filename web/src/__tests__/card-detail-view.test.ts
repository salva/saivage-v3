...
describe('CardDetailView S06 read-only detail contract', () => {
  ...
  it('shows typed initial and refresh not-found without stale detail or subordinate surfaces', async () => {
    ...
    store.selectedDetail = { cardId: 'card-a', card: cardView('card-a', { title: 'Accepted old detail' }), records: [] }; store.selectedDetailError = null; await nextTick();
    expect(wrapper.text()).toContain('Accepted old detail'); expect(wrapper.text()).toContain('accepted records');
    store.selectedDetail = null; store.selectedDetailError = { kind: 'not-found', status: 404, message: 'gone' }; await nextTick();
  ...
  it('excludes prior-route success, loading, and error while a new valid route is pending, then renders only its result', async () => {
    ...
    store.selectedCardId = 'card-a'; store.selectedDetail = { cardId: 'card-a', card: cardView('card-a', { title: 'Old route success' }), records: [] }; store.selectedDetailLoading = true; store.selectedDetailError = { kind: 'network', status: null, message: 'Old route error' };
    let release!: () => void; const pending = new Promise<void>((resolve) => { release = resolve; });
    vi.spyOn(store, 'fetchCardDetail').mockImplementation(async (id) => { store.selectedCardId = id; store.selectedDetail = null; store.selectedDetailLoading = true; store.selectedDetailError = null; await pending; store.selectedDetail = { cardId: id, card: cardView(id, { title: 'New route result' }), records: [] }; store.selectedDetailLoading = false; });
  ...
  it('keeps valid current detail and generic transient Retry behavior', async () => {
    ...
    store.selectedCardId = 'card-a'; store.selectedDetail = { cardId: 'card-a', card: cardView('card-a', { title: 'Current valid detail' }), records: [] }; await nextTick(); expect(wrapper.text()).toContain('Current valid detail');
  ...
  it('presents stopped as inactive recovery history without problematic styling or controls', async () => {
    ...
    store.selectedDetail = { cardId: 'card-a', card: cardView('card-a', { lifecycle: { status: 'stopped', result: null, error: null, completed_at: null }, allowedActions: ['card.start', 'card.cancel', 'card.delete'] }), records: [] };
    await nextTick();
