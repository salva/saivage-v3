...
describe('kill_process presenter', () => {
  it('renders a structured call presentation for kill_process', () => {
    const view = presentToolCall(callEnvelope('kill_process', { path: '.saivage/plan.json', command: 'npm test', cardId: 'card-1', id: 'card-1', goalId: 'goal-1', sessionId: 'session-1', content: 'hello' }));
