...
function createConn(initial: WsConnectionState = 'offline') {
  ...
  return {
    ...
    emitEvent(envelope: Parameters<WsEventHandler>[0]) {
...
describe('SyncClient', () => {
  ...
  it('subscribes without randomUUID, refetches on acknowledgement, and unsubscribes with the current opaque lease', async () => {
    ...
    emitOpen();
    await flush();
    const subscribe = vi.mocked(conn.sendRaw).mock.calls.at(-1)![0] as {
      t: string;
      resource: string;
      id: string;
      lease: string;
  ...
  it('shares one conversation subscription until its final consumer leaves', async () => {
    ...
    const subscriptions = vi
      ...
      .filter((frame: any) => frame.t === 'subscribe');
    ...
    expect(
      vi
        .mocked(conn.sendRaw)
        .mock.calls.map(([frame]) => frame)
        .filter((frame: any) => frame.t === 'unsubscribe'),
    ).toHaveLength(0);
    closeSecond();
    expect(conn.sendRaw).toHaveBeenLastCalledWith({
      t: 'unsubscribe',
      resource: 'conversation',
      id: 'agent:planner:project',
      lease,
    ...
    expect(vi.mocked(conn.sendRaw).mock.calls.map(([frame]) => frame)).toEqual([
      {
        ...
        resource: 'conversation',
        id: 'agent:planner:project',
        lease: '000102030405060708090a0b0c0d0e0f',
      },
      {
        t: 'subscribe',
        resource: 'conversation',
        id: 'agent:analyst:global',
        lease: '101112131415161718191a1b1c1d1e1f',
      },
    ]);
  });
  ...
  it('refetches conversations only from canonical live-sync invalidate frames', async () => {
    ...
    harness.emitOpen();
    await flush();
    const lease = (vi.mocked(harness.conn.sendRaw).mock.calls.at(-1)![0] as { lease: string })
      .lease;
    harness.emitSync({
      t: 'subscribed',
      resource: 'conversation',
