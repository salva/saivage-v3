...
describe('InlineParts', () => {
  it('renders file parts as canonical files router links and url parts as external anchors', async () => {
    ...
    const wrapper = mount(InlineParts, { props: { parts: [{ kind: 'file', root: 'output', path: '.saivage-work/a.log' }, { kind: 'url', href: 'https://example.test' }] }, global: { plugins: [r, createPinia()] } });
