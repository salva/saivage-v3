...
const CARD = 'card-a';
const snapshot = historyCard(CARD, { title: 'before', version_seq: 2 });
...
describe('CardHistoryPanel', () => {
  ...
  function success(): void { vi.mocked(listCardHistory).mockResolvedValue({ card_id: CARD, versions: [header], total: 1 }); vi.mocked(getCardHistoryEntry).mockResolvedValue({ card_id: CARD, version: 2, entry_id: entryId, published_at: publishedAt, artifact: { kind: 'card-version', card: snapshot } }); vi.mocked(getCardDiff).mockResolvedValue({ card_id: CARD, from: 2, to: 3, diff: [{ field: 'title', before: 'before', after: 'after' }] }); }
  it('renders route publication metadata, snapshot, and diff without removed change metadata', async () => { success(); const wrapper = mount(CardHistoryPanel, { props: { cardId: CARD }, global: { plugins: [pinia] } }); await flushPromises(); const text = wrapper.text(); expect(text).toContain('Card version'); expect(text).toContain('Snapshot version'); expect(text).toContain('Published at'); expect(text).toContain('before'); expect(text).toContain('after'); expect(text).not.toContain('Changed by'); expect(text).not.toContain('Reason'); expect(text).not.toContain('Initial card version'); expect(text).not.toContain('none recorded'); });
