<template>
  <span class="status-badge" :class="`tone-${status.tone}`" :title="status.description" :aria-label="ariaLabel">
    <span
      v-if="showDot || status.indicator === 'stopped-dot'"
      class="status-badge__dot"
      :class="{ 'status-badge__dot--stopped': status.indicator === 'stopped-dot' }"
      aria-hidden="true"
    ></span>
...
.status-badge__dot--stopped { background:var(--card-status-stopped); }
