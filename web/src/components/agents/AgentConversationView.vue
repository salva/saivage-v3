<template>
  <div class="conversation-container">
    ...
    <template v-else>
      ...
      <RelatedLinks label="Related" :links="relatedLinks" />
      ...
      <div ref="timelineControls.scrollAreaRef" class="conv-rounds" @scroll="timelineControls.handleTimelineScroll">
...
const relatedLinks = computed(() => {
  ...
  if (!session) return [];
  const links: Array<{ kind: 'card'; id: string; label: string }> = [];
  if (session.goal_card_id) links.push({ kind: 'card', id: session.goal_card_id, label: `Goal ${session.goal_card_id}` });
  if (session.card_id && session.card_id !== session.goal_card_id) links.push({ kind: 'card', id: session.card_id, label: `Card ${session.card_id}` });
  return links;
});
