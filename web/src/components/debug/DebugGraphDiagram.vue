...
function entryDescription(entry: Graph['entries'][number]): string { return `${entry.entry} lifecycle entry targets ${entry.node_id}${entry.prompt ? ` with prompt ${entry.prompt.reference}` : ''}`; }
