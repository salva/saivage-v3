...
<script setup lang="ts">import { useRouter } from 'vue-router'; import type { AgentConversationEntry, EntityLink } from '../../types/view-models'; import MarkdownText from '../content/MarkdownText.vue'; defineProps<{ entry: AgentConversationEntry }>(); const router = useRouter(); function navigate(link: EntityLink): void { if (link.entity_type === 'card') void router.push({ name: 'card-detail', params: { id: link.entity_id } }); else if (link.entity_type === 'process') void router.push({ name: 'debug', query: { tab: 'processes', process: link.entity_id } }); else if (link.entity_type === 'artifact' || link.entity_type === 'attachment') void router.push({ name: 'files', query: { path: link.entity_id } }); }</script>
<style scoped>
