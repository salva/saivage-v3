...
describe('analyst runtime tools', () => {
  it('projects process logs as canonical work URLs', async () => {
    ...
    try {
      ...
      if (result.success) expect(result.data).toEqual([expect.objectContaining({ card_id: 'card-1', owner_kind: 'agent', owner_id: 'agent-1', logs: { stdout: 'work:///cards/card-1/processes/proc-1/stdout.log', stderr: 'work:///cards/card-1/processes/proc-1/stderr.log' } })]);
    } finally {
