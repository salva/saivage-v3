...
describe('analyst runtime tools', () => {
  it('projects process logs as canonical work URLs', async () => {
    ...
    try {
      ...
      if (result.success) expect(result.data).toEqual([expect.objectContaining({ logs: { stdout: 'work:///processes/proc-1/stdout.log', stderr: 'work:///processes/proc-1/stderr.log' } })]);
    } finally {
