...
describe('WebProvider', () => {
  ...
  it('rejects multimodal webfetch before fetch while accepting auto and text modes', async () => {
    ...
    try {
      ...
      await expect(invokeTestTool(surface, 'webfetch', { url: 'https://93.184.216.34/auto', read_mode: 'auto' })).resolves.toMatchObject({ success: true, data: { text: 'automatic' } });
      await expect(invokeTestTool(surface, 'webfetch', { url: 'https://93.184.216.34/text', read_mode: 'text' })).resolves.toMatchObject({ success: true, data: { text: 'forced' } });
    } finally {
      fetchSpy.mockRestore();
      rmSync(root, { recursive: true, force: true });
    }
