...
describe('ToolRuntime', () => {
  it('returns schema metadata from the typed definitions', () => {
    const runtime = new ToolRuntime({ matrix: { decide } }, [echoTool]);
  ...
  it('uses the card permission matrix for state checks', async () => {
    const runtime = new ToolRuntime({ matrix: { decide } }, [deleteTool]);
    ...
    expect(result).toEqual(expect.objectContaining({ ok: false, error: expect.objectContaining({ kind: 'ToolStateRejected' }) }));
  });
  ...
  it('validates output and emits boundary audit events', async () => {
    ...
    const runtime = new ToolRuntime({ matrix: { decide }, bus }, [badOutputTool]);
