...
function manager(input: { annotations?: Record<string, unknown>; result?: unknown } = {}): McpToolInvocationPort {
  return {
    ...
    invokeTool: async (serverName, toolName, args) => input.result ?? { serverName, toolName, args },
...
describe('McpProvider', () => {
  ...
  it('returns a model-visible error when MCP manager is unavailable', async () => {
    const surface = buildInvocationSurface('executor', [createMcpProvider({ agentRole: 'executor', mcpManagerProvider: () => undefined })]);
  ...
  it('requires reviewer MCP tools to be read-only and non-destructive', async () => {
    const allowed = buildInvocationSurface('reviewer', [createMcpProvider({ agentRole: 'reviewer', mcpManagerProvider: () => manager({ annotations: { readOnlyHint: true, destructiveHint: false } }) })]);
