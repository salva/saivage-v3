...
import { createCardVersionProvider } from '../../src/tools/card-version-provider.js';
...
describe('card version provider', () => {
  it('lists index metadata and reads and diffs exact resulting versions', async () => {
    ...
    const surface = buildInvocationSurfaceFixture('planner', [createCardVersionProvider({ store: cards })]);
