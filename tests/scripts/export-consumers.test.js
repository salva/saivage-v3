...
describe('repository complete export boundary', () => {
  it('reproduces complete candidate/consumer parity and the cleanup-complete inventory', () => {
    ...
    expect(directHistogram).toEqual({
      'production-consumed': 1727, 'test-only': 199, 'local-only': 2, 'zero-use': 0,
    });
    expect(result.records).toHaveLength(1928);
    expect(result.totals).toEqual({ 'production-consumed': 1729, 'test-only': 199, 'local-only': 0, 'zero-use': 0 });
