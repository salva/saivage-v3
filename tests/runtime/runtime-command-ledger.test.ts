  ...
  it('start_project rejects before setting running intent when the canonical project card is missing', async () => {
    ...
    try {
      ...
      expect(state.runtime_commands).toEqual(expect.arrayContaining([
        expect.objectContaining({ command_id: result.command.command_id, command: 'start_project', status: 'rejected', error: expect.objectContaining({ code: 'runtime_start_project_card_missing' }) }),
      ]));
    } finally { rmSync(projectRoot, { recursive: true, force: true }); }
  });
    ...
    try {
      ...
      const agentRuntime: AgentRuntime = {
        async invokePlanner(request) {
          ...
          const exec = new PlannerControlExecutor({ projectRoot, cardStore: runtime.cardStore, activationLedger: activationLedger(projectRoot) });
