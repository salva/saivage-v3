...
function evidenceProvider(complete: (input: LlmInvocationInput) => ReturnType<SummarizerProviderPort['completeTurn']> extends Promise<infer T> ? T : never) {
  ...
  roots.push(root);
  const service = new InvocationService({ projectRoot: root, saivageDir: root, appLogs: testAppLogs(root), registry: {} as never, router: {} as never, candidateAvailability: {} as never });
