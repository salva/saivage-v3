...
describe('LLMActor', () => {
  ...
  it('uses frozen prompt identity after real compaction for a new ordinary actor activation', async () => withTempProject(async (projectRoot) => {
    ...
    const sessionId = 'planner:project';
    const provider: LLMProviderPort = { completeTurn: jest.fn(async () => completion({ kind: 'message' as const, content: 'done' })) };
    appendActivationMarker(testConversationMutations(projectRoot), sessionId, { event: 'activation_open', role: 'planner', card_id: 'project', input_id: 'initial-turn' });
    ...
    for (let ordinal = 1; ordinal <= 3; ordinal++) {
      appendActivationMarker(testConversationMutations(projectRoot), sessionId, { event: 'activation_open', role: 'planner', card_id: 'project', input_id: `history-${ordinal}` });
      appendUserContextMessage(testConversationMutations(projectRoot), sessionId, `history-${ordinal}`, 'notification', 0, { role: 'user', content: `history ${ordinal} ${'x'.repeat(120)}` });
    }
  ...
  it('invokes the initial input factory only on the idle branch', async () => withTempProject(async (projectRoot) => {
    ...
    const factory = jest.fn(() => {
      appendActivationMarker(testConversationMutations(projectRoot), 'planner:project', { event: 'activation_open', role: 'planner', card_id: 'project', input_id: 'lazy-input' });
      appendUserContextMessage(testConversationMutations(projectRoot), 'planner:project', 'lazy-input', 'notification', 0, { role: 'user', content: 'lazy notification context' });
  ...
  it('invokes the initial input factory on the idle branch', async () => withTempProject(async (projectRoot) => {
    ...
    const factory = jest.fn(() => {
      appendActivationMarker(testConversationMutations(projectRoot), 'planner:project', { event: 'activation_open', role: 'planner', card_id: 'project', input_id: 'lazy-input' });
      const context = appendUserContextMessage(testConversationMutations(projectRoot), 'planner:project', 'lazy-input', 'notification', 0, { role: 'user', content: 'lazy notification context' });
  ...
  it('fatally settles a raw post-admission provider rejection without a model issue or exchange', async () => withTempProject(async (projectRoot) => {
    initProjectTree(projectRoot);
    ...
    expect(readAppLogEntries(projectRoot, 'provider_exchange')).toEqual([]);
    consoleError.mockRestore();
  }));

  it('fatally settles a malformed provider-attempt failure without inventing an exchange', async () => withTempProject(async (projectRoot) => {
    initProjectTree(projectRoot);
    const consoleError = jest.spyOn(console, 'error').mockImplementation(() => undefined);
  ...
  it('fatally settles a non-abort pre-provider failure without calling the provider', async () => withTempProject(async (projectRoot) => {
    initProjectTree(projectRoot);
    const consoleError = jest.spyOn(console, 'error').mockImplementation(() => undefined);
    const provider: LLMProviderPort = { completeTurn: jest.fn(async () => completion({ kind: 'message', content: 'unused' })) };
  ...
  it('resets provider-boundary classification for a post-tool pre-provider failure', async () => withTempProject(async (projectRoot) => {
    initProjectTree(projectRoot);
    const consoleError = jest.spyOn(console, 'error').mockImplementation(() => undefined);
    const provider: LLMProviderPort = { completeTurn: jest.fn(async () => completion({ kind: 'tool_calls', tool_calls: [{ id: 'call-1', type: 'function', function: { name: 'inspect', arguments: '{}' } }] })) };
    class ContinuationSetupFailureLLMActor extends LLMActor {
      protected override async onBeforeProviderCall(turnInput: LlmInvocationInput): Promise<void> {
    ...
    await expect(actor.appendToolResult('call-1', { success: true })).rejects.toThrow('continuation setup failed');
    await eventually(() => expect(actor.state()).toBe('idle'));
