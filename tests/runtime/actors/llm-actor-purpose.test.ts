...
describe('ConversationLLMActor purpose authority',()=>{
  it('accepts only exact card-purpose/card-session ownership',()=>{
    expect(()=>new ConversationLLMActor({...common,agentId:'agent:planner:card-a',purpose:{kind:'autonomous-card',cardId:'card-a'}})).not.toThrow();
    expect(()=>new ConversationLLMActor({...common,agentId:'agent:planner:global',purpose:{kind:'autonomous-card',cardId:'card-a'}})).toThrow(/does not match session/);
    expect(()=>new ConversationLLMActor({...common,agentId:'agent:planner:card-b',purpose:{kind:'autonomous-card',cardId:'card-a'}})).toThrow(/does not match session/);
  });
  ...
  it('leaves Analyst content refusal terminal with one ordinary provider call',async()=>{
    ...
    const input={inputId:'00000000-0000-4000-8000-000000000001',agentId:'agent:analyst:global',agentName:'analyst' as const,sessionId:'agent:analyst:global' as const,systemPrompt:'system',providerConversation:{sourceSessionId:'agent:analyst:global' as const,messages:[]},tools:[],terminalToolNames:[],modelParams:{},preparedCompaction:prepareCompaction({input_budget_tokens:1000,trigger_fraction:.8,completion_reserve_fraction:.2,merge_line_fraction:.3,summary_line_fraction:.5,escalate_merge_line_fraction:.4,escalate_summary_line_fraction:.6,snap:'compact_straddler'},'system',[]),capabilityRequest:{},routePass:{kind:'ordinary' as const,candidateChain:[{provider:'test',account:null,model:'test-model'}]},episodeContext:{cardId:'card-a'}};
