...
describe('AgentNodeExecution authored-record absence handling', () => {
  it('treats only the concrete type as an absent candidate or cleanup target', () => {
    ...
    expect(() => absent.discardWrittenRecords(new Set(['review.md']), 'stale')).not.toThrow();
  ...
  it('closes updated requirements in declaration order and retains close returns without rereading',()=>{
    const trace:string[]=[];
    const store={readRecordCurrent:jest.fn(),closeRecord:jest.fn((_card:string,name:string)=>{trace.push(name);const sourceVersion=({['alpha.md']:1,['beta.md']:2})[name]!+1;return{headVersion:sourceVersion,currentUrl:`record:///${name}?card=project`,artifact:{accepted:{source_version:sourceVersion}}};})};
  ...
  it('prepares only clean requirements by discard then open and captures the post-preparation numeric head', () => {
    ...
    const readRecordCurrent=jest.fn()
      ...
      .mockReturnValue({kind:'found',value:{projection:{headVersion:4}}});
