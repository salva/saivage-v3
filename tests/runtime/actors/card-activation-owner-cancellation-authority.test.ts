...
describe('CardActivationOwner terminal authority', () => {
  it('starts with one open terminal axis and a normal owner phase', () => { const owner = new CardActivationOwner({ card, store: {} as never, processor, activationId: 'activation', entry: 'BACKLOG', caller: { kind: 'root' }, phase: 'prepared_root' }); expect(owner).toMatchObject({ phase: 'prepared_root', terminalWinner: 'open', childCardId: null }); });
  it('retains normal result settlement without a second ownership axis', () => { const owner = new CardActivationOwner({ card, store: {} as never, processor, activationId: 'activation', entry: 'BACKLOG', caller: { kind: 'root' }, phase: 'prepared_root' }); owner.terminalWinner = 'result'; owner.phase = 'settling'; expect(owner).toMatchObject({ terminalWinner: 'result', phase: 'settling' }); });
});
