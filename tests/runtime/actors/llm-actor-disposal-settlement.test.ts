...
async function toolCallFixture(toolName:string){
  ...
  const input={inputId,agentId:sessionId,agentName:'analyst' as const,sessionId,systemPrompt:'system',providerConversation:{sourceSessionId:sessionId,messages:[]},tools:[],terminalToolNames:[],modelParams:{},preparedCompaction:prepareCompaction({input_budget_tokens:1000,trigger_fraction:.8,completion_reserve_fraction:.2,merge_line_fraction:.3,summary_line_fraction:.5,escalate_merge_line_fraction:.4,escalate_summary_line_fraction:.6,snap:'compact_straddler'},'system',[]),capabilityRequest:{},routePass:{kind:'ordinary' as const,candidateChain:[{provider:'test',account:null,model:'test-model'}]},episodeContext:{}};
...
function expectToolResult(fixture:Awaited<ReturnType<typeof toolCallFixture>>,result:ToolResult,toolName:string):void{
