...
function call(overrides: Partial<AgentMessage> = {}): AgentMessage {
  const session_id = (overrides.session_id ?? 'planner:project') as ConversationSessionId;
...
function result(overrides: Partial<AgentMessage> = {}): AgentMessage {
  const tool_call_id = overrides.tool_call_id ?? 'call-1';
  return { id: `${source}:tool-result:${tool_call_id}`, session_id: 'planner:project', role: 'tool', kind: 'tool_result', tool: 'webfetch', tool_call_id, content: '{"success":true}', round_id: round, message_index: 1, block_index: 0, timestamp, ...overrides };
}
...
describe('canonical conversation call-pair inspection', () => {
  it('shares structural pairing while the projection inspector returns exact canonical call data', () => {
    ...
    expect(inspectConversationCallPairs([row])).toMatchObject({ sessionId: 'planner:project', sourceInputId: source, toolCallId: 'call-1', toolName: 'webfetch', startedAt: timestamp, args: { url: 'https://example.com' } });
  ...
  it('rejects malformed and mismatched row, source, session, embedded id, and embedded tool identities', () => {
    ...
    expect(() => inspectCanonicalCallSettlementPairs([call(), result({ session_id: 'reviewer:project' })])).toThrow('has no prior matching call');
  });
});
