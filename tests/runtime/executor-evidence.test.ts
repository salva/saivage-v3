...
import { buildIgnoredExecutorEvidencePatch, registerExecutorEvidence, resolveRegisterableProcessMetadataSource, summarizeExecutorEvidenceRegistrationFailure } from '../../src/runtime/phases/executor-evidence.js';
...
describe('executor evidence registration', () => {
  ...
  it('summarizes registration failure only for otherwise done executor results', () => {
    expect(summarizeExecutorEvidenceRegistrationFailure({ execStatus: 'done', artifactRegistrationErrors: ['artifact failed'], attachmentRegistrationErrors: [] })).toEqual({
      ...
      registrationError: 'Completion blocked by evidence registration failure. Artifacts: artifact failed. Attachments: none.',
    });
