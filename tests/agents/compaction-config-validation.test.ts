...
describe('compaction config validation', () => {
  it('accepts ordered widths without a second rounded-budget authority', () => {
    expect(saivageConfigSchema.safeParse({ models, compaction }).success).toBe(true);
  });

  it('requires an explicitly enabled complete structured policy', () => {
    expect(saivageConfigSchema.safeParse({}).success).toBe(false);
    expect(saivageConfigSchema.safeParse({ models, compaction: { ...compaction, enabled: false } }).success).toBe(false);
    expect(saivageConfigSchema.safeParse({ models, compaction: { ...compaction, input_budget_tokens: undefined } }).success).toBe(false);
    expect(saivageConfigSchema.safeParse({ models, compaction: { ...compaction, summarizer_candidate: undefined } }).success).toBe(false);
    ...
    ]) expect(saivageConfigSchema.safeParse({ models, compaction: { ...compaction, summarizer_candidate } }).success).toBe(false);
    expect(saivageConfigSchema.safeParse({ models, compaction: { ...compaction, summarizer_model: 'test/_/model', summarizer_candidate: undefined } }).success).toBe(false);
  });
  ...
  it.each([
    ...
    const parsed = saivageConfigSchema.safeParse({ models: max_tokens === undefined ? {} : { max_tokens }, compaction: selectedCompaction });
  ...
  it.each([
    ...
    expect(saivageConfigSchema.safeParse({ models: max_tokens === undefined ? {} : { max_tokens }, compaction: selectedCompaction }).success).toBe(true);
  });
});
