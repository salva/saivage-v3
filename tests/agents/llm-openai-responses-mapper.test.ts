...
describe('OpenAI Responses replay mapper', () => {
  it('replays private output unchanged and appends a matching failed function_call_output unchanged', () => {
    ...
    expect(responsesInputFromReplay(buildResponsesReplayProjection('s1', [privateRow, visible, result]))).toEqual([...output, { type: 'function_call_output', call_id: 'call-1', output: failedContent }]);
  });
