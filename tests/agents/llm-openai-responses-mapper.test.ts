...
describe('OpenAI Responses provider conversation mapper', () => {
  it('maps private output unchanged and appends a matching failed function_call_output unchanged', () => {
    ...
    expect(responsesInputFromProviderConversation({ sourceSessionId: 's1', messages: [privateRow, visible, result] })).toEqual([...output, { type: 'function_call_output', call_id: 'call-1', output: failedContent }]);
  });
  ...
  it('does not require private rows for unmarked generic assistant history', () => {
    ...
    expect(responsesInputFromProviderConversation({ sourceSessionId: 's1', messages: [visible] })).toEqual([{ role: 'assistant', content: [{ type: 'output_text', text: 'generic' }] }]);
  });
  ...
  it('fails on marked visible row without matching private row', () => {
    ...
    expect(() => responsesInputFromProviderConversation({ sourceSessionId: 's1', messages: [visible] })).toThrow(/missing private row/);
  });
  ...
  it('rejects compaction metadata instead of filtering it at the gateway', () => {
    ...
    expect(() => responsesInputFromProviderConversation({ sourceSessionId: 's1', messages: [metadata] })).toThrow(/contains compaction metadata/);
  });
  ...
  it('fails on orphan private rows, duplicate private rows, and mismatched bidirectional ids', () => {
    ...
    expect(() => responsesInputFromProviderConversation({ sourceSessionId: 's1', messages: [privateRow] })).toThrow(/missing marked visible projection/);
    expect(() => responsesInputFromProviderConversation({ sourceSessionId: 's1', messages: [privateRow, duplicatePrivate, visible] })).toThrow(/duplicated/);
    expect(() => responsesInputFromProviderConversation({ sourceSessionId: 's1', messages: [privateRow, { ...visible, provider_projection: { ...visible.provider_projection!, private_message_id: 'wrong-private' } }] })).toThrow(/missing private row/);
    expect(() => responsesInputFromProviderConversation({ sourceSessionId: 'other', messages: [privateRow, visible] })).toThrow(/not source session 'other'/);
  });
});
