...
function cliCtx(root: string, store: CardStore): ToolContext { return { projectRoot: root, store, actor: 'analyst', surface: 'cli' }; }
...
describe('analyst audit', () => {
  ...
  it('redacts shell params, summaries, and errors for previewed/rejected and failing shell actions', async () => {
    ...
    try {
      ...
      const rejected = await run_shell_command(cliCtx(root, store), { command, confirmed: true, preview_hash: 'wrong-hash' });
      expect(rejected.success).toBe(false);
      expect((rejected.preview as unknown as Record<string, unknown> | undefined)?.preview_hash).toBe(previewHash);
      ...
      expect(shellLines.map((line) => line.outcome)).toEqual(['preview', 'rejected', 'error']);
    } finally { rmSync(root, { recursive: true, force: true }); }
  });
});
