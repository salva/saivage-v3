...
function setupProjectDir(root: string): void {
  ...
  writeFileSync(join(sd, 'saivage.json'), JSON.stringify({ server: { port: 8080, host: '127.0.0.1' }, models: { default: ['test-model'] }, providers: { test: { priority: 10, models: ['test-model'], apiKey: 'ar-server-test-api-key' } } }, null, 2));
...
describe('Server with ActiveRuntime (createRuntime=true)', () => {
  ...
  beforeAll(async () => {
    ...
    if (!addr || typeof addr === 'string') throw new Error('Server did not listen on a network port');
    port = addr.port;
  }, 30000);
  ...
  function baseUrl(path: string): string { return `http://127.0.0.1:${port}${path}`; }
  ...
  describe('GET /api/runtime/status', () => {
    it('returns 200 with status info', async () => { const res = await fetch(baseUrl('/api/runtime/status'), { headers: authHeaders() }); expect(res.status).toBe(200); const body = await res.json() as Record<string, unknown>; expect(body).toHaveProperty('runtime'); expect(body).toHaveProperty('paused'); expect(body).toHaveProperty('currentCardId'); expect(body).toHaveProperty('goalCount'); });
    it('requires auth (returns 401 without token)', async () => { const res = await fetch(baseUrl('/api/runtime/status')); expect(res.status).toBe(401); });
    it('returns sensible runtime status value', async () => { const res = await fetch(baseUrl('/api/runtime/status'), { headers: authHeaders() }); const body = await res.json() as Record<string, unknown>; expect(['idle', 'running', 'paused', 'error', 'frozen']).toContain(body.runtime); });
  ...
  describe('Pause / Resume via API', () => {
    ...
    it('POST /api/runtime/resume sets paused to false', async () => { await fetch(baseUrl('/api/runtime/pause'), { method: 'POST', headers: authHeaders() }); const res = await fetch(baseUrl('/api/runtime/resume'), { method: 'POST', headers: authHeaders() }); expect(res.status).toBe(200); const body = await res.json() as Record<string, unknown>; expect(body.status).toBe('resumed'); expect(server.activeRuntime!.getStatus().paused).toBe(false); });
    it('GET /api/runtime/status reflects pause/resume changes', async () => { await fetch(baseUrl('/api/runtime/pause'), { method: 'POST', headers: authHeaders() }); let res = await fetch(baseUrl('/api/runtime/status'), { headers: authHeaders() }); let body = await res.json() as Record<string, unknown>; expect(body.paused).toBe(true); await fetch(baseUrl('/api/runtime/resume'), { method: 'POST', headers: authHeaders() }); res = await fetch(baseUrl('/api/runtime/status'), { headers: authHeaders() }); body = await res.json() as Record<string, unknown>; expect(body.paused).toBe(false); });
    it('analyst chat pause/resume controls ActiveRuntime in-memory state', async () => { const pauseRes = await fetch(baseUrl('/api/chats/runtime-control-live'), { method: 'POST', headers: { ...authHeaders(), 'content-type': 'application/json' }, body: JSON.stringify({ content: 'pause runtime' }) }); expect(pauseRes.status).toBe(200); const pauseBody = await pauseRes.json() as { toolInvocations?: Array<{ tool: string }> }; expect(pauseBody.toolInvocations?.some((inv) => inv.tool === 'pause_runtime')).toBe(true); expect(server.activeRuntime!.getStatus().paused).toBe(true); const resumeRes = await fetch(baseUrl('/api/chats/runtime-control-live'), { method: 'POST', headers: { ...authHeaders(), 'content-type': 'application/json' }, body: JSON.stringify({ content: 'resume runtime' }) }); expect(resumeRes.status).toBe(200); const resumeBody = await resumeRes.json() as { toolInvocations?: Array<{ tool: string }> }; expect(resumeBody.toolInvocations?.some((inv) => inv.tool === 'resume_runtime')).toBe(true); expect(server.activeRuntime!.getStatus().paused).toBe(false); });
  });

  describe('Freeze / Resume via API', () => {
    it('POST /api/runtime/freeze creates a freeze manifest', async () => { const res = await fetch(baseUrl('/api/runtime/freeze'), { method: 'POST', headers: { ...authHeaders(), 'content-type': 'application/json' }, body: JSON.stringify({ reason: 'API freeze test' }) }); expect(res.status).toBe(200); const body = await res.json(); expect(body.status).toBe('frozen'); expect(body.freeze_id).toBeDefined(); expect(body.reason).toBe('API freeze test'); expect(body.created_at).toBeDefined(); });
    it('POST /api/runtime/freeze when already frozen keeps returning frozen state', async () => { const res = await fetch(baseUrl('/api/runtime/freeze'), { method: 'POST', headers: { ...authHeaders(), 'content-type': 'application/json' }, body: JSON.stringify({ reason: 'second freeze' }) }); expect(res.status).toBe(200); const body = await res.json(); expect(body.status).toBe('frozen'); });
    it('POST /api/runtime/resume-from-freeze restores state', async () => { const res = await fetch(baseUrl('/api/runtime/resume-from-freeze'), { method: 'POST', headers: authHeaders() }); expect(res.status).toBe(200); const body = await res.json(); expect(body.status).toBe('resumed'); expect(body.freeze_id).toBeDefined(); expect(body.restored_queue).toBeDefined(); expect(body.restored_processes).toBeDefined(); });
    it('POST /api/runtime/resume-from-freeze after resume returns error', async () => { const res = await fetch(baseUrl('/api/runtime/resume-from-freeze'), { method: 'POST', headers: authHeaders() }); expect(res.status).toBe(500); const body = await res.json(); expect(body.error).toContain('Failed to resume from freeze'); });
    it('freeze requires auth', async () => { const res = await fetch(baseUrl('/api/runtime/freeze'), { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ reason: 'unauth freeze' }) }); expect(res.status).toBe(401); });
    it('resume-from-freeze requires auth', async () => { const res = await fetch(baseUrl('/api/runtime/resume-from-freeze'), { method: 'POST' }); expect(res.status).toBe(401); });
  });
});

describe('Server without ActiveRuntime (createRuntime=false)', () => {
  let tmpDir: string; let server: ServerInstance; let port: number; let originalCwd: string;
  beforeAll(async () => { originalCwd = process.cwd(); tmpDir = mkdtempSync(join(tmpdir(), 'saivage-ar-nort-')); setupProjectDir(tmpDir); process.chdir(tmpDir); process.env['SAIVAGE_API_TOKEN'] = AUTH_TOKEN; const { createServer } = await import('../../src/server/server.js'); server = await createServer(tmpDir, false); await server.fastify.listen({ host: '127.0.0.1', port: 0 }); const addr = server.fastify.server.address(); if (!addr || typeof addr === 'string') throw new Error('Server did not listen on a network port'); port = addr.port; }, 30000);
