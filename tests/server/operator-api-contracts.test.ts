...
describe('operator API runtime contract without runtime ledgers', () => {
  ...
  it('rejects representative top-level and nested extras in every REST contract family', () => {
    ...
    const session = { id: 'agent:planner:project', agent_name: 'planner', session_scope: 'card', card_id: 'project', started_at: timestamp, status: 'inactive', activity: 'idle', compaction: null };
  ...
  it('keeps hierarchy, displayed detail, records, and history as distinct exact shapes', () => {
    ...
    expect(() => parseOperatorResponse('cards.children', 200, { parent: canonicalHierarchyCard, children: [canonicalHierarchyChild,canonicalHierarchyChild] })).toThrow();
    const missingPolicy={...canonicalHierarchyChild} as Record<string,unknown>;delete missingPolicy.permitted_child_types;
