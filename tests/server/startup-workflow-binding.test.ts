...
describe('startup workflow binding authority', () => {
  ...
  it('discovers participant bindings in direct canonical state-table order and skips unused configured agents',()=>{
    ...
    source.agents.unused={prompt:{ reference: 'executor', compactable: true },tools:[],model_route:'unused',skills:false,session:'card',can_create_children:false,record_writes:[]};
    const structural=compileProjectWorkflows(source);
    const order:string[]=[];
  ...
  it('rejects a zero-operational-tool card participant when only a tool-unsupported model can serve it', () => {
    ...
    source.agents.empty = { prompt: { reference: 'executor', compactable: true }, tools: [], model_route: 'empty', skills: false, session: 'card', can_create_children: false,record_writes:['status.md'] };
