...
function setup() { const root = mkdtempSync(join(tmpdir(), 'saivage-record-stream-')); roots.push(root); initProjectTree(root); const cards = new CardService(root); const card = cards.create({ type: 'code', parent: 'project', title: 'card', brief: 'brief', status: 'backlog', tags: [], priority: 0, urgency: 'normal', created_by: 'analyst', depends_on: [], related: [] }); return { root, cards, card }; }
...
describe('authored record revision streams', () => {
  ...
  it('leaves a complete record revision visible after reported fsync failure without retry', () => {
    ...
    const faulty = new CardService(root, undefined, undefined, failing);
