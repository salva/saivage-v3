...
export function roleSurfaceCompileContracts(values: { store: CardService; processRunner: ProcessRunner; processScope: ManagedProcessScope; mcpToolInvocation: McpToolInvocationPort; toolContext: ToolContext }): void {
  ...
  createPlannerControlProvider({ projectRoot: '/project', parentCardId: 'project', sessionId: 'planner:project', store: values.store, children: planner.children, cancelCard: planner.cancelCard, appLogs: planner.appLogs, beginStructuralWait: planner.beginStructuralWait, endStructuralWait: planner.endStructuralWait });
}
