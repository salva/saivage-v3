...
import { invokeToolCall, invokeToolForLlm } from '../../src/tools/invocation.js';
...
export function llmToolInvocationCompileContract(surface: InvocationSurface, context: LlmToolInvocationContext, signal: AbortSignal): void {
  ...
  void invokeToolCall(surface, 'tool', '{}', context);
  void invokeToolCall(surface, 'tool', '{}', context, signal);

  // @ts-expect-error LLM invocation context is mandatory.
  void invokeToolForLlm(surface, 'tool', {});
  // @ts-expect-error LLM invocation context is mandatory even before raw JSON parsing.
  void invokeToolCall(surface, 'tool', '{}');
