...
describe('ResolvedConfigAuthority', () => {
  it('selects a CLI file once and routes reads and writes only to it', async () => {
    ...
    const environment = await loadEnvironment(['node', 'saivage', 'start', '--config', custom], { SAIVAGE_PROJECT_ROOT: projectRoot });
  ...
  it('selects SAIVAGE_CONFIG without probing or falling back to the default', async () => {
    ...
    const environment = await loadEnvironment(['node', 'saivage', 'start'], { SAIVAGE_PROJECT_ROOT: projectRoot, SAIVAGE_CONFIG: selected });
  ...
  it('creates only the selected missing canonical default for --create-runtime and preserves existing files', async () => {
    ...
    const environment = await loadEnvironment(['node', 'saivage', 'start', '--create-runtime', '--config', selected], { SAIVAGE_PROJECT_ROOT: projectRoot });
    ...
    await environment.configAuthority.initializeCanonicalDefaultIfMissing();
  ...
  it('captures interpolation and preserves raw placeholders across unrelated writes', async () => {
    ...
    const environment = await loadEnvironment(['node', 'saivage', 'start'], startup);
    startup.MODEL_NAME = 'model-later';
    const originalModelName = process.env.MODEL_NAME;
    process.env.MODEL_NAME = 'process-model-later';
    try {
      await environment.configAuthority.mutate({ kind: 'set_server_setting', key: 'port', value: 8181 });
  ...
  it('serializes overlapping mutations and continues after a failed queued validation', async () => {
    const projectRoot = root();
    writeSaivageConfig(projectRoot, valid());
    const environment = await loadEnvironment(['node', 'saivage', 'start'], { SAIVAGE_PROJECT_ROOT: projectRoot });
    const first = environment.configAuthority.mutate({ kind: 'set_server_setting', key: 'port', value: 8201 });
    const second = environment.configAuthority.mutate({ kind: 'set_runtime_setting', key: 'max_review_retries', value: 7 });
    expect((await first).success).toBe(true);
    expect((await second).success).toBe(true);
    const failed = environment.configAuthority.mutate({ kind: 'set_server_setting', key: 'port', value: -1 });
    const afterFailure = environment.configAuthority.mutate({ kind: 'set_runtime_setting', key: 'continuous_improvement', value: true });
    expect((await failed).success).toBe(false);
    expect((await afterFailure).success).toBe(true);
