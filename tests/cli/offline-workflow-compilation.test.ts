...
describe('offline workflow compilation and publication',()=>{
  ...
  it('preserves exact pre-existing explicit-map bytes and uses their custom bootstrap definition',async()=>{const projectRoot=root();const config=structuredClone(DEFAULT_SAIVAGE_CONFIG);config.card_types.project!.records['charter.md']={...config.card_types.project!.records['brief.md']!,bootstrap:true};delete config.card_types.project!.records['brief.md'];const configPath=join(projectRoot,'.saivage','saivage.yaml');write(configPath,YAML.stringify(config));const bytes=readFileSync(configPath);jest.spyOn(console,'log').mockImplementation(()=>{});await command(projectRoot,'init');expect(readFileSync(configPath)).toEqual(bytes);expect(existsSync(join(projectRoot,'.saivage','cards','project','records','charter.md','index.json'))).toBe(true);expect(existsSync(join(projectRoot,'.saivage','cards','project','records','brief.md','index.json'))).toBe(false);});
