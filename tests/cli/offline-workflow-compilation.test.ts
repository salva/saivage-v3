...
describe('offline workflow compilation and publication',()=>{
  ...
  it('uses a precompiled custom project bootstrap definition',async()=>{const projectRoot=root();const config=structuredClone(DEFAULT_SAIVAGE_CONFIG);config.card_types.project!.records['charter.md']={...config.card_types.project!.records['brief.md']!,bootstrap:true};delete config.card_types.project!.records['brief.md'];write(join(projectRoot,'.saivage','saivage.yaml'),YAML.stringify(config));jest.spyOn(console,'log').mockImplementation(()=>{});await command(projectRoot,'init');expect(existsSync(join(projectRoot,'.saivage','cards','project','records','charter.md','index.json'))).toBe(true);expect(existsSync(join(projectRoot,'.saivage','cards','project','records','brief.md','index.json'))).toBe(false);});
