...
describe('CLI no-live runtime controls', () => {
  ...
  it.each(['status', 'pause', 'resume', 'stop'] as const)('uses the published bearer mode for verified-live %s and never puts the token in the URL', async (command) => {
    ...
    try {
      ...
      const runtimeStatus = { runtime: command === 'pause' ? 'paused' : 'running', currentCardId: null, started_at: '2026-07-18T00:00:00.000Z', restart_server_available: true, pid: process.pid, actorRuntime: { pauseMode: command === 'pause' ? 'paused' : 'running', cards: [] }, serverAvailability: { generatedAt: '2026-07-18T00:00:00.000Z', components: { api: { state: 'available', source: 'health-check', checkedAt: '2026-07-18T00:00:00.000Z' }, runtime: { state: 'available', source: 'runtime-application', checkedAt: '2026-07-18T00:00:00.000Z' }, mcp: { state: 'idle', source: 'mcp-manager', checkedAt: '2026-07-18T00:00:00.000Z' } } } };
