...
describe('CLI no-live runtime controls', () => {
  ...
  it.each(['status', 'pause', 'resume', 'stop'] as const)('uses the published bearer mode for verified-live %s and never puts the token in the URL', async (command) => {
    ...
    try {
      ...
      const runtimeStatus = { runtime: command === 'pause' ? 'paused' : 'running', currentCardId: null, goalCount: 0, lastTickAt: null, restart_server_available: true, pid: process.pid, actorRuntime: { pauseMode: command === 'pause' ? 'paused' : 'running', activeWork: 'none', cards: [], agents: [], diagnostics: [] } };
