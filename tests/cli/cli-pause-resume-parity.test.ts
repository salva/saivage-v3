...
describe('cli pause resume parity', () => {
  it('fallback updates persisted state and writes audit when server is down', async () => {
    ...
    try {
      ...
      for (const d of ['cards/by-id','cards/tree','cards/dependencies','notes/by-card','runtime','tmp/state']) mkdirSync(join(sd, d), { recursive: true });
      ...
      const state = JSON.parse(readFileSync(runtimeStatePath(root), 'utf-8')) as { status: string; paused: boolean };
