...
describe('current runtime composition',()=>{
  it('copies a required route pass and forwards provider operations',()=>{const preparePrimaryRequestAdmission=jest.fn<InvocationService['preparePrimaryRequestAdmission']>((request)=>({kind:'local_admission_failed',routePass:request.routePass as Extract<typeof request.routePass,{kind:'ordinary'}>,candidates:[],diagnostic:{} as never}));const projectProviderExchanges=jest.fn<InvocationService['projectProviderExchanges']>();const service={preparePrimaryRequestAdmission,projectProviderExchanges} as unknown as InvocationService;const provider=createInvocationServiceProvider(service);const signal=new AbortController().signal;const candidateChain=[{provider:'test',account:null,model:'test-model'}];const input:LlmInvocationInput={inputId:'turn',agentId:'agent:planner:project',agentName:'planner',sessionId:'agent:planner:project',...preparedInvocationContextFixture('plan',['emit_result']),providerConversation:{sourceSessionId:'agent:planner:project',messages:[]},modelParams:{temperature:0},preparedCompaction:{inputBudgetTokens:1000,reservedCompletionTokens:100,requestedCompletionTokens:100} as never,capabilityRequest:{requiresTools:true},routePass:{kind:'ordinary',candidateChain},episodeContext:{}};const request=invocationRequest(input,signal);expect(request.routePass).toEqual({kind:'ordinary',candidateChain});expect(request.routePass).not.toBe(input.routePass);provider.preparePrimaryRequest(input as never,signal);expect(preparePrimaryRequestAdmission).toHaveBeenCalledWith(request);const context={assistantOutputIds:[],terminalConversationOutputId:null};provider.projectProviderExchanges!('agent:planner:project','turn',[],context);expect(projectProviderExchanges).toHaveBeenCalledWith('agent:planner:project','turn',[],context);});
