...
describe('WorkspaceFileReadModelService work URLs', () => {
  it('reads process logs and stash files through canonical work URLs', () => withRoot((root) => {
    ...
    writeFileSync(join(root, '.saivage-work', 'processes', 'proc-1', 'combined.log'), 'combined output', 'utf8');
    ...
    expect(service.readFileContent('work:///processes/proc-1/combined.log')).toEqual(expect.objectContaining({ body: expect.objectContaining({ path: 'work:///processes/proc-1/combined.log', content: 'combined output' }) }));
  ...
  it('lists work directories with work URL child paths while preserving project-relative navigation', () => withRoot((root) => {
    ...
    expect(service.listFiles('work:///processes/proc-1')).toEqual(expect.objectContaining({ body: expect.objectContaining({ files: [expect.objectContaining({ path: 'work:///processes/proc-1/combined.log' })] }) }));
