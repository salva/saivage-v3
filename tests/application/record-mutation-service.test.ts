...
describe('card-agent record mutation', () => {
  it('creates and repeatedly edits a glob-authorized free record independently of requirements', () => {
    ...
    expect(first).toMatchObject({ success: true, data: { state: 'open', head_version: 2, current_url: path } });
    ...
    expect(second).toMatchObject({ success: true, data: { state: 'open', head_version: 3, current_url: path } });
  ...
  it('exposes crash-left open content, conflicts with Analyst mutation, and lets a later authorized activation reuse and close it', () => {
    ...
    expect(mutateRecord(cards,{path,operation:'write',content:'interrupted draft',surface:'card_agent',agentName:'planner',cardId:'project',requiredTools:['write']})).toMatchObject({success:true,data:{state:'open'}});
    expect(cards.readCurrentRecord('project','brief.md').artifact.draft?.content).toBe('interrupted draft');
    expect(mutateRecord(cards,{path,operation:'write',content:'analyst replacement',surface:'analyst',agentName:'analyst',requiredTools:['write']})).toMatchObject({success:false,data:{code:'record_open_conflict'}});
