...
describe('OperatorRuntimeHttpClient', () => {
  ...
  it('uses the canonical method and path for every delegated operation', async () => {
    const status = { runtime: 'stopped', currentCardId: null, started_at: '2026-07-18T00:00:00.000Z', restart_server_available: true, pid: process.pid, actorRuntime: { pauseMode: 'idle', cards: [] }, oversight: { agent_name: 'oversight', session_id: 'agent:oversight:global', enabled: true, eligible: false, eligibility_reason: 'stopped', state: 'unavailable', next_nominal_due: null, last_attempt: null, last_successful_at: null, service_epoch: '2026-07-18T00:00:00.000Z' }, serverAvailability: { generatedAt: '2026-07-18T00:00:00.000Z', components: { api: { state: 'available', source: 'health-check', checkedAt: '2026-07-18T00:00:00.000Z' }, runtime: { state: 'idle', source: 'runtime-application', checkedAt: '2026-07-18T00:00:00.000Z' }, mcp: { state: 'idle', source: 'mcp-manager', checkedAt: '2026-07-18T00:00:00.000Z' } } } };
