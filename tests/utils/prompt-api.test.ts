...
describe('prompt template compiler',()=>{
  it('renders direct repeated fragments in semantic order and counts the expanded contract',()=>{
    ...
    expect(renderCompiledPrompt({kind:'workflow-agent',cardType:'code'},templateName,compiled,{contractDescription:'contract',cardType:'code'})).toBe('A contract B code C code');
