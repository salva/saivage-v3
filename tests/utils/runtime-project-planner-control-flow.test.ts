...
describe('Runtime caller-edge reconstruction from unresolved activate_card calls', () => {
  ...
  it('resumes parent planners by appending exactly one tool_result to the real activate_card tool_call_id', async () => {
    const projectFixture: FakeAgentFixture = { name: 'project-parent', planner: [{ status: 'done', created_cards: [{ id: 'goal-parent-1', type: 'goal', title: 'Initial top-level goal', description: 'Create the first top-level goal.', status: 'backlog', depends_on: [], priority: 1 }], summary: 'Initial project planning created one top-level goal.' }, { status: 'done', created_cards: [{ id: 'goal-parent-2', type: 'goal', title: 'Second top-level goal', description: 'Create a second top-level goal after the first completes.', status: 'backlog', depends_on: [], priority: 2 }], summary: 'Project planner resumed and created a second top-level goal.' }, { status: 'done', created_cards: [], summary: 'Project planner resumed again after the second goal completed and confirmed no further work.' }], reviewer: [{ assessment: { id: 'review-project', goal_card_id: 'project', reviewer_session_id: 'rev-project', result: 'pass', summary: 'Project planning and follow-up goals were accepted.', achieved: ['Created first top-level goal', 'Created second top-level goal after resume'], missing: [], evidence_card_ids: ['goal-parent-1', 'goal-parent-2'], created_at: new Date().toISOString() } }] };
    const goalOneReview: FakeReviewerResult = { assessment: { id: 'review-goal-1', goal_card_id: 'goal-parent-1', reviewer_session_id: 'rev-goal-1', result: 'pass', summary: 'Both leaf cards executed.', achieved: ['Execution evidence for code-parent-1', 'Execution evidence for code-parent-2'], missing: [], evidence_card_ids: ['code-parent-1', 'code-parent-2'], created_at: new Date().toISOString() } };
    const goalTwoReview: FakeReviewerResult = { assessment: { id: 'review-goal-2', goal_card_id: 'goal-parent-2', reviewer_session_id: 'rev-goal-2', result: 'pass', summary: 'The second top-level goal leaf card executed.', achieved: ['Execution evidence for code-parent-3'], missing: [], evidence_card_ids: ['code-parent-3'], created_at: new Date().toISOString() } };
