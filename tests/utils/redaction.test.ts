...
describe('outbound redaction', () => {
  ...
  describe('text and dynamic conversion', () => {
    ...
    it.each([
      [
        'JSON',
        '{"apiKey":"synthetic-json-secret","safe":"visible"}',
        '{"apiKey":"[REDACTED]","safe":"visible"}',
      ],
      [
        'YAML',
        'password: synthetic-yaml-secret\nsafe: visible',
        'password: [REDACTED]\nsafe: visible',
      ],
      [
        'escaped JSON',
        'payload={\\"accessToken\\":\\"synthetic-escaped-secret\\"}',
        'payload={\\"accessToken\\":\\"[REDACTED]\\"}',
      ],
      [
        'inline assignment',
        'run token=synthetic-assignment-secret now',
        'run token=[REDACTED] now',
      ],
      [
        'URL query credential',
        'https://example.test/path?api_key=synthetic-query-secret&safe=yes',
      ...
      [
        'credential literals',
        'sk-synthetic tid=synthetic ghu_synthetic rt_synthetic tok_synthetic',
        'sk-[REDACTED] tid-[REDACTED] ghu-[REDACTED] rt-[REDACTED] tok-[REDACTED]',
      ],
    ...
    it('converts Error and unserializable dynamic values before redaction', () => {
      expect(redactTextForOutbound(new Error('token=synthetic-error-secret'))).toBe(
        'token=[REDACTED]',
      );
