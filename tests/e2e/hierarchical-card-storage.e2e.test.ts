...
function row(session_id: ConversationSessionId, id: string) { return { id, session_id, role: 'user' as const, kind: 'text' as const, content: id, context_policy: durableContentPolicy(), round_id: 'r-user-00000000000000000000000000000000', message_index: 0, block_index: 0, timestamp: '2026-07-17T00:00:00.000Z' }; }
...
describe('reset-only hierarchical card storage', () => {
  it('survives restart with exact streams, records, conversations, reorder, and safe deletion', () => {
    ...
    const dependentStreamBefore = readFileSync(cardVersionIndexFile(root, dependent.id), 'utf8');
    ...
    expect(readFileSync(cardVersionIndexFile(root, dependency.id), 'utf8')).toBe(dependencyStreamBefore);
    expect(readFileSync(cardVersionIndexFile(root, dependent.id), 'utf8')).toBe(dependentStreamBefore);
