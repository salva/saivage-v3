...
function setup(reviewStatus: 'done' | 'blocked' | 'failed' | 'rework') {
  ...
  const actor = new PlanningCardProcessorActor({ projectRoot, cardId: 'project', store: cards, children: { get: () => null }, cancelCard: async () => { throw new Error('unused'); }, provider, conversations: { projectRoot }, appLogs: testAppLogs(projectRoot), promptTemplates: createTestPromptTemplateRegistry(), gate });
  actor.start();
