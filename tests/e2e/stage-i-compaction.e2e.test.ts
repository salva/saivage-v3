...
describe('Stage-I versioned compaction', () => {
  it('retains route-independent budgets and newest-relative partitions', () => {
    const prepared = prepareCompaction(config, 'system', []); expect(prepared.requestedCompletionTokens).toBe(200);
    const messages: AgentMessage[] = [{ id: 'm', session_id: 'agent:planner:project', role: 'user', kind: 'text', content: 'x'.repeat(4000), context_policy: durableContentPolicy(), round_id: `r-user-${'0'.repeat(32)}`, message_index: 0, block_index: 0, timestamp: '2026-08-11T00:00:00.000Z' }];
  ...
  it('publishes a compacted segment head while preserving v1 as explicit history', async () => {
    ...
    try {
      ...
      const before = readConversation(root, SESSION); const result = await compact({ strategy: 'preventive', conversations: { projectRoot: root }, input: invocationFor(SESSION, providerConversationProjection(before).messages), summarizerProvider: { candidate: TEST_CANDIDATE, completeTurn: async () => ({ result: { kind: 'message' as const, content: 'summary' }, provider_exchanges: [] }), projectProviderExchanges: jest.fn() }, signal: new AbortController().signal });
      ...
      const projected = providerConversationProjection(current.conversation).messages; expect(projected.filter((row) => row.id.endsWith(':rendered'))).toHaveLength(1);
    } finally { rmSync(root, { recursive: true, force: true }); }
  });
});

const SESSION = 'agent:planner:project' as const;
function appendRound(root: string, ordinal: number): void { const timestamp = `2026-08-11T00:${String(ordinal).padStart(2, '0')}:00.000Z`; appendConversationBatch({ projectRoot: root }, [{ id: `activation-${ordinal}`, session_id: SESSION, role: 'system', kind: 'activity', content: JSON.stringify({ event: 'activation_open', agent_name: 'planner', card_id: 'project', input_id: `00000000-0000-4000-8000-${String(ordinal).padStart(12, '0')}`, timestamp }), context_policy: structuralContextPolicy('activation_boundary'), round_id: `r-pre-${String(ordinal).padStart(32, '0')}`, message_index: 0, block_index: 0, timestamp }, { id: `message-${ordinal}`, session_id: SESSION, role: 'user', kind: 'text', content: 'x'.repeat(400), context_policy: durableContentPolicy(), round_id: `r-user-${String(ordinal).padStart(32, '0')}`, message_index: 1, block_index: 0, timestamp }]); }
function invocationFor(sessionId: ConversationSessionId, messages: readonly AgentMessage[]): PreparedLlmInvocationInput { const agentName = conversationSessionIdentity(sessionId).agentName; return { inputId: '00000000-0000-4000-8000-000000000001', agentId: sessionId, agentName, sessionId, ...preparedInvocationContextFixture(), providerConversation: { sourceSessionId: sessionId, messages: [...messages] }, modelParams: { temperature: 0 }, preparedCompaction: prepareCompaction(config, 'system', []), capabilityRequest: {}, routePass: { kind: 'ordinary', candidateChain: [TEST_CANDIDATE] }, episodeContext: {} }; }
function round(id: string, tokens: number): ClassifiedRound { const message: AgentMessage = { id: `${id}-m`, session_id: SESSION, role: 'user', kind: 'text', content: 'x', context_policy: durableContentPolicy(), round_id: `r-user-${'0'.repeat(32)}`, message_index: 0, block_index: 0, timestamp: '2026-08-11T00:00:00.000Z' }; const positioned = { message, estimated_tokens: tokens }; return { round_id: id, activation_marker: positioned, rows: [positioned], sub_rounds: [], estimated_tokens: tokens }; }
