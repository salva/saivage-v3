...
describe('Stage-I versioned compaction', () => {
  it('retains route-independent budgets and newest-relative partitions', () => {
    ...
    const messages: AgentMessage[] = [{ id: 'm', session_id: 'agent:planner:project', role: 'user', kind: 'text', content: 'x'.repeat(4000), round_id: `r-user-${'0'.repeat(32)}`, message_index: 0, block_index: 0, timestamp: '2026-08-11T00:00:00.000Z' }];
...
function invocationFor(sessionId: ConversationSessionId, messages: readonly AgentMessage[]): PreparedLlmInvocationInput { const agentName = conversationSessionIdentity(sessionId).agentName; return { inputId: '00000000-0000-4000-8000-000000000001', agentId: sessionId, agentName, sessionId, systemPrompt: 'system', providerConversation: { sourceSessionId: sessionId, messages: [...messages] }, tools: [], terminalToolNames: [], modelParams: { temperature: 0 }, preparedCompaction: prepareCompaction(config, 'system', []), capabilityRequest: {}, routePass: { kind: 'ordinary', candidateChain: [TEST_CANDIDATE] }, episodeContext: {} }; }
function round(id: string, tokens: number): ClassifiedRound { const message: AgentMessage = { id: `${id}-m`, session_id: SESSION, role: 'user', kind: 'text', content: 'x', round_id: `r-user-${'0'.repeat(32)}`, message_index: 0, block_index: 0, timestamp: '2026-08-11T00:00:00.000Z' }; const positioned = { message, estimated_tokens: tokens }; return { round_id: id, activation_marker: positioned, rows: [positioned], sub_rounds: [], estimated_tokens: tokens }; }
