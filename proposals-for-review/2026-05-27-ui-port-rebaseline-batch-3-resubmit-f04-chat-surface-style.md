# Design: UI port rebaseline batch 3 (resubmit) — F04 chat surface style against HEAD `eb98caf`
...
[2026-05-27-2026-05-27-ui-port-rebaseline-batch-3-f04-chat-surface-style.md](rejected/2026-05-27-2026-05-27-ui-port-rebaseline-batch-3-f04-chat-surface-style.md).
The rejection cited the missing F03/R2 preconditions, which the
companion resubmission
[2026-05-27-ui-port-rebaseline-batch-2-resubmit-f03-conversation-rounds.md](2026-05-27-ui-port-rebaseline-batch-2-resubmit-f03-conversation-rounds.md)
addresses. The contract is unchanged; the *only* addition is the
**intermediate-broken-state authorization** in §"Implementation
latitude" below.
...
- The harness MAY split this batch into N internal stages (the
  B0..B7 sequence in the F04 plan r2 §4 is the recommended
  shape, but any decomposition that covers every row in the F04
  rebaseline §3 is acceptable).
...
- If the harness genuinely cannot reach a green PR tip, it must
  stop and file a delta-proposal cycle; it must NOT open a red
  PR and must NOT archive this proposal to `done/`.

The "no alias period, no parallel old+new class names" rule
still applies to the PR tip.
...
## Files / tests / docs to DELETE (PR tip)
...
- All hex literals and all nine forbidden selector families in
...
No alias period, no parallel old+new class names at the PR tip.
Intermediate stages MAY carry temporarily stale or duplicated
code as a side-effect of staged sequencing, provided the
duplicates are removed before the PR opens.
...
- Per-batch (B0–B7) acceptance commands per F04 plan r2 §5 as
  PR-tip checks (the per-batch commands are run once against
  the merge tip, not against every intermediate commit).
...
- The §0.1 class renames are NEW non-legacy identifiers, not
  ...
  same PR (B4/B5) that introduces the renamed classes; at no
  point in the PR's *tip* do both the old and the new selector
  coexist.
...
`architecture-audit/mailbox-<NNN>-ui-port-rebaseline-batch-3-resubmit/classification.md`
identifying this as Branch B, and
`architecture-audit/mailbox-<NNN>-ui-port-rebaseline-batch-3-resubmit/stage-plan.md`
