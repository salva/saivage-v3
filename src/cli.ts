...
async function handleInit(options: CliOptions): Promise<void> { const { initProjectTree, isInitialized } = await import('./persistence/file-tree.js'); const projectRoot = process.cwd(); if (!options.force && isInitialized(projectRoot)) { console.log(`Project already initialized at ${projectRoot}`); return; } initProjectTree(projectRoot); console.log(`Project initialized at ${projectRoot}`); }
async function handleStart(options: CliOptions, args: string[]): Promise<void> { const { startApp } = await import('./boot/app.js'); const app = await startApp({ argv: args, createRuntime: options['create-runtime'] === true }); console.log(`Saivage server listening on http://${app.environment.server.host}:${app.environment.server.port}`); }
async function handleStatus(): Promise<void> { const { findProjectRoot } = await import('./persistence/discovery.js'); const { readRuntimeState } = await import('./runtime/state.js'); const projectRoot = findProjectRoot(); if (projectRoot === null) { console.log('Not in a Saivage project'); return; } const state = readRuntimeState(projectRoot); if (state === null) { console.log(`Project root: ${projectRoot}`); console.log('Runtime state: not initialized (no state.json)'); return; } console.log(`Project root: ${projectRoot}`); console.log(`Status:       ${state.status}`); console.log(`PID:          ${state.pid}`); console.log(`Paused:       ${state.paused}`); console.log(`Current card: ${state.current_card_id ?? '(none)'}`); console.log(`Started at:   ${state.started_at}`); console.log(`Queue length: ${state.queue.length}`); }
async function restBaseUrl(projectRoot: string): Promise<string> { const cfgPath = join(projectRoot, '.saivage', 'saivage.json'); let host = '127.0.0.1'; let port = 8080; if (existsSync(cfgPath)) { try { const cfg = JSON.parse(readFileSync(cfgPath, 'utf-8')) as { server?: { host?: string; port?: number } }; host = cfg.server?.host === '0.0.0.0' ? '127.0.0.1' : (cfg.server?.host ?? host); port = cfg.server?.port ?? port; } catch {} } return `http://${host}:${port}`; }
