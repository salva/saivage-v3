...
export const EventRegistry = {
  ...
  card_history_appended: { schema: payload({ entry_id: z.string().uuid(), entry_kind: z.enum(['update', 'status', 'mutate', 'depends', 'delete', 'archive']), card_id: z.string(), version_seq: z.number(), changed_fields: z.array(z.string()), changed_at: z.string() }), severity: 'info', tracked: false, audit: true, broadcast: true, outbound: 'operator' },
  notification_added: { schema: payload({ session_id: z.string().nullable(), kind: z.string() }), severity: 'info', tracked: false, audit: true, broadcast: true, outbound: 'operator' },
