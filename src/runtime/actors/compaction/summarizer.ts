...
export async function summarizeRound(args: { sourceSessionId: string; round_id: string; rows: AgentMessage[]; summarizerProvider: SummarizerProviderPort; modelSpec: string; signal: AbortSignal }): Promise<string> {
...
export async function summarizeMerge(args: { entries: MergeSummaryInput[]; summarizerProvider: SummarizerProviderPort; modelSpec: string; signal: AbortSignal }): Promise<string> {
  ...
  const completion = await args.summarizerProvider.completeTurn(buildSummaryInput(randomUUID(), 'summary:merge', args.modelSpec, 'Merge these ordered Saivage round summaries into one concise historical summary. Do not include recoverable-evidence pointer sections.', providerConversation), args.signal);
  args.signal.throwIfAborted();
  return validateSummaryResult(completion.result, 'summarizeMerge');
}

function buildSummaryInput(inputId: string, sessionId: string, modelSpec: string, systemPrompt: string, providerConversation: LlmInvocationInput['providerConversation']): LlmInvocationInput {
  return { inputId, agentId: 'llm:compaction-summarizer', role: 'analyst', sessionId, systemPrompt: `${systemPrompt}\nModel: ${modelSpec}`, providerConversation, tools: [], terminalToolNames: [], modelParams: { temperature: 0, maxTokens: 2000 }, capabilityRequest: {}, episodeContext: { compaction: true, model_spec: modelSpec } };
}
