...
export function buildContentPolicyRetryMessage(sessionId: ConversationSessionId, sourceInputId: string): AgentMessage {
  ...
  return { id: randomUUID(), session_id: sessionId, role: 'user', kind: 'content_policy_retry', content: CONTENT_POLICY_RETRY_TEXT, round_id: inputRoundId('user', sourceInputId), message_index: 2, block_index: 0, timestamp: new Date().toISOString() };
}
...
export function buildContentPolicyRefusalMessage(args: { sessionId: ConversationSessionId; sourceInputId: string; candidate: Candidate; providerResponse: string }): AgentMessage {
  ...
  return { id: randomUUID(), session_id: args.sessionId, role: 'system', kind: 'content_policy_refusal', content: canonicalJson(contentPolicyRefusalContentSchema.parse(content)), round_id: inputRoundId('assistant', args.sourceInputId), message_index: 3, block_index: 0, timestamp: new Date().toISOString() };
}

function inputRoundId(kind: 'user' | 'assistant', sourceInputId: string): string { return `r-${kind}-${createHash('sha256').update(sourceInputId).digest('hex').slice(0, 32)}`; }
