...
export interface AgentNodeExecutionDeps {
  ...
  cardId: string;
  store: CardService;
  children: { get(cardId: string): CardActor | null };
...
export class AgentNodeExecution {
      ...
      return await runContractRepairLoop<AcceptedNodeResult>({
        ...
        onNonTerminalTool: async (toolOutcome) => {
          const toolResult = surface.tools.has(toolOutcome.toolName)
            ? await invokeToolForLlm(surface, toolOutcome.toolName, toolOutcome.args, signal, this.host.toolContext(llm, toolOutcome))
  ...
  private discardOpenRecord(filename: string, reason: string): void { try { const open = this.deps.store.readRecord(this.deps.cardId, filename, 'open'); this.deps.store.discardRecord(this.deps.cardId, filename, open.version, reason); } catch (error) { if (isMissingRecord(error)) return; throw error; } }
...
function readCandidate(store: CardService, cardId: string, filename: string, requireNonEmpty: boolean): RecordProjection | null { let open: RecordProjection | null = null; try { open = store.readRecord(cardId, filename, 'open'); } catch (error) { if (!isMissingRecord(error)) throw error; } if (open && (!requireNonEmpty || open.artifact.content.trim())) return open; try { const closed = store.readRecord(cardId, filename, 'latest'); return !requireNonEmpty || closed.artifact.content.trim() ? closed : null; } catch (error) { if (isMissingRecord(error)) return null; throw error; } }
