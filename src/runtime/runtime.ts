...
export class Runtime extends EventEmitter {
    ...
    while (activationCards.length > 0 && !this._shuttingDown) {
      ...
      for (const card of activationCards) {
        ...
        try {
          ...
          const result = this.agentRuntime.invokeExecutor({ cardId: card.id, goalId, systemPrompt: executorPrompt, contract: createExecutorContract({ cardId: card.id, goalId }) }); execResult = result instanceof Promise ? await result : result;
  ...
  async invokeReviewer(goalId: string, _planCardId: string, assessmentId: string, reviewerSessionId: string): Promise<ReviewerResult> {
    ...
    const result = await this.agentRuntime.invokeReviewer({ goalId, systemPrompt: reviewerPrompt, contextMessages: [], assessmentId, reviewerSessionId, contract: createReviewerContract({ goalId, assessmentId }) });
