...
export class Runtime extends EventEmitter {
  ...
  private diagnosticStamp(sessionId: string) {
    if (!this._activeRuntime) throw new Error('Runtime diagnostic stamp requires ActiveRuntime.');
    return this._activeRuntime.stampDiagnosticInCurrentRound(sessionId);
  }

  private userStamp(sessionId: string) {
    if (!this._activeRuntime) throw new Error('Runtime user stamp requires ActiveRuntime.');
  ...
  async dispatchGoal(goalId: string): Promise<void> {
    ...
    for (let iter = 0; iter < MAX_ITERATIONS && !plannerDone && !this._shuttingDown; iter++) {
      ...
      try {
        ...
        this.appendPlannerResumeContext(goalId, `planner:${goalId}`, resumeReason); injectQueuedSyntheticPlannerNotes(this.projectRoot, `planner:${goalId}`, { stampUserMessage: (id: string) => this.userStamp(id) }); const result = this.agentRuntime.invokePlanner({ goalId, systemPrompt: plannerPrompt }); plannerResult = result instanceof Promise ? await result : result;
