...
export class Runtime extends EventEmitter {
  ...
  constructor(config: RuntimeConfig, agentRuntime?: AgentRuntime) {
    ...
    this._stateMachine = new RuntimeStateMachine({
      ...
      clock: () => new Date(),
      scheduler,
      redispatchGoal: (cardId) => { void cardId; /* Step 6 wires redispatch */ },
  ...
  private _syncRunningProcesses(): void { try { updateRuntimeState(this.projectRoot, { running_processes: Array.from(this.runningProcesses) }); } catch { void 0; } }
  ...
  async startProject(source: 'operator' | 'tool' | 'runtime' = 'operator'): Promise<{ success: true; command: RuntimeCommandRecord; intent: RuntimeState['runtime_intent']; run: RuntimeRunRecord } | { success: false; command: RuntimeCommandRecord; error: ActionableErrorEnvelope }> {
    ...
    if (!this._paused) {
      this.trackBackgroundDispatch(this.dispatchGoal('project')
        ...
        .catch(() => {
          try { updateRuntimeState(this.projectRoot, { status: 'idle', current_card_id: null, current_agent_session_id: null, queue: [], active_card_run: null } as Partial<RuntimeState> as never); } catch { void 0; }
  ...
  async dispatchGoal(goalId: string): Promise<void> {
    ...
    for (let iter = 0; iter < MAX_ITERATIONS && !plannerDone && !this._shuttingDown; iter++) {
      ...
      try {
        ...
        const goalContext = this.buildGoalContextBlock(goalId, resumeReason); void goalContext;
  ...
  private async safeTick(): Promise<void> {
    ...
    try {
      ...
      if (state?.active_card_run) {
        if (state.active_card_run.phase === 'planner') { try { await this.dispatchGoal(state.active_card_run.card_id); } catch { void 0; } return; }
