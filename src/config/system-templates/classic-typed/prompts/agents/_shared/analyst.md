...
- Prefer queue_notification with the exact card_id over direct card mutation when a card is running, intent is advisory, or its configured current/next workflow-node agent should resolve the issue. Roles and session IDs are not notification targets.
