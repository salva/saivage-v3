...
function redactEscapedJsonSecretValues(content: string): string {
  return content.replace(
    ...
    (
      ...
      !isSecretKey(key) || shouldPreserveValue(secretValue)
        ? match
...
function redactInlineSecretAssignments(content: string): string {
  return content
    .replace(INLINE_SECRET_ASSIGNMENT_RE, (match, key: string, value: string) =>
      isSecretKey(key) && !value.startsWith(SECRET_REDACTION_PLACEHOLDER)
        ? `${key}=${SECRET_REDACTION_PLACEHOLDER}`
        : match,
    )
    .replace(URL_SECRET_QUERY_PARAM_RE, (match, prefix: string, value: string) =>
      value.startsWith(SECRET_REDACTION_PLACEHOLDER)
        ? match
