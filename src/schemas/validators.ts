...
export const cardStatusSchema = z.enum(['drafting','backlog','active','running','blocked','done','failed','cancelled']);
...
export const reviewAssessmentSchema: z.ZodType<import('./types.js').ReviewAssessment> = z.lazy(() => z.object({ id: z.string().min(1), goal_card_id: z.string().min(1), reviewer_session_id: z.string().min(1), result: z.enum(['pass', 'fail']), summary: z.string(), achieved: z.array(z.string()), missing: z.array(z.string()), evidence_card_ids: z.array(z.string()), created_at: z.string().datetime() }));
export const plannerDispatchCompletionSchema = z.object({ outcome: z.enum(['done', 'failed', 'blocked', 'cancelled', 'timed_out']), summary: z.string(), child_result: z.record(z.string(), z.unknown()).nullable(), review: reviewAssessmentSchema.nullable(), artifacts: z.array(artifactRefSchema), attachments: z.array(attachmentRefSchema), evidence_card_ids: z.array(z.string()), error: z.string().nullable() });
export const plannerFrameRecordSchema = z.object({ frame_id: z.string().min(1), planner_card_id: z.string().min(1), planner_role: z.literal('planner'), planner_scope: z.enum(['project', 'goal']), status: plannerFrameStatusSchema, resume_reason: plannerResumeReasonSchema, waiting_on_dispatch_ids: z.array(z.string()), last_resume_cursor: z.string().datetime().nullable(), last_dispatch_id: z.string().nullable(), created_at: z.string().datetime(), updated_at: z.string().datetime() });
export const plannerDispatchRecordSchema = z.object({ dispatch_id: z.string().min(1), parent_frame_id: z.string().min(1), parent_card_id: z.string().min(1), target_card_id: z.string().min(1), target_kind: z.enum(['goal', 'terminal_card']), requested_by_role: z.literal('planner'), requested_by_scope: z.enum(['project', 'goal']), status: plannerDispatchStatusSchema, completion: plannerDispatchCompletionSchema.nullable(), idempotency_key: z.string().min(1), created_at: z.string().datetime(), started_at: z.string().datetime().nullable(), completed_at: z.string().datetime().nullable() });
...
export const processRecordSchema = z.object({ id: z.string().min(1), card_id: z.string().min(1), command: z.string().min(1), cwd: z.string().min(1), status: processStatusSchema, pid: z.number().int().nullable().optional(), started_at: z.string().datetime(), completed_at: z.string().datetime().nullable().optional(), exit_code: z.number().int().nullable().optional(), required_for_card_completion: z.boolean(), output_dir: z.string().min(1), stdout_path: z.string().min(1), stderr_path: z.string().min(1), combined_log_path: z.string().min(1), agent_session_id: z.string().nullable().optional(), goal_id: z.string().nullable().optional(), launch_reason: z.string().nullable().optional(), owner_kind: z.enum(['agent', 'operator', 'runtime']).nullable().optional(), background_policy: z.enum(['foreground', 'background_required', 'background_optional', 'detach', 'kill_on_freeze']).nullable().optional(), process_group_id: z.number().int().nonnegative().nullable().optional() });
