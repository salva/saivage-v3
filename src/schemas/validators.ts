...
export const runtimeDispatchOwnershipSchema: z.ZodType<import('./types.js').RuntimeDispatchOwnership> = z.union([
  ...
  z.object({ kind: z.literal('activation'), parent_card_id: cardIdSchema, parent_tool_call: z.object({ session_id: z.string().min(1), source_input_id: z.string().min(1), tool_call_id: z.string().min(1) }).strict() }).strict(),
]);
