...
export const cardRecordSchema: z.ZodType<import('./types.js').CardRecord> = z.lazy(() => z.object({ id: z.string().min(1), type: cardTypeSchema, parent: z.string().nullable(), depth: z.number().int().min(0), position: z.number().int().nonnegative(), title: z.string().min(1), description: z.string(), status: cardStatusSchema, lifecycle: cardLifecycleStateSchema, planner_state: cardStatusSchema.optional(), plannerState: cardStatusSchema.optional(), subtype: z.string().nullable().optional(), instructions_file: z.string().nullable().optional(), tags: z.array(z.string()), priority: z.number().int(), urgency: urgencySchema, created_by: createdBySchema, created_at: z.string().datetime(), updated_at: z.string().datetime(), version_seq: z.number().int().positive(), assigned_to: z.string().nullable().optional(), depends_on: z.array(z.string()), blocks: z.array(z.string()), related: z.array(z.string()), acceptance: z.string(), metrics: z.record(z.string(), z.union([z.number(), z.string(), z.boolean(), z.null()])).nullable().optional(), artifacts: z.array(artifactRefSchema), attachments: z.array(attachmentRefSchema), estimate: z.string().nullable().optional(), started_at: z.string().datetime().nullable().optional(), duration_ms: z.number().int().nonnegative().nullable().optional(), status_text: z.string().nullable().optional(), status_text_updated_at: z.string().datetime().nullable().optional(), status_text_author_session_id: z.string().nullable().optional(), latest_self_report: z.record(z.string(), z.unknown()).nullable().optional(), metadata: cardMetadataSchema.nullable().optional(), allowedActions: z.array(cardActionSchema).optional(), retries: z.number().int().nonnegative() }).superRefine((card, ctx) => { if (card.status !== card.lifecycle.status) ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Card status must match lifecycle.status', path: ['status'] }); }));
