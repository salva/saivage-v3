...
export const processRecordSchema = z.object({ id: z.string().min(1), card_id: z.string().min(1), owner_id: z.string().nullable(), command: z.string().min(1), command_hash: z.string().min(32), cwd: z.string().min(1), cwd_canonical: z.string().min(1), status: processStatusSchema, pid: z.number().int().nullable().optional(), started_at: z.string().datetime(), started_at_monotonic: z.number().finite(), completed_at: z.string().datetime().nullable().optional(), exit_code: z.number().int().nullable().optional(), signal: z.string().nullable().optional(), terminal_reason: z.enum(['exit', 'signal', 'spawn_error', 'lost', 'kill_unattached']).nullable().optional(), required_for_card_completion: z.boolean(), output_dir: z.string().min(1), stdout_path: z.string().min(1), stderr_path: z.string().min(1), agent_session_id: z.string().nullable().optional(), goal_id: z.string().nullable().optional(), launch_reason: z.string().nullable().optional(), owner_kind: z.enum(['agent', 'operator', 'runtime']).nullable().optional(), background_policy: z.enum(['foreground']).nullable().optional(), process_group_id: z.number().int().nonnegative().nullable().optional(), failure_classification: z.enum(['lost', 'spawn_error']).nullable().optional(), reattach_error: z.string().nullable().optional() });
...
export const runtimeStateSchema = z.object({ status: runtimeStatusSchema, project_id: z.literal('project'), pid: z.number().int().positive(), started_at: z.string().datetime(), active_card_run: activeCardRunSchema.nullable(), updated_at: z.string().datetime(), last_tick_at: z.string().datetime().nullable().optional(), runtime_commands: z.array(runtimeCommandRecordSchema), runtime_runs: z.array(runtimeRunRecordSchema), runtime_activations: z.array(runtimeActivationRecordSchema) }).strict();
