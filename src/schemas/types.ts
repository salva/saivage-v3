...
export interface PlannerDispatchCompletion { outcome: 'done' | 'failed' | 'blocked' | 'cancelled' | 'timed_out'; summary: string; child_result: Record<string, unknown> | null; review: ReviewAssessment | null; artifacts: ArtifactRef[]; attachments: AttachmentRef[]; evidence_card_ids: string[]; error: string | null; }
...
export interface ReviewAssessment { id: string; goal_card_id: string; reviewer_session_id: string; result: 'pass' | 'fail'; summary: string; achieved: string[]; missing: string[]; evidence_card_ids: string[]; created_at: string; }
