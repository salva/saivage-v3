...
export type LlmFailureClass = 'auth_permanent' | 'rate_limit' | 'server_transient' | 'timeout' | 'contract_mismatch' | 'capability_mismatch' | 'token_budget_exceeded' | 'parse_error' | 'cancelled' | 'unknown';
...
export interface LlmInvocationSummaryEvent extends BaseEvent { kind: 'llm_invocation_summary'; session_id: string; role: AgentRole; goal_id: string; card_id: string; attempts_count: number; total_duration_ms: number; verdict: 'succeeded' | 'exhausted' | 'cancelled'; final_provider?: string; final_model?: string; final_account?: string; final_terminal_tool?: 'emit_planner_result' | 'emit_executor_result' | 'emit_reviewer_result'; last_failure_class?: LlmFailureClass; }
