...
export interface ReviewerResult { result: 'pass' | 'needs_corrections' | 'fail'; summary: string; achieved: string[]; issues?: ReviewerIssue[]; evidence_card_ids: string[]; missing?: string[]; }
...
export interface ProcessRecord { id: string; card_id: string; command: string; cwd: string; status: ProcessStatus; pid?: number | null; started_at: string; completed_at?: string | null; exit_code?: number | null; required_for_card_completion: boolean; output_dir: string; stdout_path: string; stderr_path: string; combined_log_path: string; agent_session_id?: string | null; goal_id?: string | null; launch_reason?: string | null; owner_kind?: 'agent' | 'operator' | 'runtime' | null; background_policy?: 'foreground' | 'background_required' | 'background_optional' | 'detach' | 'kill_on_freeze' | null; process_group_id?: number | null; }
...
export type RuntimeEventKind = 'goal_report_rejected' | 'started' | 'goal_completed' | 'goal_failed' | 'card_failed' | 'review_complete' | 'review_failed' | 'dispatch_blocked' | 'dispatch_interrupted' | 'dispatch_held_for_notification' | 'escalation' | 'plan_updated' | 'paused' | 'resumed' | 'shutdown' | 'error' | 'stuck_supervisor_started' | 'stuck_supervisor_stopped' | 'stuck_verdict' | 'abort_target_selected' | 'force_cancel_sent' | 'project_run_completed';
...
export type LoggedEvent = StartedEvent | GoalCompletedEvent | GoalFailedEvent | CardFailedEvent | ReviewCompleteEvent | ReviewFailedEvent | DispatchBlockedEvent | DispatchInterruptedEvent | EscalationEvent | PlanUpdatedEvent | PausedEvent | ResumedEvent | ShutdownEvent | ErrorEvent | ProjectRunCompletedEvent | StuckSupervisorStartedEvent | StuckSupervisorStoppedEvent | StuckVerdictEvent | AbortTargetSelectedEvent | ForceCancelSentEvent | SessionStartedEvent | ModelSelectedEvent | InvocationSucceededEvent | InvocationFailedEvent | RetryAttemptedEvent | CompactionTriggeredEvent | SelfCheckTriggeredEvent | SessionCancelledEvent | SessionForceCancelledEvent | McpToolInvocationEvent;
