...
export const cardVersionChangeSchema = z.object({
  ...
  terminal_summary: z.object({ status: z.enum(['done', 'failed', 'blocked']), result_kind: z.enum(['workflow-result', 'runtime-failure', 'content-policy-refusal']), summary: z.string().min(1), content_policy: z.object({ session_id: ConversationSessionIdSchema, marker_id: z.string().min(1), evidence_url: z.string().min(1), blocked_at: z.string().datetime() }).strict().nullable() }).strict().nullable(),
