...
const RecordMutationStateFailureSchema = z.discriminatedUnion('error', [
  ...
  z.object({ kind: z.literal('rejected'), error: z.literal('old_string was not found in current record content.'), data: z.object({ code: z.literal('record_edit_old_string_not_found'), ...commonIdentity, current_head: positiveSafeIntegerSchema }).strict() }).strict(),
  z.object({ kind: z.literal('rejected'), error: z.literal('old_string matched multiple locations; set replace_all to true.'), data: z.object({ code: z.literal('record_edit_old_string_multiple_matches'), ...commonIdentity, current_head: positiveSafeIntegerSchema, occurrences: positiveSafeIntegerSchema.min(2), replace_all_required: z.literal(true) }).strict() }).strict(),
  z.object({ kind: z.literal('rejected'), error: z.literal('Record content must not be empty.'), data: z.object({ code: z.literal('record_result_content_empty'), ...commonIdentity, current_head: positiveSafeIntegerSchema.nullable(), operation: operationSchema }).strict() }).strict(),
]);
...
export const RecordMutationSuccessSchema = z.object({ kind: z.literal('applied'), data: z.object({
