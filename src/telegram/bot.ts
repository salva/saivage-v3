...
export class TelegramBot {
  ...
  private async _pollLoop(signal: AbortSignal): Promise<void> { while (!signal.aborted) { try { const updates = await this._getUpdates(signal); for (const update of updates) { if (signal.aborted) break; await this._handleUpdate(update); this.lastUpdateId = Math.max(this.lastUpdateId, update.update_id); } } catch (err) { if (signal.aborted) break; console.error(`[telegram] Poll error: ${err instanceof Error ? err.message : String(err)}`); await sleepWithSignal(POLL_INTERVAL_MS, signal); } } }
  ...
  private async _handleUpdate(update: TelegramUpdate): Promise<void> { const msg = update.message ?? update.edited_message; if (!msg || !msg.text) return; const from = msg.from; if (!from) return; const userId = from.id; const chatId = msg.chat.id; if (!this.isAuthorized(userId)) return; const sessionId = `telegram-${chatId}`; try { getOrCreateAnalystSession(this.projectRoot, sessionId); } catch (err) { console.error(`[telegram] Failed to create session for chat ${chatId}: ${err instanceof Error ? err.message : String(err)}`); return; } try { const response = await this.analystHandler.handleMessage(sessionId, msg.text); await this.sendMessage(chatId, response.message.content, { parseMode: 'Markdown' }); } catch (err) { console.error(`[telegram] Error handling message from ${userId}: ${err instanceof Error ? err.message : String(err)}`); try { await this.sendMessage(chatId, 'Sorry, something went wrong processing your request.'); } catch {} } }
