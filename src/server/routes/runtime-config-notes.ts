...
function legacyConversationEntry(raw: unknown, sessionId: string, index: number): Record<string, unknown> { const entry = raw && typeof raw === 'object' ? raw as Record<string, unknown> : {}; return { id: typeof entry['id'] === 'string' ? entry['id'] : `${sessionId}:${index}`, session_id: typeof entry['session_id'] === 'string' ? entry['session_id'] : sessionId, role: typeof entry['role'] === 'string' ? entry['role'] : 'assistant', kind: typeof entry['kind'] === 'string' ? entry['kind'] : 'text', content: typeof entry['content'] === 'string' ? entry['content'] : JSON.stringify(entry), round_id: typeof entry['round_id'] === 'string' ? entry['round_id'] : `r-assistant-${index + 1}`, message_index: typeof entry['message_index'] === 'number' ? entry['message_index'] : index, block_index: typeof entry['block_index'] === 'number' ? entry['block_index'] : 0, tool: entry['tool'], tool_call_id: entry['tool_call_id'], timestamp: typeof entry['timestamp'] === 'string' ? entry['timestamp'] : new Date(0).toISOString(), links: entry['links'], model_spec: entry['model_spec'], requested_model_spec: entry['requested_model_spec'] }; }
function readConversationEntries(projectRoot: string, sessionId: string): unknown[] { const messagesPath = join(projectRoot, '.saivage', 'agents', 'messages', `${sessionId}.jsonl`); if (!existsSync(messagesPath)) return []; const entries: unknown[] = []; for (const line of readFileSync(messagesPath, 'utf-8').split('\n')) if (line.trim()) { const raw = JSON.parse(line) as unknown; const parsed = agentMessageSchema.safeParse(raw); entries.push(parsed.success ? parsed.data : agentMessageSchema.parse(legacyConversationEntry(raw, sessionId, entries.length))); } return entries; }
...
function firstMessageTimestamp(projectRoot: string, sessionId: string): string | null {
  const messages = readConversationEntries(projectRoot, sessionId);
