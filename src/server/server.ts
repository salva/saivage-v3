...
export interface ServerInstance { fastify: FastifyInstance; config: ServerConfig; saivageConfig: SaivageConfig; mcpManager: McpManager; telegramBot?: TelegramBot; runtimeApplication: RuntimeApplication; }
...
export async function createServer(options: CreateServerOptions): Promise<ServerInstance> {
  ...
  return { fastify: services.fastify, config: serverConfig, saivageConfig: services.config, mcpManager: services.mcpManager, telegramBot: services.telegramBot, runtimeApplication: services.runtimeApplication };
}
