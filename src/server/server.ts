...
export interface CreateServerOptions { environment: Environment; scope?: ResourceScope; restartPort?: RestartPort; }
...
export async function createServer(options: CreateServerOptions): Promise<ServerInstance> {
  ...
  const services = await createServerServices({ environment, scope, restartPort: options.restartPort });
