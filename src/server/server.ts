...
function registerHealth(fastify: FastifyInstance, projectRoot: string, _saivageConfig: SaivageConfig): void { fastify.get('/health', async () => { let runtimeStatus = 'unknown'; let frozenReason: string | undefined; try { const { readRuntimeState } = await import('../utils/runtime-state.js'); const state = readRuntimeState(projectRoot); if (state) { runtimeStatus = state.status; if (state.status === 'frozen') { const manifest = readFreezeManifest(projectRoot); if (manifest) frozenReason = manifest.reason; } } } catch {} const response: Record<string, unknown> = { status: 'ok', version: '0.1.0', project: 'saivage-v3', runtime: runtimeStatus }; if (frozenReason !== undefined) response.frozen_reason = frozenReason; return response; }); }
