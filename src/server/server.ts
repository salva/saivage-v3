...
export interface CreateServerOptions { environment: Environment; authority: ProjectStoreRepository; persistenceHealth: ApplicationPersistenceHealth; scope?: ResourceScope; restartPort?: RestartPort; }
...
export async function createServer(options: CreateServerOptions): Promise<ServerInstance> {
  ...
  const services = await createServerServices({ environment, authority: options.authority, persistenceHealth: options.persistenceHealth, scope, restartPort: options.restartPort });
