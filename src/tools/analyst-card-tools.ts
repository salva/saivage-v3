...
export const analystCardToolBinders: readonly ToolBinder<ToolContext, any>[] = Object.freeze([
  defineToolBinder({ name: 'create_card', description: `Create a card without dispatching work. Analyst use requires runtime status stopped or paused and an existing non-running parent. Every created child receives backlog lifecycle.`, inputSchema: (ctx) => createAnalystCreateCardInputSchema(ctx.cardTypeVocabulary), resultPolicyTemplate: PRIMARY_TOOL_RESULT_POLICY_TEMPLATE, executor: (ctx, args, signal) => noneToolExecutionPromise(create_card(ctx, args, signal)) }),
  defineToolBinder({ name: 'reorder_child', description: 'Reorder children of a non-running parent while runtime status is stopped or paused. Denies running parents and running children; orderedChildIds must be a permutation of the current child set.', inputSchema: () => analystReorderChildInputSchema, resultPolicyTemplate: PRIMARY_TOOL_RESULT_POLICY_TEMPLATE, executor: (ctx, args, signal) => noneToolExecutionPromise(reorder_child(ctx, args, signal)) }),
  defineToolBinder({ name: 'reopen_card', description: 'Reopen a done, failed, or blocked card without editing its content while Analyst intervention is ready (runtime stopped or settled paused). Changes the target and eligible resting ancestors through normal changed propagation.', inputSchema: () => analystReopenCardInputSchema, resultPolicyTemplate: PRIMARY_TOOL_RESULT_POLICY_TEMPLATE, executor: (ctx, args, signal) => noneToolExecutionPromise(reopen_card(ctx, args, signal)) }),
  defineToolBinder({ name: 'get_status', description: 'Get the overall project status.', inputSchema: () => emptyInput, resultPolicyTemplate: OBSERVATIONAL_TOOL_RESULT_POLICY_TEMPLATE, executor: (ctx, args) => observationalToolExecutionPromise(get_status(ctx, args)) }),
  defineToolBinder({ name: 'cancel_card', description: 'Cancel non-completed work. Analyst cancellation allows every status except done and cancelled, rejects the root project card, and requires exact runtime ownership for running work.', inputSchema: () => analystCancelCardInputSchema, resultPolicyTemplate: PRIMARY_TOOL_RESULT_POLICY_TEMPLATE, executor: (ctx, args, signal) => noneToolExecutionPromise(cancel_card(ctx, args, signal)) }),
  defineToolBinder({ name: 'delete_card', description: 'Delete one or more non-running card subtrees while runtime status is stopped or paused. Deleted ids remain reserved; no card restore/archive content is produced. Denies the root project card and any running subtree member.', inputSchema: () => analystDeleteCardInputSchema, resultPolicyTemplate: PRIMARY_TOOL_RESULT_POLICY_TEMPLATE, executor: (ctx, args, signal) => noneToolExecutionPromise(delete_card(ctx, args, signal)) }),
]);
