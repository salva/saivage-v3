...
import { defineToolBinder, noneToolExecutionPromise, observationalToolExecutionPromise, type ToolBinder, type ToolResult } from './invocation.js';
...
export const workspaceToolBinders: readonly ToolBinder<WorkspaceProviderContext, any>[] = Object.freeze([
  defineToolBinder({ name: 'read', description: readDescription, inputSchema: () => readWorkspaceInputSchema, resultPolicyTemplate: OBSERVATIONAL_TOOL_RESULT_POLICY_TEMPLATE, executor: (ctx, args) => observationalToolExecutionPromise(runWorkspaceTool(() => readProject(ctx, args))) }),
  defineToolBinder({ name: 'write', description: 'Create or replace a project, record, tmp, or system file according to the named agent contract.', inputSchema: () => writeWorkspaceInputSchema, resultPolicyTemplate: PRIMARY_TOOL_RESULT_POLICY_TEMPLATE, executor: (ctx, args) => noneToolExecutionPromise(runWorkspaceTool(() => writeProject(ctx, args))) }),
  defineToolBinder({ name: 'edit', description: 'Replace exact text in a project, record, tmp, or system file according to the named agent contract.', inputSchema: () => editWorkspaceInputSchema, resultPolicyTemplate: PRIMARY_TOOL_RESULT_POLICY_TEMPLATE, executor: (ctx, args) => noneToolExecutionPromise(runWorkspaceTool(() => editProject(ctx, args))) }),
  defineToolBinder({ name: 'glob', description: 'Search files by glob pattern under a scoped directory, including read-only work:/// process-output and stash directories.', inputSchema: () => globWorkspaceInputSchema, resultPolicyTemplate: OBSERVATIONAL_TOOL_RESULT_POLICY_TEMPLATE, executor: (ctx, args) => observationalToolExecutionPromise(runWorkspaceTool(() => globProject(ctx, args))) }),
  defineToolBinder({ name: 'grep', description: grepDescription, inputSchema: () => grepWorkspaceInputSchema, resultPolicyTemplate: OBSERVATIONAL_TOOL_RESULT_POLICY_TEMPLATE, executor: (ctx, args) => observationalToolExecutionPromise(runWorkspaceTool(() => grepProject(ctx, args))) }),
]);
export const patchToolBinders: readonly ToolBinder<WorkspaceProviderContext, any>[] = Object.freeze([
  defineToolBinder({ name: 'apply_patch', description: 'Apply a text-only unified diff. Patch paths are project-relative only; scoped URL paths such as work:/// are rejected in diff headers.', inputSchema: () => applyPatchInputSchema, resultPolicyTemplate: PRIMARY_TOOL_RESULT_POLICY_TEMPLATE, executor: (ctx, args) => noneToolExecutionPromise(runWorkspaceTool(() => applyProjectPatch(ctx, args))) }),
]);
export const analystWorkspaceToolBinders: readonly ToolBinder<AnalystToolContext, any>[] = Object.freeze([
  defineToolBinder({ name: 'read', description: readDescription, inputSchema: () => readWorkspaceInputSchema, resultPolicyTemplate: OBSERVATIONAL_TOOL_RESULT_POLICY_TEMPLATE, executor: (ctx, args) => observationalToolExecutionPromise(runWorkspaceTool(() => readProject(analystWorkspace(ctx), args))) }),
  defineToolBinder({ name: 'write', description: 'Create or replace a project, record, tmp, or system file according to the named agent contract.', inputSchema: () => writeWorkspaceInputSchema, resultPolicyTemplate: PRIMARY_TOOL_RESULT_POLICY_TEMPLATE, executor: (ctx, args, signal) => noneToolExecutionPromise(args.path.startsWith('record:///') ? runAuditedAnalystTool(ctx, args, { action: 'record.write', safety_class: 'low', target_kind: 'card', getTargetId: (input) => input.path, lifecycle: { kind: 'intervention_ready', timing: 'immediate_before_mutation' }, mutate: (_prepared, input, mutation) => mutation.services.recordMutations.write(input.path, input.content) }, signal) : runWorkspaceTool(() => writeProject(analystWorkspace(ctx), args))) }),
  defineToolBinder({ name: 'edit', description: 'Replace exact text in a project, record, tmp, or system file according to the named agent contract.', inputSchema: () => editWorkspaceInputSchema, resultPolicyTemplate: PRIMARY_TOOL_RESULT_POLICY_TEMPLATE, executor: (ctx, args, signal) => noneToolExecutionPromise(args.path.startsWith('record:///') ? runAuditedAnalystTool(ctx, args, { action: 'record.edit', safety_class: 'low', target_kind: 'card', getTargetId: (input) => input.path, lifecycle: { kind: 'intervention_ready', timing: 'immediate_before_mutation' }, mutate: (_prepared, input, mutation) => mutation.services.recordMutations.edit(input.path, input.old_string, input.new_string, input.replace_all === true) }, signal) : runWorkspaceTool(() => editProject(analystWorkspace(ctx), args))) }),
  defineToolBinder({ name: 'glob', description: 'Search files by glob pattern under a scoped directory, including read-only work:/// process-output and stash directories.', inputSchema: () => globWorkspaceInputSchema, resultPolicyTemplate: OBSERVATIONAL_TOOL_RESULT_POLICY_TEMPLATE, executor: (ctx, args) => observationalToolExecutionPromise(runWorkspaceTool(() => globProject(analystWorkspace(ctx), args))) }),
  defineToolBinder({ name: 'grep', description: grepDescription, inputSchema: () => grepWorkspaceInputSchema, resultPolicyTemplate: OBSERVATIONAL_TOOL_RESULT_POLICY_TEMPLATE, executor: (ctx, args) => observationalToolExecutionPromise(runWorkspaceTool(() => grepProject(analystWorkspace(ctx), args))) }),
]);
export const analystPatchToolBinders: readonly ToolBinder<AnalystToolContext, any>[] = Object.freeze([
  defineToolBinder({ name: 'apply_patch', description: 'Apply a text-only unified diff.', inputSchema: () => applyPatchInputSchema, resultPolicyTemplate: PRIMARY_TOOL_RESULT_POLICY_TEMPLATE, executor: (ctx, args) => noneToolExecutionPromise(runWorkspaceTool(() => applyProjectPatch({ projectRoot: ctx.projectRoot,agentName:ctx.actor,filesystemWrite:true }, args))) }),
]);
