...
export function createWorkspaceProvider(ctx: WorkspaceProviderContext): ToolProvider {
  return {
    ...
    tools: [
      ...
      defineTool({ name: 'write', description: 'Create or replace a project, record, tmp, or system file according to the named agent contract.', inputSchema: writeWorkspaceInputSchema, executor: (args) => runWorkspaceTool(() => writeProject(ctx, args)) }),
      defineTool({ name: 'edit', description: 'Replace exact text in a project, record, tmp, or system file according to the named agent contract.', inputSchema: editWorkspaceInputSchema, executor: (args) => runWorkspaceTool(() => editProject(ctx, args)) }),
...
export function createAnalystWorkspaceProvider(ctx: AnalystToolContext): ToolProvider {
  const workspace: WorkspaceProviderContext = { projectRoot: ctx.projectRoot, agentName:ctx.actor,filesystemWrite:true,store: ctx.store, notifyCard: ctx.runtime?.notifyCard };
  return {
    providerName: 'workspace',
    tools: [
      defineTool({ name: 'read', description: 'Read a project:///, record:///, tmp:///, system:///, or read-only work:/// file or directory through scoped URLs. Use work:/// to page through runtime process output and stash files. Text reads return at most 2000 lines, 2000 characters per line, and about 256KB total inline content; files larger than about 10MB are not read inline. Set metadata_only to inspect file size/mtime or visible directory entry counts without reading content.', inputSchema: readWorkspaceInputSchema, executor: (args) => runWorkspaceTool(() => readProject(workspace, args)) }),
      defineTool({ name: 'write', description: 'Create or replace a project, record, tmp, or system file according to the named agent contract.', inputSchema: writeWorkspaceInputSchema, executor: (args, signal) => args.path.startsWith('record:///')
        ? runAuditedAnalystTool(ctx, args, { action: 'record.write', safety_class: 'low', target_kind: 'card', getTargetId: (input) => input.path, lifecycle: 'intervention_ready', mutate: (_prepared, input, mutation) => mutation.services.recordMutations.write(input.path, input.content) }, signal)
        : runWorkspaceTool(() => writeProject(workspace, args)) }),
      defineTool({ name: 'edit', description: 'Replace exact text in a project, record, tmp, or system file according to the named agent contract.', inputSchema: editWorkspaceInputSchema, executor: (args, signal) => args.path.startsWith('record:///')
        ? runAuditedAnalystTool(ctx, args, { action: 'record.edit', safety_class: 'low', target_kind: 'card', getTargetId: (input) => input.path, lifecycle: 'intervention_ready', mutate: (_prepared, input, mutation) => mutation.services.recordMutations.edit(input.path, input.old_string, input.new_string, input.replace_all === true) }, signal)
...
export function createAnalystPatchProvider(ctx: AnalystToolContext): ToolProvider {
  return { providerName: 'patch', tools: [defineTool({ name: 'apply_patch', description: 'Apply a text-only unified diff.', inputSchema: applyPatchInputSchema, executor: (args) => runWorkspaceTool(() => applyProjectPatch({ projectRoot: ctx.projectRoot,agentName:ctx.actor,filesystemWrite:true }, args)) })] };
}
