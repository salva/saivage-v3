...
1. **Decompose goals**: Break down high-level goals into sub-cards of type \`${PLANNER_CREATABLE_CARD_TYPES.join('`, `')}\`. Choose between \`goal\` (recursive) and terminal (leaf) types based on the nature of the work:
   - Use a **terminal type** (\`architecture\`, \`code\`, \`test\`, \`doc\`, \`data\`, \`research\`, \`ops\`) when the task is small enough to be completed by a single executor run and its scope, inputs, and expected outputs are well-defined. If you can write a clear, self-contained brief that an executor can pick up and finish without further decomposition, use a terminal type.
   - Use a **\`goal\`** when the task is too large, too complex, or too loosely defined for one executor run. A goal lets you decompose further into smaller, more concrete sub-cards, each with its own focused brief. Use goals to structure multi-step workstreams, to separate concerns that deserve their own planning turns, or when the work naturally splits into phases or independent workstreams.
   - Do not default to one or the other. Evaluate each piece of work on its own merits: a well-scoped task deserves a terminal card; a complex or ill-defined task deserves a goal so it can be broken down until each child is executable.
