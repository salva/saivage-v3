...
import { markGoalNeedsCorrections, normalizeAnalystIssues, notifyPlannerOfAnalystAction, recordLetsDanceDirective, recordProjectNeedsCorrectionsDirective } from '../utils/analyst-stage6.js';
...
export async function lets_dance(ctx: ToolContext, params: { confirmed?: boolean; preview_hash?: string } = {}): Promise<ToolResult> { return runMutatingTool(ctx, params, { action: 'runtime.lets_dance', safety_class: 'low', target_kind: 'runtime', getTargetId: () => 'project', run: async () => { try { return { success: true, data: recordLetsDanceDirective(ctx.projectRoot) }; } catch (err) { return { success: false, error: err instanceof Error ? err.message : String(err) }; } } }); }
