...
export interface ToolContext { projectRoot: string; store?: CardStore; sessionId?: string; activeRuntime?: ActiveRuntime; requestServerRestart?: () => Promise<void>; actor: ActorRole; surface: ControlActionSurface; }
...
export async function start_project(ctx: ToolContext, params: Record<string, never> = {}): Promise<ToolResult> { return runAuditedAnalystTool(ctx, params, { action: 'runtime.start_project', safety_class: 'low', target_kind: 'runtime', getTargetId: () => PROJECT_CARD_ID, run: async () => { if (!ctx.activeRuntime) return { success: false, error: 'Active runtime is not available.' }; const data = await ctx.activeRuntime.runtime.startProject('analyst'); return { success: data.success, ...(data.success ? { data } : { error: data.error.message, data }) }; } }); }

export async function stop_project(ctx: ToolContext, params: Record<string, never> = {}): Promise<ToolResult> { return runAuditedAnalystTool(ctx, params, { action: 'runtime.stop_project', safety_class: 'destructive', target_kind: 'runtime', getTargetId: () => PROJECT_CARD_ID, run: async () => { if (!ctx.activeRuntime) return { success: false, error: 'Active runtime is not available.' }; const data = await ctx.activeRuntime.runtime.stopProject('analyst'); return { success: true, data }; } }); }
