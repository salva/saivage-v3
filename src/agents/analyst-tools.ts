...
import { listProcesses, tailOutput, getProcess, killProcess } from '../runtime/process-api.js';
...
import {
  ...
  notifyPlannerOfAnalystAction,
} from '../agents/analyst-stage6.js';
import {
  CARD_STATUS_VALUES,
  CARD_TYPE_VALUES,
...
function humanizeToolError(toolName: string, raw: string): string {
  ...
  const enumIssueRe =
    /"received":\s*"([^"]*)"[\s\S]*?"path":\s*\[\s*"([^"]+)"[\s\S]*?Expected\s+([^,]+(?:\s*\|\s*[^,]+)+)/g;
  ...
  if (enumHints.length === 0) {
    if (/\bstatus\b/i.test(raw))
      enumHints.push(`'status' allowed values: ${CARD_STATUS_VALUES.join(', ')}`);
    else if (/\burgency\b/i.test(raw))
      enumHints.push(`'urgency' allowed values: ${URGENCY_VALUES.join(', ')}`);
    else if (/\btype\b/i.test(raw))
      enumHints.push(`'type' allowed values: ${CARD_TYPE_VALUES.join(', ')}`);
  }
...
function preflightEnum<T extends string>(
  value: unknown,
  allowed: readonly T[],
  field: string,
  toolName: string,
  ...
  if (typeof value !== 'string')
    return {
      ok: false,
      error: `${toolName} failed: field '${field}' must be a string. Allowed values: ${allowed.join(', ')}. See the '${toolName}' tool's parameter schema.`,
    };
  if (!(allowed as readonly string[]).includes(value))
    return {
      ok: false,
      error: `${toolName} failed: field '${field}' received '${value}', which is not a valid value. Allowed values: ${allowed.join(', ')}. See the '${toolName}' tool's parameter schema.`,
    };
  return { ok: true };
}
...
type NormalizedShellCommandParams = {
  command: string;
  cwd: string;
  timeoutMs: number;
  maxOutputBytes: number;
...
function defaultParentForCreate(store: CardStore, type: CardType): string | null | undefined {
  ...
  const activeGoals = store
    ...
    .filter(
      (card) =>
        card.type === 'goal' &&
        ['active', 'running', 'backlog', 'drafting', 'blocked'].includes(card.status),
    )
...
function buildDeletePreview(projectRoot: string, store: CardStore, id: string): ActionPreview {
  ...
  return {
    ...
    affectedProcesses: listProcesses(projectRoot)
      .filter((p) => allAffectedIds.includes(p.card_id))
      .map((p) => ({ id: p.id, command: p.command, status: p.status })),
    warnings:
      descendantIds.length > 0
        ? [`This will permanently delete ${descendantIds.length} descendant card(s).`]
        : [],
  };
}
...
function summarizeShellOutcome(
  ...
  return timedOut
    ? 'command timed out'
    : `exit=${exitCode === null ? 'null' : String(exitCode)}${truncated ? ' truncated' : ''}`;
}
function captureLimited(
  buffer: Buffer,
  limit: number,
): { text: string; truncated: boolean; truncatedBytes: number } {
  if (buffer.length <= limit)
    return { text: buffer.toString('utf8'), truncated: false, truncatedBytes: 0 };
...
async function runShellCommandWithCapture(
  command: string,
  cwd: string,
  timeoutMs: number,
  maxOutputBytes: number,
  ...
  return await new Promise((resolveResult) => {
    ...
    child.on('error', (error) => {
      resolveResult({
        ...
        stdout: '',
        stderr: redactShellText(error.message),
        truncated: false,
        timedOut: false,
      });
    });
    ...
    child.on('close', (code, signal) => {
      ...
      resolveResult({
        ...
        stderr: redactShellText(
          stderrCapture.text || (timedOut ? `Command timed out after ${timeoutMs}ms.` : ''),
        ),
        truncated:
...
function normalizeShellCwd(projectRoot: string, cwd: unknown): string {
  ...
  if (rel !== '' && (rel === '..' || rel.startsWith('../') || rel.startsWith('..\\')))
    throw new Error('cwd must stay within the project root.');
  if (!existsSync(resolved) || !statSync(resolved).isDirectory())
    throw new Error('cwd is not a readable directory within the project root.');
  assertSafeShellCwd(resolved);
  return resolved;
}
function normalizeShellNumeric(
  value: unknown,
  fallback: number,
  max: number,
  field: 'timeoutMs' | 'maxOutputBytes',
): number {
...
function normalizeShellParams(
  ctx: ToolContext,
  params: ShellCommandParams,
): NormalizedShellCommandParams {
  if (params === null || typeof params !== 'object' || Array.isArray(params))
    throw new Error('run_shell_command params must be an object.');
...
export async function mark_goal_needs_corrections(
  ctx: ToolContext,
  params: { goalId: string; issues: unknown[]; note?: string },
): Promise<ToolResult> {
  return runAuditedAnalystTool(ctx, params, {
    action: 'goal.needs_corrections',
    safety_class: 'destructive',
    target_kind: 'card',
    getTargetId: (p) => p.goalId,
    run: async () => {
      try {
        const issues = normalizeAnalystIssues(params.issues);
        return {
          success: true,
          data: markGoalNeedsCorrections(ctx.projectRoot, params.goalId, issues, params.note),
        };
      } catch (err) {
  ...
  return runAuditedAnalystTool(ctx, params, {
    ...
    run: async () => {
      try {
        ...
        if (!typeCheck.ok) return { success: false, error: typeCheck.error };
        const statusCheck = preflightEnum(
          params.status,
          CARD_STATUS_VALUES,
          'status',
          'create_card',
        ...
        const urgencyCheck = preflightEnum(
          params.urgency,
          URGENCY_VALUES,
          'urgency',
          'create_card',
        );
        if (!urgencyCheck.ok) return { success: false, error: urgencyCheck.error };
        ...
        return {
          success: false,
          error: humanizeToolError('create_card', err instanceof Error ? err.message : String(err)),
        };
      }
    },
  });
...
export async function edit_card(
  ...
  return runAuditedAnalystTool(ctx, params, {
    ...
    preview: () => ({
      type: 'edit_card',
      summary: `Edit card '${params.id}'.`,
      affectedCards: getStore(ctx).read(params.id)
        ? [cardSummary(getStore(ctx).read(params.id)!)]
        : [],
      affectedProcesses: [],
      warnings: [],
    }),
    run: async () => {
      try {
        const statusCheck = preflightEnum(params.status, CARD_STATUS_VALUES, 'status', 'edit_card');
        if (!statusCheck.ok) return { success: false, error: statusCheck.error };
        const urgencyCheck = preflightEnum(params.urgency, URGENCY_VALUES, 'urgency', 'edit_card');
        if (!urgencyCheck.ok) return { success: false, error: urgencyCheck.error };
        const typeCheck = preflightEnum(params.type, CARD_TYPE_VALUES, 'type', 'edit_card');
        if (!typeCheck.ok) return { success: false, error: typeCheck.error };
        const store = getStore(ctx);
        ...
        if (Object.keys(changes).length === 0)
          return {
            success: false,
            error: `edit_card failed: no allowed fields to update. Rejected fields: ${rejected.join(', ') || '(none)'}. Allowed fields include: ${Array.from(ALLOWED_EDIT_FIELDS).join(', ')}. See the 'edit_card' tool's parameter schema.`,
          };
        const updated = store.mutateCard(params.id, changes as Partial<CardRecord>, {
          actor: ctx.actor,
          surface: ctx.surface,
          reason: 'analyst edit',
        });
        ...
        return {
          success: false,
          error: humanizeToolError('edit_card', err instanceof Error ? err.message : String(err)),
        };
      }
    },
  });
...
export async function delete_card(
  ...
  return runAuditedAnalystTool(ctx, params, {
    ...
    safety_class: 'destructive',
    target_kind: 'card',
    getTargetId: (p) => p.ids.join(','),
    preview: () => {
      const store = getStore(ctx);
      const previews = params.ids.map((id) => buildDeletePreview(ctx.projectRoot, store, id));
      return {
        type: 'delete_card',
        summary: `Delete ${params.ids.length} card target(s) and their descendants.`,
        affectedCards: previews.flatMap((preview) => preview.affectedCards),
        affectedProcesses: previews.flatMap((preview) => preview.affectedProcesses),
    ...
    run: async () => {
      ...
      for (const targetId of params.ids) {
        try {
          const card = store.read(targetId);
          if (!card) {
            failures.push({ id: targetId, reason: `Card '${targetId}' not found.` });
            continue;
          }
          const cards = [targetId, ...store.getDescendantIds(targetId)]
            .map((id) => store.read(id))
            .filter((c): c is CardRecord => c !== null)
            .sort((a, b) => b.depth - a.depth);
          const denied = cards.find(
            (c) =>
              !decide({ role: 'analyst', action: 'card.delete', targetState: c.status }).allowed,
          );
          if (denied) {
            const decision = decide({
              role: 'analyst',
              action: 'card.delete',
              targetState: denied.status,
            });
            failures.push({
              id: targetId,
              reason: `delete_card denied by permission matrix for card '${denied.id}' in state '${denied.status}' (${decision.allowed ? 'not_allowed' : decision.reason}).`,
            });
            continue;
          }
          for (const c of cards) {
            store.delete(c.id);
            deletedAll.push(c.id);
          }
          deletedTopLevel.push(targetId);
        } catch (err) {
          failures.push({ id: targetId, reason: err instanceof Error ? err.message : String(err) });
        }
      }
      if (deletedTopLevel.length > 0 && failures.length > 0)
        return {
          success: true,
          data: {
            partial: true,
            total: params.ids.length,
            succeeded: deletedTopLevel.length,
            failures,
          },
        };
      if (failures.length > 0)
        return {
          success: false,
          error: failures.map((failure) => `${failure.id}: ${failure.reason}`).join('; '),
          data: { failures },
        };
    ...
    if (params.parent !== undefined)
      cards =
        params.parent === null
          ...
          : cards.filter((c) => store.listChildren(params.parent!).includes(c.id));
...
export async function get_tree(ctx: ToolContext, params: { rootId?: string }): Promise<ToolResult> {
  try {
    const store = getStore(ctx);
    const rootId = params.rootId ?? PROJECT_CARD_ID;
    if (!store.read(rootId)) return { success: false, error: `Root card '${rootId}' not found.` };
...
export async function get_card_output(
  ctx: ToolContext,
  params: { cardId: string; lines?: number; processId?: string },
): Promise<ToolResult> {
  try {
    const store = getStore(ctx);
    if (!store.read(params.cardId))
      return { success: false, error: `Card '${params.cardId}' not found.` };
    const numLines = params.lines ?? 50;
    if (params.processId) {
      const proc = getProcess(ctx.projectRoot, params.processId);
      if (!proc) return { success: false, error: `Process '${params.processId}' not found.` };
      if (proc.card_id !== params.cardId)
        return {
          success: false,
          error: `Process '${params.processId}' is not associated with card '${params.cardId}'.`,
        };
      return {
        success: true,
        data: {
          process: { id: proc.id, command: proc.command, status: proc.status, pid: proc.pid },
          output: tailOutput(ctx.projectRoot, params.processId, numLines),
        },
      };
    }
    return {
      success: true,
      data: listProcesses(ctx.projectRoot, { cardId: params.cardId }).map((proc) => ({
        id: proc.id,
        ...
        output: tailOutput(ctx.projectRoot, proc.id, numLines),
      })),
    };
  } catch (err) {
    return { success: false, error: err instanceof Error ? err.message : String(err) };
...
export async function get_status(
  ...
  try {
    ...
    const plannerStateCounts = allCards.reduce<Record<string, number>>((counts, card) => {
      counts[card.status] = (counts[card.status] ?? 0) + 1;
      return counts;
    }, {});
    const activeCardRun = runtimeState?.active_card_run ?? null;
    const runtimeIntent = runtimeState?.runtime_intent ?? null;
    const runtimeRuns = runtimeState?.runtime_runs ?? [];
    const activationRecords = runtimeState?.runtime_activations ?? [];
    return {
      success: true,
      data: {
        ...
        runtimeSummary: {
          ...
          activations: activationRecords.map((activation) => ({
            activation_id: activation.activation_id,
            parent_card_id: activation.parent_card_id,
            child_card_id: activation.child_card_id,
            status: activation.status,
            requested_at: activation.requested_at,
            runtime_run_id: activation.runtime_run_id ?? null,
          })),
        },
        runningProcesses: runningProcesses.length,
        plannerStateCounts,
        counts: {
          done: plannerStateCounts.done ?? 0,
          failed: plannerStateCounts.failed ?? 0,
          blocked: plannerStateCounts.blocked ?? 0,
...
export async function list_card_history(
  ctx: ToolContext,
...
export async function get_card_history_entry(
  ctx: ToolContext,
  params: { cardId: string; version_seq: number },
): Promise<ToolResult> {
  try {
    const store = getStore(ctx);
    const entry = store
      .listCardHistory(params.cardId)
      .find((candidate) => candidate.version_seq === params.version_seq);
    if (!entry)
      return {
        success: false,
        error: `Card '${params.cardId}' has no history entry for version ${params.version_seq}.`,
      };
...
export async function diff_card(
  ctx: ToolContext,
  params: { cardId: string; fromSeq?: number; toSeq?: number },
): Promise<ToolResult> {
  try {
    const store = getStore(ctx);
    ...
    return {
      ...
      data: {
        ...
        from_version_seq: fromSeq,
        to_version_seq: toSeq,
        diff: store.diffCard(params.cardId, fromSeq, toSeq),
      },
    };
  } catch (err) {
    return { success: false, error: err instanceof Error ? err.message : String(err) };
...
export async function start_project(
  ...
  return runAuditedAnalystTool(ctx, params, {
    action: 'runtime.start_project',
    safety_class: 'low',
    target_kind: 'runtime',
    getTargetId: () => PROJECT_CARD_ID,
    run: async () => {
      ...
      return {
        ...
        ...(data.success ? { data } : { error: data.error.message, data }),
      };
    },
  });
}
...
export async function stop_project(
  ...
  return runAuditedAnalystTool(ctx, params, {
    action: 'runtime.stop_project',
    safety_class: 'destructive',
    target_kind: 'runtime',
    getTargetId: () => PROJECT_CARD_ID,
    run: async () => {
      if (!ctx.runtime) return { success: false, error: 'Active runtime is not available.' };
      const data = await ctx.runtime.stopProject('analyst');
...
export async function terminate_process(
  ...
  return runAuditedAnalystTool(ctx, params, {
    ...
    safety_class: 'destructive',
    target_kind: 'process',
    getTargetId: (p) => p.processId,
    run: async () => {
      const proc = await killProcess(ctx.projectRoot, params.processId, 'SIGTERM');
      if (!proc) return { success: false, error: `Process '${params.processId}' not found.` };
  ...
  return runAuditedAnalystTool(ctx, params, {
    ...
    run: async () => {
      ...
      if (!result.ok)
        return {
          success: false,
          error: result.message ?? result.error ?? 'Failed to pause runtime',
        };
  ...
  return runAuditedAnalystTool(ctx, params, {
    ...
    run: async () => {
      ...
      if (state?.status === 'frozen' || state?.status === 'error')
        return {
          success: false,
          error: `${state.status === 'frozen' ? FROZEN_RUNTIME_RECOVERY_MESSAGE : 'Runtime is in error state. Inspect Debug errors/timeline and fix the underlying failure before attempting recovery.'}`,
        };
      const result = resumeRuntimeControl({
        projectRoot: ctx.projectRoot,
        runtimeApi: ctx.runtime,
      });
      if (!result.ok)
        return {
          success: false,
          error: result.message ?? result.error ?? 'Failed to resume runtime',
        };
      return { success: true, data: { status: result.status, paused: result.paused } };
    },
  });
}
export async function abort_goal_subtree(
  ctx: ToolContext,
  params: { goalId: string },
): Promise<ToolResult> {
  return runAuditedAnalystTool(ctx, params, {
    ...
    target_kind: 'card',
    getTargetId: (p) => p.goalId,
    preview: () => buildAbortPreview(ctx.projectRoot, getStore(ctx), params.goalId),
    run: async () => {
      const store = getStore(ctx);
      try {
        const goal = store.read(params.goalId);
...
export async function restart_card_or_subtree(
  ctx: ToolContext,
  params: { id: string },
): Promise<ToolResult> {
  return runAuditedAnalystTool(ctx, params, {
    ...
    preview: () => {
      ...
      return card
        ...
        : {
            type: 'restart_card',
            summary: `Restart card '${params.id}'.`,
            affectedCards: [],
            affectedProcesses: [],
            warnings: [],
          };
    ...
    run: async () => {
      ...
      try {
        ...
        if (card.type === 'goal' || card.type === 'project')
          return restart_goal(ctx, { goalId: params.id });
        if (!decide({ role: 'analyst', action: 'card.restart', targetState: card.status }).allowed)
          return {
            success: false,
            error: `Card '${params.id}' has status '${card.status}'. Only matrix-allowed states can be restarted by analyst.`,
          };
        return {
          success: true,
          data: store.repairTerminalLifecycle(params.id, {
            status: 'backlog',
...
export async function restart_goal(
  ...
  return runAuditedAnalystTool(ctx, params, {
    ...
    target_kind: 'card',
    getTargetId: (p) => p.goalId,
    preview: () => buildRestartGoalPreview(ctx.projectRoot, getStore(ctx), params.goalId),
    run: async () => {
      const store = getStore(ctx);
      try {
        const goal = store.read(params.goalId);
        ...
        store.repairTerminalLifecycle(params.goalId, {
          status: 'backlog',
          lifecycle: { status: 'backlog', result: null, error: null, completed_at: null },
        });
        return {
          ...
          data: {
            ...
            status: 'backlog',
            descendantIds: store.getDescendantIds(params.goalId),
          },
        };
      } catch (err) {
        return { success: false, error: err instanceof Error ? err.message : String(err) };
...
export async function queue_notification(
  ctx: ToolContext,
  params: { recipient: string; kind: string; body: string },
): Promise<ToolResult> {
  return runAuditedAnalystTool(
    ctx,
    { recipient: params.recipient, kind: params.kind },
    {
      action: 'notification.queue',
      safety_class: 'low',
      target_kind: 'session',
      getTargetId: () => params.recipient,
      run: async () => {
        const resolved = resolveRecipient(ctx.projectRoot, params.recipient);
        if (resolved === null)
          return {
            success: false,
            data: { reason: 'unknown_recipient', recipient: params.recipient },
          };
        queueNotification(ctx.projectRoot, resolved, params.kind, params.body, {
          actor: 'analyst',
          surface: ctx.surface,
        });
...
export async function reorder_child(
  ctx: ToolContext,
  params: { parentId: string; orderedChildIds: string[] },
): Promise<ToolResult> {
  return runAuditedAnalystTool(ctx, params, {
    action: 'card.reorder_child',
    safety_class: 'low',
    target_kind: 'card',
    getTargetId: (p) => p.parentId,
    run: async () => {
      try {
        const store = getStore(ctx);
        const r = store.reorderChildren(params.parentId, params.orderedChildIds, {
          actor: 'analyst',
          surface: ctx.surface,
          reason: 'analyst reorder_child',
        });
        if (r.ok)
...
export async function navigate_workspace(
  ...
  return runAuditedAnalystTool(ctx, params, {
    ...
    run: async () => ({
      success: true,
      data: { intent: 'navigate_workspace', target: params.target },
    }),
  });
}
...
export async function navigate_back(
  ...
  return runAuditedAnalystTool(ctx, params, {
    ...
    run: async () => ({ success: true, data: { intent: 'navigate_back' } }),
  });
}
...
export async function show_config(
  ...
  try {
    const path = join(ctx.projectRoot, '.saivage', 'saivage.json');
    assertAnalystInspectionTarget(path);
    const result = getRedactedConfig(ctx.projectRoot);
    if (!result.success)
      return {
        success: false,
        data: { reason: 'invalid_argument', fieldPath: result.fieldPath, detail: result.message },
      };
    return { success: true, data: { config: redactAnalystSecretValue(result.config) } };
  } catch (err) {
    return { success: false, error: err instanceof Error ? err.message : String(err) };
  }
}
...
export async function restart_server(
  ...
  return runAuditedAnalystTool(ctx, params, {
    ...
    getTargetId: () => 'server',
    run: async () => {
      if (!ctx.requestServerRestart)
        return { success: false, error: 'Server restart primitive is not available.' };
      await ctx.requestServerRestart();
      return { success: true, data: { restart_requested: true } };
    },
  });
}
...
type ReconfigureParams = {
  ...
  for_model?: string;
  ordered_failover_models?: string[];
  name?: string;
  command?: string;
  args?: string[];
...
export async function reconfigure(
  ...
  return runAuditedAnalystTool(ctx, params as ReconfigureParams & Record<string, unknown>, {
    action: actionName,
    safety_class: 'low',
    target_kind: targetKind,
    getTargetId: targetId,
    run: async () => {
      const invalid = (fieldPath: string, detail: string): ToolResult => ({
        success: false,
        data: { reason: 'invalid_argument', fieldPath, detail },
        error: detail,
      });
      let result;
      switch (params.action) {
        case 'set_role_routing':
          result = setRoleRouting(ctx.projectRoot, params.role!, params.model_candidate!);
          break;
        case 'set_failover_chain':
          result = setFailoverChain(
            ctx.projectRoot,
            params.for_model!,
            params.ordered_failover_models!,
          );
          break;
        case 'mcp_add':
          result = mcpAdd(ctx.projectRoot, params.name!, params.command!, params.args, params.env);
          if (result.success) {
            ctx.mcpManager?.reloadServersFromConfig();
            await ctx.mcpManager?.startServer(params.name!);
          }
          break;
        case 'mcp_edit':
          result = mcpEdit(ctx.projectRoot, params.name!, {
            command: params.command,
            args: params.args,
            env: params.env,
          });
          if (result.success) {
            ctx.mcpManager?.reloadServersFromConfig();
            await ctx.mcpManager?.restartServer(params.name!);
          }
          break;
        case 'mcp_remove':
          await ctx.mcpManager?.stopServer(params.name!);
          result = mcpRemove(ctx.projectRoot, params.name!);
          if (result.success) ctx.mcpManager?.reloadServersFromConfig();
          break;
        case 'set_runtime_setting':
          result = setRuntimeSetting(ctx.projectRoot, params.key!, params.value);
          break;
        case 'set_server_setting':
          result = setServerSetting(ctx.projectRoot, params.key!, params.value);
          break;
        default:
          return invalid('action', 'Unknown reconfigure action.');
      }
      if (!result.success) return invalid(result.fieldPath, result.message);
      if (params.action === 'set_server_setting' && result.requires_restart)
        return { success: true, data: { applied: true, requires_restart: true, key: params.key } };
...
export async function run_shell_command(
  ...
  try {
    if (ctx.surface === 'telegram')
      return { success: false, error: 'run_shell_command is not available on Telegram.' };
    const normalized = normalizeShellParams(ctx, params);
    for (const token of normalized.command.split(/\s+/)) {
      if (token && looksLikeSecretPath(token)) throw new SecretPathError(token);
    }
    const classifiedAs = classifyShellCommand(normalized.command, normalized.cwd);
    const verdict = evaluateAuthz({
      actor: ctx.actor,
      surface: ctx.surface,
      safety_class: classifiedAs,
    });
    const auditBase = {
      actor: ctx.actor,
      surface: ctx.surface,
      ...
      target_kind: null,
      target_id: null,
      params_summary: `shell.exec [classified=${classifiedAs}] ${summarizeShellCommand(normalized.command)}`,
    };
    if (verdict === 'deny') {
      recordControlAction(ctx.projectRoot, {
        ...auditBase,
        outcome: 'denied',
        outcome_summary: `shell denied [classified=${classifiedAs}]`,
      });
      return {
        success: false,
        error: `Denied by authorization policy for ${ctx.actor}/${ctx.surface}/${classifiedAs}.`,
        data: { classified_as: classifiedAs },
      };
    }
    const result = await runShellCommandWithCapture(
      normalized.command,
      normalized.cwd,
    ...
    const payload = {
      ...
      cwd: normalized.cwd,
      command: redactShellText(normalized.command),
    };
    if (classifiedAs !== 'read_only') {
      recordControlAction(ctx.projectRoot, {
        ...auditBase,
        outcome: result.exitCode === 0 && !result.timedOut ? 'ok' : 'error',
        outcome_summary:
          `${summarizeShellOutcome(result.exitCode, result.truncated, result.timedOut)} stdout=${result.stdout} stderr=${result.stderr}`.slice(
            0,
            2000,
          ),
        ...(result.exitCode === 0 && !result.timedOut
          ...
          : {
              error:
                result.stderr ||
                `shell command failed: ${summarizeShellOutcome(result.exitCode, result.truncated, result.timedOut)}`,
            }),
      });
    }
    if (result.timedOut)
      return {
        success: false,
        error: `Command timed out after ${normalized.timeoutMs}ms.`,
        data: payload,
      };
    return {
      ...
      ...(result.exitCode === 0
        ? { data: payload }
        : { error: result.stderr || `Command exited with code ${result.exitCode}`, data: payload }),
    };
  } catch (err) {
    if (err instanceof SecretPathError)
      return {
        success: false,
        error:
          'Access denied: secret-bearing path is off-limits ([SECRET_PATH]). Use safer inspection paths that do not touch secrets.',
      };
    return {
      success: false,
      error:
        err instanceof Error
          ? redactShellText(err.message).replaceAll(ctx.projectRoot, '[PROJECT_ROOT]')
          : String(err),
    };
  }
}
...
export async function read_file(
  ...
  try {
    ...
    if (st.isDirectory())
      return { success: false, error: `Path is a directory; use list_directory instead: ${abs}` };
    if (!st.isFile()) return { success: false, error: `Path is not a regular file: ${abs}` };
    const cap = Math.min(
      Math.max(1, params.maxBytes ?? FILE_READ_DEFAULT_BYTES),
      FILE_READ_MAX_BYTES,
    );
...
export async function list_directory(
  _ctx: ToolContext,
  params: { path: string; maxEntries?: number },
): Promise<ToolResult> {
  try {
    if (typeof params.path !== 'string' || params.path.length === 0)
      return { success: false, error: 'path is required.' };
    ...
    for (const name of slice) {
      ...
      try {
        ...
        entries.push({
          name,
          type: cs.isDirectory() ? 'directory' : cs.isFile() ? 'file' : 'other',
          size: cs.isFile() ? cs.size : undefined,
          symlink,
          modified_at: cs.mtime.toISOString(),
        });
      } catch (err) {
        entries.push({
          name,
...
export async function read_runtime_events(
  ctx: ToolContext,
  params: { limit?: number; kind?: string },
): Promise<ToolResult> {
  try {
    const limit = Math.min(Math.max(1, params.limit ?? JSONL_TAIL_DEFAULT), JSONL_TAIL_MAX);
    const { entries, total } = readJsonlTail(
      join(ctx.projectRoot, '.saivage', 'runtime', 'events.jsonl'),
      limit,
    );
    const filtered = params.kind
      ? entries.filter(
          (e) =>
            typeof e === 'object' &&
            e !== null &&
            (e as Record<string, unknown>)['kind'] === params.kind,
        )
      : entries;
    return {
      success: true,
      data: { total_lines: total, returned: filtered.length, events: filtered },
    };
  } catch (err) {
    return { success: false, error: err instanceof Error ? err.message : String(err) };
  }
...
export async function read_runtime_errors(
  ctx: ToolContext,
  params: { limit?: number },
): Promise<ToolResult> {
  try {
    const limit = Math.min(Math.max(1, params.limit ?? JSONL_TAIL_DEFAULT), JSONL_TAIL_MAX);
    const { entries, total } = readJsonlTail(
      join(ctx.projectRoot, '.saivage', 'runtime', 'errors.jsonl'),
      limit,
    );
    return {
      success: true,
      data: { total_lines: total, returned: entries.length, errors: entries },
    };
  } catch (err) {
    return { success: false, error: err instanceof Error ? err.message : String(err) };
  }
...
export async function read_control_actions(
  ctx: ToolContext,
  params: { limit?: number; since?: string },
): Promise<ToolResult> {
  try {
    const limit = Math.min(Math.max(1, params.limit ?? JSONL_TAIL_DEFAULT), JSONL_TAIL_MAX);
    const all = listControlActions(
      ctx.projectRoot,
    ...
    return {
      success: true,
      data: { total_lines: all.length, returned: tail.length, actions: tail },
    };
  } catch (err) {
    return { success: false, error: err instanceof Error ? err.message : String(err) };
  }
...
export async function list_processes_tool(
  ctx: ToolContext,
  params: { status?: string; cardId?: string },
): Promise<ToolResult> {
  try {
    const procs = listProcesses(
      ctx.projectRoot,
      params.cardId ? { cardId: params.cardId } : undefined,
    );
...
export async function list_agent_sessions(
  ...
  try {
    ...
    const files = readdirSync(dir)
      ...
      .sort();
    const sessions: Array<Record<string, unknown>> = files.flatMap(
      (file): Array<Record<string, unknown>> => {
        try {
          const data = JSON.parse(readFileSync(join(dir, file), 'utf-8')) as Record<
            string,
            unknown
          >;
          const id = (data['id'] as string) ?? file.replace('.json', '');
          if (data['role'] === 'analyst' && id !== GLOBAL_ANALYST_SESSION_ID) return [];
          return [
            {
              id,
              role: data['role'] ?? null,
...
export async function read_agent_session(
  ...
  try {
    ...
    return {
      success: true,
      data: { session, total_messages: total, returned: entries.length, messages: entries },
    };
  } catch (err) {
    return { success: false, error: err instanceof Error ? err.message : String(err) };
  }
