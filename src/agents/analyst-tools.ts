...
import { CARD_STATUS_VALUES, CARD_TYPE_VALUES, URGENCY_VALUES } from './analyst-tool-schemas.js';
...
export interface ToolContext { projectRoot: string; store?: CardStore; sessionId?: string; runtime?: Pick<RuntimeApi, 'startProject' | 'stopProject' | 'pause' | 'resume'>; mcpManager?: McpManager; requestServerRestart?: () => Promise<void>; actor: ActorRole; surface: ControlActionSurface; }
...
export async function start_project(ctx: ToolContext, params: Record<string, never> = {}): Promise<ToolResult> { return runAuditedAnalystTool(ctx, params, { action: 'runtime.start_project', safety_class: 'low', target_kind: 'runtime', getTargetId: () => PROJECT_CARD_ID, run: async () => { if (!ctx.runtime) return { success: false, error: 'Active runtime is not available.' }; const data = await ctx.runtime.startProject('analyst'); return { success: data.success, ...(data.success ? { data } : { error: data.error.message, data }) }; } }); }

export async function stop_project(ctx: ToolContext, params: Record<string, never> = {}): Promise<ToolResult> { return runAuditedAnalystTool(ctx, params, { action: 'runtime.stop_project', safety_class: 'destructive', target_kind: 'runtime', getTargetId: () => PROJECT_CARD_ID, run: async () => { if (!ctx.runtime) return { success: false, error: 'Active runtime is not available.' }; const data = await ctx.runtime.stopProject('analyst'); return { success: true, data }; } }); }
...
export async function pause_runtime(ctx: ToolContext, params: Record<string, never> = {}): Promise<ToolResult> { return runAuditedAnalystTool(ctx, params, { action: 'runtime.pause', safety_class: 'low', target_kind: 'runtime', getTargetId: () => 'project', run: async () => { const result = pauseRuntimeControl({ projectRoot: ctx.projectRoot, runtimeApi: ctx.runtime }); if (!result.ok) return { success: false, error: result.message ?? result.error ?? 'Failed to pause runtime' }; return { success: true, data: { status: result.status, paused: result.paused } }; } }); }
export async function resume_runtime(ctx: ToolContext, params: Record<string, never> = {}): Promise<ToolResult> { return runAuditedAnalystTool(ctx, params, { action: 'runtime.resume', safety_class: 'low', target_kind: 'runtime', getTargetId: () => 'project', run: async () => { const state = readRuntimeState(ctx.projectRoot); if (state?.status === 'frozen' || state?.status === 'error') return { success: false, error: `${state.status === 'frozen' ? FROZEN_RUNTIME_RECOVERY_MESSAGE : 'Runtime is in error state. Inspect Debug errors/timeline and fix the underlying failure before attempting recovery.'}` }; const result = resumeRuntimeControl({ projectRoot: ctx.projectRoot, runtimeApi: ctx.runtime }); if (!result.ok) return { success: false, error: result.message ?? result.error ?? 'Failed to resume runtime' }; return { success: true, data: { status: result.status, paused: result.paused } }; } }); }
  ...
  return runAuditedAnalystTool(ctx, params as ReconfigureParams & Record<string, unknown>, { action: actionName, safety_class: 'low', target_kind: targetKind, getTargetId: targetId, run: async () => {
    ...
    switch (params.action) {
      ...
      case 'mcp_add':
        ...
        if (result.success) { ctx.mcpManager?.reloadServersFromConfig(); await ctx.mcpManager?.startServer(params.name!); }
        break;
      case 'mcp_edit':
        result = mcpEdit(ctx.projectRoot, params.name!, { command: params.command, args: params.args, env: params.env });
        if (result.success) { ctx.mcpManager?.reloadServersFromConfig(); await ctx.mcpManager?.restartServer(params.name!); }
        break;
      case 'mcp_remove':
...
export async function list_agent_sessions(ctx: ToolContext, _params: Record<string, never>): Promise<ToolResult> {
  try {
    ...
    const sessions: Array<Record<string, unknown>> = files.flatMap((file): Array<Record<string, unknown>> => {
      try {
        ...
        return [{ id, role: data['role'] ?? null, status: data['status'] ?? null, started_at: data['started_at'] ?? null, card_id: data['card_id'] ?? null }];
      } catch (err) {
