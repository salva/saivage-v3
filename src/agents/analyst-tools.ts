...
import { markGoalNeedsCorrections, normalizeAnalystIssues, notifyPlannerOfAnalystAction, recordLetsDanceDirective, withLetsDanceWakeupObservation, recordProjectNeedsCorrectionsDirective } from '../utils/analyst-stage6.js';
...
export async function lets_dance(ctx: ToolContext, params: { confirmed?: boolean; preview_hash?: string } = {}): Promise<ToolResult> { return runMutatingTool(ctx, params, { action: 'runtime.lets_dance', safety_class: 'low', target_kind: 'runtime', getTargetId: () => 'project', run: async () => { try { const recorded = recordLetsDanceDirective(ctx.projectRoot, { runtime_available: Boolean(ctx.activeRuntime) }); const wakeup = ctx.activeRuntime && recorded.outcome !== 'queued_paused' && recorded.outcome !== 'blocked_project_status' && recorded.outcome !== 'active_run_present' ? await ctx.activeRuntime.requestProjectDirectiveWakeup('lets_dance') : null; return { success: true, data: withLetsDanceWakeupObservation(recorded, wakeup) }; } catch (err) { return { success: false, error: err instanceof Error ? err.message : String(err) }; } } }); }

export async function mark_goal_needs_corrections(ctx: ToolContext, params: { goalId: string; issues: unknown[]; note?: string; confirmed?: boolean; preview_hash?: string }): Promise<ToolResult> { return runMutatingTool(ctx, params, { action: 'goal.needs_corrections', safety_class: 'high', target_kind: 'card', getTargetId: (p) => p.goalId, run: async () => { try { const issues = normalizeAnalystIssues(params.issues); return { success: true, data: markGoalNeedsCorrections(ctx.projectRoot, params.goalId, issues, params.note) }; } catch (err) { return { success: false, error: err instanceof Error ? err.message : String(err) }; } } }); }

export async function mark_project_needs_corrections(ctx: ToolContext, params: { issues: unknown[]; note?: string; confirmed?: boolean; preview_hash?: string }): Promise<ToolResult> { return runMutatingTool(ctx, params, { action: 'runtime.project_needs_corrections', safety_class: 'high', target_kind: 'runtime', getTargetId: () => 'project', run: async () => { try { const issues = normalizeAnalystIssues(params.issues); return { success: true, data: recordProjectNeedsCorrectionsDirective(ctx.projectRoot, issues, params.note) }; } catch (err) { return { success: false, error: err instanceof Error ? err.message : String(err) }; } } }); }
