...
import { appendMessage, completeSession, createSession, markSessionWaiting, type RoundStamp } from '../agents/session-persistence.js';
...
export class FakeAgentAdapter implements AgentExecutionPort {
  ...
  private fixtureRoundCounters = new Map<string, number>();
  private fixtureBlockCounters = new Map<string, number>();
  ...
  private openAssistantRound(sessionId: string): void { const next = (this.fixtureRoundCounters.get(sessionId) ?? 0) + 1; this.fixtureRoundCounters.set(sessionId, next); this.fixtureBlockCounters.set(sessionId, 0); }
  private stampInRound(sessionId: string): RoundStamp { if (!this.fixtureRoundCounters.has(sessionId)) this.openAssistantRound(sessionId); const round = this.fixtureRoundCounters.get(sessionId) ?? 1; const block = this.fixtureBlockCounters.get(sessionId) ?? 0; this.fixtureBlockCounters.set(sessionId, block + 1); return { round_id: `r-assistant-${round}`, message_index: block, block_index: block }; }
  private closeAssistantRound(sessionId: string): void { this.fixtureBlockCounters.set(sessionId, 0); }
  ...
  private invokePlannerForGoal(goalId: string): PlannerResult { const fixture = this.resolveFixture(goalId); if (!fixture.planner || fixture.planner.length === 0) throw new Error(`FakeAgent fixture '${fixture.name}' has no planner results.`); const count = this.plannerCounters.get(fixture.name) ?? 0; if (count >= fixture.planner.length) throw new Error(`FakeAgent fixture '${fixture.name}' exhausted planner results (called ${count + 1} times, only ${fixture.planner.length} available).`); const sessionId = this.registerSession('planner', goalId, goalId, `Planning goal ${goalId}`, fixture.name); let persistedSessionId: string | null = null; let converted: PlannerResult | null = null; try { if (this.config.saivageDir) { persistedSessionId = createSession(this.config.saivageDir, 'planner', goalId, goalId).id; this.openAssistantRound(persistedSessionId); } if (this.cancelledSessions.has(sessionId)) throw new Error(`Fake planner session cancelled: ${sessionId}`); const result = fixture.planner[count]; this.plannerCounters.set(fixture.name, count + 1); converted = convertPlannerResult(result); if (this.config.autoActivateCreatedCards !== false && this.config.saivageDir && persistedSessionId && converted.created_cards.length > 0) { const toolCalls = converted.created_cards.map((card) => ({ id: `activate:${goalId}:${card.id}:${count}`, type: 'function', function: { name: 'activate_card', arguments: JSON.stringify({ cardId: card.id }) } })); for (const toolCall of toolCalls) appendMessage(this.config.saivageDir, persistedSessionId, { role: 'assistant', kind: 'tool_call', content: JSON.stringify({ toolCalls: [toolCall] }), tool: 'activate_card', tool_call_id: toolCall.id }, this.stampInRound(persistedSessionId)); this.createFixtureActivationRecords(goalId, persistedSessionId, toolCalls); } return converted; } finally { if (this.config.saivageDir && persistedSessionId) { this.closeAssistantRound(persistedSessionId); if (converted?.status === 'continue') markSessionWaiting(this.config.saivageDir, persistedSessionId); else if (converted?.status === 'blocked') completeSession(this.config.saivageDir, persistedSessionId, 'blocked'); else completeSession(this.config.saivageDir, persistedSessionId, 'done'); } this.completeSession(sessionId); } }
