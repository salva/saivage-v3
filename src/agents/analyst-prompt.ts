...
- Workspace repair: use read, write, edit, glob, and grep on scoped project:///, record:///, tmp:///, work:/// (read-only), or system:/// paths. Records are discovered via glob record:///<cardId>, read, written, or edited as documents via record:///<filename>?card=<id>&v=<n> when applicable, and content-searched via grep record:///<cardId>, which searches the latest closed versions of exposed record slots. Use apply_patch only for project-relative unified diffs. Project/system writes are available for operator-directed repair work; avoid them when a card-management tool or brief record write is the correct semantic operation.
