...
import { invokeToolForLlm, providerResultFromSettlement, surfaceCompiledInvocationTools, syntheticToolSettlement, type InvocationSurface, type ToolResult, type ToolSettlementInput,
} from '../tools/invocation.js';
import { UNSUPPORTED_TOOL_RESULT_POLICY_TEMPLATE } from '../runtime/actors/llm-invocation.js';
...
export class AnalystSession {
  ...
  private async runAnalystTurn(
    operation: AnalystTurnOperation,
    signal: AbortSignal,
  ): Promise<AnalystResponse> {
    this.assertCurrent(operation, signal);
    if (operation.step.kind === 'confirmed_restart_preparing')
      return this.runConfirmedRestart(operation);
    ...
    for (;;) {
      ...
      if (!surface.tools.has(outcome.toolName)) {
        ...
        settlement = syntheticToolSettlement('unsupported_tool', UNSUPPORTED_TOOL_RESULT_POLICY_TEMPLATE, `Unsupported tool '${outcome.toolName}' for agent '${surface.agentName}'. ${ANALYST_UNSUPPORTED_ACTION_TEMPLATE('Analyst', Array.from(surface.tools.keys()))}`);
        ...
        const definition = surface.tools.get(outcome.toolName);
        settlement = syntheticToolSettlement(definition ? 'rejected_before_execution' : 'unsupported_tool', definition?.resultPolicyTemplate ?? UNSUPPORTED_TOOL_RESULT_POLICY_TEMPLATE, JSON.stringify(violation));
      } else {
        params = parsed.args;
      ...
      const result = providerResultFromSettlement(settlement);
  ...
  private terminalHandoff(operation: AnalystTurnOperation): LlmTerminalHandoff {
    return (completion) => {
      ...
      if (completion.outcome.type === 'blocked')
        throw new Error('Analyst-purpose LLM actor handed off an autonomous-card blocked outcome.');
      operation.step = {
        kind: 'settling_llm',
        completion: { input: completion.input, outcome: completion.outcome },
        noticeEntered: false,
      };
    };
  }

  private settleTerminalCompletion(
    operation: AnalystTurnOperation,
    outcome: Extract<LLMActorOutcome, { type: 'result' | 'error' }>,
  ): AnalystResponse {
    if (operation.step.kind !== 'settling_llm' || operation.step.completion.outcome !== outcome)
      throw new Error('Analyst terminal promise disagrees with synchronous handoff.');
    if (outcome.type === 'error') {
      operation.step.noticeEntered = true;
      appendConversationBatch(this.#conversations, [
        buildLlmTurnMessage(operation.step.completion.input, this.errorMessage(outcome.error)),
      ]);
    }
    return this.response(operation);
  }
  ...
  private response(
    operation: AnalystTurnOperation,
    restart: RestartChatAcknowledgement | null = null,
  ): AnalystResponse {
    const acknowledgement =
      restart ??
      (operation.restartConfirmation || operation.newlyRequestedRestart
        ? { status: 'confirmation_required' as const, confirmationMessage: 'RESTART SERVER' }
        : null);
    return {
      sessionId: this.#sessionId,
  ...
  private async consumeTurn(
    operation: AnalystTurnOperation,
    wrapper: Promise<AnalystResponse>,
  ): Promise<void> {
    let response: AnalystResponse | null = null;
    let failure: unknown;
    let rejected = false;
    try {
  ...
  private pruneRetiredTrackers(): void {
    for (const tracker of this.#retiredOperationTrackers)
      void tracker.join().then(
        (outcome) => {
          if (outcome.status === 'joined') this.#retiredOperationTrackers.delete(tracker);
        },
        (error) => {
          this.#deliverPublicationFatal(error);
        },
  ...
  disposeSession(reason: unknown): void {
    ...
    if (this.#phase.kind === 'failed' || this.#phase.kind === 'idle') {
      this.#phase = { kind: 'disposed', reason, settling: null };
    ...
    if (disposition === 'revoked_before_owned_completion') {
      operation.tracker.revoke(reason);
  ...
  async joinSession(): Promise<readonly InvocationJoinOutcome[]> {
    ...
    if (this.#phase.kind === 'disposed' && this.#phase.settling)
      trackers.add(this.#phase.settling.tracker);
    const joins = [...trackers].map((tracker) => tracker.join());
    const llmJoin = this.#llm.join();
    const settled = await Promise.allSettled([...joins, llmJoin]);
    const failure = settled.find(
      (entry): entry is PromiseRejectedResult => entry.status === 'rejected',
    );
    if (failure) {
      this.#deliverPublicationFatal(failure.reason);
...
export class AnalystRuntime {
  ...
  async cleanupForApplicationStop(): Promise<void> {
    ...
    try {
      directContainment = session ? session.shutdownProcesses() : Promise.resolve();
    } catch (error) {
    ...
    try {
      ...
      rootContainment = Promise.reject(error);
    }
    let sessionJoin: Promise<readonly InvocationJoinOutcome[]>;
    try {
      sessionJoin = session ? session.joinSession() : Promise.resolve([]);
    } catch (error) {
      sessionJoin = Promise.reject(error);
    }
    const [directSettlement, rootSettlement, joinSettlement] = await Promise.allSettled([
      directContainment,
      rootContainment,
      sessionJoin,
    ]);
    ...
    if (
      rootSettlement.value.failed.length !== 0 ||
      directSettlement.status === 'rejected' ||
      joinSettlement.status === 'rejected'
    )
      throw new Error('Analyst application cleanup failed.');
  }
