...
export function buildPlannerStateContextText(input: PlannerStateContextInput): string {
  ...
  const activeRun = runtimeState?.active_card_run;
  const openRuns = activeRun && (activeRun.card_id === input.goalId || childIds.has(activeRun.card_id))
    ? [{ card_id: activeRun.card_id, status: activeRun.runtime_status, phase: activeRun.phase, activated_at: activeRun.started_at }]
    : [];
  const state = {
