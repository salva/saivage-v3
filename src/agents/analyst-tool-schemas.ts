...
export const ANALYST_TOOL_DEFINITIONS: ToolDefinition[] = [
  ...
  tool('lets_dance','Record a lets_dance directive on the project card and request a best-effort runtime wakeup when available. Inspect returned outcome, project_status, runtime_paused, wakeup_requested, and expected_next_step. Pre-conditions: the project card status must be `active` (not `backlog`) and the runtime must not be paused, frozen, or in error; otherwise the directive is queued but no work begins. If the project is in `backlog`, call edit_card to set status=active before calling lets_dance.',{},[]),
