...
export const ANALYST_TOOL_DEFINITIONS: ToolDefinition[] = [
  ...
  tool('reconfigure','Reconfigure role routing, failover, MCP servers, runtime, or server settings.',{ action: { type: 'string', enum: ['set_role_routing','set_failover_chain','mcp_add','mcp_edit','mcp_remove','set_runtime_setting','set_server_setting'] }, role: str('Role name.'), model_candidate: str('Model candidate.'), for_model: str('Primary model name whose failover chain is being set (per-model, not per-role).'), ordered_failover_models: arr(str('Failover model name'),'Ordered failover models for the primary model.'), name: str('MCP server name.'), command: str('MCP command.'), args: arr(str('Argument'),'MCP args.'), env: { type: 'object', additionalProperties: { type: 'string' } }, key: str('Setting key.'), value: { description: 'Setting value.' } },['action']),
