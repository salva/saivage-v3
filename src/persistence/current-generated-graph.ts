...
export function initializeAndValidateCurrentGeneratedState(projectRoot: string, workflows: CompiledProjectWorkflows): void {
  ...
  const globalSessionId = globalAgentSessionId(workflows.analyst.name);
  initializeMissingConversation(projectRoot, globalSessionId);
  truncateCurrentConversationUnterminatedSuffix(projectRoot, globalSessionId);
