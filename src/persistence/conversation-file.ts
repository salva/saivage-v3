...
export function readConversation(
  ...
  try {
    return validateConversation(sessionId, rows);
  } catch (error) {
    throw new Error(
...
export function readConversationSummary(
  ...
  try {
    validateConversation(sessionId, first.rows);
  } catch (error) {
    throw new Error(
      `Conversation '${sessionId}' is invalid: ${error instanceof Error ? error.message : String(error)}`,
    );
...
export function foldConversation(
  ...
  const validation = createCanonicalConversationValidationState(sessionId);
  ...
  const result = foldCanonicalGrowingFileRows({
    ...
    reduce(state, row, checkpoint, replay) {
      reduceCanonicalConversationRow(state, row, checkpoint, replay);
      cursor = row.id;
...
export function appendConversationBatch(
  ...
  try {
    ...
    current = validateConversation(sessionId, []);
  }
  ...
  validateConversation(sessionId, [...current.physicalRows, ...parsed]);
