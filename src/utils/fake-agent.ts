...
import { appendMessage, completeSession, createSession } from '../agents/session-persistence.js';
...
export interface FakeAgentConfig { mapping: Record<string, string>; fixtureDir: string; saivageDir?: string; autoActivateCreatedCards?: boolean; }
...
export class FakeAgentAdapter implements AgentRuntime {
  ...
  invokePlanner(goalId: string, _systemPrompt?: string, _contextMessages?: AgentMessage[]): PlannerResult { const fixture = this.resolveFixture(goalId); if (!fixture.planner || fixture.planner.length === 0) throw new Error(`FakeAgent fixture '${fixture.name}' has no planner results.`); const count = this.plannerCounters.get(fixture.name) ?? 0; if (count >= fixture.planner.length) throw new Error(`FakeAgent fixture '${fixture.name}' exhausted planner results (called ${count + 1} times, only ${fixture.planner.length} available).`); const sessionId = this.registerSession('planner', goalId, goalId, `Planning goal ${goalId}`, fixture.name); let persistedSessionId: string | null = null; try { if (this.config.saivageDir) persistedSessionId = createSession(this.config.saivageDir, 'planner', goalId, goalId).id; if (this.cancelledSessions.has(sessionId)) throw new Error(`Fake planner session cancelled: ${sessionId}`); const result = fixture.planner[count]; this.plannerCounters.set(fixture.name, count + 1); const converted = convertPlannerResult(result); if (this.config.autoActivateCreatedCards !== false && this.config.saivageDir && persistedSessionId && converted.created_cards.length > 0) { const toolCalls = converted.created_cards.map((card) => ({ id: `activate:${goalId}:${card.id}:${count}`, type: 'function', function: { name: 'activate_card', arguments: JSON.stringify({ cardId: card.id }) } })); appendMessage(this.config.saivageDir, persistedSessionId, { role: 'assistant', kind: 'tool_call', content: JSON.stringify({ toolCalls }), tool: 'activate_card' }); this.createFixtureActivationRecords(goalId, persistedSessionId, toolCalls); } return converted; } finally { if (this.config.saivageDir && persistedSessionId) completeSession(this.config.saivageDir, persistedSessionId, 'done'); this.completeSession(sessionId); } }
  ...
  private createFixtureActivationRecords(goalId: string, plannerSessionId: string, toolCalls: Array<{ id: string; function: { arguments: string } }>): void {
    ...
    const projectRoot = dirname(this.config.saivageDir);
  ...
  invokeExecutor(cardId: string, goalId: string, _systemPrompt?: string, _contextMessages?: AgentMessage[]): ExecutorResult { const fixture = this.resolveFixture(goalId); if (!fixture.executor) throw new Error(`FakeAgent fixture '${fixture.name}' has no executor results.`); const sessionId = this.registerSession('executor', goalId, cardId, `Executing card ${cardId}`, fixture.name); try { if (this.cancelledSessions.has(sessionId)) throw new Error(`Fake executor session cancelled: ${sessionId}`); const result = fixture.executor[cardId]; if (!result) throw new Error(`FakeAgent fixture '${fixture.name}' has no executor result for card '${cardId}'.`); return convertExecutorResult(result); } finally { this.completeSession(sessionId); } }
  invokeReviewer(goalId: string, _systemPrompt?: string, _contextMessages?: AgentMessage[], options: { assessmentId?: string; reviewerSessionId?: string } = {}): ReviewerResult { const fixture = this.resolveFixture(goalId); if (!fixture.reviewer || fixture.reviewer.length === 0) throw new Error(`FakeAgent fixture '${fixture.name}' has no reviewer results.`); const count = this.reviewerCounters.get(fixture.name) ?? 0; if (count >= fixture.reviewer.length) throw new Error(`FakeAgent fixture '${fixture.name}' exhausted reviewer results (called ${count + 1} times, only ${fixture.reviewer.length} available).`); const sessionId = this.registerSession('reviewer', goalId, null, `Reviewing goal ${goalId}`, fixture.name, options.reviewerSessionId); try { if (this.cancelledSessions.has(sessionId)) throw new Error(`Fake reviewer session cancelled: ${sessionId}`); const result = fixture.reviewer[count]; this.reviewerCounters.set(fixture.name, count + 1); return convertReviewerResult(result); } finally { this.completeSession(sessionId); } }
  reinvokeSession(sessionId: string): ExecutorResult | ReviewerResult { const session = this.sessionFixtures.get(sessionId); if (!session) throw new Error(`Unknown fake session '${sessionId}'.`); if (session.role === 'executor') return this.invokeExecutor(session.cardId ?? '', session.goalId); if (session.role === 'reviewer') return this.invokeReviewer(session.goalId, undefined, undefined, { reviewerSessionId: sessionId }); throw new Error(`Session '${sessionId}' is not reinvokable.`); }
