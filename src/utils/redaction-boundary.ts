...
function redactObjectValue(value: unknown, context: RedactionContext, keyHint: string | undefined, depth: number, seen: WeakSet<object>): unknown {
  ...
  if (Array.isArray(value)) {
    const output = value.slice(0, maxEntries).map((entry) => redactObjectValue(entry, context, undefined, depth + 1, seen));
...
export const RedactionBoundary = {
  ...
  object<T>(value: T, context: RedactionContext): T {
    return redactObjectValue(value, context, undefined, 0, new WeakSet<object>()) as T;
  },
