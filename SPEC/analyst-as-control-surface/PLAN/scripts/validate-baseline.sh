...
jq -e --slurpfile b "$PLAN_DIR/baseline-gates.json" '([.gates[].id] == ["tsc-build","web-vite-build","web-vitest","analyst-e2e"]) and ([.gates[].id] == [$b[0].gates[].id]) and all(.gates[]; (.failing_ids|type)=="array" and all(.failing_ids[]; type=="string"))' "$OUT" >/dev/null
