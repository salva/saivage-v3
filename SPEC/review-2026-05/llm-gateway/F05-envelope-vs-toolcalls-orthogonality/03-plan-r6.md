# F05 — Envelope-vs-toolcalls orthogonality (implementation plan, r6)
...
- `cd web && npx vitest run src/__tests__/tool-presenters.test.ts src/__tests__/analystChat.test.ts src/__tests__/llm-exchange-viewer-terminal-tool.test.ts src/__tests__/event-log-terminal-tool.test.ts` — GREEN (Vitest, path patterns relative to `web/`).
