# F05 — Envelope-vs-toolcalls orthogonality (implementation plan, r7)
...
**Test-runner conventions used throughout this plan.** The root project runs Jest; the `web/` sub-project runs Vitest (see [web/package.json#L15](../../../web/package.json#L15)). Therefore:

- Root **typecheck**: `npx tsc --noEmit`.
- Root **full suite**: `npm test` (Jest is configured via `npm test`; no extra flags are required to run the whole suite).
- Root **specific test files** (by exact path): `npm test -- --runTestsByPath path/to/test1.ts path/to/test2.ts`. Jest's `--runTestsByPath` accepts one or more exact file paths and is the correct way to scope Jest to specific files; the Vitest-style `--run <file>` invocation is wrong for this project and is not used anywhere in this plan.
- Root **path-pattern** (run any test file whose path matches a substring): `npm test -- <substring1> <substring2>`. Jest interprets bare positional arguments as `testPathPatterns`.
- Web **typecheck**: `cd web && npx tsc --noEmit` (or whatever the web `tsc` entry is — Vue SFC builds use Vue's own tooling but type-checking the TS-only test/source surface is via `tsc`).
- Web **full suite**: `cd web && npm test` (which is `vitest run` per [web/package.json#L15](../../../web/package.json#L15)).
- Web **specific test files / patterns**: `cd web && npx vitest run <pattern1> <pattern2>` — Vitest treats bare args as path patterns relative to the `web/` root. When this plan lists a web test at the workspace-relative path `web/src/__tests__/foo.test.ts`, the corresponding Vitest invocation uses the `web/`-relative path `src/__tests__/foo.test.ts`.

**Build-output layout.** The root `tsconfig.json` emits compiled JS for `src/**/*.ts` and `tests/**/*.ts` only, mirroring the source tree under `dist/`. Files placed at `src/scripts/foo.ts` therefore compile to `dist/src/scripts/foo.js`. Files placed at the top-level `scripts/` directory are NOT emitted by `tsc` and cannot be invoked from `dist/`. This plan places every TypeScript build artifact under `src/` so it ends up in `dist/`; shell scripts (which are not compiled) stay under `scripts/`.
...
npx tsc --noEmit                             # baseline typecheck green (root)
npm test                                     # baseline Jest suite green (root)
(cd web && npx tsc --noEmit)                 # baseline web typecheck green
(cd web && npm test)                         # baseline Vitest suite green (web)
...
- `npm test -- --runTestsByPath tests/agents/zod-to-jsonschema-mini.test.ts tests/agents/persisted-tool-call.test.ts tests/agents/terminal-protocol.test.ts tests/agents/parse-role-envelope-arguments.test.ts` — GREEN (Jest, exact file paths).
- `npm test -- --runTestsByPath tests/agents/result-parser.test.ts` — GREEN (existing parser behaviour unchanged because the schemas are identical Zod objects in a new module).
- `npm test` — full root suite GREEN.
- `npm run build` — GREEN.
...
The two `*__never_sends_response_format` assertions above ARE legitimate negative properties of the new wire contract and remain part of the suite indefinitely. The B6 sweep is scoped to exclude them by name (see §B6, sweep command #2).
...
- `npm run build` — GREEN.
- `npm test` — full root Jest suite GREEN. Specifically (each runnable as a focused Jest invocation via `npm test -- --runTestsByPath <paths>`): `tests/agents/llm-options-factory.test.ts`, `tests/agents/llm-openai-chat-gateway-request.test.ts`, `tests/agents/llm-openai-chat-gateway-response.test.ts`, `tests/agents/llm-openai-codex-gateway-request.test.ts`, `tests/agents/llm-openai-codex-gateway-response.test.ts`, `tests/agents/llm-stream-parser.test.ts`, `tests/agents/llm-codex-parser.test.ts`, `tests/agents/agent-llm-gateway.test.ts`, `tests/agents/agent-adapter-invoke-happy-path.test.ts`, `tests/agents/agent-adapter-invoke-direct-terminal.test.ts`, `tests/agents/agent-adapter-invoke-escalation.test.ts`, `tests/agents/analyst-handler-result-union.test.ts`, `tests/agents/invocation-recovery-policy-contract-mismatch.test.ts`, `tests/contracts/llm-exchange-terminal-tool.test.ts`, `tests/schemas/event-catalog-terminal-tool.test.ts`, `tests/schemas/validators-terminal-tool.test.ts`, `tests/schemas/types-terminal-tool.test.ts`, `tests/agents/session-persistence-legacy-wrapper.test.ts`, `tests/runtime/runtime-legacy-wrapper.test.ts`, plus every rewritten fixture file listed above.
- `cd web && npx tsc --noEmit` — GREEN.
- `cd web && npm test` — full web Vitest suite GREEN (covers the rewritten `web/src/__tests__/*` fixtures).
...
- `npx tsc --noEmit` — GREEN.
- `npm test -- --runTestsByPath tests/agents/integration.test.ts` — GREEN.
- `npm test` — full root Jest suite GREEN.
...
- `npx tsc --noEmit` — GREEN.
- `npm test -- --runTestsByPath tests/agents/provider-capabilities-axis.test.ts tests/agents/agent-adapter-recovery.test.ts tests/agents/model-router.test.ts tests/agents/provider.test.ts tests/agents/config-schema.test.ts tests/agents/llm-client-integration.test.ts` — GREEN.
- `npm test` — full root Jest suite GREEN.
- Grep: `grep -rn 'envelopeMode\|responseShape' src/ tests/` → no hits. (`response_format` is allowed to survive ONLY in the B2 negative-property request tests named `*__never_sends_response_format`; the B6 sweep excludes those by name.)
...
Goal: migrate `web/` to the one-row-per-tool-call format and render the `terminal_tool` badge. All web-test invocations in this batch use Vitest because `web/` runs Vitest (see [web/package.json#L15](../../../web/package.json#L15)).

**Files modified (existing web tests rewritten in this batch)**
...
- [web/src/__tests__/tool-presenters/registry.test.ts](../../../web/src/__tests__/tool-presenters/registry.test.ts) — rewrite the persisted-row fixtures to single-`ToolCall` row strings (matches the helper rename and the new one-row-per-call format); add a case asserting `readToolCallMessage` raises on a legacy wrapper input.
- [web/src/__tests__/analyst-chat-store.test.ts](../../../web/src/__tests__/analyst-chat-store.test.ts) — rewrite the `toolInvocationMatchesMessage` fixtures to single-`ToolCall` rows.

**Files created (new web tests added in this batch)**

- [web/src/__tests__/tool-presenters/terminal-tool-viewer.test.ts](../../../web/src/__tests__/tool-presenters/terminal-tool-viewer.test.ts) — assert the LLM-exchange viewer badge renders per role (`emit_planner_result`, `emit_executor_result`, `emit_reviewer_result`); hidden when `attempt.terminalTool === null`.
- [web/src/__tests__/event-log-terminal-tool.test.ts](../../../web/src/__tests__/event-log-terminal-tool.test.ts) — assert the event-log row renders the `terminal_tool` label per role; absent on events that do not carry the field.
...
- `cd web && npx vitest run src/__tests__/analyst-chat-store.test.ts src/__tests__/tool-presenters/registry.test.ts src/__tests__/tool-presenters/terminal-tool-viewer.test.ts src/__tests__/event-log-terminal-tool.test.ts` — GREEN (Vitest, path patterns relative to `web/`; lists ONLY the existing web tests touched in this batch plus the two new tests created in this batch).
- `cd web && npm test` — full web Vitest suite GREEN.
- `npm test` — full root Jest suite GREEN (root tests should not regress from a web-only change, but rerun as a guardrail).
...
**Build-output reminder.** The root `tsconfig.json` emits compiled JS only for `src/**/*.ts` and `tests/**/*.ts`. The probe script is a TypeScript file that needs a compiled artifact deployed inside the container, so it MUST live under `src/` to be emitted by `tsc`. A file placed at the top-level `scripts/` directory would NOT appear in `dist/` and could not be invoked from the container. The probe is therefore placed at [src/scripts/probe-llm-contract.ts](../../../src/scripts/probe-llm-contract.ts), which `npm run build` compiles to `dist/src/scripts/probe-llm-contract.js`. The shell sweep script (which is not compiled) stays under `scripts/`.

**Sweep-scope rule (read first).** The B2 negative-property tests `tests/agents/llm-openai-chat-gateway-request.test.ts::chat_request__never_sends_response_format` and `tests/agents/llm-openai-codex-gateway-request.test.ts::codex_request__never_sends_response_format` are legitimate, permanent assertions that the wire request body never contains `response_format`. They mention the string `response_format` and the identifier `never_sends_response_format` by design and are NOT in scope for the sweep. Sweep checks #1 and #2 below are therefore PRODUCTION-SOURCE-ONLY (`src/`, with `--include='*.ts'`) and additionally exclude the sweep script itself plus deprecated-marker comments. The fixture sweep (#3) and the wire-shape grep (#4) remain as before.

**Files created**

- [src/scripts/probe-llm-contract.ts](../../../src/scripts/probe-llm-contract.ts) — per provider configured in `.saivage/saivage.json`, issue one role invocation that (a) executes at least one action tool call and (b) terminates via `emit_<role>_result`. Assert the envelope parses against the role's Zod schema. Reads ONLY `.saivage/saivage.json` (never `.saivage/auth-profiles.json`, never any `apiKey` / `token` field). Output: machine-readable JSON, one line per `{provider, role, status}`. Compiled artifact after `npm run build`: `dist/src/scripts/probe-llm-contract.js`.
...
- [scripts/check-no-legacy-toolcalls-wrapper.sh](../../../scripts/check-no-legacy-toolcalls-wrapper.sh) — CI-callable script that runs the sweep commands below and exits non-zero on any hit. The script is self-excluded from its own grep (the grep paths target `src/` only) and ignores lines containing the markers `never_sends` or `deprecated_marker`.

**Sweep commands (must all return empty; the shell script exits non-zero on any hit)**

```bash
# 1. Combined production-source sweep: no production .ts file under src/ either
#    reads the legacy {toolCalls:[...]} wrapper or mentions the deleted
#    response_format option. Restricted to src/ (production source only),
#    *.ts only, and excludes the negative-property markers and deprecated
#    comment markers.
! grep -rn "response_format\|\.toolCalls" src/ --include='*.ts' \
    | grep -v 'never_sends\|deprecated_marker'
# Expected: empty. NOTE: this sweep does NOT scan tests/ — the B2 request tests
# `chat_request__never_sends_response_format` and `codex_request__never_sends_response_format`
# are legitimate negative-property assertions and remain.

# 2. No other deleted-surface name survives in production source.
! grep -rn "extractJson\|parsePersistedToolCalls\|parseToolCallsFromResponse\|envelopeMode\|responseShape\|forceFinalAnswer\|handleToolCallsLoop\|ResultParseError\|buildExecutorFallbackResult\|parsePlannerResult\|parseExecutorResult\|parseReviewerResult\|LlmEnvelopeOptions" src/ --include='*.ts' \
    | grep -v 'deprecated_marker'
# Expected: empty.

# 3. No persisted wrapper literal survives in test fixtures.
! grep -rn "toolCalls:" tests/ web/src/__tests__/ web/src/utils/ \
    | grep -v "capabilities:"
# Expected: empty.
...
# Expected: empty.
! grep -n "parallel_tool_calls" src/agents/llm-openai-chat-gateway.ts src/agents/llm-openai-codex-gateway.ts \
    | grep -v "false"
# Expected: empty.
```

The shell script `scripts/check-no-legacy-toolcalls-wrapper.sh` wraps the four `!`-prefixed pipelines above; any single non-empty pipeline causes the script to `exit 1`. It is paths-only (no recursion into itself because the grep targets are explicit directories: `src/`, `tests/`, `web/src/__tests__/`, `web/src/utils/`, and the two gateway files).
...
#    The compiled artifact lives at dist/src/scripts/probe-llm-contract.js because the
#    tsconfig.json rootDir mirrors the src/ tree under dist/.
ssh salva@10.0.3.112 'cd /work/saivage-v3 && node /opt/saivage-v3/dist/src/scripts/probe-llm-contract.js'
...
- `npx tsc --noEmit` — GREEN.
- `npm run build` — GREEN. Post-build check: `ls dist/src/scripts/probe-llm-contract.js` MUST succeed (confirms the probe was emitted under `dist/src/scripts/` as expected).
- `npm test` — full root Jest suite GREEN. In particular, the B2 negative-property tests `chat_request__never_sends_response_format` and `codex_request__never_sends_response_format` continue to pass and continue to be part of the suite; they are not removed.
- `cd web && npm test` — full web Vitest suite GREEN.
...
```
F05: add legacy-wrapper sweep script (production-source-scoped) and probe-llm-contract live playbook
```
...
The B6 sweep (`grep -rn "response_format\|\.toolCalls" src/ --include='*.ts' | grep -v 'never_sends\|deprecated_marker'`) is the post-condition that no `.toolCalls` wrapper reader survives in production source.
...
| B6 | Live probe returns `contract_mismatch:<subtype>` for a real provider. | NOT a rollback — the runtime change is correct; the probe just discovered a wire bug. File a follow-up F-issue. If the probe script itself crashes or the `dist/src/scripts/probe-llm-contract.js` artifact is missing after build, revert only the script-add commit. |
...
- Live probe on `saivage-v3` returns `status: "ok"` for every `(provider, role)` row in `.saivage/saivage.json`. The compiled probe lives at `dist/src/scripts/probe-llm-contract.js` (mirroring the `src/scripts/` source path under `dist/`).
