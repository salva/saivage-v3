# F05 — Envelope-vs-toolcalls orthogonality (implementation plan, r2)
...
Six batches, B1–B6, each one commit and one green checkpoint. The previous twelve-batch plan is collapsed to honour the rule that every batch ends green: B2 is intentionally large because the contract flip cannot be split without temporary shims (forbidden by the project guidelines).

### Batch B1 — Substrate-only additions (purely additive, fully green)

Goal: add the new typed surfaces, helpers, terminal-protocol validator, role-envelope schemas, and persisted-row helpers without touching any existing call site. The existing `LlmCompleteOptions` / `LlmCompleteResult` / `LlmCallFn` shapes are NOT changed in this batch, so every existing consumer keeps compiling and existing tests keep passing.

**Files created**

- [src/agents/role-envelope-schemas.ts](../../../src/agents/role-envelope-schemas.ts) — re-exports `PlannerResultSchema`, `ExecutorResultSchema`, `ReviewerResultSchema` from their current homes in [src/agents/result-parser.ts](../../../src/agents/result-parser.ts) (re-exports only; B3 moves the definitions in-place when result-parser.ts is deleted). Exports `type EnvelopeBearingRole = 'planner' | 'executor' | 'reviewer'` and `ENVELOPE_SCHEMAS: Record<EnvelopeBearingRole, ZodSchema>`.
...
- [tests/agents/terminal-protocol.test.ts](../../../tests/agents/terminal-protocol.test.ts) — one test per name-based rejection subtype (`terminal_prose_only` for both `terminal_text` and empty-tools variants, `terminal_duplicate`, `terminal_mixed_with_actions`, `terminal_wrong_role`, `terminal_missing_on_forced_turn`).
- [tests/agents/parse-role-envelope-arguments.test.ts](../../../tests/agents/parse-role-envelope-arguments.test.ts) — `terminal_arguments_not_json` and `terminal_arguments_schema_mismatch` for all three envelope-bearing roles, using the re-exported schemas.
...
- `npx tsc --noEmit` — GREEN (no existing surface changed).
- `npm test -- --run tests/agents/persisted-tool-call.test.ts tests/agents/terminal-protocol.test.ts tests/agents/parse-role-envelope-arguments.test.ts` — GREEN.
...
```
F05: add typed substrate (role schemas, terminal-protocol validator, persisted-row helpers, contract errors)
```
...
- [src/agents/llm-options-factory.ts](../../../src/agents/llm-options-factory.ts) — `buildLlmOptions(role, phase: 'tools' | 'terminal', tools, modelParams, signal, recorder): LlmCompleteOptions`. Translates `phase` into `TerminalChoice`: `'tools'` → `{ kind: 'auto' }`; `'terminal'` → `{ kind: 'required_named', toolName: ROLE_RESULT_TOOL_NAMES[role] }`. Throws `Error('analyst has no terminal tool')` when `phase === 'terminal'` and `ROLE_RESULT_TOOL_NAMES[role] === null`. Always sets `mode: 'tools'` and `parallel_tool_calls: false`.
- [tests/agents/llm-options-factory.test.ts](../../../tests/agents/llm-options-factory.test.ts) — covers both phases per role, the analyst-terminal rejection, and the `parallel_tool_calls: false` invariant.
- [tests/agents/llm-openai-chat-gateway-request.test.ts](../../../tests/agents/llm-openai-chat-gateway-request.test.ts) — chat-only request-shape: `response_format` absent; `parallel_tool_calls: false`; auto-phase `tool_choice: 'auto'`; terminal-phase `tool_choice: { type: 'function', function: { name: <role tool> } }` (nested form).
- [tests/agents/llm-openai-codex-gateway-request.test.ts](../../../tests/agents/llm-openai-codex-gateway-request.test.ts) — Codex-only request-shape: `tool_choice: { type: 'function', name: <role tool> }` (FLAT form, no nested `function` object); `parallel_tool_calls: false`; tool entries use flat `name` field.
...
- [tests/agents/agent-adapter-invoke-escalation.test.ts](../../../tests/agents/agent-adapter-invoke-escalation.test.ts) — escalation to `mustBeTerminal === true` on the final turn; assert `validateTerminalProtocol` runs BEFORE any `executeToolCall` on that turn (use a side-effect-recording fake tool executor).
...
Goal: result-parser.ts and its tests are dead after B2 (no consumer left in `src/`). Delete them, move the Zod schemas in-place into `role-envelope-schemas.ts`, and rewrite the integration test that still references the deleted parser surface.

**Files modified**

- [src/agents/role-envelope-schemas.ts](../../../src/agents/role-envelope-schemas.ts) — MOVE the `PlannerResultSchema`, `ExecutorResultSchema`, `ReviewerResultSchema` Zod definitions in-place (replacing the re-exports added in B1). Continue to export `ENVELOPE_SCHEMAS` and `type EnvelopeBearingRole`.
...
- [src/agents/result-parser.ts](../../../src/agents/result-parser.ts) — DELETE outright (the schemas have moved; `extractJson`, `parsePlannerResult`, `parseExecutorResult`, `parseReviewerResult`, `buildExecutorFallbackResult`, `ResultParseError` are gone). If any unrelated helper remains, move it into a topical neighbour file and then delete; do not keep the file alive as a near-empty module.
...
```
F05: delete result-parser.ts family and migrate role schemas into role-envelope-schemas.ts
```
...
The two production-source readers of the legacy persisted `{ toolCalls: [...] }` wrapper are migrated in **Batch B2** (the contract-flip commit). They are repeated here so an implementer cannot finish the plan with either surviving:

- [src/agents/session-persistence.ts](../../../src/agents/session-persistence.ts) — `parseToolCalls(content)` at L379 (consumed by `findUniqueUnresolvedActivateCardToolCall` at L380–L384 iteration and L415 matches loop). Migration in **B2**: delete the local helper; replace each row read with one `parseToolCallMessage(message.content)` call (one persisted `ToolCall` per row); raise `LegacyMessageShapeError` on a wrapper row (no silent ignore, no fallback). New test: [tests/agents/session-persistence-legacy-wrapper.test.ts](../../../tests/agents/session-persistence-legacy-wrapper.test.ts).
- [src/runtime/runtime.ts](../../../src/runtime/runtime.ts) — `findUnresolvedActivateCards` at L234–L264 with two inline `JSON.parse(message.content) as { toolCalls?: ... }` loops at L239–L242 and L254–L255. Migration in **B2**: delete both inline parses; each row yields exactly one `ToolCall` via `parseToolCallMessage`; legacy wrapper raises `LegacyMessageShapeError`. New test: [tests/runtime/runtime-legacy-wrapper.test.ts](../../../tests/runtime/runtime-legacy-wrapper.test.ts).
...
| --- | --- | --- |
| B1 | New module unit test red, or import of `LlmContractMismatchError` fails downstream. | `git revert HEAD`. New modules are additive; redo the batch after fixing the local module. |
...
| B3 | Residual caller of a deleted parser surface or schema relocation regression. | `git revert HEAD`. Re-run the B3 grep; address each survivor in the same commit before reapplying. |
...
| --- | --- |
| 1. Single-carrier per turn | `tests/agents/llm-options-factory.test.ts > buildLlmOptions__rejects_phase_terminal_for_analyst`; structural — `LlmCompleteOptions` is `LlmToolsOptions` only (no `LlmEnvelopeOptions` export, enforced by B6 grep). |
...
| 4. No per-turn option drift | `tests/agents/agent-adapter-invoke-happy-path.test.ts > uses_buildLlmOptions_for_every_turn`; structural — `agent-adapter.ts` contains zero inline `LlmCompleteOptions` literals. |
