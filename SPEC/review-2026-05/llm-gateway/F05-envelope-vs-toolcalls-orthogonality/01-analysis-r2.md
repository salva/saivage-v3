...
Scope of this document: a self-contained functional analysis of how Saivage v3's LLM gateway treats the JSON result envelope and the provider tool-call channel as independent carriers, why that assumption diverges from real provider semantics, the failure modes it spawns, and the conceptual design alternatives a correct contract must choose between. A reader with no prior context on this issue should be able to implement from this file alone. All file references are workspace-relative to `/home/salva/g/ml/saivage-v3/` and clickable as markdown links; line numbers are 1-based and validated against current HEAD.

This file does NOT prescribe a single implementation. It frames the problem, inventories every consumer of the affected contract, surveys the coherent design alternatives, and lists the invariants the chosen design must satisfy.
...
Concretely, for this issue: do not introduce a feature flag, a "legacy envelope mode" toggle, or any branch that keeps the current orthogonal-channels assumption alive in parallel. The contract chosen below replaces the current one outright; on-disk session message rows, persisted tool-call shapes, result-parser fallbacks, and the analyst-side `{ content, toolCalls }` consumer shape that exist purely to bridge the two carriers should all be removed in the same pass.

## 1. Problem statement

For three operational roles — `planner`, `executor`, `reviewer` — every LLM turn issued by [src/agents/agent-adapter.ts#L341](src/agents/agent-adapter.ts#L341) is constructed as if the model has TWO independent ways to answer:
...
(See [src/agents/agent-adapter.ts#L342](src/agents/agent-adapter.ts#L342) for the spread assembly.)

The `tools` array is non-empty for all three roles by construction — see [src/agents/agent-tool-catalog.ts#L77](src/agents/agent-tool-catalog.ts#L77) (`ROLE_TOOL_NAMES`) and [src/agents/agent-tool-executor.ts#L45](src/agents/agent-tool-executor.ts#L45) (`buildToolsForRole`). So the request always says: "Here are tools you may call AND give me a JSON object as your final answer."

The result type assumes the same orthogonality — [src/agents/llm-contracts.ts#L25](src/agents/llm-contracts.ts#L25):
...
The flatten rule is "content wins by `??`". If `content` is `null`/`undefined`, the gateway synthesises a JSON blob `{"toolCalls":[…]}` and pretends it was the model's text answer. The adapter's tool loop ([src/agents/agent-adapter.ts#L220](src/agents/agent-adapter.ts#L220), `handleToolCallsLoop`) then text-parses that synthesised blob back into tool calls via `parsePersistedToolCalls` ([src/agents/llm-contracts.ts#L60](src/agents/llm-contracts.ts#L60)).
...
A separate non-envelope role, `analyst`, consumes the SAME `LlmCompleteResult` type and the SAME `LlmProviderGateway.complete` entry-point directly, bypassing `AgentAdapter` and `createLlmCallFn`. That consumer relies on the two-field shape too. The contract change must therefore land coherently in both call paths; see Section 2.4.
...
[src/agents/provider-capabilities.ts#L119](src/agents/provider-capabilities.ts#L119) (`capabilityRequestForLlmOptions`) builds a `CapabilityRequest` from `{ toolCalls, toolChoice, responseShape, streaming }`. There is no `responseFormat` axis, no `mixedCarriersPerTurn` axis, and no `toolsAndJsonModeCompatible` axis. Therefore `ModelRouter` cannot skip a candidate that will reject the combined request — the rejection has to happen at the wire and the failure has to be recovered after the fact.

The built-in capabilities table at [src/agents/provider-capabilities.ts#L53](src/agents/provider-capabilities.ts#L53) makes `opencode`, `opencode-go`, and `github-copilot` all identical to `GLOBAL_DEFAULT_CAPABILITIES` ([src/agents/provider-capabilities.ts#L44](src/agents/provider-capabilities.ts#L44)). There is no axis on which `opencode-go` is differentiated from `opencode`, even though they have materially different acceptance of the `tools[] + response_format` combination.

### 2.4 Direct consumers of `LlmProviderGateway.complete` and `LlmCompleteResult`

The contract under analysis is consumed from two distinct paths. Any redesign of `LlmCompleteOptions` and `LlmCompleteResult` must land coherently in BOTH, or it will leave the second path stranded with a stale shape.

Path A — adapter loop (envelope-bearing roles `planner` / `executor` / `reviewer` and any other adapter-driven role):

- Caller: `AgentAdapter.invokeAgent` ([src/agents/agent-adapter.ts#L341](src/agents/agent-adapter.ts#L341)) assembles `LlmCompleteOptions`.
- Wrapper: `AgentLlmInvocationGateway.createLlmCallFn` ([src/agents/agent-llm-gateway.ts#L48](src/agents/agent-llm-gateway.ts#L48)) invokes `client.complete(...)` and string-flattens the result at [src/agents/agent-llm-gateway.ts#L58](src/agents/agent-llm-gateway.ts#L58).
- Loop: `AgentAdapter.handleToolCallsLoop` ([src/agents/agent-adapter.ts#L220](src/agents/agent-adapter.ts#L220)) re-parses the flattened string back into tool calls. Its follow-up turn re-builds `LlmCompleteOptions` independently at [src/agents/agent-adapter.ts#L280](src/agents/agent-adapter.ts#L280) and DOES NOT include `response_format` (this is the per-turn option-drift bug; see Section 3, F11).
- Parser: `result-parser.ts` `extractJson` at [src/agents/result-parser.ts#L278](src/agents/result-parser.ts#L278) consumes the flattened final content string.

Path B — analyst resolver (role `analyst`, tools-only by design, no envelope):

- Caller: `LlmIntentResolver` in [src/agents/analyst-llm-resolver.ts](src/agents/analyst-llm-resolver.ts) (the file is `analyst-llm-resolver.ts`; the exported class name is `LlmIntentResolver`).
- It instantiates its OWN `LlmProviderGateway` at [src/agents/analyst-llm-resolver.ts#L160](src/agents/analyst-llm-resolver.ts#L160), bypassing the adapter's invocation-gateway wrapper entirely.
- It calls `client.complete(candidate, systemPrompt, messages, sessionId, { tools, tool_choice: 'auto', stream: false, … })` at [src/agents/analyst-llm-resolver.ts#L163](src/agents/analyst-llm-resolver.ts#L163). The request is tools-only by construction: `response_format` is NEVER set on this path, and `expectsJsonEnvelope` is irrelevant because `role === 'analyst'` is excluded from the planner/executor/reviewer triplet at [src/agents/agent-adapter.ts#L341](src/agents/agent-adapter.ts#L341) — but the analyst does not go through the adapter anyway.
- It iterates `result.toolCalls` at [src/agents/analyst-llm-resolver.ts#L171](src/agents/analyst-llm-resolver.ts#L171) to enforce the analyst tool surface (`RoleToolPolicy.assertAnalystSurfaceTool`).
- It returns `{ content: result.content ?? '', toolCalls: result.toolCalls }` at [src/agents/analyst-llm-resolver.ts#L179](src/agents/analyst-llm-resolver.ts#L179) — a public method signature that the rest of the analyst surface depends on.
- Its capability request is built at [src/agents/analyst-llm-resolver.ts#L204](src/agents/analyst-llm-resolver.ts#L204) via the same `capabilityRequestForLlmOptions` factory (so it is subject to the same capability-surface gap described in Section 2.3).

Implication for the redesign: under any new `LlmCompleteResult` discriminated union, Path B must consume the `tools` variant and the `terminal_text` variant cleanly. The current method shape `{ content; toolCalls }` of `LlmIntentResolver.chat` MUST be replaced with the typed-union shape; do not preserve the old return shape with a translation shim. The analyst caller already treats the two cases distinctly in practice — it inspects `result.toolCalls` for policy enforcement and returns `result.content ?? ''` as a fallback — so it can be rewritten to switch on `result.kind` without semantic loss.

The pre-redesign call-graph for the contract under analysis is therefore:

```
AgentAdapter.invokeAgent ──► createLlmCallFn ──► LlmProviderGateway.complete ──► {OpenAIChatGateway | OpenAICodexGateway}.complete
LlmIntentResolver.chat ─────────────────────────► LlmProviderGateway.complete ──► {OpenAIChatGateway | OpenAICodexGateway}.complete
```

Both arrows on the right are the same `complete(candidate, systemPrompt, messages, sessionId, opts) -> LlmCompleteResult` shape. Both call sites consume the result. Any change to the result type ripples to both; this fact is not optional and not negotiable under the architecture-first guideline.
...
The first-turn assembly is [src/agents/agent-adapter.ts#L342](src/agents/agent-adapter.ts#L342); the follow-up assembly is [src/agents/agent-adapter.ts#L280](src/agents/agent-adapter.ts#L280) — note the absence of any `response_format` spread on the second site. (a) and (b) are both downstream consequences of treating the two channels as a single bag of independent fields. A typed discriminated union for `LlmCompleteResult` removes (b) by construction; a single per-call (not per-turn) `LlmCompleteOptions` builder removes (a) by construction.
...
The analyst path (Section 2.4) is not directly affected by F01/F02/F09/F11 because it never asks for an envelope. But it consumes the SAME `LlmCompleteResult` type, so any redesign of that type ripples to it; and it instantiates its OWN `LlmProviderGateway` outside of the adapter's invocation-gateway wrapper, so any new request-side factory must be reachable from there too. The chosen design must not silently regress analyst tool-call delivery when the result type is replaced.
...
- A.1 **Tools-only result**: define one synthetic tool per role, e.g. `emit_planner_result`, whose `parameters` schema IS the role envelope. The role-specific tool catalog already exists ([src/agents/agent-tool-catalog.ts#L27](src/agents/agent-tool-catalog.ts#L27)); add the result-tool to each catalog and require the model to call it as its terminal action. `response_format` is never requested. The result-parser becomes a tool-call argument unpacker validated by a per-role Zod schema. This matches OpenAI chat-completions semantics exactly (the wire spec already says "per turn the model picks tool_calls XOR content"). It also matches the Codex Responses transport (tool calls are first-class events; the parsed arguments are typed objects).
...
- A.1 wins on analyst alignment: the analyst path (Section 2.4) is already tools-only; under A.1 the contract converges (every role's result is a tool-call argument), and `LlmIntentResolver` becomes a degenerate case of the same code path rather than a parallel implementation.
...
- A.2 wins on staying close to Saivage v2's mental model and on keeping the runtime tool surface separate from the LLM tool surface. It loses on requiring a parallel "action vocabulary" that is essentially a duplicate of the tool catalog, and it still has to text-parse JSON out of `content`, so F02 and F09 do not fully disappear. It also creates a wider gap between the envelope-bearing roles and the analyst role, which would still consume tool calls directly.

For the architecture-first guideline, A.1 is the cleaner choice: it collapses two carriers into one, deletes `response_format` from the call-site entirely, unifies the envelope-bearing roles with the analyst path on a single result kind (`tools`), and lets `extractJson` and its three fallbacks be deleted alongside.
...
- B leaves the analyst path unchanged in spirit (analyst is phase-1-only — no envelope is ever requested) but still requires updating the result-consumer in [src/agents/analyst-llm-resolver.ts#L171](src/agents/analyst-llm-resolver.ts#L171) and [src/agents/analyst-llm-resolver.ts#L179](src/agents/analyst-llm-resolver.ts#L179) to the new discriminated `LlmCompleteResult` union.
...
- C leaves the analyst-vs-adapter shape drift untouched: both paths keep consuming the same orthogonal `{ content, toolCalls }` result, and the recovery logic for "one of the two carriers is missing" remains scattered between `agent-llm-gateway.ts`, `agent-adapter.ts`, `analyst-llm-resolver.ts`, and `result-parser.ts`.
...
1. **Single-carrier per turn.** For any single `LlmCompleteOptions` value, at most one of `{ tools, response_format }` is set. Encoded in the type: `LlmCompleteOptions` becomes a discriminated union — `{ mode: 'tools'; tools: ToolDefinition[]; tool_choice?: … }` vs `{ mode: 'envelope'; envelopeSchema: ZodSchema | { type: 'json_object' } }`. The current shape at [src/agents/llm-contracts.ts#L31](src/agents/llm-contracts.ts#L31) is replaced.

2. **Single-carrier per result.** `LlmCompleteResult` is a discriminated union of `{ kind: 'tools'; calls: ToolCall[] }` vs `{ kind: 'envelope'; envelope: unknown }` vs `{ kind: 'terminal_text'; text: string }` (the last for non-envelope roles that genuinely emit free text — e.g. analyst end-of-turn replies). The current shape at [src/agents/llm-contracts.ts#L25](src/agents/llm-contracts.ts#L25), with both `content` and `toolCalls` populated independently, is replaced. There is no `flatten` step.

3. **No string-flatten in the gateway.** [src/agents/agent-llm-gateway.ts#L58](src/agents/agent-llm-gateway.ts#L58) (`return result.content ?? JSON.stringify({ toolCalls: result.toolCalls })`) is deleted. The gateway returns the structured result. The adapter consumes the structured result. No code anywhere stringifies tool calls to re-parse them later. `parsePersistedToolCalls` at [src/agents/llm-contracts.ts#L60](src/agents/llm-contracts.ts#L60) is deleted as part of the same pass (no live caller remains under the new contract).

4. **No per-turn option drift inside a single call.** `LlmCompleteOptions` for a given role invocation is built by exactly one named factory function (e.g. `buildLlmOptionsForRole(role, phase, tools, modelParams, signal)`). Both the first turn and any follow-up turns inside the same invocation route through the same factory. The current second build site inside `handleToolCallsLoop` ([src/agents/agent-adapter.ts#L280](src/agents/agent-adapter.ts#L280)) is replaced. The first build site at [src/agents/agent-adapter.ts#L342](src/agents/agent-adapter.ts#L342) is replaced by the same factory.

5. **Envelope is parsed by a typed schema, not by text heuristics.** Each role declares a Zod schema for its envelope. Under A.1, the envelope IS the validated tool-call arguments. Under B, the envelope is the validated parse of the phase-2 `content`. Either way, `extractJson` at [src/agents/result-parser.ts#L278](src/agents/result-parser.ts#L278) is deleted, along with the three-layer brace-span fallback (and the throw at [src/agents/result-parser.ts#L292](src/agents/result-parser.ts#L292)). The Zod failure carries the schema diff and the first 200 characters of the raw response.

6. **Capability surface admits the new shape.** `EffectiveProviderCapabilities` and `CapabilityRequest` are extended to declare, per provider, which of the two modes it supports (tools-mode always; envelope-mode where applicable). `capabilityRequestForLlmOptions` at [src/agents/provider-capabilities.ts#L119](src/agents/provider-capabilities.ts#L119) is regenerated from the new request shape. The router skips candidates that cannot serve the requested mode BEFORE the wire call, not after.
...
9. **No backward-compatibility shim survives.** No code path remains that constructs the old `tools + response_format` request shape. No on-disk session-history row remains that encodes the old `{toolCalls: [...]}`-stringified-as-content format; rows already on disk that have that shape are not migrated, they are read with a parser that REJECTS them so the older sessions are clearly broken rather than silently mis-parsed. The analyst-side return signature `{ content; toolCalls }` at [src/agents/analyst-llm-resolver.ts#L179](src/agents/analyst-llm-resolver.ts#L179) is removed and replaced with a typed-union return; no translation shim survives between the new `LlmCompleteResult` union and the analyst's public surface. (Per project guideline: actively remove the old shape; do not bridge it.)

10. **Tests cover the matrix AND every direct consumer.** For each provider entry in Section 2.1 and each mode in Section 2.2, there is a test that asserts the chosen contract is honoured. The matrix is the test plan. In addition, there is at least one test per direct consumer of `LlmProviderGateway.complete`:
    - Adapter path: a planner invocation that exercises `AgentAdapter.invokeAgent` → `createLlmCallFn` → `LlmProviderGateway.complete` against a fake transport returning each of the three `LlmCompleteResult` kinds, asserting that `handleToolCallsLoop` consumes `kind: 'tools'` directly (no string round-trip), `kind: 'envelope'` is passed to the Zod parser, and `kind: 'terminal_text'` is treated as an envelope error (planner cannot emit free text under either A.1 or B).
    - Analyst path: a `LlmIntentResolver.chat` invocation against a fake `LlmProviderGateway` returning each of the three result kinds, asserting that `kind: 'tools'` is iterated for `RoleToolPolicy.assertAnalystSurfaceTool` enforcement, `kind: 'terminal_text'` is returned to the caller as the assistant reply, and `kind: 'envelope'` raises because the analyst path never requests an envelope mode (and therefore should never receive one).
...
- **F01** — option-assembler fix at [src/agents/agent-adapter.ts#L341](src/agents/agent-adapter.ts#L341). Owned by `F01-response-format-tools-mutex.md`. Under F05's chosen contract, F01's specific bug ("both spreads always emitted") is impossible by construction; F01's implementation plan should therefore reduce to "delete the old assembler and re-implement the call site against the new typed factory from F05". Do not write the F01 fix in this issue.
