...
Bare `Promise.race` is explicitly insufficient. The current invocation loop already creates an inner `AbortController` per LLM turn and passes `abortController.signal` into LLM transport. After Wave 5A runner extraction, this loop lives in `AgentInvocationRunner`; before that extraction lands it lives in `AgentAdapter.invokeAgent()`. The missing design is an outer runtime-supplied `AbortSignal` that can cancel that controller and the planner iteration. `plannerIterationTimeoutMs` is a default fallback in `RuntimeConfig`, not a standalone constant. Make `RepairBudget` immutable now regardless.
...
| `src/agents/invocation-runner.ts` / `src/agents/agent-adapter.ts` | **Modify.** `invokePlanner()` accepts `request.signal` and forwards it to the runner. `AgentInvocationRunner.invoke()` accepts an optional external `AbortSignal`; if the runner has not landed yet, apply the same change in `AgentAdapter.invokeAgent()` first and move it with the runner. When a turn creates its inner `AbortController`, bridge external abort to `abortController.abort()`, remove the listener in `finally`, and check `signal.aborted` before starting candidate attempts. Keep the existing session cancellation controller tracking. |
...
Use `rg` to count importers before deletion. Preserve/move `redactProviderErrorText` before deleting the `llm-errors.ts` barrel. Recount after Wave 5A completes because the new `invocation-runner.ts`, `invocation-model-context.ts`, and helper modules may change importer ownership.
