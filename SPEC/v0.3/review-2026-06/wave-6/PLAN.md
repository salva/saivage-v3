...
| File | Action |
|------|--------|
| `src/contracts/operator-api-core.ts` | **Modify.** Add a `defineRoute<TParams, TQuery, TBody, TSuccess, TError>(spec)` helper returning `OperatorRouteContract`. `spec.auth` must use the existing `ContractAuthClass` values (`'public'`, `'operator-session'`, `'agent-session'`, `'mcp-tool-token'`), not a new `'protected'` alias. Preserve explicit `response`, `error`, `successSchemaName`, `permissions`, `audit`, and `describe` fields. Helper may derive common `requiresAuth`, `successSchemaName`, and default response maps, but explicit values win. |
...
|---|---|
| `src/agents/llm-errors.ts` | 9 source files: `src/agents/llm-openai-codex-gateway.ts`, `src/agents/llm-openai-chat-gateway.ts`, `src/agents/llm-failure-classifiers.ts`, `src/agents/llm-codex-parser.ts`, `src/agents/llm-recording.ts`, `src/agents/llm-stream-parser.ts`, `src/agents/llm-provider-gateway.ts`, `src/agents/invocation-recovery-policy.ts`, `src/scripts/probe-llm-contract.ts`; plus tests `tests/runtime/planner-context-length-blocker.test.ts` and `tests/agents/llm-codex-parser-context-length.test.ts`. Only `llm-failure-classifiers.ts` and `llm-codex-parser.ts` import `redactProviderErrorText`; move `redactProviderErrorText` to `llm-failure.ts` or `redaction/index.ts`. Others import `LlmRequestError` or `unwrapFailure` — change source import paths to `./llm-failure.js` or `../agents/llm-failure.js` as appropriate. |
...
| File | Action |
|------|--------|
| `src/runtime/process-runner.ts` | **Modify.** (1) Extend `ProcessReconcileOptions` with `eventLogger?: EventLogger`. Change `auditProcessReconciliation` to accept an optional injected logger and use it when provided; only construct/close a temporary `EventLogger` as compatibility fallback. Thread the logger through `markLost()` and `reconcileProcessRecordsForService()`. (2) Merge `ProcessRunnerService` class methods with their module-level `*ForService` function counterparts — move the function bodies into the class methods and delete the `*ForService` functions. The class already stores `projectRoot` and process records; the module-level functions mostly pass `service` or `service.projectRoot`. Keep `serviceFor()` and `disposeProcessRuntimeScope()` for compatibility, but mark them `@internal`. |
