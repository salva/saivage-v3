# F18 — Design (round 2, WRITER)

Round 2 changes vs r1:

1. Lock-file references corrected to
   ...
   The r1 phrase "live holder lives in `.saivage/.lock`" was wrong; the new
   `readLiveLockHolder` helper reuses the existing module-local
   `lockPath()` so the path stays single-sourced.
2. CLI description now uses the helper output directly (no path name in
   the user-facing CLI doc).

No other surface contracts change.
...
Renders the holder pid read from the runtime lock at
`lockPath(projectRoot)` (resolved by the existing helper in
[src/runtime/lock.ts](../../../src/runtime/lock.ts#L31-L36)), rather than
the persisted `RuntimeState.pid`. Output gains a clear "(not running)"
line when no live holder exists.

```
Project root: /work/saivage-v3-getrich
Status:       running
PID:          5485        # holder pid from the runtime lock, or "(not running)"
Paused:       false
...
Add a tiny read-only accessor (new export) that opens the runtime lock at
`lockPath(projectRoot)` — the same path used by `acquireLock` /
`releaseLock` — parses the `{ pid, started_at }` payload, and returns the
holder only when `isPidAlive(pid)` is true:

```ts
export function readLiveLockHolder(projectRoot: string): { pid: number; started_at: string } | null {
  // Reuses the existing module-local lockPath() and isPidAlive() helpers.
  // No lock acquisition, no mutation, no side effects.
}
```

No change to `acquireLock` / `releaseLock` semantics, no new constant, and
the `.saivage-work/tmp/runtime/runtime.lock` path stays defined exactly
once in this module.
...
Tests that interact with the runtime lock directly (e.g.
