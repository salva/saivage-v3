# F18 — Analysis (round 2, WRITER)

Round 2 changes vs r1 (per reviewer findings in
[01-analysis-review-r1.md](01-analysis-review-r1.md)):

1. Corrected lock-file path everywhere: it is
   `.saivage-work/tmp/runtime/runtime.lock`, written by `lockPath()` in
   [src/runtime/lock.ts](../../../src/runtime/lock.ts#L31-L36) — **not**
   `.saivage/.lock`. Wording updated to refer to the file via the
   `lockPath(projectRoot)` helper rather than naming a non-existent path.
2. The deployment / Jest changes live in the plan
   ([03-plan-r2.md](03-plan-r2.md)); no analysis content moved.
...
- **`saivage status` CLI**
  ...
  last-written pid, not the live server pid. The authoritative
  live-holder pid is in the runtime lock at
  ...
  where `acquireLock` writes `{ pid, started_at }` and verifies liveness
  via `isPidAlive` (`process.kill(pid, 0)`).
...
The independent runtime lock file at `lockPath(projectRoot)` already
records the holder's pid and already validates liveness with `isPidAlive`
— that is the authoritative record of "which process owns this Saivage
project", and is the right source for any out-of-process consumer (CLI,
recovery tools) that needs to ask "who is currently running".
...
- **CLI:** `saivage status`
  ...
  reading the lock-holder pid via a new read-only helper in
  [src/runtime/lock.ts](../../../src/runtime/lock.ts) that opens the file
  at `lockPath(projectRoot)`. When no lock is held (or the holder pid is
  dead), the CLI emits `(not running)`.
