# F14 r1 — Design: surface deployment identity on `/api/state`
...
- `projectRoot` is **already** the value the operator started the server with
  and is reachable via `ps`, systemd unit inspection, and the `.saivage/`
  layout. Exposing it on a session-authenticated route to the same operator is
  not a leak.
- `redactOperatorErrorMessage` continues to strip `projectRoot` from **error
  messages**. We do **not** route this typed identity field through that
  helper — redaction is for unstructured exception text, not for the canonical
  identity contract.
- `/api/state` requires operator session
  ([operator-api.ts#L264](../../../src/contracts/operator-api.ts) inherits
  `operatorSessionContract`). No anonymous probe can read these fields.
...
- Any backend integration test that parses
  `RuntimeGetStateResponseSchema` (search in r2; expected: zero or few).
...
- F14 does **not** change `health.liveness`'s hard-coded `project:
  'saivage-v3'`. Documented in r1 analysis as an open question.
