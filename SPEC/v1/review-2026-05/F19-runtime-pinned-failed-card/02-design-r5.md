...
Supersedes [02-design-r4.md](02-design-r4.md). This revision addresses the r4 review (`01-analysis-review-r4.md`) — specifically the design concern that follow-up non-status `cardStore.update` writes were shown without `await` even though F13 r4 makes `CardStore.update` async. Proposal B is still recommended. Proposal A is unchanged from r3/r4 and is referenced by name rather than copied; only Proposal B is revised below.

## Proposal B — Runtime state machine (recommended; revised)

### New module: `src/runtime/state-machine.ts`

`RuntimeStateMachine` owns every runtime-layer transition. Boundaries (owned fields, owned card-status writes, out-of-scope analyst/operator/CardStore/freeze-fallback surfaces) are unchanged from r3/r4.
...
`transition(event, payload)` for runtime-state events stays `async` for the same reason (it writes through the awaited `writeState` dep). The Step 5 conversion table in [03-plan-r5.md](03-plan-r5.md) marks every call site `await`ed; the Step 7 gate fails if any unawaited call survives.

### Every post-`transitionCard` `cardStore.update` follow-up is awaited (NEW in r5)

**Binding rule.** Every runtime-layer `cardStore.update(...)` call that writes non-status payload after a `transitionCard(...)` call **must be `await`ed**. There are **no fire-and-forget `cardStore.update` writes in the runtime** under Proposal B.

Sites the rule covers (one entry per conversion in [03-plan-r5.md](03-plan-r5.md) §Step 5):

- Startup executor-phase repair (post-L278 transition): `await this.cardStore.update(run.card_id, { error, result })`.
- Planner-exception catch (post-L635 transition): `await this.cardStore.update(goalId, { error, status_text })`.
- Executor terminal restructure (post-L725-733 transition; folds in the deleted L740 site): a single `await this.cardStore.update(card.id, { result, error, status_text, status_text_updated_at, status_text_author_session_id, latest_self_report })`.
- Ignored evidence-registration payload write (currently L737, after restructure executes before the `executor_finish` transition): `await this.cardStore.update(card.id, { result: { ..., evidence_registration_ignored: ... } })`.
- Planner-status rejection bookkeeping (post-L766–L782 transition, when the machine rejects the requested status): `await this.cardStore.update(...)` is not used (the rejection is bookkeeping inside `untrackedChanges.rejected`); no fire-and-forget store call is introduced.

**Rationale.** Three reasons make `await` mandatory rather than optional here:

1. **Durability before the runtime proceeds.** Executor results, registration failures, and human-visible `error` / `status_text` text must be on disk before subsequent runtime work observes the post-transition card. The post-transition continuation of `dispatchPendingActivations` (and of the startup repair / planner-exception branches) reads the card and the runtime state to decide the next step; if the non-status payload is still in flight the next step can race against a half-written card. F13 r4 routed `cardStore.update` through async `applyMutation` precisely to make this race observable; F19 must honor the contract and `await` the result.
2. **Error surface.** A fire-and-forget `cardStore.update` swallows promise rejections (unhandled rejection becomes a noisy crash with no per-site context). `await` ensures the existing `try`/`catch` in the runtime catch sites surfaces store-layer failures (disk full, JSON-parse error, concurrent-write reject) into the same `errorLogger` the rest of F19 uses.
3. **Audit-log ordering.** `CardStore.applyMutation` writes the card-history entry inside the awaited path. If the non-status payload write is not awaited, a later `transitionCard` (e.g., the post-reviewer `complete` step) can interleave its own history entries with the payload-write entry, producing out-of-order history that violates the F12 contiguous-set invariant.

**The Step 7 multiline `rg` gate** ([03-plan-r5.md](03-plan-r5.md) §Step 7) is augmented to also fail on any unawaited `cardStore.update(...)` call inside `src/runtime/runtime.ts`. The gate uses a plain `rg` match for `\bcardStore\.update\(` not preceded by `await ` on the same line, scoped to the runtime file; the small explicit allowlist (currently empty) is documented inline at the gate site.
...
Two new actions vs r3 (unchanged from r4):
...
Source-of-truth constants: [`STARTABLE_STATES = {drafting, backlog, changed}`](../../../../src/permissions/card-permissions.ts#L29), [`RESTARTABLE_STATES = {blocked, changed, done, failed, cancelled}`](../../../../src/permissions/card-permissions.ts#L28), [`VALID_TRANSITIONS`](../../../../src/cards/card-store.ts#L217-L227). Per [03-plan-r5.md](03-plan-r5.md) §Step 1.5, the two constants are exported from `src/permissions/card-permissions.ts` so the runtime and the machine can import them directly.
...
// 3. Optional: write ignored registrations (non-status payload; unchanged from L737, NOW awaited).
if (ignoredArtifactRegistrations.length > 0 || ignoredAttachmentRegistrations.length > 0)
  await this.cardStore.update(card.id, { result: { ..., evidence_registration_ignored: ... } });
...
// 6. Non-status payload (combines existing L725-733 non-status fields + L740 registration_failures). AWAITED.
...
Net effect: exactly **one** legal status step is emitted per executor turn (`running → done` or `running → failed`). The L725-733 site and the L740 site collapse into the single `'executor_finish'` transition in step 5 above, plus one awaited non-status `cardStore.update` in step 6.
...
Unchanged from r3/r4. F13 r4 lands first; F19 rebases Step 3 wiring into `Runtime.open(config)` / `ActiveRuntime.open(projectRoot, config, mcpManager)`. `RuntimeStateMachine` itself is sync-constructible (constructor body does no I/O), so the seam composes with either construction style. The new r5 await-everywhere rule for follow-up `cardStore.update` writes composes cleanly with F13 r4's awaited `applyMutation` surface — the runtime catches and the machine never observe an in-flight promise.

### Files deleted or reduced

Unchanged from r3/r4 (`_status` field/getter; the eight inline `updateRuntimeState({ status, current_card_id: null, … } as Partial<RuntimeState> as never)` blobs and their casts; `safeTick` self-heal; `_safeTickInFlight`; `_autoDispatchFirstBacklogGoal`; `mirrorRuntimeState`; the `enforceInvariants` flag itself in Step 7).
...
The unit test fixture passes a spy `cardStore` whose `setStatus(id, status)` and `update(id, patch)` calls are recorded as `(id, kind, status)` tuples in order. Each test case asserts the exact ordered list. Examples (full matrix lives in [03-plan-r5.md](03-plan-r5.md) Step 5 test additions):
...
The full action × source-state matrix is encoded in [03-plan-r5.md](03-plan-r5.md) Step 5 test additions; every cell asserts either an exact emitted sequence or a no-write rejection.

### Failure modes

Unchanged from r3/r4 (bigger surface change, tick interval = wall-clock dep, wider blast radius if invariants are wrong, cross-process operator/runtime write race). The `enforceInvariants: false` observe-only window in Step 3 still bounds the auto-correction blast radius; Step 4 lands the corrective bodies and the flip together so design and plan agree on what is live when.
...
Proposal B. Reasoning unchanged from r3/r4.

## Changes vs r4

- **NEW: every post-`transitionCard` `cardStore.update` follow-up is `await`ed** (own subsection added). Rationale spelled out: durability before runtime proceeds, error surface, audit-log ordering. The executor terminal restructure pseudo-diff now explicitly `await`s steps 3 (ignored evidence registration) and 6 (combined non-status payload). [03-plan-r5.md](03-plan-r5.md) §Step 5 mirrors the rule per site and §Step 7 augments the final gate to fail on any unawaited `cardStore.update` inside the runtime file. (Reviewer §B1.)
- **Permission constants now imported, not redefined.** The action-table footnote pinning `STARTABLE_STATES` / `RESTARTABLE_STATES` to `src/permissions/card-permissions.ts` now also notes that those constants are exported in [03-plan-r5.md](03-plan-r5.md) §Step 1.5 so the runtime and machine can import them directly. (Reviewer §P2-companion / §B-coverage of plan.)
- Cross-references updated to point at `01-analysis-r5.md` / `03-plan-r5.md`.
- All other content (actions, action-matrix, executor terminal restructure, staging lifecycle, failure modes, API impact, test strategy, test-contract examples) is unchanged from r4.
...
- **New `'reviewer_repair_resume'` action** for the L266 startup repair; precondition `card.status ∈ {'active', 'running'}` with construction proof in [01-analysis-r5.md](01-analysis-r5.md). (Reviewer §A2.)
