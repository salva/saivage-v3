# F04 — Chat / analyst surface style — Implementation plan (r2)

Writer round 2. Binding inputs:

- Prior draft: [03-plan-r1.md](03-plan-r1.md).
...
`.chat-composer` / `.composer-input` / `.pending-tool-*` /
`.message-badges` / `.on-screen-section` / `.state-panel`
survivors anywhere under `web/src/components/chat/`, no chat-local
chip API, and no shims around the F02 primitives or the F03 shared
`<ToolChip>`.

---

## 0. Review coverage map (r1 → r2)

[03-plan-review-r1.md](03-plan-review-r1.md) returned
**CHANGES_REQUESTED** with one blocking finding and a set of
non-blocking confirmations. r2 addresses them as follows:

| # | Reviewer item | Resolution in r2 |
| - | ------------- | ---------------- |
| **R1 (blocking)** | r1's B4 SFC sketches reintroduce three forbidden selector families inside the new children (`composer-input`, `pending-tool-list`, `message-badges`), and r1's PR merge-gate `rg` would have failed on its own output. r1's `.chat-composer` class also matches design r2 §1.2's deletion list. | §1 (renamed-class table) renames the new layout classes to non-legacy names: `chat-composer` → `composer-form`, `composer-input` → `composer-textarea`, `pending-tool-list` → `pending-invocations`, `message-badges` → `badge-stack`. B4 (§4) instructs the developer to apply those renames over design r2 §1.4 / §1.5 / §1.7's verbatim sketches. B5 (§4) deletion checklist now lists all eight forbidden families. B6 (§4) selector assumptions are updated. The merge-gate `rg` in §5/§9 includes `chat-composer`. The renamed names are **new, non-legacy** identifiers; they do not alias the old classes — the old `.composer-input` / `.pending-tool-list` / `.message-badges` / `.chat-composer` rules are deleted in the same commit that introduces the new SFCs (B5). |
| **R-N1** (non-blocking) | Per-file implementability strong. | Retained verbatim — no changes to file inventory, ownership, or commit ordering. |
| **R-N2** | Commit ordering sound (B0 → B1 → B2 → B3 → B4 → B5 → B6 → B7). | Retained verbatim. |
| **R-N3** | `ChatMessage` additive metadata boundary correct; no `thinking` field on the store. | Retained verbatim (B0; B5 local `thinking` derivation). |
| **R-N4** | Test rewrite scope appropriate, `.on-screen-children li` selector continues to work, raw-source guard retargets the rewritten container. | Retained verbatim. The `on-screen-children` class is preserved per design r2 §1.4 / C1 — it is **not** a forbidden family (`on-screen-section` is the forbidden one; `on-screen-children` is the new layout class introduced by C1 alongside the `data-testid`). |
| **R-N5** | Cross-issue dependencies correctly stated; in-place chip swap stays in F03's PR. | Retained verbatim. |
| **R-N6** | "Plan does not introduce a new alias period in intent." | Confirmed: the renames in R1 are **not** parallel aliases. The old families are deleted in B5; the new SFCs ship with the renamed classes from B4 onward; at no point in the PR's history do both the old and the new selector exist in the same HEAD. |

### 0.1 Authoritative class-rename table (resolves R1)

These renames apply over design r2's verbatim SFC sketches. Every
sketch in design r2 §1.4 / §1.5 / §1.7 that uses the old class
must use the new class in the implementation. No other text or
behavior changes; the renames are a pure identifier substitution.

| File | Old class (in design r2 sketch) | New class (implementation) |
| --- | --- | --- |
| `chat/ChatComposer.vue` | `.chat-composer` (root `<form>`) | `.composer-form` |
| `chat/ChatComposer.vue` | `.composer-input` (textarea) | `.composer-textarea` |
| `chat/MessageList.vue` | `.pending-tool-list` (pending footer `<section>`) | `.pending-invocations` |
| `chat/MessageItem.vue` | `.message-badges` (badge `<ul>`) | `.badge-stack` |

Classes **not** renamed (already non-legacy):

- `chat/AnalystChatPanel.vue` root: `.analyst-chat-panel` (unchanged — distinct from the forbidden `.chat-composer` etc.).
- `chat/MessageList.vue` scroll body: `.message-list` (unchanged; not in the forbidden list).
- `chat/MessageList.vue` on-screen block: `.on-screen-children` (unchanged; preserved per design r2 §1.4 / C1 so the legacy `.on-screen-children li` test selector keeps working).
- `chat/MessageItem.vue` row root: `.message-item` and inner `.message-text` (unchanged; not in the forbidden list).
- `chat/JumpToLatest.vue`: `.jump-to-latest`, `.label`, `.icon` (unchanged).
- `chat/UnauthorizedNotice.vue`: `.unauth-notice`, `.copy`, `.icon` (unchanged).
- `chat/ChatHeader.vue`: `.chat-header`, `.connection-chip`, `.chip-icon`, `.session-id`, `.header-state` (unchanged).

The forbidden-family merge-gate (§5 B5 / §9 gate 4) is updated to
match the **complete** design r2 §1.2 deletion list:
...
(That is the r1 regex plus the missing `chat-composer` alternative.
With the renames in §0.1 applied, the regex returns no matches at
the F04 PR's tip commit.)
...
by the listed batch, with the §0.1 renames overlayed where
applicable.
...
| §1.4 | `chat/MessageList.vue` — apply §0.1 rename `pending-tool-list` → `pending-invocations`; B3 resize-emit invariant | B4 |
| §1.5 | `chat/MessageItem.vue` — apply §0.1 rename `message-badges` → `badge-stack`; B2 model-pill gating via `modelLabel` | B4 |
| §1.6 | `chat/JumpToLatest.vue` | B4 |
| §1.7 | `chat/ChatComposer.vue` — apply §0.1 renames `chat-composer` → `composer-form` and `composer-input` → `composer-textarea`; resize-to-content, keymap, send | B4 |
...
- **B1** (no `thinking` field on the live store) — discharged by
  ...
  by the `thinking` prop on `MessageList` in B4, and by the unit
  case in B6.
...
- **R1** (this round; class renames) — discharged across §0.1, B4,
  B5, B6, and the merge gates in §5 / §9.

---

## 2. Selected approach — Proposal A verbatim (modulo §0.1 renames)
...
This plan implements Proposal A literally except for the four
class identifiers tabulated in §0.1. The container script in §B5
is the script from design r2 §1.2 verbatim. The child components
in §B4 are the SFCs from design r2 §1.3–§1.8 verbatim, with the
§0.1 identifier substitutions applied. No deviation, no
...
web/src/components/chat/
  ...
  MessageList.vue            NEW      (design r2 §1.4; §0.1 rename: .pending-invocations)
  MessageItem.vue            NEW      (design r2 §1.5; §0.1 rename: .badge-stack)
  JumpToLatest.vue           NEW      (design r2 §1.6)
  ChatComposer.vue           NEW      (design r2 §1.7; §0.1 renames: .composer-form, .composer-textarea)
...
hex literals and all eight forbidden class families (the seven
listed in r1 plus `.chat-composer`) are gone in the same commit.
...
Files (NEW), in implementation order. The SFC bodies are design
r2 §1.3–§1.8 verbatim, with the §0.1 identifier substitutions
applied to the three affected files.

1. `web/src/components/chat/UnauthorizedNotice.vue`
   (design r2 §1.8) — smallest, no dependencies on other new
   SFCs; emits `openTokenEntry`. No §0.1 renames.
2. `web/src/components/chat/JumpToLatest.vue`
   (design r2 §1.6) — pure presentational; props `unseen`,
   `bottomOffsetPx`; emits `jump`. No §0.1 renames.
3. `web/src/components/chat/ChatHeader.vue`
   ...
   `chipLabel` helpers are module-local. No §0.1 renames.
4. `web/src/components/chat/ChatComposer.vue`
   ...
   **§0.1 renames applied:** the `<form>` root carries
   `class="composer-form"` (NOT `chat-composer`); the
   `<textarea>` carries `class="composer-textarea"` (NOT
   `composer-input`). The `<style scoped>` block renames the
   matching rules (`.composer-form { … }`,
   `.composer-textarea { … }`); the `.composer-footer` and
   `.icon` rules are unchanged.
5. `web/src/components/chat/MessageItem.vue`
   ...
   - Badges render as `<ul class="badge-stack"><li><Pill tone="accent">…</Pill></li></ul>`.
   **§0.1 rename applied:** the badge `<ul>` carries
   `class="badge-stack"` (NOT `message-badges`); the `<style
   scoped>` block renames the matching rule
   (`.badge-stack { … }`).
...
6. `web/src/components/chat/MessageList.vue`
   ...
   `<section ref="pendingFooterEl" class="pending-invocations" data-testid="pending-tool-list">`
   ...
   - The single `ResizeObserver` is the only path that emits
     ...
     `onBeforeUnmount`.
   **§0.1 rename applied:** the pending-footer `<section>` carries
   `class="pending-invocations"` (NOT `pending-tool-list`); the
   `<style scoped>` block renames the matching rule
   (`.pending-invocations { … }`). The `data-testid` attribute
   value stays as `"pending-tool-list"` — that is a test
   identifier, not a CSS class, and the design r2 §1.13 test
   inventory binds tests to the testid, not the class. Keeping
   the testid stable keeps the test surface unchanged across the
   class rename.
...
- No hex literals; all colours flow from F01 tokens (`--bg`,
  ...
  `--text-muted`, `--accent`).
- No class on the file's root element or any inner element
  matches the forbidden-family regex from §0.1 / §5 B4 / §9 gate 4
  (after applying the §0.1 renames, the regex returns no matches
  in the file).
...
- Single `<script setup>` block per file
  ...
  exactly 1 per file).
- `rg -n "(tool-chip|message-bubble|primary-btn|chat-composer|composer-input|pending-tool-|message-badges|state-panel|on-screen-section)" web/src/components/chat/`
  returns no matches across the six new SFCs (the monolithic
  `AnalystChatPanel.vue` is still on HEAD at this batch tip and
  may still match; B5 closes that gap).
...
3. **Delete in the same commit:**
   ...
   - The eight forbidden class families
     (`.tool-chip*`, `.message-bubble`, `.primary-btn`,
     `.chat-composer`, `.composer-input`, `.pending-tool-*`,
     `.message-badges`, `.on-screen-section`, `.state-panel`).
     The bulk of `.tool-chip*` is already gone from the F03 PR;
     this step verifies and removes any tail.
   - Any markup in the container template that referenced the
     removed classes (the rewrite replaces the entire template,
     so this is automatic; the verification step is the
     forbidden-family `rg` below).

Exit criteria:

- The `AnalystChatPanel.vue` file shrinks to roughly the size of
  design r2 §1.2 (container script + minimal grid CSS).
- `grep -nE "(#[0-9a-f]{3,8})" web/src/components/chat/AnalystChatPanel.vue`
  prints nothing (case-insensitive variant also checked).
- `rg -n "(tool-chip|message-bubble|primary-btn|chat-composer|composer-input|pending-tool-|message-badges|state-panel|on-screen-section)" web/src/components/chat/`
  prints nothing — across the whole `chat/` subtree, including
  the rewritten container and the six children from B4. This is
  the merge-gate regex from §0.1 / §9 gate 4.
...
- The Vue compile succeeds and the container template references
  exactly the five children created in B4 (plus the inline
  `UnauthorizedNotice` slot child).

### Batch B6 — Tests (rewrites + new files)

All paths are relative to `web/src/__tests__/`. Files are
ordered to match design r2 §1.13. Selector references below use
the **renamed** class identifiers from §0.1 where applicable.
...
1. `analyst-chat-panel.test.ts` — **rewrite** (existing file):
   ...
   - Pending chip → `[data-testid="tool-chip"][data-status="pending"]`.
     The pending footer is located via
     `[data-testid="pending-tool-list"]` (the testid is stable
     across the §0.1 rename; the class behind it is
     `.pending-invocations`).
   ...
   - **R1 selector hygiene:** no `wrapper.find('.composer-input')`,
     no `wrapper.find('.message-badges')`, no
     `wrapper.find('.pending-tool-list')`, no
     `wrapper.find('.chat-composer')` anywhere in the rewritten
     file. Composer-text input is located via
     `wrapper.find('textarea[aria-label="Analyst chat composer"]')`
     or via the `defineExpose`d ref through the mounted wrapper
     (whichever the existing test scaffold already uses).
...
3. `components/AnalystChatPanel.children.test.ts` —
   **rewrite (C1)**:
   - The three existing behavior tests mount the rewritten
     ...
     must also continue to work (the class is preserved — it is
     NOT in the forbidden list; only `.on-screen-section` is).
...
5. `components/chat/MessageList.resize.test.ts` — **NEW (B3)**:
   ...
   - **R1 selector hygiene:** locate the pending footer via
     `wrapper.find('[data-testid="pending-tool-list"]')`, not
     via the class selector. The test must not contain the
     string `pending-tool-list` outside the `data-testid`
     attribute selector.
...
9. `utils/analyst-timeline.test.ts` — **NEW or EXTEND**:
...
- No test references any of the eight forbidden class names as a
  CSS selector (i.e. `wrapper.find('.<forbidden>')` does not
  appear in the test suite under `web/src/__tests__/`).
...
| B4 | `npx vue-tsc --noEmit`; `for f in web/src/components/chat/*.vue; do echo $(grep -c '<script setup' "$f") $f; done` — every line starts with `1`; `rg -n "#[0-9a-fA-F]{3,8}" web/src/components/chat/` prints nothing; and **on the six new SFCs only** (excluding the still-monolithic container) `rg -n "(tool-chip\|message-bubble\|primary-btn\|chat-composer\|composer-input\|pending-tool-\|message-badges\|state-panel\|on-screen-section)" web/src/components/chat/{ChatHeader,MessageList,MessageItem,JumpToLatest,ChatComposer,UnauthorizedNotice}.vue` prints nothing |
| B5 | as B4 plus the merge-gate regex across the whole subtree: `rg -n "(tool-chip\|message-bubble\|primary-btn\|chat-composer\|composer-input\|pending-tool-\|message-badges\|state-panel\|on-screen-section)" web/src/components/chat/` prints nothing |
| B6 | `npx vitest run` — full suite green; `rg -n "\\.(composer-input\|message-badges\|pending-tool-list\|chat-composer)\b" web/src/__tests__/` prints nothing (no test uses the renamed classes as CSS selectors) |
...
beyond design r2 §1.13 plus the R1 selector-hygiene line items.**

| Test file | Status | Owner test cases (high level) |
| --- | --- | --- |
| `analyst-chat-panel.test.ts` | REWRITE | selectors migration; B2 model-pill gating (positive + negative); B1 thinking-dots (3 fixtures); read-only tooltip; connection chip `data-state`; state-panel text queries; `saivage:focus-chat`; P3 expansion-body content check; R1 selector hygiene (no `.composer-input` / `.message-badges` / `.pending-tool-list` / `.chat-composer` finds) |
| `analyst-chat-store.test.ts` | EDIT | accept additive `ChatMessage` fields in fixtures; `PendingToolInvocation` import path |
| `components/AnalystChatPanel.children.test.ts` | REWRITE | three behavior tests via `[data-testid="on-screen-children"]`; raw-source guard targets new container; legacy `.on-screen-children li` selector continues to work (class is preserved, not forbidden) |
| `jump-to-latest.test.ts` | NEW | offset style, label text, unseen class, click → `jump`, aria-label, ellipsis rules |
| `components/chat/MessageList.resize.test.ts` | NEW | B3 invariant: initial `0`, empty-transition `0`, non-empty transition does not emit spurious `0`, element-null watcher emits `0`; R1 hygiene: locate footer via `[data-testid="pending-tool-list"]` only |
...
| Implementer copies design r2 §1.4 / §1.5 / §1.7 SFC sketches verbatim and forgets to apply the §0.1 renames | medium | high (merge-gate failure) | §4 B4 per-file checklist and §5 B4/B5 commands include the forbidden-family `rg`. The merge cannot succeed if any of the four legacy class names re-appears. The §0.1 rename table is also restated in §4 B4 per file. |
...
3. `rg -n "(#[0-9a-fA-F]{3,8})" web/src/components/chat/`
   prints nothing.
4. `rg -n "(tool-chip|message-bubble|primary-btn|chat-composer|composer-input|pending-tool-|message-badges|state-panel|on-screen-section)" web/src/components/chat/`
   prints nothing. **This is the full forbidden-family list from
   design r2 §1.2; it includes `chat-composer`, which r1's
   merge-gate omitted (R1).**
...
where two chip renderers coexist, and no intermediate HEAD where
both an old forbidden class and its renamed §0.1 successor coexist.
