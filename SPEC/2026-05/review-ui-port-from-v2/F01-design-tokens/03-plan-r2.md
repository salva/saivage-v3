# F01 — Design tokens & semantic CSS layer — Implementation plan (r2)
...
## Coverage map — r1-plan review required changes

| r1-plan-review item | Where addressed in r2 |
| --- | --- |
| Blocking #1 — Replace every `pnpm -C web …` command with the repo-native `npm --prefix web run …` form (root and `web/` both ship `package-lock.json`, not `pnpm-lock.yaml`); update the "pnpm workspace is bootstrapped" precondition accordingly | §1 "Preconditions" (npm bootstrap check), §5 Step 12 (typecheck), Step 13 (tests), Step 14 (build), §7 "Validation commands" (all four pnpm invocations replaced), §10 Risks (Vite import-order row no longer mentions pnpm) |
| Blocking #2 — Add the missing visually affected surfaces to the diff matrix (`WorkspaceHeader.vue`, `RawLlmExchangePanel.vue`, `AnalystToaster.vue`, `CardsView.vue`, `CardsLeaderboardView.vue`, `CardsTreeView.vue`, `CardHistoryPanel.vue`, `StaleWarningRibbon.vue`, `NotFound.vue`) with per-surface checks | §8 "Visual diff matrix" — rows 17–25 added, with explicit per-surface checks. Existing rows 1–16 retained. |
| Confirmation — single atomic F01 commit | §6 unchanged |
| Confirmation — no backward-compatibility leak (`--legacy-*`, fallbacks, shims, dual visual system, global `.tool-chip*`) | §4 (no deletions/shims), §11 (out of scope still excludes the F02 composite families), §7 gates still enforce zero shim |
| Confirmation — edit ordering (add styles → import in `main.ts` → mechanical hex replacement → grep/typecheck/test/build gates) | §5 Steps 1–14 unchanged in shape |
| Confirmation — rollback realism (single `git revert <F01-sha>` restores pre-F01 `main.ts`, removes `web/src/styles/`, restores scoped hex blocks) | §9 unchanged |

No design or analysis content changes between r1 and r2 of this plan — the corrections are strictly procedural (package manager) and editorial (matrix coverage).
...
- Repository state at [/home/salva/g/ml/saivage-v3](../../../../) currently has:
  ...
  - Verify:
    ```bash
    test -d web/src/styles || echo "absent (expected)"
    grep -rEoh '#[0-9a-fA-F]{3,8}\b' web/src --include='*.vue' --include='*.ts' | sort -u | wc -l
    # ≈ 51 distinct hex values
    ```
- npm workspace is bootstrapped. Both the root and `web/` package use npm (lockfiles: `/home/salva/g/ml/saivage-v3/package-lock.json` and `/home/salva/g/ml/saivage-v3/web/package-lock.json`; no `pnpm-lock.yaml` exists at either level). Verify:
  ```bash
  ...
  ```
  Both `npm` commands must pass on `main` before F01 begins.
...
- Verification:
  ...
  npm --prefix web run test
  ```
- Expectation: passes. Snapshot churn, if any, is bounded to embedded color values inside [web/src/__tests__/__snapshots__/](../../../../web/src/__tests__/__snapshots__/). Re-baseline by running with the project's snapshot-update flag (vitest `--update` or equivalent — `npm --prefix web run test -- -u` if the script forwards) **only after** manually reviewing every snapshot diff and confirming that:
...
- Action: serve the built UI (`npm --prefix web run dev`, or preview the build artefact) and walk through every surface listed in §8 of this plan. Compare against the pre-F01 baseline. Document any unexpected delta and reconcile before committing.
...
- Commit message (suggested):
  ...
  See SPEC/2026-05/review-ui-port-from-v2/F01-design-tokens/{01-analysis-r2,02-design-r2,03-plan-r2}.md.
  ```
- Single commit (see §6).
...
# Build pipeline (npm — root and web both ship package-lock.json):
npm --prefix web run typecheck
npm --prefix web run test
npm --prefix web run build
```

If any of the four grep gates is non-empty, or any of the three `npm` commands fails, the commit is **not ready**.
...
The matrix lists every `.vue` file currently carrying a hex literal under [web/src/](../../../../web/src/) (rows 1–15, 17–25). Rows 16 and 26 cover cross-surface gates that are not file-scoped.
...
| 16 | Scrollers (cross-surface) | n/a | global scrollbar paints `--border-strong` thumb on `--bg` track on every long surface (rows 1, 4, 5, 8, 9, 11, 13, 18, 20, 21, 22, 23). Overflow-hidden regions unaffected. |
| 17 | Workspace header | [web/src/components/layout/WorkspaceHeader.vue](../../../../web/src/components/layout/WorkspaceHeader.vue) | header surface uses `--surface-1`; title text on `--text`; meta/timestamps on `--text-muted`; bottom rule on `--border`; any status pip in the header maps to `--accent` / `--warn` / `--danger` per its state. No raw hex remains in Computed styles of the header bar or its embedded badges. |
| 18 | Raw LLM exchange panel | [web/src/components/agents/RawLlmExchangePanel.vue](../../../../web/src/components/agents/RawLlmExchangePanel.vue) | request / response panes use `--surface-1` (panel) and `--code-block-bg` (inner pre); JSON syntax tones come from `--syn-*` only; role labels on `--text-muted`; error rows tinted `--entry-danger-bg` / `--entry-danger-border`. Inner `<pre>` integrates with `highlight-overrides.css` (no double-border, no background "pop" against `--bg`). |
| 19 | Analyst toaster | [web/src/components/chat/AnalystToaster.vue](../../../../web/src/components/chat/AnalystToaster.vue) | toast surfaces use `--surface-2` with `--border` rule; success toast tint `--entry-accent-*`; warn `--entry-warn-*`; error `--entry-danger-*`; toast text on `--text`; dismiss icon on `--text-muted`. Backdrop / overlay (if any) reads as `--overlay-bg`. |
| 20 | Cards view (shell) | [web/src/views/CardsView.vue](../../../../web/src/views/CardsView.vue) | view chrome (tab strip, filter bar, toolbar) reads as `--surface-1` / `--surface-2`; active tab indicator on `--accent-2`; empty-state placeholders on `--text-muted` / `--text-faint`. No mixing with the inner board / timeline surfaces (rows 10–11) at the chrome boundary. |
| 21 | Cards leaderboard | [web/src/components/cards/CardsLeaderboardView.vue](../../../../web/src/components/cards/CardsLeaderboardView.vue) | leaderboard rows alternate using `--surface-1` / `--surface-2`; rank badges use `--accent` (top), `--warn` (middle), `--text-muted` (rest) or the equivalent semantic per the analysis mapping; row hover from `--hover-bg`; metric numbers on `--text`. |
| 22 | Cards tree | [web/src/components/cards/CardsTreeView.vue](../../../../web/src/components/cards/CardsTreeView.vue) | tree gutters / connector lines on `--border`; selected node highlight uses `--entry-accent-bg` + `--entry-accent-border`; collapsed-node carets on `--text-faint`; node labels on `--text`; status pips on the canonical `--accent` / `--warn` / `--danger`. |
| 23 | Card history panel | [web/src/components/cards/CardHistoryPanel.vue](../../../../web/src/components/cards/CardHistoryPanel.vue) | history entries arranged as `.entry-*`-shaped strips: created → `--entry-accent-*`, updated → `--entry-user-*` (blue), warning → `--entry-warn-*`, error → `--entry-danger-*`; meta timestamps on `--text-muted`; entry separators on `--border-subtle`. |
| 24 | Stale warning ribbon | [web/src/components/cards/StaleWarningRibbon.vue](../../../../web/src/components/cards/StaleWarningRibbon.vue) | ribbon surface uses `--entry-warn-bg` + `--entry-warn-border`; emphasis word on `--warn`; body copy on `--text`; dismiss / refresh action icon on `--text-muted` with `--hover-bg` on hover. No raw `#d29922` / `#9e6a03` / `#241f18` / `#5e4b16` remains in Computed styles. |
| 25 | Not found | [web/src/views/NotFound.vue](../../../../web/src/views/NotFound.vue) | full-bleed `--bg`; heading on `--text`; secondary copy on `--text-muted`; any CTA button uses `--btn-primary-bg` + `--btn-primary-text` (if present) or the secondary `.btn` look. |
| 26 | Inline `:style` bindings (cross-surface) | enumerated by `grep -rn ":style=" web/src --include='*.vue'` | every runtime-coloured site (status dot fill, progress bar, ribbon background, badge background, hover overlay) emits a `var(--…)` string in the rendered DOM `style` attribute — no `#…` literal appears in DevTools Elements panel `style="…"`. Cover at least one site per file in rows 1–25 plus the cards composites. |

If any row reveals an unmapped colour, return to Step 9 for the offending file, re-run Step 10, and re-validate.

Optional, non-blocking: a Playwright pixel-diff over rows 1–15 and 17–25 if a runner is available in the repo at implementation time (per §A.4.5.opt of the design).
...
| --- | --- | --- |
| Hex-rewrite misses a site (typo, dynamic-string, ternary). | Medium | Step 10's full-tree grep gate is the backstop; it must be empty before commit. Step 9 also runs per-hex grep after each value pass. Row 26 of the visual matrix re-checks every `:style=` binding in the running app. |
...
| highlight.js wrapper "pops" against `--bg` (background mismatch with `--code-block-bg`). | Low | `highlight-overrides.css` (Step 6) overrides only `pre.hljs` background / border / border-radius. Token rules (`.hljs-*`) remain from github-dark.css. Verified live in rows 13, 14, 18 of §8. |
...
| Re-running `npm --prefix web run build` re-imports `github-dark.css` in a different order. | Very low | Vite preserves source order of static imports in `main.ts`. Verified in Step 7. |
| `npm run` script names diverge between F01 design time and implementation time. | Very low | Step 12/13/14 verify the exact script names (`typecheck`, `test`, `build`) exist by running them; if a script is renamed, the implementer pauses and reconciles (not a F01 design issue). |
...
**Absolute path of this plan:** `/home/salva/g/ml/saivage-v3/SPEC/2026-05/review-ui-port-from-v2/F01-design-tokens/03-plan-r2.md`
